...
u_char *nxt_utf8_encode(u_char *p, uint32_t u)
{
    if (u < 0x80)
    {
        *p++ = (u_char)(u & 0xFF);
...
uint32_t nxt_utf8_decode(const u_char **start, const u_char *end)
{
    uint32_t u;
...
uint32_t nxt_utf8_decode2(const u_char **start, const u_char *end)
{
    u_char c;
    size_t n;
    uint32_t u, overlong;
...
nxt_int_t nxt_utf8_casecmp(const u_char *start1, const u_char *start2, size_t len1, size_t len2)
{
    int32_t n;
    uint32_t u1, u2;
...
uint32_t nxt_utf8_lowcase(const u_char **start, const u_char *end)
{
    uint32_t u;
...
ssize_t nxt_utf8_length(const u_char *p, size_t len)
{
    ssize_t length;
    const u_char *end;
...
nxt_bool_t nxt_utf8_is_valid(const u_char *p, size_t len)
{
    const u_char *end;
