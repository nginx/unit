...
static void nxt_gnutls_conn_init(nxt_thread_t *thr, nxt_ssltls_conf_t *conf, nxt_event_conn_t *c);
static void nxt_gnutls_session_cleanup(void *data);
static ssize_t nxt_gnutls_pull(gnutls_transport_ptr_t data, void *buf, size_t size);
static ssize_t nxt_gnutls_push(gnutls_transport_ptr_t data, const void *buf, size_t size);
#if (NXT_HAVE_GNUTLS_VEC_PUSH)
static ssize_t nxt_gnutls_vec_push(gnutls_transport_ptr_t data, const giovec_t *iov, int iovcnt);
#endif
...
static ssize_t nxt_gnutls_conn_io_write_chunk(nxt_thread_t *thr, nxt_event_conn_t *c, nxt_buf_t *b, size_t limit);
static ssize_t nxt_gnutls_conn_io_send(nxt_event_conn_t *c, void *buf, size_t size);
static void nxt_gnutls_conn_io_shutdown(nxt_thread_t *thr, void *obj, void *data);
static nxt_int_t nxt_gnutls_conn_test_error(nxt_thread_t *thr, nxt_event_conn_t *c, ssize_t err,
                                            nxt_work_handler_t handler);
static void nxt_cdecl nxt_gnutls_conn_log_error(nxt_event_conn_t *c, ssize_t err, const char *fmt, ...);
static nxt_uint_t nxt_gnutls_log_error_level(nxt_event_conn_t *c, ssize_t err);
static void nxt_cdecl nxt_gnutls_log_error(nxt_uint_t level, nxt_log_t *log, int err, const char *fmt, ...);
...
static nxt_int_t nxt_gnutls_start(void)
{
    int ret;
    ...
    {
        nxt_gnutls_log_error(NXT_LOG_ALERT, nxt_thread_log(), ret, "gnutls_global_init() failed");
        return NXT_ERROR;
    }

    nxt_thread_log_error(NXT_LOG_INFO, "GnuTLS version: %s", gnutls_check_version(NULL));
...
static nxt_int_t nxt_gnutls_server_init(nxt_ssltls_conf_t *conf)
{
    ...
    if (ret != GNUTLS_E_SUCCESS)
    {
        nxt_gnutls_log_error(NXT_LOG_ALERT, thr->log, ret, "gnutls_certificate_allocate_credentials() failed");
        return NXT_ERROR;
    }
    ...
    ret = gnutls_certificate_set_x509_key_file(ctx->certificate, certificate, key, GNUTLS_X509_FMT_PEM);
    if (ret != GNUTLS_E_SUCCESS)
    {
        nxt_gnutls_log_error(NXT_LOG_ALERT, thr->log, ret,
                             "gnutls_certificate_set_x509_key_file(\"%s\", \"%s\") failed", certificate, key);
    ...
    if (conf->ca_certificate != NULL)
    {
        ...
        ret = gnutls_certificate_set_x509_trust_file(ctx->certificate, ca_certificate, GNUTLS_X509_FMT_PEM);
        if (ret < 0)
        {
            nxt_gnutls_log_error(NXT_LOG_ALERT, thr->log, ret, "gnutls_certificate_set_x509_trust_file(\"%s\") failed",
                                 ca_certificate);
...
static nxt_int_t nxt_gnutls_set_ciphers(nxt_ssltls_conf_t *conf)
{
    ...
    case GNUTLS_E_INVALID_REQUEST:
        nxt_gnutls_log_error(NXT_LOG_ALERT, nxt_thread_log(), ret, "gnutls_priority_init(\"%s\") failed at \"%s\"",
                             ciphers, err);
        return NXT_ERROR;

    default:
        nxt_gnutls_log_error(NXT_LOG_ALERT, nxt_thread_log(), ret, "gnutls_priority_init() failed");
        return NXT_ERROR;
    }
}
...
static void nxt_gnutls_conn_init(nxt_thread_t *thr, nxt_ssltls_conf_t *conf, nxt_event_conn_t *c)
{
    ...
    if (ret != GNUTLS_E_SUCCESS)
    {
        nxt_gnutls_log_error(NXT_LOG_ALERT, c->socket.log, ret, "gnutls_init() failed");
        goto fail;
    }
    ...
    if (ret != GNUTLS_E_SUCCESS)
    {
        nxt_gnutls_log_error(NXT_LOG_ALERT, c->socket.log, ret, "gnutls_priority_set() failed");
        goto fail;
    }
    ...
    ret = gnutls_credentials_set(sess, GNUTLS_CRD_CERTIFICATE, ctx->certificate);
    if (ret != GNUTLS_E_SUCCESS)
    {
        nxt_gnutls_log_error(NXT_LOG_ALERT, c->socket.log, ret, "gnutls_credentials_set() failed");
        goto fail;
    }
...
fail:
    ...
    nxt_event_conn_io_handle(thr, c->read_work_queue, c->read_state->error_handler, c, c->socket.data);
}
...
static void nxt_gnutls_conn_handshake(nxt_thread_t *thr, void *obj, void *data)
{
    ...
    if (ret == NXT_ERROR)
    {
        ...
        nxt_event_conn_io_handle(thr, c->read_work_queue, c->read_state->error_handler, c, data);
    }
    else if (err == GNUTLS_E_AGAIN && ssltls->times < 2 && gnutls_record_get_direction(ssltls->session) == 0)
    {
        ssltls->times++;
    }
}
...
static void nxt_gnutls_conn_io_read(nxt_thread_t *thr, void *obj, void *data)
{
    ...
    if (b != NULL)
    {
        ...
        n = gnutls_record_recv(ssltls->session, b->mem.free, b->mem.end - b->mem.free);

        nxt_log_debug(thr->log, "gnutls_record_recv(%d, %p, %uz): %z", c->socket.fd, b->mem.free,
                      b->mem.end - b->mem.free, n);
        ...
        {
            ret = nxt_gnutls_conn_test_error(thr, c, n, nxt_gnutls_conn_io_read);
            ...
            nxt_gnutls_conn_log_error(c, n, "gnutls_record_recv(%d, %p, %uz): failed", c->socket.fd, b->mem.free,
...
static ssize_t nxt_gnutls_conn_io_send(nxt_event_conn_t *c, void *buf, size_t size)
{
    ...
    nxt_log_debug(c->socket.log, "gnutls_record_send(%d, %p, %uz): %z", c->socket.fd, buf, size, n);
    ...
    ret = nxt_gnutls_conn_test_error(nxt_thread(), c, n, nxt_event_conn_io_write);

    if (nxt_slow_path(ret == NXT_ERROR))
    {
        nxt_gnutls_conn_log_error(c, n, "gnutls_record_send(%d, %p, %uz): failed", c->socket.fd, buf, size);
    }
...
static void nxt_gnutls_conn_io_shutdown(nxt_thread_t *thr, void *obj, void *data)
{
    ...
    nxt_log_debug(c->socket.log, "gnutls_bye(%d, %d): %d", c->socket.fd, how, err);
    ...
    {
        ret = nxt_gnutls_conn_test_error(thr, c, err, nxt_gnutls_conn_io_shutdown);
        ...
        nxt_gnutls_conn_log_error(c, err, "gnutls_bye(%d) failed", c->socket.fd);
...
static void nxt_gnutls_conn_log_error(nxt_event_conn_t *c, ssize_t err, const char *fmt, ...)
{
    va_list args;
    nxt_uint_t level;
    ...
    if (nxt_log_level_enough(c->socket.log, level))
    {
        ...
        nxt_log_error(level, c->socket.log, "%*s (%d: %s)", p - msg, msg, err, gnutls_strerror(err));
    }
}
...
static void nxt_gnutls_log_error(nxt_uint_t level, nxt_log_t *log, int err, const char *fmt, ...)
{
    va_list args;
    u_char *p, msg[NXT_MAX_ERROR_STR];
    ...
    nxt_log_error(level, log, "%*s (%d: %s)", p - msg, msg, err, gnutls_strerror(err));
}
