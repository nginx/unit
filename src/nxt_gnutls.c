...
static void nxt_cdecl nxt_gnutls_conn_log_error (nxt_event_conn_t *c,
                                                 ...
                                                 ...);
static nxt_uint_t nxt_gnutls_log_error_level (nxt_event_conn_t *c,
                                              ssize_t err);
  ...
  if (b != NULL)
    {
      ...
      if (n > 0)
      ...
      else
        {
          ...
          nxt_gnutls_conn_log_error (
              c, n, "gnutls_record_recv(%d, %p, %uz): failed", c->socket.fd,
              b->mem.free, b->mem.end - b->mem.free);
...
static nxt_int_t
nxt_gnutls_conn_test_error (nxt_thread_t *thr, nxt_event_conn_t *c,
                            ssize_t err, nxt_work_handler_t handler)
{
  int ret;
  nxt_gnutls_conn_t *ssltls;
...
static void
nxt_gnutls_conn_log_error (nxt_event_conn_t *c, ssize_t err, const char *fmt,
                           ...)
{
  va_list args;
  nxt_uint_t level;
