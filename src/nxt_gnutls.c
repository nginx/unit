...
static void nxt_gnutls_conn_init(nxt_thread_t* thr,
                                 nxt_ssltls_conf_t* conf,
                                 nxt_event_conn_t* c);
static void nxt_gnutls_session_cleanup(void* data);
static ssize_t nxt_gnutls_pull(gnutls_transport_ptr_t data,
                               void* buf,
                               size_t size);
static ssize_t nxt_gnutls_push(gnutls_transport_ptr_t data,
                               const void* buf,
                               size_t size);
...
static ssize_t nxt_gnutls_conn_io_write_chunk(nxt_thread_t* thr,
                                              ...
                                              nxt_buf_t* b,
                                              size_t limit);
static ssize_t nxt_gnutls_conn_io_send(nxt_event_conn_t* c,
                                       void* buf,
                                       size_t size);
static void nxt_gnutls_conn_io_shutdown(nxt_thread_t* thr,
                                        void* obj,
                                        void* data);
...
static void nxt_cdecl nxt_gnutls_log_error(nxt_uint_t level,
                                           nxt_log_t* log,
                                           int err,
                                           const char* fmt,
                                           ...);
...
static nxt_int_t nxt_gnutls_server_init(nxt_ssltls_conf_t* conf) {
  ...
  ret = gnutls_certificate_set_x509_key_file(ctx->certificate, certificate, key,
                                             GNUTLS_X509_FMT_PEM);
  if (ret != GNUTLS_E_SUCCESS) {
    nxt_gnutls_log_error(
        NXT_LOG_ALERT, thr->log, ret,
  ...
  if (conf->ca_certificate != NULL) {
    ...
    ret = gnutls_certificate_set_x509_trust_file(
        ctx->certificate, ca_certificate, GNUTLS_X509_FMT_PEM);
    if (ret < 0) {
      nxt_gnutls_log_error(
          NXT_LOG_ALERT, thr->log, ret,
...
static void nxt_gnutls_conn_handshake(nxt_thread_t* thr,
                                      void* obj,
                                      void* data) {
  int err;
  nxt_int_t ret;
  ...
  } else if (err == GNUTLS_E_AGAIN && ssltls->times < 2 &&
...
static void nxt_gnutls_conn_io_read(nxt_thread_t* thr, void* obj, void* data) {
  ...
  if (b != NULL) {
    ...
    nxt_log_debug(thr->log, "gnutls_record_recv(%d, %p, %uz): %z", c->socket.fd,
                  b->mem.free, b->mem.end - b->mem.free, n);
    ...
    if (n > 0) {
      ...
      nxt_gnutls_conn_log_error(c, n, "gnutls_record_recv(%d, %p, %uz): failed",
...
static ssize_t nxt_gnutls_conn_io_send(nxt_event_conn_t* c,
  ...
  if (nxt_slow_path(ret == NXT_ERROR)) {
    nxt_gnutls_conn_log_error(c, n, "gnutls_record_send(%d, %p, %uz): failed",
...
static void nxt_gnutls_conn_io_shutdown(nxt_thread_t* thr,
  ...
  nxt_log_debug(c->socket.log, "gnutls_bye(%d, %d): %d", c->socket.fd, how,
                err);
...
static void nxt_gnutls_conn_log_error(nxt_event_conn_t* c,
                                      ssize_t err,
                                      const char* fmt,
                                      ...) {
  va_list args;
  ...
  if (nxt_log_level_enough(c->socket.log, level)) {
    ...
    nxt_log_error(level, c->socket.log, "%*s (%d: %s)", p - msg, msg, err,
                  gnutls_strerror(err));
  }
}
...
static void nxt_gnutls_log_error(nxt_uint_t level,
                                 nxt_log_t* log,
                                 int err,
                                 const char* fmt,
                                 ...) {
  ...
  nxt_log_error(level, log, "%*s (%d: %s)", p - msg, msg, err,
                gnutls_strerror(err));
}
