...
nxt_int_t nxt_clone_credential_setgroups(nxt_task_t *task, pid_t child_pid, const char *str);
nxt_int_t nxt_clone_credential_map_set(nxt_task_t *task, const char *mapfile, pid_t pid, nxt_int_t default_container,
                                       nxt_int_t default_host, nxt_clone_credential_map_t *map);
nxt_int_t nxt_clone_credential_map_write(nxt_task_t *task, const char *mapfile, pid_t pid, u_char *mapinfo);
...
nxt_int_t nxt_clone_credential_map_write(nxt_task_t *task, const char *mapfile, pid_t pid, u_char *mapinfo)
{
    ...
    *p = '\0';
    ...
    if (nxt_slow_path(n != len))
    {
        ...
        {
            nxt_alert(task,
                      "failed to write %s: Check kernel maximum "
                      "allowed lines %E",
                      buf, nxt_errno);
        }
        else
        {
            nxt_alert(task, "failed to write proc map (%s) %E", buf, nxt_errno);
        }
                                       ...
                                       nxt_int_t default_host, nxt_clone_credential_map_t *map)
{
    ...
    if (map->size > 0)
    {
        ...
        {
            p = nxt_sprintf(p, end, "%L %L %L", map->map[i].container, map->map[i].host, map->map[i].size);
    ...
    else
    {
        ...
        p = nxt_sprintf(mapinfo, end, "%d %d 1", default_container, default_host);
        *p = '\0';
...
nxt_int_t nxt_clone_credential_map(nxt_task_t *task, pid_t pid, nxt_credential_t *app_creds, nxt_clone_t *clone)
{
    nxt_int_t ret;
    ...
    ret = nxt_clone_credential_map_set(task, "uid_map", pid, app_creds->uid, default_host_uid, &clone->uidmap);
...
nxt_int_t nxt_clone_vldt_credential_uidmap(nxt_task_t *task, nxt_clone_credential_map_t *map, nxt_credential_t *creds)
{
    nxt_int_t id;
    nxt_uint_t i;
    nxt_runtime_t *rt;
    ...
    if (!rt->capabilities.setid)
    {
        ...
        {
            nxt_log(task, NXT_LOG_NOTICE,
                    "\"uidmap\" field has an entry for "
                    "host uid %d but unprivileged unit can only map itself "
                    "(uid %d) into child namespaces.",
                    id, nxt_euid);
    ...
    for (i = 0; i < map->size; i++)
    {
        ...
        if (creds->uid >= (nxt_uid_t)m.container && creds->uid < (nxt_uid_t)(m.container + m.size))
        {
            return NXT_OK;
        }
    }
    ...
    nxt_log(task, NXT_LOG_NOTICE,
            "\"uidmap\" field has no \"container\" "
            "entry for user \"%s\" (uid %d)",
            creds->user, creds->uid);

    return NXT_ERROR;
}

nxt_int_t nxt_clone_vldt_credential_gidmap(nxt_task_t *task, nxt_clone_credential_map_t *map, nxt_credential_t *creds)
{
    ...
    if (!rt->capabilities.setid)
    {
        if (creds->ngroups > 0 && !(creds->ngroups == 1 && creds->gids[0] == creds->base_gid))
        {
            nxt_log(task, NXT_LOG_NOTICE,
        ...
        if (nxt_slow_path((nxt_gid_t)m.host != nxt_egid))
        {
            nxt_log(task, NXT_LOG_ERR,
                    "\"gidmap\" field has an entry for "
                    "host gid %L but unprivileged unit can only map itself "
                    "(gid %d) into child namespaces.",
                    m.host, nxt_egid);
        ...
        {
            nxt_log(task, NXT_LOG_ERR,
                    "\"gidmap\" field has an entry with "
        ...
        {
            nxt_log(task, NXT_LOG_ERR, "\"gidmap\" field has no \"container\" entry for gid %d.", creds->base_gid);
    ...
    for (i = 0; i < creds->ngroups; i++)
    {
        ...
        for (j = 0; j < map->size; j++)
        {
            ...
            if (creds->gids[i] >= (nxt_gid_t)m.container && creds->gids[i] < (nxt_gid_t)(m.container + m.size))
            {
                gid_ok = 1;
                break;
            }
    ...
    if (!base_ok)
    {
        for (i = 0; i < map->size; i++)
        {
            ...
            if (creds->base_gid >= (nxt_gid_t)m.container && creds->base_gid < (nxt_gid_t)(m.container + m.size))
            {
                base_ok = 1;
                break;
            }
    ...
    {
        nxt_log(task, NXT_LOG_ERR,
                "\"gidmap\" field has missing "
                "suplementary gid mappings (found %d out of %d).",
                gids_ok, creds->ngroups);
