...
static nxt_int_t nxt_controller_prefork(nxt_task_t *task, nxt_process_t *process, nxt_mp_t *mp);
static nxt_int_t nxt_controller_file_read(nxt_task_t *task, const char *name, nxt_str_t *str, nxt_mp_t *mp);
static nxt_int_t nxt_controller_start(nxt_task_t *task, nxt_process_data_t *data);
static void nxt_controller_process_new_port_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg);
static void nxt_controller_send_current_conf(nxt_task_t *task);
static void nxt_controller_router_ready_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg);
static void nxt_controller_remove_pid_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg);
static nxt_int_t nxt_controller_conf_default(void);
static void nxt_controller_conf_init_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg, void *data);
static void nxt_controller_flush_requests(nxt_task_t *task);
static nxt_int_t nxt_controller_conf_send(nxt_task_t *task, nxt_mp_t *mp, nxt_conf_value_t *conf,
...
static nxt_msec_t nxt_controller_conn_timeout_value(nxt_conn_t *c, uintptr_t data);
static void nxt_controller_conn_read_error(nxt_task_t *task, void *obj, void *data);
static void nxt_controller_conn_read_timeout(nxt_task_t *task, void *obj, void *data);
static void nxt_controller_conn_body_read(nxt_task_t *task, void *obj, void *data);
static void nxt_controller_conn_write(nxt_task_t *task, void *obj, void *data);
static void nxt_controller_conn_write_error(nxt_task_t *task, void *obj, void *data);
static void nxt_controller_conn_write_timeout(nxt_task_t *task, void *obj, void *data);
...
static nxt_int_t nxt_controller_request_content_length(void *ctx, nxt_http_field_t *field, uintptr_t data);

static void nxt_controller_process_request(nxt_task_t *task, nxt_controller_request_t *req);
static void nxt_controller_process_config(nxt_task_t *task, nxt_controller_request_t *req, nxt_str_t *path);
static nxt_bool_t nxt_controller_check_postpone_request(nxt_task_t *task);
static void nxt_controller_process_status(nxt_task_t *task, nxt_controller_request_t *req);
static void nxt_controller_status_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg, void *data);
static void nxt_controller_status_response(nxt_task_t *task, nxt_controller_request_t *req, nxt_str_t *path);
#if (NXT_TLS)
static void nxt_controller_process_cert(nxt_task_t *task, nxt_controller_request_t *req, nxt_str_t *path);
static void nxt_controller_process_cert_save(nxt_task_t *task, nxt_port_recv_msg_t *msg, void *data);
static nxt_bool_t nxt_controller_cert_in_use(nxt_str_t *name);
static void nxt_controller_cert_cleanup(nxt_task_t *task, void *obj, void *data);
#endif
#if (NXT_HAVE_NJS)
static void nxt_controller_process_script(nxt_task_t *task, nxt_controller_request_t *req, nxt_str_t *path);
static void nxt_controller_process_script_save(nxt_task_t *task, nxt_port_recv_msg_t *msg, void *data);
static nxt_bool_t nxt_controller_script_in_use(nxt_str_t *name);
static void nxt_controller_script_cleanup(nxt_task_t *task, void *obj, void *data);
#endif
static void nxt_controller_process_control(nxt_task_t *task, nxt_controller_request_t *req, nxt_str_t *path);
static void nxt_controller_app_restart_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg, void *data);
static void nxt_controller_conf_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg, void *data);
static void nxt_controller_conf_store(nxt_task_t *task, nxt_conf_value_t *conf);
static void nxt_controller_response(nxt_task_t *task, nxt_controller_request_t *req, nxt_controller_response_t *resp);
static u_char *nxt_controller_date(u_char *buf, nxt_realtime_t *now, struct tm *tm, size_t size, const char *format);
...
static nxt_int_t nxt_controller_prefork(nxt_task_t *task, nxt_process_t *process, nxt_mp_t *mp)
{
    ...
    nxt_mp_cleanup(mp, nxt_controller_script_cleanup, task, ctrl_init.scripts, rt);
#endif
...
static nxt_int_t nxt_controller_start(nxt_task_t *task, nxt_process_data_t *data)
{
    ...
    ret = nxt_http_fields_hash(&nxt_controller_fields_hash, nxt_controller_request_fields,
    ...
    if (nxt_slow_path(ret != NXT_OK))
    {
        ...
        if (ret == NXT_DECLINED)
        {
            nxt_alert(task, "the previous configuration is invalid: %V", &vldt.error);
...
static void nxt_controller_process_new_port_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
    ...
    if (msg->u.new_port->type != NXT_PROCESS_ROUTER || !nxt_controller_router_ready)
    {
        return;
    }
...
static void nxt_controller_send_current_conf(nxt_task_t *task)
{
    ...
    if (conf != NULL)
    {
        rc = nxt_controller_conf_send(task, nxt_controller_conf.pool, conf, nxt_controller_conf_init_handler, NULL);
...
static nxt_int_t nxt_controller_conf_default(void)
{
    ...
    static const nxt_str_t json = nxt_string("{ \"listeners\": {}, \"routes\": [], \"applications\": {} }");
                                          ...
                                          nxt_port_rpc_handler_t handler, void *data)
{
    ...
    stream = nxt_port_rpc_register_handler(task, controller_port, handler, handler, router_port->pid, data);
    if (nxt_slow_path(stream == 0))
    {
        goto fail;
    }

    rc = nxt_port_socket_write(task, router_port, NXT_PORT_MSG_DATA_LAST | NXT_PORT_MSG_CLOSE_FD, fd, stream,
                               controller_port->id, b);
...
static void nxt_controller_conn_init(nxt_task_t *task, void *obj, void *data)
{
    ...
    if (nxt_slow_path(nxt_http_parse_request_init(&r->parser, c->mem_pool) != NXT_OK))
    {
        nxt_controller_conn_free(task, c, NULL);
        return;
    }
...
static const nxt_event_conn_state_t nxt_controller_conn_read_state nxt_aligned(64) = {
...
static void nxt_controller_conn_read(nxt_task_t *task, void *obj, void *data)
{
    ...
    rc = nxt_http_fields_process(r->parser.fields, &nxt_controller_fields_hash, r);
...
static const nxt_event_conn_state_t nxt_controller_conn_body_read_state nxt_aligned(64) = {
...
static const nxt_event_conn_state_t nxt_controller_conn_write_state nxt_aligned(64) = {
...
static const nxt_event_conn_state_t nxt_controller_conn_close_state nxt_aligned(64) = {
...
static nxt_int_t nxt_controller_request_content_length(void *ctx, nxt_http_field_t *field, uintptr_t data)
{
    ...
    if (nxt_fast_path(length >= 0))
    {
        ...
        {
            nxt_log_error(NXT_LOG_ERR, &r->conn->log, "Content-Length is too big");
            return NXT_ERROR;
        }
...
static void nxt_controller_process_request(nxt_task_t *task, nxt_controller_request_t *req)
{
    ...
    if (nxt_str_start(&path, "/config", 7) && (path.length == 7 || path.start[7] == '/'))
    {
        if (path.length == 7)
        {
            path.length = 1;
    ...
    if (nxt_str_start(&path, "/status", 7) && (path.length == 7 || path.start[7] == '/'))
    {
    ...
    if (nxt_str_start(&path, "/certificates", 13) && (path.length == 13 || path.start[13] == '/'))
    {
        if (path.length == 13)
        {
            path.length = 1;
    ...
    if (nxt_str_start(&path, "/js_modules", 11) && (path.length == 11 || path.start[11] == '/'))
    {
        if (path.length == 11)
        {
            path.length = 1;
...
static void nxt_controller_process_config(nxt_task_t *task, nxt_controller_request_t *req, nxt_str_t *path)
{
    ...
    if (post || nxt_str_eq(&req->parser.method, "PUT", 3))
    {
        ...
        /* Skip UTF-8 BOM. */
        if (nxt_buf_mem_used_size(mbuf) >= 3 && memcmp(mbuf->pos, "\xEF\xBB\xBF", 3) == 0)
        {
            mbuf->pos += 3;
        }
        ...
        if (value == NULL)
        {
            ...
            nxt_conf_json_position(mbuf->pos, error.pos, &resp.line, &resp.column);
        ...
        if (path->length != 1)
        {
            rc = nxt_conf_op_compile(c->mem_pool, &ops, nxt_controller_conf.root, path, value, post);
        ...
        rc = nxt_controller_conf_send(task, mp, value, nxt_controller_conf_handler, req);
    ...
    if (nxt_str_eq(&req->parser.method, "DELETE", 6))
    {
        ...
        {
            rc = nxt_conf_op_compile(c->mem_pool, &ops, nxt_controller_conf.root, path, NULL, 0);
...
static nxt_bool_t nxt_controller_check_postpone_request(nxt_task_t *task)
{
    ...
    if (!nxt_queue_is_empty(&nxt_controller_waiting_requests) || nxt_controller_waiting_init_conf ||
...
static void nxt_controller_process_status(nxt_task_t *task, nxt_controller_request_t *req)
{
    ...
    stream = nxt_port_rpc_register_handler(task, controller_port, nxt_controller_status_handler,
    ...
    rc = nxt_port_socket_write(task, router_port, NXT_PORT_MSG_STATUS, -1, stream, controller_port->id, NULL);
...
static void nxt_controller_status_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg, void *data)
{
    ...
    if (msg->port_msg.type == NXT_PORT_MSG_RPC_READY)
    {
        status = nxt_status_get((nxt_status_report_t *)msg->buf->mem.pos, req->conn->mem_pool);
    }
    else
    {
        status = NULL;
...
static void nxt_controller_process_cert(nxt_task_t *task, nxt_controller_request_t *req, nxt_str_t *path)
{
    ...
    if (nxt_str_eq(&req->parser.method, "PUT", 3))
    {
        ...
        nxt_cert_store_get(task, &name, c->mem_pool, nxt_controller_process_cert_save, req);
        return;
    }
...
static void nxt_controller_process_script(nxt_task_t *task, nxt_controller_request_t *req, nxt_str_t *path)
{
    ...
    if (nxt_str_eq(&req->parser.method, "PUT", 3))
    {
        ...
        script = nxt_script_new(task, &name, bm->pos, nxt_buf_mem_used_size(bm), error);
        if (script == NULL)
        {
            goto invalid_script;
        }
        ...
        nxt_script_store_get(task, &name, c->mem_pool, nxt_controller_process_script_save, req);
        return;
    }
...
static void nxt_controller_conf_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg, void *data)
{
    ...
    nxt_debug(task, "controller conf ready: %*s", nxt_buf_mem_used_size(&msg->buf->mem), msg->buf->mem.pos);
...
static void nxt_controller_process_control(nxt_task_t *task, nxt_controller_request_t *req, nxt_str_t *path)
{
    ...
    if (!nxt_str_start(path, "applications/", 13) || memcmp(path->start + path->length - 8, "/restart", 8) != 0)
    {
        goto not_found;
    }
    ...
    stream = nxt_port_rpc_register_handler(task, controller_port, nxt_controller_app_restart_handler,
    ...
    rc = nxt_port_socket_write(task, router_port, NXT_PORT_MSG_APP_RESTART, -1, stream, 0, b);
    if (nxt_slow_path(rc != NXT_OK))
    {
        nxt_port_rpc_cancel(task, controller_port, stream);
...
static void nxt_controller_conf_store(nxt_task_t *task, nxt_conf_value_t *conf)
{
    ...
    (void)nxt_port_socket_write(task, main_port, NXT_PORT_MSG_CONF_STORE | NXT_PORT_MSG_CLOSE_FD, fd, 0, -1, b);
...
static void nxt_controller_response(nxt_task_t *task, nxt_controller_request_t *req, nxt_controller_response_t *resp)
{
    ...
    if (value == NULL)
    {
        n = 1 + (resp->detail.length != 0) + (resp->status >= 400 && resp->offset != -1);
        ...
        if (resp->status >= 400 && resp->offset != -1)
        {
            ...
            location = nxt_conf_create_object(c->mem_pool, resp->line != 0 ? 3 : 1);
            ...
            if (resp->line != 0)
            {
                ...
                nxt_conf_set_member_integer(location, &column_str, resp->column, 2);
            }
        }
    }
    ...
    b->mem.free = nxt_cpymem(b->mem.free, status_line.start, status_line.length);
    ...
    b->mem.free = nxt_thread_time_string(task->thread, &date_cache, b->mem.free);
    ...
    b->mem.free = nxt_sprintf(b->mem.free, b->mem.end, "%uz", nxt_buf_mem_used_size(&body->mem));
...
static u_char *nxt_controller_date(u_char *buf, nxt_realtime_t *now, struct tm *tm, size_t size, const char *format)
{
    ...
    return nxt_sprintf(buf, buf + size, format, week[tm->tm_wday], tm->tm_mday, month[tm->tm_mon], tm->tm_year + 1900,
