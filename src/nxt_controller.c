...
static nxt_int_t nxt_controller_prefork(nxt_task_t* task,
                                        ...
                                        nxt_mp_t* mp);
static nxt_int_t nxt_controller_file_read(nxt_task_t* task,
                                          const char* name,
                                          nxt_str_t* str,
                                          nxt_mp_t* mp);
...
static nxt_int_t nxt_controller_conf_send(nxt_task_t* task,
                                          nxt_mp_t* mp,
                                          nxt_conf_value_t* conf,
                                          nxt_port_rpc_handler_t handler,
                                          void* data);
...
static nxt_msec_t nxt_controller_conn_timeout_value(nxt_conn_t* c,
                                                    uintptr_t data);
static void nxt_controller_conn_read_error(nxt_task_t* task,
                                           void* obj,
                                           void* data);
static void nxt_controller_conn_read_timeout(nxt_task_t* task,
                                             void* obj,
                                             void* data);
static void nxt_controller_conn_body_read(nxt_task_t* task,
                                          void* obj,
                                          void* data);
static void nxt_controller_conn_write(nxt_task_t* task, void* obj, void* data);
static void nxt_controller_conn_write_error(nxt_task_t* task,
                                            void* obj,
                                            void* data);
static void nxt_controller_conn_write_timeout(nxt_task_t* task,
                                              void* obj,
                                              void* data);
...
static void nxt_controller_cert_cleanup(nxt_task_t* task,
                                        void* obj,
                                        void* data);
#endif
#if (NXT_HAVE_NJS)
...
static void nxt_controller_script_cleanup(nxt_task_t* task,
                                          void* obj,
                                          void* data);
#endif
...
static u_char* nxt_controller_date(u_char* buf,
                                   nxt_realtime_t* now,
                                   struct tm* tm,
                                   size_t size,
                                   const char* format);
...
static nxt_int_t nxt_controller_conf_default(void) {
  ...
  static const nxt_str_t json =
      nxt_string("{ \"listeners\": {}, \"routes\": [], \"applications\": {} }");
...
static nxt_int_t nxt_controller_conf_send(nxt_task_t* task,
  ...
  rc = nxt_port_socket_write(task, router_port,
                             NXT_PORT_MSG_DATA_LAST | NXT_PORT_MSG_CLOSE_FD, fd,
...
static void nxt_controller_conn_read(nxt_task_t* task, void* obj, void* data) {
  ...
  rc =
      nxt_http_fields_process(r->parser.fields, &nxt_controller_fields_hash, r);
...
static void nxt_controller_process_config(nxt_task_t* task,
  ...
  if (post || nxt_str_eq(&req->parser.method, "PUT", 3)) {
    ...
    if (path->length != 1) {
      rc = nxt_conf_op_compile(c->mem_pool, &ops, nxt_controller_conf.root,
                               path, value, post);
    ...
    rc = nxt_controller_conf_send(task, mp, value, nxt_controller_conf_handler,
                                  req);
...
static void nxt_controller_process_status(nxt_task_t* task,
  ...
  rc = nxt_port_socket_write(task, router_port, NXT_PORT_MSG_STATUS, -1, stream,
                             controller_port->id, NULL);
...
static void nxt_controller_process_script(nxt_task_t* task,
  ...
  if (nxt_str_eq(&req->parser.method, "PUT", 3)) {
    ...
    script =
        nxt_script_new(task, &name, bm->pos, nxt_buf_mem_used_size(bm), error);
    if (script == NULL) {
      goto invalid_script;
    }
...
static u_char* nxt_controller_date(u_char* buf,
                                   ...
                                   size_t size,
                                   const char* format) {
  static const char* const week[] = {"Sun", "Mon", "Tue", "Wed",
                                     "Thu", "Fri", "Sat"};
