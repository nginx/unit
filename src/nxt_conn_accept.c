...
static nxt_conn_t *nxt_conn_accept_alloc(nxt_task_t *task, nxt_listen_event_t *lev);
static void nxt_conn_listen_handler(nxt_task_t *task, void *obj, void *data);
static nxt_conn_t *nxt_conn_accept_next(nxt_task_t *task, nxt_listen_event_t *lev);
static void nxt_conn_accept_close_idle(nxt_task_t *task, nxt_listen_event_t *lev);
static void nxt_conn_accept_close_idle_handler(nxt_task_t *task, void *obj, void *data);
static void nxt_conn_listen_event_error(nxt_task_t *task, void *obj, void *data);
static void nxt_conn_listen_timer_handler(nxt_task_t *task, void *obj, void *data);

nxt_listen_event_t *nxt_listen_event(nxt_task_t *task, nxt_listen_socket_t *ls)
{
...
void nxt_conn_accept(nxt_task_t *task, nxt_listen_event_t *lev, nxt_conn_t *c)
{
    nxt_conn_t *next;
    nxt_event_engine_t *engine;
    ...
    nxt_debug(task, "client: %*s", (size_t)c->remote->address_length, nxt_sockaddr_address(c->remote));
    ...
    if (lev->listen->read_after_accept)
    {
        ...
        nxt_work_queue_add(c->read_work_queue, lev->listen->handler, &c->task, c, lev);
    }
    else
    {
        nxt_work_queue_add(c->write_work_queue, lev->listen->handler, &c->task, c, lev);
    }
    ...
    {
        nxt_work_queue_add(lev->socket.read_work_queue, lev->accept, task, lev, next);
    }
}
...
static void nxt_conn_accept_close_idle(nxt_task_t *task, nxt_listen_event_t *lev)
{
    ...
    nxt_work_queue_add(&engine->close_work_queue, nxt_conn_accept_close_idle_handler, task, NULL, NULL);
...
static void nxt_conn_accept_close_idle_handler(nxt_task_t *task, void *obj, void *data)
{
    ...
    static nxt_log_moderation_t nxt_idle_close_log_moderation = {NXT_LOG_INFO, 2, "idle connections closed",
                                                                 NXT_LOG_MODERATION};
    ...
    for (link = nxt_queue_last(idle); link != nxt_queue_head(idle); link = next)
    {
        next = nxt_queue_next(link);

        c = nxt_queue_link_data(link, nxt_conn_t, link);

        nxt_debug(c->socket.task, "idle connection: %d rdy:%d", c->socket.fd, c->socket.read_ready);

        if (!c->socket.read_ready)
        {
            nxt_log_moderate(&nxt_idle_close_log_moderation, NXT_LOG_INFO, task->log,
...
void nxt_conn_accept_error(nxt_task_t *task, nxt_listen_event_t *lev, const char *accept_syscall, nxt_err_t err)
{
    static nxt_log_moderation_t nxt_accept_log_moderation = {NXT_LOG_INFO, 2, "accept() failed", NXT_LOG_MODERATION};
    ...
    case ECONNABORTED:
        nxt_log_moderate(&nxt_accept_log_moderation, NXT_LOG_WARN, task->log, "%s(%d) failed %E", accept_syscall,
                         lev->socket.fd, err);
        return;
    ...
    case ENOBUFS:
    case ENOMEM:
        nxt_alert(task, "%s(%d) failed %E", accept_syscall, lev->socket.fd, err);
