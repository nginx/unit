...
static nxt_log_moderation_t nxt_malloc_log_moderation = {NXT_LOG_ALERT, 2, "memory allocation failed",
                                                         NXT_LOG_MODERATION};

static nxt_log_t *nxt_malloc_log(void)
{
    nxt_thread_t *thr;
...
void *nxt_malloc(size_t size)
{
    ...
    {
        nxt_log_alert_moderate(&nxt_malloc_log_moderation, nxt_malloc_log(), "malloc(%uz) failed %E", size, nxt_errno);
    }
...
void *nxt_realloc(void *p, size_t size)
{
    void *n;
    uintptr_t ptr;
    ...
    {
        nxt_log_alert_moderate(&nxt_malloc_log_moderation, nxt_malloc_log(), "realloc(%p, %uz) failed %E", ptr, size,
                               nxt_errno);
    }
...
void *nxt_lvlhsh_alloc(void *data, size_t size)
{
    return nxt_memalign(size, size);
}

void nxt_lvlhsh_free(void *data, void *p)
{
    nxt_free(p);
}
...
void *nxt_memalign(size_t alignment, size_t size)
{
    ...
    {
        nxt_thread_log_debug("posix_memalign(%uz, %uz): %p", alignment, size, p);
        return p;
    }

    nxt_log_alert_moderate(&nxt_malloc_log_moderation, nxt_malloc_log(), "posix_memalign(%uz, %uz) failed %E",
                           alignment, size, err);
    return NULL;
}
...
void *nxt_memalign(size_t alignment, size_t size)
{
    ...
    nxt_log_alert_moderate(&nxt_malloc_log_moderation, nxt_malloc_log(), "memalign(%uz, %uz) failed %E", alignment,
                           size, nxt_errno);
    return NULL;
}
...
fail:
    ...
    nxt_thread_log_alert("nxt_memalign(%uz, %uz) failed %E", alignment, size, err);
    return NULL;
}
