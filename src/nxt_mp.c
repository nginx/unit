...
nxt_mp_t *nxt_mp_create(size_t cluster_size, size_t page_alignment,
...
nxt_bool_t nxt_mp_test_sizes(size_t cluster_size, size_t page_alignment,
                             size_t page_size, size_t min_chunk_size) {
  nxt_bool_t valid;
  ...
  valid = nxt_expect(
      1, (page_size >= 64 && page_size >= page_alignment &&
          page_size >= min_chunk_size && min_chunk_size * 32 >= page_size &&
          cluster_size >= page_size && cluster_size / page_size <= 256 &&
...
nxt_inline u_char *nxt_mp_page_addr(nxt_mp_t *mp, nxt_mp_page_t *page) {
  ...
  page_offset =
      page->number * sizeof(nxt_mp_page_t) + offsetof(nxt_mp_block_t, pages);
...
static void *nxt_mp_get_small(nxt_mp_t *mp, nxt_queue_t *pages, size_t size) {
  ...
  for (link = nxt_queue_first(pages); link != nxt_queue_tail(pages);
       link = next) {
...
nxt_int_t nxt_mp_cleanup(nxt_mp_t *mp, nxt_work_handler_t handler,
...
void nxt_mp_lvlhsh_free(void *pool, void *p) { nxt_mp_free(pool, p); }
