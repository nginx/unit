...
#define nxt_mp_chunk_get_free(map) (__builtin_ffs(map) - 1)

#define nxt_mp_chunk_is_free(map, chunk) ((map & (1 << chunk)) != 0)

#define nxt_mp_chunk_set_busy(map, chunk) map &= ~(1 << chunk)

#define nxt_mp_chunk_set_free(map, chunk) map |= (1 << chunk)

#define nxt_mp_free_junk(p, size) memset((p), 0x5A, size)
...
#endif
static void *nxt_mp_alloc_large(nxt_mp_t *mp, size_t alignment, size_t size, nxt_bool_t freeable);
static intptr_t nxt_mp_rbtree_compare(nxt_rbtree_node_t *node1, nxt_rbtree_node_t *node2);
static nxt_mp_block_t *nxt_mp_find_block(nxt_rbtree_t *tree, const u_char *p);
static const char *nxt_mp_chunk_free(nxt_mp_t *mp, nxt_mp_block_t *cluster, u_char *p);

#if (NXT_HAVE_BUILTIN_CLZ)

#define nxt_lg2(value) (31 - __builtin_clz(value))

#else

static const int nxt_lg2_tab64[64] = {63, 0,  58, 1,  59, 47, 53, 2,  60, 39, 48, 27, 54, 33, 42, 3,
...
nxt_inline void nxt_mp_thread_assert(nxt_mp_t *mp)
{
    nxt_tid_t tid;
    nxt_thread_t *thread;
...
void nxt_mp_thread_adopt(nxt_mp_t *mp)
{
#if (NXT_DEBUG)
    mp->pid = nxt_pid;
...
nxt_mp_t *nxt_mp_create(size_t cluster_size, size_t page_alignment, size_t page_size, size_t min_chunk_size)
{
    nxt_mp_t *mp;
    ...
    nxt_debug_alloc("mp %p create(%uz, %uz, %uz, %uz)", mp, cluster_size, page_alignment, page_size, min_chunk_size);

    return mp;
}

void nxt_mp_retain(nxt_mp_t *mp)
{
    mp->retain++;
...
void nxt_mp_release(nxt_mp_t *mp)
{
    mp->retain--;
...
void nxt_mp_destroy(nxt_mp_t *mp)
{
    void *p;
...
nxt_bool_t nxt_mp_test_sizes(size_t cluster_size, size_t page_alignment, size_t page_size, size_t min_chunk_size)
{
    nxt_bool_t valid;

    /* Alignment and sizes must be a power of 2. */

    valid = nxt_expect(1, (nxt_is_power_of_two(page_alignment) && nxt_is_power_of_two(page_size) &&
    ...
    valid = nxt_expect(1, (page_size >= 64 && page_size >= page_alignment && page_size >= min_chunk_size &&
                           min_chunk_size * 32 >= page_size && cluster_size >= page_size &&
                           cluster_size / page_size <= 256 && cluster_size % page_size == 0));
    if (!valid)
    {
        return 0;
    }
...
nxt_bool_t nxt_mp_is_empty(nxt_mp_t *mp)
{
    return (nxt_rbtree_is_empty(&mp->blocks) && nxt_queue_is_empty(&mp->free_pages));
}

void *nxt_mp_alloc(nxt_mp_t *mp, size_t size)
{
    void *p;
...
void *nxt_mp_zalloc(nxt_mp_t *mp, size_t size)
{
    void *p;
...
void *nxt_mp_zalign(nxt_mp_t *mp, size_t alignment, size_t size)
{
    void *p;
...
nxt_inline nxt_uint_t nxt_mp_chunk_pages_index(nxt_mp_t *mp, size_t size)
{
    nxt_int_t n, index;
...
nxt_inline u_char *nxt_mp_page_addr(nxt_mp_t *mp, nxt_mp_page_t *page)
{
    size_t page_offset;
    nxt_mp_block_t *block;

    page_offset = page->number * sizeof(nxt_mp_page_t) + offsetof(nxt_mp_block_t, pages);
...
static void *nxt_mp_alloc_small(nxt_mp_t *mp, size_t size)
{
    ...
    nxt_debug_alloc("mp %p chunk:%uz alloc: %p", mp, page->size << mp->chunk_size_shift, p);
...
static void *nxt_mp_get_small(nxt_mp_t *mp, nxt_queue_t *pages, size_t size)
{
    ...
    for (link = nxt_queue_first(pages); link != nxt_queue_tail(pages); link = next)
    {
...
static nxt_mp_block_t *nxt_mp_alloc_cluster(nxt_mp_t *mp)
{
    ...
    while (n != 0)
    {
        n--;
        cluster->pages[n].number = n;
        nxt_queue_insert_before(&cluster->pages[n + 1].link, &cluster->pages[n].link);
    }
...
void *nxt_mp_nget(nxt_mp_t *mp, size_t size)
{
    void *p;
...
void *nxt_mp_get(nxt_mp_t *mp, size_t size)
{
    void *p;
...
void *nxt_mp_zget(nxt_mp_t *mp, size_t size)
{
    void *p;
...
nxt_int_t nxt_mp_cleanup(nxt_mp_t *mp, nxt_work_handler_t handler, nxt_task_t *task, void *obj, void *data)
{
    nxt_work_t *work;
...
void *nxt_mp_lvlhsh_alloc(void *pool, size_t size)
{
    return nxt_mp_align(pool, size, size);
}

void nxt_mp_lvlhsh_free(void *pool, void *p)
{
    nxt_mp_free(pool, p);
}
