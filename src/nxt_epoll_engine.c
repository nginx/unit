...
static void nxt_epoll_enable_accept(nxt_event_engine_t* engine,
                                    nxt_fd_event_t* ev);
static void nxt_epoll_change(nxt_event_engine_t* engine,
                             nxt_fd_event_t* ev,
                             int op,
                             uint32_t events);
...
static void nxt_epoll_edge_conn_io_connect(nxt_task_t* task,
                                           void* obj,
                                           void* data);
static void nxt_epoll_edge_conn_connected(nxt_task_t* task,
                                          void* obj,
                                          void* data);
...
static void nxt_epoll_commit_changes(nxt_event_engine_t* engine) {
  ...
  nxt_debug(&engine->task, "epoll %d changes:%ui", engine->u.epoll.fd,
            engine->u.epoll.nchanges);
  ...
  do {
    ...
    nxt_debug(ev->task, "epoll_ctl(%d): fd:%d op:%d ev:%XD", engine->u.epoll.fd,
              ev->fd, change->op, change->event.events);

    ret = epoll_ctl(engine->u.epoll.fd, change->op, ev->fd, &change->event);

    if (nxt_slow_path(ret != 0)) {
      nxt_alert(ev->task, "epoll_ctl(%d, %d, %d) failed %E", engine->u.epoll.fd,
                change->op, ev->fd, nxt_errno);
...
static void nxt_epoll_signalfd_handler(nxt_task_t* task,
                                       void* obj,
                                       void* data) {
  int n;
  nxt_fd_event_t* ev;
...
static nxt_int_t nxt_epoll_enable_post(nxt_event_engine_t* engine,
  ...
  ret = epoll_ctl(engine->u.epoll.fd, EPOLL_CTL_ADD, engine->u.epoll.eventfd.fd,
                  &ee);
...
static void nxt_epoll_eventfd_handler(nxt_task_t* task, void* obj, void* data) {
  ...
  if (engine->u.epoll.neventfd++ >= 0xFFFFFFFE) {
    ...
    nxt_debug(task, "read(%d): %d events:%uL", engine->u.epoll.eventfd.fd, n,
              events);

    if (n != sizeof(uint64_t)) {
      nxt_alert(task, "read eventfd(%d) failed %E", engine->u.epoll.eventfd.fd,
                nxt_errno);
    }
  }
...
static void nxt_epoll_poll(nxt_event_engine_t* engine, nxt_msec_t timeout) {
  ...
  nxt_debug(&engine->task, "epoll_wait(%d) timeout:%M", engine->u.epoll.fd,
            timeout);
  ...
  for (i = 0; i < nevents; i++) {
    ...
    nxt_debug(ev->task, "epoll: fd:%d ev:%04XD d:%p rd:%d wr:%d", ev->fd,
              events, ev, ev->read, ev->write);
    ...
    if (error && ev->read <= NXT_EVENT_BLOCKED &&
        ev->write <= NXT_EVENT_BLOCKED) {
      error = 0;
    }
    ...
    if ((events & EPOLLIN) != 0) {
      ...
      if (ev->read != NXT_EVENT_BLOCKED) {
        ...
        nxt_work_queue_add(ev->read_work_queue, ev->read_handler, ev->task, ev,
                           ev->data);
    ...
    if ((events & EPOLLOUT) != 0) {
      ...
      if (ev->write != NXT_EVENT_BLOCKED) {
        ...
        nxt_work_queue_add(ev->write_work_queue, ev->write_handler, ev->task,
                           ev, ev->data);
