...
void nxt_conn_wait(nxt_conn_t* c) {
  ...
  nxt_debug(c->socket.task, "conn wait fd:%d rdy:%d", c->socket.fd,
            c->socket.read_ready);
...
void nxt_conn_io_read(nxt_task_t* task, void* obj, void* data) {
  ...
  nxt_debug(task, "conn read fd:%d rdy:%d cl:%d er:%d bl:%d", c->socket.fd,
            c->socket.read_ready, c->socket.closed, c->socket.error,
            c->block_read);
  ...
  if (c->socket.read_ready) {
    ...
    if (n > 0) {
      ...
      nxt_work_queue_add(c->read_work_queue, state->ready_handler, task, c,
                         data);
      return;
    }
...
ssize_t nxt_conn_io_recv(nxt_conn_t* c,
  ...
  for (;;) {
    ...
    nxt_debug(c->socket.task, "recv(%d, %p, %uz, 0x%ui): %z", c->socket.fd, buf,
              size, flags, n);
    ...
    switch (err) {
      ...
      default:
        c->socket.error = err;
        nxt_log(c->socket.task, nxt_socket_error_level(err),
                "recv(%d, %p, %uz, %ui) failed %E", c->socket.fd, buf, size,
                flags, err);
