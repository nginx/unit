...
nxt_conn_io_read(nxt_task_t *task, void *obj, void *data)
{
    ...
    if (c->socket.read_ready) {
        ...
        if (n > 0) {
            ...
            nxt_work_queue_add(c->read_work_queue, state->ready_handler, task,
                               c, data);
            return;
        }
        ...
        if (n != NXT_AGAIN) {
            ...
            nxt_work_queue_add(&engine->fast_work_queue, handler, task, c,
                               data);
            return;
        }
...
nxt_conn_io_recv(nxt_conn_t *c, void *buf, size_t size, nxt_uint_t flags)
{
    ...
    for (;;) {
        ...
        nxt_debug(c->socket.task, "recv(%d, %p, %uz, 0x%ui): %z", c->socket.fd,
                  buf, size, flags, n);
