...
void nxt_conn_wait(nxt_conn_t *c)
{
    ...
    nxt_debug(c->socket.task, "conn wait fd:%d rdy:%d", c->socket.fd, c->socket.read_ready);
    ...
    {
        nxt_work_queue_add(&engine->fast_work_queue, state->ready_handler, c->socket.task, c, c->socket.data);
        return;
    }
...
void nxt_conn_io_read(nxt_task_t *task, void *obj, void *data)
{
    ...
    nxt_debug(task, "conn read fd:%d rdy:%d cl:%d er:%d bl:%d", c->socket.fd, c->socket.read_ready, c->socket.closed,
    ...
    if (c->socket.read_ready)
    {
        ...
        if (n > 0)
        {
            ...
            nxt_work_queue_add(c->read_work_queue, state->ready_handler, task, c, data);
            return;
        }
        ...
        if (n != NXT_AGAIN)
        {
            ...
            nxt_work_queue_add(&engine->fast_work_queue, handler, task, c, data);
            return;
        }
...
ssize_t nxt_conn_io_recvbuf(nxt_conn_t *c, nxt_buf_t *b)
{
    ...
    for (;;)
    {
        ...
        default:
            c->socket.error = err;
            nxt_log(c->socket.task, nxt_socket_error_level(err), "readv(%d, %ui) failed %E", c->socket.fd, niov, err);
...
ssize_t nxt_conn_io_recv(nxt_conn_t *c, void *buf, size_t size, nxt_uint_t flags)
{
    ...
    for (;;)
    {
        ...
        nxt_debug(c->socket.task, "recv(%d, %p, %uz, 0x%ui): %z", c->socket.fd, buf, size, flags, n);
        ...
        default:
            c->socket.error = err;
            nxt_log(c->socket.task, nxt_socket_error_level(err), "recv(%d, %p, %uz, %ui) failed %E", c->socket.fd, buf,
                    size, flags, err);
