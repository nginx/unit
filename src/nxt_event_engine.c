...
static void nxt_event_engine_signal_pipe_close(nxt_task_t* task,
                                               void* obj,
                                               void* data);
static void nxt_event_engine_signal_pipe(nxt_task_t* task,
                                         void* obj,
                                         void* data);
static void nxt_event_engine_post_handler(nxt_task_t* task,
                                          void* obj,
                                          void* data);
static void nxt_event_engine_signal_pipe_error(nxt_task_t* task,
                                               void* obj,
                                               void* data);
static void nxt_event_engine_signal_handler(nxt_task_t* task,
                                            void* obj,
                                            void* data);
...
nxt_event_engine_t* nxt_event_engine_create(
    nxt_task_t* task,
...
static void nxt_event_engine_signal_pipe_error(nxt_task_t* task,
  ...
  nxt_alert(task, "engine pipe(%FD:%FD) event error", pipe->fds[0],
            pipe->fds[1]);
...
static void nxt_event_engine_signal_handler(nxt_task_t* task,
  ...
  for (sigev = task->thread->engine->signals->sigev; sigev->signo != 0;
       sigev++) {
...
nxt_int_t nxt_event_engine_change(nxt_event_engine_t* engine,
                                  const nxt_event_interface_t* interface,
                                  nxt_uint_t batch) {
  nxt_uint_t events;
  ...
  if (engine->pipe != NULL && interface->enable_post != NULL) {
    ...
    nxt_work_queue_add(&engine->fast_work_queue,
                       nxt_event_engine_signal_pipe_close, &engine->task,
                       engine->pipe, NULL);
...
void* nxt_event_engine_mem_alloc(nxt_event_engine_t* engine,
  ...
  if (n == NXT_EVENT_ENGINE_NO_MEM_HINT) {
    if (mem_cache == NULL) {
      ...
      mem_cache =
          nxt_array_create(engine->mem_pool, items, sizeof(nxt_mem_cache_t));
...
void nxt_event_engine_mem_free(nxt_event_engine_t* engine,
  ...
  if (nxt_slow_path(n == NXT_EVENT_ENGINE_NO_MEM_HINT)) {
    if (size != 0) {
      ...
      nxt_alert(&engine->task, "event engine mem free(%p, %z) not found", p,
                size);
    }
