...
nxt_app_module_t nxt_external_module = {
    0, NULL, nxt_string("external"), "*", NULL, 0, NULL, nxt_external_start,
};

extern char **environ;

nxt_inline nxt_int_t nxt_external_fd_no_cloexec(nxt_task_t *task, nxt_socket_t fd)
{
    int res, flags;
...
static nxt_int_t nxt_external_start(nxt_task_t *task, nxt_process_data_t *data)
{
    ...
    if (nxt_slow_path(proto_port == NULL || my_port == NULL || router_port == NULL))
    {
        return NXT_ERROR;
    }
    ...
    p = nxt_sprintf(buf, end,
                    ...
                    "%d,%d;"
                    "%d,%z,%uD,%Z",
                    NXT_VERSION, my_port->process->stream, proto_port->pid, proto_port->id, proto_port->pair[1],
                    router_port->pid, router_port->id, router_port->pair[1], my_port->pid, my_port->id,
                    my_port->pair[0], my_port->pair[1], conf->shared_port_fd, conf->shared_queue_fd, 2, conf->shm_limit,
                    conf->request_limit);
    ...
    {
        nxt_alert(task, "setenv(" NXT_UNIT_INIT_ENV ", %s) failed %E", buf, nxt_errno);
