...
static const u_char* nxt_sha1_body(nxt_sha1_t* ctx,
                                   const u_char* data,
                                   size_t size);

void nxt_sha1_init(nxt_sha1_t* ctx) {
  ctx->a = 0x67452301;
  ctx->b = 0xefcdab89;
  ctx->c = 0x98badcfe;
  ctx->d = 0x10325476;
...
void nxt_sha1_update(nxt_sha1_t* ctx, const void* data, size_t size) {
  size_t used, free;
...
void nxt_sha1_final(u_char result[20], nxt_sha1_t* ctx) {
  size_t used, free;
...
#define GET(n)                                                \
  (((uint32_t)p[n * 4 + 3]) | ((uint32_t)p[n * 4 + 2] << 8) | \
   ((uint32_t)p[n * 4 + 1] << 16) | ((uint32_t)p[n * 4] << 24))
...
static const u_char* nxt_sha1_body(nxt_sha1_t* ctx,
  ...
  do {
    ...
    for (i = 16; i < 80; i++) {
      words[i] = ROTATE(
          1, words[i - 3] ^ words[i - 8] ^ words[i - 14] ^ words[i - 16]);
    }
