...
static void nxt_runtime_thread_pool_destroy(nxt_task_t* task,
                                            nxt_runtime_t* rt,
                                            nxt_runtime_cont_t cont);
static void nxt_runtime_thread_pool_init(void);
static void nxt_runtime_thread_pool_exit(nxt_task_t* task,
                                         void* obj,
                                         void* data);
...
nxt_int_t nxt_runtime_create(nxt_task_t* task) {
  ...
  nxt_work_queue_add(&task->thread->engine->fast_work_queue, nxt_runtime_start,
                     task, rt, NULL);
...
static nxt_int_t nxt_runtime_inherited_listen_sockets(nxt_task_t* task,
  ...
  inherited_sockets =
      nxt_array_create(rt->mem_pool, 1, sizeof(nxt_listen_socket_t));
...
static nxt_int_t nxt_runtime_systemd_listen_sockets(nxt_task_t* task,
  ...
  inherited_sockets =
      nxt_array_create(rt->mem_pool, n, sizeof(nxt_listen_socket_t));
  if (inherited_sockets == NULL) {
    return NXT_ERROR;
  }
...
static nxt_int_t nxt_runtime_event_engines(nxt_task_t* task,
                                           nxt_runtime_t* rt) {
  nxt_thread_t* thread;
  ...
  engine =
      nxt_event_engine_create(task, interface, nxt_main_process_signals, 0, 0);
...
static nxt_int_t nxt_runtime_thread_pools(nxt_thread_t* thr,
                                          nxt_runtime_t* rt) {
  nxt_int_t ret;
...
void nxt_runtime_quit(nxt_task_t* task, nxt_uint_t status) {
  ...
  if (done) {
    nxt_work_queue_add(&engine->fast_work_queue, nxt_runtime_exit, task, rt,
                       engine);
  }
}
...
static void nxt_runtime_close_idle_connections(nxt_event_engine_t* engine) {
  ...
  for (link = nxt_queue_first(idle); link != nxt_queue_tail(idle);
       link = next) {
...
static void nxt_runtime_stop_all_processes(nxt_task_t* task,
                                           nxt_runtime_t* rt) {
  nxt_port_t* port;
  nxt_process_t* process;
...
nxt_int_t nxt_runtime_thread_pool_create(nxt_thread_t* thr,
                                         nxt_runtime_t* rt,
  ...
  thread_pool =
      nxt_thread_pool_create(max_threads, timeout, nxt_runtime_thread_pool_init,
                             thr->engine, nxt_runtime_thread_pool_exit);
...
static nxt_int_t nxt_runtime_conf_read_cmd(nxt_task_t* task,
                                           nxt_runtime_t* rt) {
  char *p, **argv;
  u_char* end;
  u_char buf[1024];
...
nxt_listen_socket_t* nxt_runtime_listen_socket_add(nxt_runtime_t* rt,
  ...
  ls->sockaddr =
      nxt_sockaddr_create(mp, &sa->u.sockaddr, sa->socklen, sa->length);
  if (ls->sockaddr == NULL) {
    return NULL;
  }
...
static nxt_int_t nxt_runtime_lvlhsh_pid_test(nxt_lvlhsh_query_t* lhq,
                                             void* data) {
  nxt_process_t* process;
...
nxt_inline void nxt_runtime_process_lhq_pid(nxt_lvlhsh_query_t* lhq,
                                            nxt_pid_t* pid) {
...
static nxt_process_t* nxt_runtime_process_get(nxt_runtime_t* rt,
                                              nxt_pid_t pid) {
  nxt_process_t* process;
  nxt_lvlhsh_query_t lhq;
...
nxt_port_t* nxt_runtime_process_port_create(nxt_task_t* task,
                                            nxt_runtime_t* rt,
                                            nxt_pid_t pid,
                                            nxt_port_id_t id,
...
nxt_port_t* nxt_runtime_port_find(nxt_runtime_t* rt,
                                  nxt_pid_t pid,
