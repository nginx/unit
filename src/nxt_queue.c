...
nxt_queue_link_t *nxt_queue_middle(nxt_queue_t *queue)
{
...
void nxt_queue_sort(nxt_queue_t *queue,
                    nxt_int_t (*cmp)(const void *data, const nxt_queue_link_t *, const nxt_queue_link_t *),
                    const void *data)
{
    ...
    for (link = nxt_queue_next(link); link != nxt_queue_tail(queue); link = next)
    {
