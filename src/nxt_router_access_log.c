...
static void nxt_router_access_log_writer(nxt_task_t *task, nxt_http_request_t *r, nxt_router_access_log_t *access_log,
                                         nxt_tstr_t *format);
static void nxt_router_access_log_write(nxt_task_t *task, nxt_http_request_t *r, nxt_router_access_log_ctx_t *ctx);
static void nxt_router_access_log_ready(nxt_task_t *task, nxt_port_recv_msg_t *msg, void *data);
static void nxt_router_access_log_error(nxt_task_t *task, nxt_port_recv_msg_t *msg, void *data);
static void nxt_router_access_log_reopen_completion(nxt_task_t *task, void *obj, void *data);
static void nxt_router_access_log_reopen_ready(nxt_task_t *task, nxt_port_recv_msg_t *msg, void *data);
static void nxt_router_access_log_reopen_error(nxt_task_t *task, nxt_port_recv_msg_t *msg, void *data);
...
nxt_int_t nxt_router_access_log_create(nxt_task_t *task, nxt_router_conf_t *rtcf, nxt_conf_value_t *value)
{
    nxt_int_t ret;
    nxt_str_t str;
    nxt_tstr_t *format;
    ...
    {
        ret = nxt_conf_map_object(rtcf->mem_pool, value, nxt_router_access_log_conf,
                                  nxt_nitems(nxt_router_access_log_conf), &alcf);
        if (ret != NXT_OK)
        {
    ...
    {
        access_log = nxt_malloc(sizeof(nxt_router_access_log_t) + alcf.path.length);
        if (access_log == NULL)
        {
        ...
        access_log->path.start = (u_char *)access_log + sizeof(nxt_router_access_log_t);

        nxt_memcpy(access_log->path.start, alcf.path.start, alcf.path.length);
    }

    format = nxt_tstr_compile(rtcf->tstr_state, &alcf.format, NXT_TSTR_LOGGING | NXT_TSTR_NEWLINE);
...
void nxt_router_access_log_open(nxt_task_t *task, nxt_router_temp_conf_t *tmcf)
{
    ...
    *b->mem.free++ = '\0';
    ...
    stream = nxt_port_rpc_register_handler(task, router_port, nxt_router_access_log_ready, nxt_router_access_log_error,
                                           -1, tmcf);
    ...
    ret = nxt_port_socket_write(task, main_port, NXT_PORT_MSG_ACCESS_LOG, -1, stream, router_port->id, b);
...
void nxt_router_access_log_use(nxt_thread_spinlock_t *lock, nxt_router_access_log_t *access_log)
{
    if (access_log == NULL)
    {
        return;
...
void nxt_router_access_log_release(nxt_task_t *task, nxt_thread_spinlock_t *lock, nxt_router_access_log_t *access_log)
{
    if (access_log == NULL)
    {
        return;
...
void nxt_router_access_log_reopen_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
    nxt_mp_t *mp;
    uint32_t stream;
    nxt_int_t ret;
    ...
    *b->mem.free++ = '\0';
    ...
    stream = nxt_port_rpc_register_handler(task, router_port, nxt_router_access_log_reopen_ready,
                                           nxt_router_access_log_reopen_error, -1, reopen);
    if (nxt_slow_path(stream == 0))
    {
        goto fail;
    }
...
static void nxt_router_access_log_reopen_ready(nxt_task_t *task, nxt_port_recv_msg_t *msg, void *data)
{
    ...
    if (access_log == nxt_router->access_log)
    {
        ...
        {
            nxt_alert(task, "dup2(%FD, %FD) failed %E", msg->fd[0], access_log->fd, nxt_errno);
        }
    }
