...
static ssize_t
nxt_sys_sendfile (int out_fd, int in_fd, off_t *offset, size_t count)
{
  return -1;
}
      ...
      switch (err)
        ...
        default:
          ...
          nxt_log (c->socket.task, nxt_socket_error_level (err),
                   ...
                   c->socket.fd, fb->file->fd, fb->file_pos, size, err,
                   fb->file->name);
...
nxt_linux_send (nxt_event_conn_t *c, void *buf, size_t size, nxt_uint_t flags)
{
  ...
  nxt_debug (c->socket.task, "send(%d, %p, %uz, 0x%uXi): %z", c->socket.fd,
             buf, size, flags, n);
  ...
  if (n == -1)
    {
      switch (err)
        ...
        default:
          c->socket.error = err;
          nxt_log (c->socket.task, nxt_socket_error_level (err),
                   "send(%d, %p, %uz, 0x%uXi) failed %E", c->socket.fd, buf,
                   size, flags, err);
                   ...
                   nxt_uint_t niov, nxt_uint_t flags)
{
  ...
  nxt_debug (c->socket.task, "sendmsg(%d, %ui, 0x%uXi): %z", c->socket.fd,
             niov, flags, n);
  ...
  if (n == -1)
    {
      switch (err)
        ...
        default:
          c->socket.error = err;
          nxt_log (c->socket.task, nxt_socket_error_level (err),
                   "sendmsg(%d, %ui, 0x%uXi) failed %E", c->socket.fd, niov,
                   flags, err);
