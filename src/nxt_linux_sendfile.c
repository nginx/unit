...
ssize_t nxt_linux_event_conn_io_sendfile(nxt_event_conn_t* c,
                                         nxt_buf_t* b,
                                         size_t limit);

static ssize_t nxt_sys_sendfile(int out_fd,
                                int in_fd,
                                off_t* offset,
                                size_t count) {
  return -1;
...
static ssize_t nxt_linux_send(nxt_event_conn_t* c,
                              void* buf,
                              size_t size,
                              nxt_uint_t flags);
...
ssize_t nxt_linux_event_conn_io_sendfile(nxt_event_conn_t* c,
  ...
  nxt_debug(c->socket.task, "sendfile(%d, %FD, @%O, %uz)", c->socket.fd,
            fb->file->fd, fb->file_pos, size);
    ...
    switch (err) {
      ...
      default:
        ...
        nxt_log(c->socket.task, nxt_socket_error_level(err),
                "sendfile(%d, %FD, %O, %uz) failed %E \"%FN\"", c->socket.fd,
                fb->file->fd, fb->file_pos, size, err, fb->file->name);
...
static ssize_t nxt_linux_send(nxt_event_conn_t* c,
  ...
  nxt_debug(c->socket.task, "send(%d, %p, %uz, 0x%uXi): %z", c->socket.fd, buf,
            size, flags, n);
  ...
  if (n == -1) {
    switch (err) {
      ...
      default:
        c->socket.error = err;
        nxt_log(c->socket.task, nxt_socket_error_level(err),
                "send(%d, %p, %uz, 0x%uXi) failed %E", c->socket.fd, buf, size,
                flags, err);
...
static ssize_t nxt_linux_sendmsg(nxt_event_conn_t* c,
  ...
  nxt_debug(c->socket.task, "sendmsg(%d, %ui, 0x%uXi): %z", c->socket.fd, niov,
            flags, n);
  ...
  if (n == -1) {
    switch (err) {
      ...
      default:
        c->socket.error = err;
        nxt_log(c->socket.task, nxt_socket_error_level(err),
                "sendmsg(%d, %ui, 0x%uXi) failed %E", c->socket.fd, niov, flags,
                err);
