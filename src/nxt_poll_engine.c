...
static nxt_int_t nxt_poll_create(nxt_event_engine_t *engine, nxt_uint_t mchanges, nxt_uint_t mevents);
...
static void nxt_poll_disable(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static nxt_bool_t nxt_poll_close(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_poll_enable_read(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_poll_enable_write(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_poll_disable_read(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_poll_disable_write(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_poll_block_read(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_poll_block_write(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_poll_oneshot_read(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_poll_oneshot_write(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_poll_change(nxt_event_engine_t *engine, nxt_fd_event_t *ev, nxt_uint_t op, nxt_uint_t events);
static nxt_int_t nxt_poll_commit_changes(nxt_event_engine_t *engine);
static nxt_int_t nxt_poll_set_add(nxt_event_engine_t *engine, nxt_fd_event_t *ev, int events);
static nxt_int_t nxt_poll_set_change(nxt_event_engine_t *engine, nxt_fd_t fd, int events);
...
static nxt_poll_hash_entry_t *nxt_poll_fd_hash_get(nxt_event_engine_t *engine, nxt_fd_t fd);
static nxt_int_t nxt_poll_fd_hash_test(nxt_lvlhsh_query_t *lhq, void *data);
static void nxt_poll_fd_hash_destroy(nxt_event_engine_t *engine, nxt_lvlhsh_t *lh);
...
static void nxt_poll_oneshot_read(nxt_event_engine_t *engine, nxt_fd_event_t *ev)
{
    ...
    op = (ev->read == NXT_EVENT_INACTIVE && ev->write == NXT_EVENT_INACTIVE) ? NXT_POLL_ADD : NXT_POLL_CHANGE;
...
static nxt_int_t nxt_poll_set_change(nxt_event_engine_t *engine, nxt_fd_t fd, int events)
{
    ...
    nxt_debug(&engine->task, "poll change event: fd:%d ev:%04Xi", fd, events);
...
static void nxt_poll(nxt_event_engine_t *engine, nxt_msec_t timeout)
{
    ...
    nxt_debug(&engine->task, "poll() events:%ui timeout:%M", engine->u.poll.nfds, timeout);
    ...
    for (i = 0; i < engine->u.poll.nfds && nevents != 0; i++)
    {
        ...
        if (nxt_slow_path(phe == NULL))
        {
            nxt_alert(&engine->task, "poll() returned invalid fd:%d ev:%04Xd rev:%04uXi", fd, pfd->events, events);
        ...
        nxt_debug(ev->task, "poll: fd:%d ev:%04uXi rd:%d wr:%d", fd, events, ev->read, ev->write);

        if (nxt_slow_path((events & POLLNVAL) != 0))
        {
            nxt_alert(ev->task, "poll() error fd:%d ev:%04Xd rev:%04uXi", fd, pfd->events, events);
        ...
        error = (((events & (POLLERR | POLLHUP)) != 0) && ((events & (POLLIN | POLLOUT)) == 0));
...
static nxt_int_t nxt_poll_fd_hash_test(nxt_lvlhsh_query_t *lhq, void *data)
{
    ...
    nxt_alert(&engine->task, "fd %d in hash mismatches fd %d in poll set", phe->fd, engine->u.poll.set[phe->index].fd);
