...
static nxt_int_t nxt_poll_set_change(nxt_event_engine_t *engine, nxt_fd_t fd,
                                     int events);
...
static void nxt_poll_block_read(nxt_event_engine_t *engine,
                                nxt_fd_event_t *ev) {
  if (ev->read != NXT_EVENT_INACTIVE) {
...
static void nxt_poll(nxt_event_engine_t *engine, nxt_msec_t timeout) {
  ...
  nxt_debug(&engine->task, "poll() events:%ui timeout:%M", engine->u.poll.nfds,
            timeout);
  ...
  for (i = 0; i < engine->u.poll.nfds && nevents != 0; i++) {
    ...
    if (nxt_slow_path(phe == NULL)) {
      nxt_alert(&engine->task,
                "poll() returned invalid fd:%d ev:%04Xd rev:%04uXi", fd,
                pfd->events, events);
    ...
    nxt_debug(ev->task, "poll: fd:%d ev:%04uXi rd:%d wr:%d", fd, events,
              ev->read, ev->write);

    if (nxt_slow_path((events & POLLNVAL) != 0)) {
      nxt_alert(ev->task, "poll() error fd:%d ev:%04Xd rev:%04uXi", fd,
                pfd->events, events);
    ...
    if ((events & POLLOUT) || (error && ev->write_handler != NULL)) {
      ...
      nxt_work_queue_add(ev->write_work_queue, ev->write_handler, ev->task, ev,
                         ev->data);
    }
