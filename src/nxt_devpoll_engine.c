...
static nxt_int_t nxt_devpoll_create(nxt_event_engine_t *engine, nxt_uint_t mchanges, nxt_uint_t mevents);
...
static nxt_bool_t nxt_devpoll_close(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_devpoll_enable_read(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_devpoll_enable_write(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_devpoll_disable_read(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_devpoll_disable_write(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_devpoll_block_read(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_devpoll_block_write(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_devpoll_oneshot_read(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_devpoll_oneshot_write(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_devpoll_change(nxt_event_engine_t *engine, nxt_fd_event_t *ev, nxt_uint_t op, nxt_uint_t events);
static nxt_int_t nxt_devpoll_commit_changes(nxt_event_engine_t *engine);
static void nxt_devpoll_change_error(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_devpoll_remove(nxt_event_engine_t *engine, nxt_fd_t fd);
static nxt_int_t nxt_devpoll_write(nxt_event_engine_t *engine, struct pollfd *pfd, size_t n);
static void nxt_devpoll_poll(nxt_event_engine_t *engine, nxt_msec_t timeout);
...
static void nxt_devpoll_change(nxt_event_engine_t *engine, nxt_fd_event_t *ev, nxt_uint_t op, nxt_uint_t events)
{
    ...
    nxt_debug(ev->task, "devpoll %d change fd:%d op:%ui ev:%04Xi", engine->u.devpoll.fd, ev->fd, op, events);
...
static nxt_int_t nxt_devpoll_commit_changes(nxt_event_engine_t *engine)
{
    ...
    nxt_debug(&engine->task, "devpoll %d changes:%ui", engine->u.devpoll.fd, engine->u.devpoll.nchanges);
    ...
    do
    {
        ...
        nxt_debug(&engine->task, "devpoll fd:%d op:%d ev:%04Xd", ev->fd, change->op, change->events);
    ...
    do
    {
        ...
        {
            nxt_fd_event_hash_delete(&engine->task, &engine->u.devpoll.fd_hash, ev->fd, 0);
        }
...
static void nxt_devpoll_remove(nxt_event_engine_t *engine, nxt_fd_t fd)
{
    ...
    nxt_debug(&engine->task, "ioctl(%d, DP_ISPOLLED, %d): %d", engine->u.devpoll.fd, fd, n);
    ...
    if (n == -1)
    {
        nxt_alert(&engine->task, "ioctl(%d, DP_ISPOLLED, %d) failed %E", engine->u.devpoll.fd, fd, nxt_errno);
        /* Fall through. */
    }

    /* n == 1: the file descriptor is in the set. */

    nxt_debug(&engine->task, "devpoll %d remove fd:%d", engine->u.devpoll.fd, fd);
...
static void nxt_devpoll_poll(nxt_event_engine_t *engine, nxt_msec_t timeout)
{
    ...
    nxt_debug(&engine->task, "ioctl(%d, DP_POLL) timeout:%M", engine->u.devpoll.fd, timeout);
    ...
    nxt_debug(&engine->task, "ioctl(%d, DP_POLL): %d", engine->u.devpoll.fd, nevents);

    if (nevents == -1)
    {
        level = (err == NXT_EINTR) ? NXT_LOG_INFO : NXT_LOG_ALERT;

        nxt_log(&engine->task, level, "ioctl(%d, DP_POLL) failed %E", engine->u.devpoll.fd, err);
    ...
    for (i = 0; i < nevents; i++)
    {
        ...
        ev = nxt_fd_event_hash_get(&engine->task, &engine->u.devpoll.fd_hash, fd);
        ...
        nxt_debug(ev->task, "devpoll: fd:%d ev:%04uXi rd:%d wr:%d", fd, events, ev->read, ev->write);

        if (nxt_slow_path(events & (POLLERR | POLLHUP | POLLNVAL)) != 0)
        {
            nxt_alert(ev->task, "ioctl(%d, DP_POLL) error fd:%d ev:%04Xd rev:%04uXi", engine->u.devpoll.fd, fd,
                      pfd->events, events);
