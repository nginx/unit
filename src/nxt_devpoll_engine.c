...
static void nxt_devpoll_oneshot_write(nxt_event_engine_t* engine,
                                      nxt_fd_event_t* ev);
static void nxt_devpoll_change(nxt_event_engine_t* engine,
                               nxt_fd_event_t* ev,
                               nxt_uint_t op,
                               nxt_uint_t events);
...
static void nxt_devpoll_disable(nxt_event_engine_t* engine,
                                nxt_fd_event_t* ev) {
...
static nxt_int_t nxt_devpoll_commit_changes(nxt_event_engine_t* engine) {
  ...
  do {
    ...
    nxt_debug(&engine->task, "devpoll fd:%d op:%d ev:%04Xd", ev->fd, change->op,
              change->events);
...
static void nxt_devpoll_poll(nxt_event_engine_t* engine, nxt_msec_t timeout) {
  ...
  for (i = 0; i < nevents; i++) {
    ...
    if (nxt_slow_path(events & (POLLERR | POLLHUP | POLLNVAL)) != 0) {
      nxt_alert(ev->task, "ioctl(%d, DP_POLL) error fd:%d ev:%04Xd rev:%04uXi",
                engine->u.devpoll.fd, fd, pfd->events, events);
