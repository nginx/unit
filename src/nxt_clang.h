...
nxt_inline int nxt_popcount(unsigned int x)
{
    int count;
...
#define nxt_alloca(size) alloca(size)

#define nxt_container_of(p, type, field) (type *)((u_char *)(p) - offsetof(type, field))

#define nxt_pointer_to(p, offset) ((void *)((char *)(p) + (offset)))

#define nxt_value_at(type, p, offset) *(type *)((u_char *)p + offset)

#define nxt_nitems(x) (sizeof(x) / sizeof((x)[0]))

/* GCC and Clang use __builtin_abs() instead of libc abs(). */

#define nxt_abs(val) abs(val)

#define nxt_max(val1, val2) ((val1 < val2) ? (val2) : (val1))

#define nxt_min(val1, val2) ((val1 > val2) ? (val2) : (val1))

#define nxt_bswap32(val) (((val) >> 24) | (((val) & 0x00FF0000) >> 8) | (((val) & 0x0000FF00) << 8) | ((val) << 24))

#define nxt_is_power_of_two(value) ((((value) - 1) & (value)) == 0)

#define nxt_align_size(d, a) (((d) + ((size_t)(a) - 1)) & ~((size_t)(a) - 1))

#define nxt_align_ptr(p, a) (u_char *)(((uintptr_t)(p) + ((uintptr_t)(a) - 1)) & ~((uintptr_t)(a) - 1))

#define nxt_trunc_ptr(p, a) (u_char *)((uintptr_t)(p) & ~((uintptr_t)(a) - 1))
