...
static nxt_int_t
nxt_ssl_conf_commands(nxt_task_t *task, SSL_CTX *ctx, nxt_conf_value_t *value,
                      nxt_mp_t *mp);
#endif
                        ...
                        nxt_tls_init_t *tls_init, nxt_bool_t last)
{
    ...
    */
    ...
    if (tls_init->conf_cmds != NULL
        && nxt_ssl_conf_commands(task, ctx, tls_init->conf_cmds, mp)
               != NXT_OK) {
        goto fail;
    }
#endif
                            ...
                            EVP_CIPHER_CTX *ectx, HMAC_CTX *hctx, int enc)
{
    ...
    found:
        ...
        if (EVP_DecryptInit_ex(ectx, cipher, NULL, ticket[i].aes_key, iv)
            != 1) {
            nxt_openssl_log_error(c->socket.task, NXT_LOG_ALERT,
