...
#endif
static nxt_int_t nxt_openssl_server_init(nxt_task_t *task, nxt_mp_t *mp, nxt_tls_init_t *tls_init, nxt_bool_t last);
static nxt_int_t nxt_openssl_chain_file(nxt_task_t *task, SSL_CTX *ctx, nxt_tls_conf_t *conf, nxt_mp_t *mp,
                                        nxt_bool_t single);
#if (NXT_HAVE_OPENSSL_CONF_CMD)
static nxt_int_t nxt_ssl_conf_commands(nxt_task_t *task, SSL_CTX *ctx, nxt_conf_value_t *value, nxt_mp_t *mp);
#endif
#if (NXT_HAVE_OPENSSL_TLSEXT)
static nxt_int_t nxt_tls_ticket_keys(nxt_task_t *task, SSL_CTX *ctx, nxt_tls_init_t *tls_init, nxt_mp_t *mp);
static int nxt_tls_ticket_key_callback(SSL *s, unsigned char *name, unsigned char *iv, EVP_CIPHER_CTX *ectx,
                                       HMAC_CTX *hctx, int enc);
#endif
static void nxt_ssl_session_cache(SSL_CTX *ctx, size_t cache_size, time_t timeout);
static nxt_uint_t nxt_openssl_cert_get_names(nxt_task_t *task, X509 *cert, nxt_tls_conf_t *conf, nxt_mp_t *mp);
static nxt_int_t nxt_openssl_bundle_hash_test(nxt_lvlhsh_query_t *lhq, void *data);
static nxt_int_t nxt_openssl_bundle_hash_insert(nxt_task_t *task, nxt_lvlhsh_t *lvlhsh,
                                                nxt_tls_bundle_hash_item_t *item, nxt_mp_t *mp);
static nxt_int_t nxt_openssl_servername(SSL *s, int *ad, void *arg);
static nxt_tls_bundle_conf_t *nxt_openssl_find_ctx(nxt_tls_conf_t *conf, nxt_str_t *sn);
static void nxt_openssl_server_free(nxt_task_t *task, nxt_tls_conf_t *conf);
static void nxt_openssl_conn_init(nxt_task_t *task, nxt_tls_conf_t *conf, nxt_conn_t *c);
...
static ssize_t nxt_openssl_conn_io_send(nxt_task_t *task, nxt_sendbuf_t *sb, void *buf, size_t size);
static void nxt_openssl_conn_io_shutdown(nxt_task_t *task, void *obj, void *data);
static nxt_int_t nxt_openssl_conn_test_error(nxt_task_t *task, nxt_conn_t *c, int ret, nxt_err_t sys_err,
                                             nxt_openssl_io_t io);
static void nxt_openssl_conn_io_shutdown_timeout(nxt_task_t *task, void *obj, void *data);
static void nxt_cdecl nxt_openssl_conn_error(nxt_task_t *task, nxt_err_t err, const char *fmt, ...);
...
static nxt_int_t nxt_openssl_library_init(nxt_task_t *task)
{
    ...
    nxt_log(task, NXT_LOG_INFO, "%s, %xl", SSLeay_version(SSLEAY_VERSION), nxt_openssl_version);
    ...
    if (index == -1)
    {
        nxt_openssl_log_error(task, NXT_LOG_ALERT, "SSL_get_ex_new_index() failed");
        return NXT_ERROR;
    }
...
static u_long nxt_openssl_thread_id(void)
{
...
static nxt_int_t nxt_openssl_server_init(nxt_task_t *task, nxt_mp_t *mp, nxt_tls_init_t *tls_init, nxt_bool_t last)
{
    ...
    if (nxt_openssl_chain_file(task, ctx, conf, mp, last && bundle->next == NULL) != NXT_OK)
    {
        goto fail;
    }
    /*
    ...
    */
    ...
    if (conf->ciphers)
    { /* else use system crypto policy */
        if (SSL_CTX_set_cipher_list(ctx, conf->ciphers) == 0)
        {
            nxt_openssl_log_error(task, NXT_LOG_ALERT, "SSL_CTX_set_cipher_list(\"%s\") failed", conf->ciphers);
            goto fail;
        }
    }

#if (NXT_HAVE_OPENSSL_CONF_CMD)
    if (tls_init->conf_cmds != NULL && nxt_ssl_conf_commands(task, ctx, tls_init->conf_cmds, mp) != NXT_OK)
    {
        goto fail;
    }
#endif
    ...
    if (conf->ca_certificate != NULL)
    {
        ...
        {
            nxt_openssl_log_error(task, NXT_LOG_ALERT, "SSL_CTX_load_verify_locations(\"%s\") failed", ca_certificate);
            goto fail;
        }
        ...
        if (list == NULL)
        {
            nxt_openssl_log_error(task, NXT_LOG_ALERT, "SSL_load_client_CA_file(\"%s\") failed", ca_certificate);
            goto fail;
        }
...
#if (OPENSSL_VERSION_NUMBER >= 0x1010100fL && OPENSSL_VERSION_NUMBER < 0x1010101fL)
                                        ...
                                        nxt_bool_t single)
{
    ...
    if (ret != NXT_OK)
    {
        nxt_openssl_log_error(task, NXT_LOG_ALERT, "nxt_openssl_chain_file() failed");
    }
...
static nxt_int_t nxt_ssl_conf_commands(nxt_task_t *task, SSL_CTX *ctx, nxt_conf_value_t *conf, nxt_mp_t *mp)
{
    int ret;
    char *zcmd, *zvalue;
    uint32_t index;
    ...
    {
        nxt_openssl_log_error(task, NXT_LOG_ALERT, "SSL_CONF_CTX_new() failed");
        return NXT_ERROR;
    }
    ...
    {
        nxt_openssl_log_error(task, NXT_LOG_ALERT, "SSL_CONF_finish() failed");
        goto fail;
    }
...
static nxt_int_t nxt_tls_ticket_keys(nxt_task_t *task, SSL_CTX *ctx, nxt_tls_init_t *tls_init, nxt_mp_t *mp)
{
    ...
    tickets = nxt_mp_get(mp, sizeof(nxt_tls_tickets_t) + count * sizeof(nxt_tls_ticket_t));
    ...
    if (SSL_CTX_set_tlsext_ticket_key_cb(ctx, nxt_tls_ticket_key_callback) == 0)
    {
        nxt_openssl_log_error(task, NXT_LOG_ALERT,
                                       ...
                                       HMAC_CTX *hctx, int enc)
{
    ...
    if (enc == 1)
    {
        ...
        {
            nxt_openssl_log_error(c->socket.task, NXT_LOG_ALERT, "RAND_bytes() failed");
            return -1;
        }
        ...
        {
            nxt_openssl_log_error(c->socket.task, NXT_LOG_ALERT, "EVP_EncryptInit_ex() failed");
            return -1;
        }
    }
    else
    ...
    found:
        ...
        nxt_debug(c->socket.task, "TLS session ticket decrypt, key number: \"%d\"", i);
        ...
        {
            nxt_openssl_log_error(c->socket.task, NXT_LOG_ALERT, "EVP_DecryptInit_ex() failed");
            return -1;
        }
    }
    ...
    if (HMAC_Init_ex(hctx, ticket[i].hmac_key, ticket[i].size, digest, NULL) != 1)
    {
        nxt_openssl_log_error(c->socket.task, NXT_LOG_ALERT, "HMAC_Init_ex() failed");
        return -1;
    }
...
static nxt_uint_t nxt_openssl_cert_get_names(nxt_task_t *task, X509 *cert, nxt_tls_conf_t *conf, nxt_mp_t *mp)
{
    ...
    if (alt_names != NULL)
    {
        ...
        for (i = 0; i != n; i++)
        {
            ...
            if (nxt_openssl_bundle_hash_insert(task, &conf->bundle_hash, item, mp) == NXT_ERROR)
            {
                goto fail;
            }
        }
    ...
    else
    {
        ...
        len = X509_NAME_get_text_by_NID(x509_name, NID_commonName, NULL, 0);
        if (len <= 0)
        {
            nxt_log(task, NXT_LOG_WARN,
                    "certificate \"%V\" has neither "
                    "Subject Alternative Name nor Common Name",
                    &bundle->name);
            return NXT_OK;
        }
        ...
        domain.length = X509_NAME_get_text_by_NID(x509_name, NID_commonName, (char *)domain.start, len + 1);
...
static const nxt_lvlhsh_proto_t nxt_openssl_bundle_hash_proto nxt_aligned(64) = {
    NXT_LVLHSH_DEFAULT,
                                                ...
                                                nxt_tls_bundle_hash_item_t *item, nxt_mp_t *mp)
{
    ...
    if (item->name.start[0] == '*')
    {
        ...
        {
            nxt_log(task, NXT_LOG_WARN,
                    "ignored invalid name \"%V\" "
                    "in certificate \"%V\": missing \".\" "
                    "after wildcard symbol",
    ...
    {
        nxt_debug(task, "name \"%V\" for certificate \"%V\" is inserted", &str, &item->bundle->name);
        return NXT_OK;
    }
...
static nxt_int_t nxt_openssl_servername(SSL *s, int *ad, void *arg)
{
    ...
    {
        nxt_debug(c->socket.task,
                  "new tls context found for \"%V\": \"%V\" "
                  "(old: \"%V\")",
                  &str, &bundle->name, &conf->bundle->name);

        if (bundle != conf->bundle)
        {
            if (SSL_set_SSL_CTX(s, bundle->ctx) == NULL)
            {
                nxt_openssl_log_error(c->socket.task, NXT_LOG_ALERT, "SSL_set_SSL_CTX() failed");
...
static void nxt_openssl_server_free(nxt_task_t *task, nxt_tls_conf_t *conf)
{
    ...
    if (conf->tickets)
    {
        nxt_memzero(conf->tickets->tickets, conf->tickets->count * sizeof(nxt_tls_ticket_t));
    }
...
static void nxt_openssl_conn_init(nxt_task_t *task, nxt_tls_conf_t *conf, nxt_conn_t *c)
{
    ...
    if (ret == 0)
    {
        nxt_openssl_log_error(task, NXT_LOG_ALERT, "SSL_set_fd(%d) failed", c->socket.fd);
        goto fail;
    }
...
fail:
    ...
    nxt_work_queue_add(c->read_work_queue, c->read_state->error_handler, task, c, c->socket.data);
}

nxt_inline void nxt_openssl_conn_free(nxt_task_t *task, nxt_conn_t *c)
{
    nxt_openssl_conn_t *tls;
...
static void nxt_openssl_conn_handshake(nxt_task_t *task, void *obj, void *data)
{
    ...
    else
    {
        ...
        n = nxt_openssl_conn_test_error(task, c, ret, err, NXT_OPENSSL_HANDSHAKE);
        switch (n)
        {
        ...
        default:
        case NXT_ERROR:
            nxt_openssl_conn_error(task, err, "SSL_do_handshake(%d) failed", c->socket.fd);
...
static ssize_t nxt_openssl_conn_io_recvbuf(nxt_conn_t *c, nxt_buf_t *b)
{
    ...
    nxt_debug(c->socket.task, "SSL_read(%d, %p, %uz): %d err:%d", c->socket.fd, b->mem.free, size, ret, err);
    ...
    n = nxt_openssl_conn_test_error(c->socket.task, c, ret, err, NXT_OPENSSL_READ);
    if (n == NXT_ERROR)
    {
        nxt_openssl_conn_error(c->socket.task, err, "SSL_read(%d, %p, %uz) failed", c->socket.fd, b->mem.free, size);
    }
...
static ssize_t nxt_openssl_conn_io_send(nxt_task_t *task, nxt_sendbuf_t *sb, void *buf, size_t size)
{
    ...
    nxt_debug(task, "SSL_write(%d, %p, %uz): %d err:%d", sb->socket, buf, size, ret, err);
    ...
    if (n == NXT_ERROR)
    {
        ...
        nxt_openssl_conn_error(task, err, "SSL_write(%d, %p, %uz) failed", sb->socket, buf, size);
    }
...
static void nxt_openssl_conn_io_shutdown(nxt_task_t *task, void *obj, void *data)
{
    ...
    for (;;)
    {
        ...
        nxt_debug(task, "SSL_shutdown(%d, %d, %b): %d err:%d", c->socket.fd, mode, quiet, ret, err);
    ...
    default:
    case NXT_ERROR:
        nxt_openssl_conn_error(task, err, "SSL_shutdown(%d) failed", c->socket.fd);
...
void nxt_cdecl nxt_openssl_log_error(nxt_task_t *task, nxt_uint_t level, const char *fmt, ...)
{
    u_char *p, *end;
    va_list args;
    u_char msg[NXT_MAX_ERROR_STR];
