...
static nxt_int_t nxt_openssl_server_init(nxt_task_t* task,
                                         ...
                                         nxt_bool_t last);
static nxt_int_t nxt_openssl_chain_file(nxt_task_t* task,
                                        SSL_CTX* ctx,
                                        nxt_tls_conf_t* conf,
                                        nxt_mp_t* mp,
                                        nxt_bool_t single);
#if (NXT_HAVE_OPENSSL_CONF_CMD)
static nxt_int_t nxt_ssl_conf_commands(nxt_task_t* task,
                                       SSL_CTX* ctx,
                                       nxt_conf_value_t* value,
                                       nxt_mp_t* mp);
#endif
#if (NXT_HAVE_OPENSSL_TLSEXT)
static nxt_int_t nxt_tls_ticket_keys(nxt_task_t* task,
                                     SSL_CTX* ctx,
                                     nxt_tls_init_t* tls_init,
                                     nxt_mp_t* mp);
static int nxt_tls_ticket_key_callback(SSL* s,
                                       unsigned char* name,
                                       unsigned char* iv,
                                       EVP_CIPHER_CTX* ectx,
                                       HMAC_CTX* hctx,
                                       int enc);
#endif
static void nxt_ssl_session_cache(SSL_CTX* ctx,
                                  size_t cache_size,
                                  time_t timeout);
static nxt_uint_t nxt_openssl_cert_get_names(nxt_task_t* task,
                                             X509* cert,
                                             nxt_tls_conf_t* conf,
                                             nxt_mp_t* mp);
static nxt_int_t nxt_openssl_bundle_hash_test(nxt_lvlhsh_query_t* lhq,
                                              void* data);
static nxt_int_t nxt_openssl_bundle_hash_insert(
    nxt_task_t* task,
    nxt_lvlhsh_t* lvlhsh,
...
static void nxt_openssl_conn_init(nxt_task_t* task,
                                  nxt_tls_conf_t* conf,
                                  nxt_conn_t* c);
...
static ssize_t nxt_openssl_conn_io_send(nxt_task_t* task,
                                        nxt_sendbuf_t* sb,
                                        void* buf,
                                        size_t size);
static void nxt_openssl_conn_io_shutdown(nxt_task_t* task,
                                         void* obj,
                                         void* data);
static nxt_int_t nxt_openssl_conn_test_error(nxt_task_t* task,
                                             ...
                                             nxt_err_t sys_err,
                                             nxt_openssl_io_t io);
static void nxt_openssl_conn_io_shutdown_timeout(nxt_task_t* task,
                                                 void* obj,
                                                 void* data);
...
static nxt_int_t nxt_openssl_library_init(nxt_task_t* task) {
  ...
  nxt_log(task, NXT_LOG_INFO, "%s, %xl", SSLeay_version(SSLEAY_VERSION),
          nxt_openssl_version);
...
static nxt_int_t nxt_tls_ticket_keys(nxt_task_t* task,
  ...
  tickets = nxt_mp_get(
      mp, sizeof(nxt_tls_tickets_t) + count * sizeof(nxt_tls_ticket_t));
...
#else
  nxt_alert(task,
            "Setting custom session ticket keys is not supported with "
...
static int nxt_tls_ticket_key_callback(SSL* s,
  ...
  found:
    ...
    nxt_debug(c->socket.task, "TLS session ticket decrypt, key number: \"%d\"",
              i);
...
static nxt_uint_t nxt_openssl_cert_get_names(nxt_task_t* task,
  ...
  if (alt_names != NULL) {
    ...
    for (i = 0; i != n; i++) {
      ...
      if (nxt_openssl_bundle_hash_insert(task, &conf->bundle_hash, item, mp) ==
          NXT_ERROR) {
        goto fail;
      }
    }
...
static nxt_int_t nxt_openssl_bundle_hash_insert(
  ...
  if (nxt_fast_path(ret == NXT_OK)) {
    nxt_debug(task, "name \"%V\" for certificate \"%V\" is inserted", &str,
              &item->bundle->name);
    return NXT_OK;
  }
...
fail:
  ...
  nxt_work_queue_add(c->read_work_queue, c->read_state->error_handler, task, c,
                     c->socket.data);
}
...
static void nxt_openssl_conn_handshake(nxt_task_t* task,
                                       void* obj,
                                       void* data) {
  int ret;
  nxt_int_t n;
...
static ssize_t nxt_openssl_conn_io_recvbuf(nxt_conn_t* c, nxt_buf_t* b) {
  ...
  nxt_debug(c->socket.task, "SSL_read(%d, %p, %uz): %d err:%d", c->socket.fd,
            b->mem.free, size, ret, err);
  ...
  n = nxt_openssl_conn_test_error(c->socket.task, c, ret, err,
                                  NXT_OPENSSL_READ);
  if (n == NXT_ERROR) {
    nxt_openssl_conn_error(c->socket.task, err, "SSL_read(%d, %p, %uz) failed",
...
static ssize_t nxt_openssl_conn_io_sendbuf(nxt_task_t* task,
                                           nxt_sendbuf_t* sb) {
  nxt_uint_t niov;
  struct iovec iov;
...
static ssize_t nxt_openssl_conn_io_send(nxt_task_t* task,
  ...
  nxt_debug(task, "SSL_write(%d, %p, %uz): %d err:%d", sb->socket, buf, size,
            ret, err);
...
static void nxt_openssl_conn_io_shutdown(nxt_task_t* task,
  ...
  for (;;) {
    ...
    nxt_debug(task, "SSL_shutdown(%d, %d, %b): %d err:%d", c->socket.fd, mode,
              quiet, ret, err);
...
void nxt_cdecl nxt_openssl_log_error(nxt_task_t* task,
                                     nxt_uint_t level,
                                     const char* fmt,
                                     ...) {
  u_char *p, *end;
