      ...
      if (SSL_CTX_load_verify_locations (ctx, ca_certificate, NULL) == 0)
        {
          nxt_openssl_log_error (
              task, NXT_LOG_ALERT,
              "SSL_CTX_load_verify_locations(\"%s\") failed", ca_certificate);
          goto fail;
        }
  ...
  if (enc == 1)
    {
      ...
      cipher
          = (ticket[0].size == 16) ? EVP_aes_128_cbc () : EVP_aes_256_cbc ();
    ...
    found:
      ...
      cipher
          = (ticket[i].size == 16) ? EVP_aes_128_cbc () : EVP_aes_256_cbc ();
                            ...
                            nxt_mp_t *mp)
{
  ...
  if (alt_names != NULL)
  ...
  else
    {
      ...
      domain.length = X509_NAME_get_text_by_NID (
          x509_name, NID_commonName, (char *)domain.start, len + 1);
...
fail:
  ...
  nxt_work_queue_add (c->read_work_queue, c->read_state->error_handler, task,
                      c, c->socket.data);
}
  ...
  if (n == NXT_ERROR)
    {
      nxt_openssl_conn_error (c->socket.task, err,
                              "SSL_read(%d, %p, %uz) failed", c->socket.fd,
                              b->mem.free, size);
    }
  ...
  for (;;)
    {
      ...
      nxt_debug (task, "SSL_shutdown(%d, %d, %b): %d err:%d", c->socket.fd,
                 mode, quiet, ret, err);
