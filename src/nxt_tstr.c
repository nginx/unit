...
nxt_tstr_t *nxt_tstr_compile(nxt_tstr_state_t *state, const nxt_str_t *str, nxt_tstr_flags_t flags)
{
    u_char *p;
    nxt_tstr_t *tstr;
...
nxt_int_t nxt_tstr_cond_compile(nxt_tstr_state_t *state, nxt_str_t *str, nxt_tstr_cond_t *cond)
{
...
nxt_int_t nxt_tstr_query_init(nxt_tstr_query_t **query_p, nxt_tstr_state_t *state, nxt_tstr_cache_t *cache, void *ctx,
                              nxt_mp_t *mp)
{
...
nxt_int_t nxt_tstr_query(nxt_task_t *task, nxt_tstr_query_t *query, nxt_tstr_t *tstr, nxt_str_t *val)
{
    nxt_int_t ret;
    ...
    {
        ret = nxt_var_interpreter(task, query->state, &query->cache->var, tstr->u.var, val, query->ctx,
    ...
    {
#if (NXT_HAVE_NJS)
        ret = nxt_js_call(task, query->state->jcf, &query->cache->js, tstr->u.js, val, query->ctx);
