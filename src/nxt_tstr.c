...
nxt_tstr_t* nxt_tstr_compile(nxt_tstr_state_t* state,
                             const nxt_str_t* str,
                             nxt_tstr_flags_t flags) {
  u_char* p;
  nxt_tstr_t* tstr;
...
nxt_int_t nxt_tstr_test(nxt_tstr_state_t* state,
                        nxt_str_t* str,
                        u_char* error) {
  u_char* p;
...
nxt_int_t nxt_tstr_cond_compile(nxt_tstr_state_t* state,
                                nxt_str_t* str,
...
nxt_int_t nxt_tstr_query(nxt_task_t* task,
                         nxt_tstr_query_t* query,
                         nxt_tstr_t* tstr,
                         nxt_str_t* val) {
  nxt_int_t ret;
  ...
  if (tstr->type == NXT_TSTR_VAR) {
    ret =
        nxt_var_interpreter(task, query->state, &query->cache->var, tstr->u.var,
                            val, query->ctx, tstr->flags & NXT_TSTR_LOGGING);
...
#if (NXT_HAVE_NJS)
    ret = nxt_js_call(task, query->state->jcf, &query->cache->js, tstr->u.js,
                      val, query->ctx);
