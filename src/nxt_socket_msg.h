...
#if (NXT_CRED_USECMSG)
#define NXT_OOB_RECV_SIZE (CMSG_SPACE(2 * sizeof(int)) + CMSG_SPACE(sizeof(nxt_socket_cred_t)))
#else
#define NXT_OOB_RECV_SIZE CMSG_SPACE(2 * sizeof(int))
#endif

#if (NXT_HAVE_MSGHDR_CMSGCRED)
#define NXT_OOB_SEND_SIZE (CMSG_SPACE(2 * sizeof(int)) + CMSG_SPACE(sizeof(nxt_socket_cred_t)))
#else
 ...
 */
NXT_EXPORT ssize_t nxt_sendmsg(nxt_socket_t s, nxt_iobuf_t *iob, nxt_uint_t niob, const nxt_send_oob_t *oob);

/**
 * The nxt_recvmsg is a wrapper for recvmsg.
 * The oob buffer must be consumed by using nxt_socket_msg_oob_get().
 */
NXT_EXPORT ssize_t nxt_recvmsg(nxt_socket_t s, nxt_iobuf_t *iob, nxt_uint_t niob, nxt_recv_oob_t *oob);

nxt_inline struct cmsghdr *NXT_CMSG_NXTHDR(struct msghdr *msgh, struct cmsghdr *cmsg)
{
...
nxt_inline void nxt_socket_msg_oob_init(nxt_send_oob_t *oob, int *fds)
{
    int nfds;
    struct cmsghdr *cmsg;
...
nxt_inline nxt_int_t nxt_socket_msg_oob_get_fds(nxt_recv_oob_t *oob, nxt_fd_t *fd)
{
    size_t size;
    struct msghdr msg;
    struct cmsghdr *cmsg;
    ...
    for (cmsg = CMSG_FIRSTHDR(&msg); cmsg != NULL; cmsg = NXT_CMSG_NXTHDR(&msg, cmsg))
    {
...
nxt_inline nxt_int_t nxt_socket_msg_oob_get(nxt_recv_oob_t *oob, nxt_fd_t *fd, nxt_pid_t *pid)
{
    size_t size;
    struct msghdr msg;
    struct cmsghdr *cmsg;
...
#if (NXT_CRED_USECMSG)
        else if (cmsg->cmsg_level == SOL_SOCKET && cmsg->cmsg_type == NXT_CRED_CMSGTYPE)
        {
