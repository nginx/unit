...
njs_mod_t *nxt_js_module_loader(njs_vm_t *vm, njs_external_ptr_t external, njs_str_t *name)
{
    nxt_str_t text;
    nxt_uint_t i, n;
    nxt_js_conf_t *jcf;
    ...
    return njs_vm_compile_module(vm, name, &text.start, &text.start[text.length]);
}
...
void nxt_js_set_proto(nxt_js_conf_t *jcf, njs_external_t *proto, njs_uint_t n)
{
    jcf->protos = n;
    jcf->proto = proto;
}

static njs_vm_t *nxt_js_vm_create(nxt_js_conf_t *jcf)
{
    u_char *p;
    size_t size;
    njs_vm_t *vm;
    ...
    for (i = 0; i < jcf->modules->nelts; i++)
    {
        ...
        size += import_str.length + 1 + mod->name.length + 1 + from_str.length + 2 + mod->name.length + 3;

        size += global_str.length + 1 + mod->name.length + 3 + mod->name.length + 2;
    }
...
nxt_int_t nxt_js_add_module(nxt_js_conf_t *jcf, nxt_str_t *name, nxt_str_t *text)
{
...
nxt_js_t *nxt_js_add_tpl(nxt_js_conf_t *jcf, nxt_str_t *str, nxt_uint_t flags)
{
    ...
    static const nxt_str_t func_str = nxt_string("function(uri, host, remoteAddr, "
...
nxt_int_t nxt_js_compile(nxt_js_conf_t *jcf)
{
    ...
    *p++ = ']';
    ...
    nxt_js_proto_id = njs_vm_external_prototype(jcf->vm, jcf->proto, jcf->protos);
...
nxt_int_t nxt_js_call(nxt_task_t *task, nxt_js_conf_t *jcf, nxt_js_cache_t *cache, nxt_js_t *js, nxt_str_t *str,
                      void *ctx)
{
    njs_vm_t *vm;
    njs_int_t ret;
    ...
    static const njs_str_t js_args[] = {
        njs_str("uri"),     njs_str("host"),    njs_str("remoteAddr"), njs_str("args"),
        njs_str("headers"), njs_str("cookies"), njs_str("vars"),
    };
    ...
    ret = njs_vm_external_create(vm, njs_value_arg(&opaque_value), nxt_js_proto_id, ctx, 0);
    if (nxt_slow_path(ret != NJS_OK))
    {
        return NXT_ERROR;
    }
    ...
    for (i = 0; i < n; i++)
    {
        value = njs_vm_object_prop(vm, njs_value_arg(&opaque_value), &js_args[i], &arguments[i]);
    ...
    ret = njs_vm_invoke(vm, func, njs_value_arg(&arguments), n, njs_value_arg(&retval));
