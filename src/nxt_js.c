...
njs_mod_t* nxt_js_module_loader(njs_vm_t* vm,
...
static njs_vm_t* nxt_js_vm_create(nxt_js_conf_t* jcf) {
  ...
  for (i = 0; i < jcf->modules->nelts; i++) {
    ...
    size += import_str.length + 1 + mod->name.length + 1 + from_str.length + 2 +
            mod->name.length + 3;
...
nxt_int_t nxt_js_add_module(nxt_js_conf_t* jcf,
                            nxt_str_t* name,
                            nxt_str_t* text) {
...
nxt_js_t* nxt_js_add_tpl(nxt_js_conf_t* jcf, nxt_str_t* str, nxt_uint_t flags) {
  ...
  static const nxt_str_t func_str = nxt_string(
      "function(uri, host, remoteAddr, "
...
nxt_int_t nxt_js_call(nxt_task_t* task,
                      nxt_js_conf_t* jcf,
                      nxt_js_cache_t* cache,
                      nxt_js_t* js,
                      nxt_str_t* str,
                      void* ctx) {
  njs_vm_t* vm;
  ...
  static const njs_str_t js_args[] = {
      njs_str("uri"),  njs_str("host"),    njs_str("remoteAddr"),
      njs_str("args"), njs_str("headers"), njs_str("cookies"),
      njs_str("vars"),
  };
  ...
  for (i = 0; i < n; i++) {
    value = njs_vm_object_prop(vm, njs_value_arg(&opaque_value), &js_args[i],
                               &arguments[i]);
