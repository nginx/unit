...
njs_mod_t *
nxt_js_module_loader (njs_vm_t *vm, njs_external_ptr_t external,
                      njs_str_t *name)
{
  nxt_str_t text;
  nxt_uint_t i, n;
  ...
  for (i = 0; i < jcf->modules->nelts; i++)
    {
      ...
      size += import_str.length + 1 + mod->name.length + 1 + from_str.length
              + 2 + mod->name.length + 3;

      size += global_str.length + 1 + mod->name.length + 3 + mod->name.length
              + 2;
    }
...
nxt_js_compile (nxt_js_conf_t *jcf)
{
  ...
  *p++ = ']';
  ...
  nxt_js_proto_id
      = njs_vm_external_prototype (jcf->vm, jcf->proto, jcf->protos);
