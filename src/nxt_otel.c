...
nxt_otel_propagate_header(nxt_task_t *task, nxt_http_request_t *r)
{
    ...
    if (traceval == NULL) {
        ...
        nxt_log(task, NXT_LOG_ERR,
                "couldn't allocate traceparent header. span will not propagate");
        return;
    }
    ...
    /*
     * if we didnt inherit a trace id then we need to add then
...
nxt_otel_span_add_status(nxt_task_t *task, nxt_http_request_t *r)
{
    ...
    // add specific 3 byte status to span
    status_buf = nxt_mp_alloc(r->mem_pool, 4);
    if (status_buf == NULL) {
        return;
    }
