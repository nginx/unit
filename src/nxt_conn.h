...
NXT_EXPORT void nxt_conn_timer(nxt_event_engine_t* engine,
                               nxt_conn_t* c,
...
NXT_EXPORT void nxt_conn_accept(nxt_task_t* task,
                                nxt_listen_event_t* lev,
                                nxt_conn_t* c);
void nxt_conn_accept_error(nxt_task_t* task,
                           nxt_listen_event_t* lev,
                           const char* accept_syscall,
                           nxt_err_t err);
...
ssize_t nxt_conn_io_recv(nxt_conn_t* c,
                         void* buf,
                         size_t size,
                         nxt_uint_t flags);
...
ssize_t nxt_conn_io_writev(nxt_task_t* task,
                           nxt_sendbuf_t* sb,
                           nxt_iobuf_t* iob,
                           nxt_uint_t niob);
ssize_t nxt_conn_io_send(nxt_task_t* task,
                         nxt_sendbuf_t* sb,
                         void* buf,
                         size_t size);
...
nxt_bool_t nxt_event_conn_write_delayed(nxt_event_engine_t* engine,
                                        nxt_conn_t* c,
                                        size_t sent);
ssize_t nxt_event_conn_io_writev(nxt_conn_t* c,
                                 nxt_iobuf_t* iob,
                                 nxt_uint_t niob);
...
#define nxt_conn_read(engine, c)                                            \
  do {                                                                      \
    ...
    c->socket.read_work_queue = &e->read_work_queue;                        \
                                                                            \
    nxt_work_queue_add(&e->read_work_queue, c->io->read, c->socket.task, c, \
                       c->socket.data);                                     \
  } while (0)
...
#define nxt_conn_write(engine, c)                                             \
  do {                                                                        \
    ...
    c->socket.write_work_queue = &e->write_work_queue;                        \
                                                                              \
    nxt_work_queue_add(&e->write_work_queue, c->io->write, c->socket.task, c, \
                       c->socket.data);                                       \
  } while (0)
