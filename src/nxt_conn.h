...
typedef struct
{
    ...
    ssize_t (*recv)(nxt_conn_t *c, void *buf, size_t size, nxt_uint_t flags);
     ...
     */
    ssize_t (*sendbuf)(nxt_task_t *task, nxt_sendbuf_t *sb);
    /*
     * The sendbuf() is an interface for OS-specific sendfile
     * implementations or simple writev().
     */
    ssize_t (*old_sendbuf)(nxt_conn_t *c, nxt_buf_t *b, size_t limit);
    /*
     * The writev() is an interface to write several nxt_iobuf_t buffers.
     */
    ssize_t (*writev)(nxt_conn_t *c, nxt_iobuf_t *iob, nxt_uint_t niob);
    /*
     * The send() is an interface to write a single buffer.  SSL/TLS
     * libraries' send() interface handles also the libraries' errors.
     */
    ssize_t (*send)(nxt_conn_t *c, void *buf, size_t size);
...
#define nxt_read_timer_conn(ev) nxt_timer_data(ev, nxt_conn_t, read_timer)

#define nxt_write_timer_conn(ev) nxt_timer_data(ev, nxt_conn_t, write_timer)
...
#define nxt_conn_tcp_nodelay_on(task, c)                                                                               \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            ret = nxt_socket_setsockopt(task, (c)->socket.fd, IPPROTO_TCP, TCP_NODELAY, 1);                            \
                                                                                                                       \
...
NXT_EXPORT void nxt_conn_timer(nxt_event_engine_t *engine, nxt_conn_t *c, const nxt_conn_state_t *state,
                               nxt_timer_t *tev);
NXT_EXPORT void nxt_conn_work_queue_set(nxt_conn_t *c, nxt_work_queue_t *wq);
NXT_EXPORT nxt_sockaddr_t *nxt_conn_local_addr(nxt_task_t *task, nxt_conn_t *c);
...
NXT_EXPORT nxt_listen_event_t *nxt_listen_event(nxt_task_t *task, nxt_listen_socket_t *ls);
void nxt_conn_io_accept(nxt_task_t *task, void *obj, void *data);
NXT_EXPORT void nxt_conn_accept(nxt_task_t *task, nxt_listen_event_t *lev, nxt_conn_t *c);
...
ssize_t nxt_conn_io_recv(nxt_conn_t *c, void *buf, size_t size, nxt_uint_t flags);

void nxt_conn_io_write(nxt_task_t *task, void *obj, void *data);
ssize_t nxt_conn_io_sendbuf(nxt_task_t *task, nxt_sendbuf_t *sb);
ssize_t nxt_conn_io_writev(nxt_task_t *task, nxt_sendbuf_t *sb, nxt_iobuf_t *iob, nxt_uint_t niob);
ssize_t nxt_conn_io_send(nxt_task_t *task, nxt_sendbuf_t *sb, void *buf, size_t size);

size_t nxt_event_conn_write_limit(nxt_conn_t *c);
nxt_bool_t nxt_event_conn_write_delayed(nxt_event_engine_t *engine, nxt_conn_t *c, size_t sent);
ssize_t nxt_event_conn_io_writev(nxt_conn_t *c, nxt_iobuf_t *iob, nxt_uint_t niob);
ssize_t nxt_event_conn_io_send(nxt_conn_t *c, void *buf, size_t size);

NXT_EXPORT void nxt_event_conn_job_sendfile(nxt_task_t *task, nxt_conn_t *c);

#define nxt_conn_connect(engine, c)                                                                                    \
    nxt_work_queue_add(&engine->socket_work_queue, nxt_conn_sys_socket, c->socket.task, c, c->socket.data)
...
#define nxt_conn_read(engine, c)                                                                                       \
    ...
    {                                                                                                                  \
        ...
        c->socket.read_work_queue = &e->read_work_queue;                                                               \
                                                                                                                       \
        nxt_work_queue_add(&e->read_work_queue, c->io->read, c->socket.task, c, c->socket.data);                       \
    } while (0)
...
#define nxt_conn_write(engine, c)                                                                                      \
    ...
    {                                                                                                                  \
        ...
        c->socket.write_work_queue = &e->write_work_queue;                                                             \
                                                                                                                       \
        nxt_work_queue_add(&e->write_work_queue, c->io->write, c->socket.task, c, c->socket.data);                     \
    } while (0)
