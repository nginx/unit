...
ssize_t
nxt_conn_io_writev(nxt_task_t *task, nxt_sendbuf_t *sb, nxt_iobuf_t *iob,
                   nxt_uint_t niob);
ssize_t
...
#define nxt_conn_read(engine, c)                                               \
    do {                                                                       \
        ...
        c->socket.read_work_queue = &e->read_work_queue;                       \
                                                                               \
        nxt_work_queue_add(&e->read_work_queue, c->io->read, c->socket.task,   \
                           c, c->socket.data);                                 \
    } while (0)
...
#define nxt_conn_write(engine, c)                                              \
    do {                                                                       \
        ...
        c->socket.write_work_queue = &e->write_work_queue;                     \
                                                                               \
        nxt_work_queue_add(&e->write_work_queue, c->io->write, c->socket.task, \
                           c, c->socket.data);                                 \
    } while (0)
...
NXT_EXPORT nxt_conn_proxy_t *
nxt_conn_proxy_create(nxt_conn_t *c);
NXT_EXPORT void
