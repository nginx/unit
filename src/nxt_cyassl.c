...
static void nxt_cyassl_conn_init(nxt_thread_t *thr, nxt_ssltls_conf_t *conf, nxt_event_conn_t *c);
...
static void nxt_cyassl_conn_io_shutdown(nxt_thread_t *thr, void *obj, void *data);
static ssize_t nxt_cyassl_conn_io_write_chunk(nxt_thread_t *thr, nxt_event_conn_t *c, nxt_buf_t *b, size_t limit);
static ssize_t nxt_cyassl_conn_io_send(nxt_event_conn_t *c, void *buf, size_t size);
static nxt_int_t nxt_cyassl_conn_test_error(nxt_thread_t *thr, nxt_event_conn_t *c, int err,
                                            nxt_work_handler_t handler);
static void nxt_cdecl nxt_cyassl_conn_error(nxt_event_conn_t *c, nxt_err_t err, const char *fmt, ...);
static nxt_uint_t nxt_cyassl_log_error_level(nxt_event_conn_t *c, nxt_err_t err, int ssl_error);
static void nxt_cdecl nxt_cyassl_log_error(nxt_uint_t level, nxt_log_t *log, int ret, const char *fmt, ...);
...
static nxt_int_t nxt_cyassl_start(void)
{
    int err;
    nxt_thread_t *thr;
    ...
    if (err != SSL_SUCCESS)
    {
        nxt_cyassl_log_error(NXT_LOG_ALERT, thr->log, err, "CyaSSL_Init() failed");
        return NXT_ERROR;
    }
...
static nxt_int_t nxt_cyassl_server_init(nxt_ssltls_conf_t *conf)
{
    ...
    if (ctx == NULL)
    {
        nxt_cyassl_log_error(NXT_LOG_ALERT, thr->log, 0, "CyaSSL_CTX_new() failed");
        return NXT_ERROR;
    }
    ...
    if (err != SSL_SUCCESS)
    {
        nxt_cyassl_log_error(NXT_LOG_ALERT, thr->log, err, "CyaSSL_CTX_use_certificate_file(\"%s\") failed",
                             certificate);
        goto fail;
    }
    ...
    if (err != SSL_SUCCESS)
    {
        nxt_cyassl_log_error(NXT_LOG_ALERT, thr->log, err, "CyaSSL_CTX_use_PrivateKey_file(\"%s\") failed", key);
        goto fail;
    }
    ...
    if (conf->ciphers != NULL)
    {
        ...
        if (err != SSL_SUCCESS)
        {
            nxt_cyassl_log_error(NXT_LOG_ALERT, thr->log, err, "CyaSSL_CTX_set_cipher_list(\"%s\") failed",
                                 conf->ciphers);
            goto fail;
        }
    }
...
static void nxt_cyassl_conn_init(nxt_thread_t *thr, nxt_ssltls_conf_t *conf, nxt_event_conn_t *c)
{
    ...
    if (s == NULL)
    {
        nxt_cyassl_log_error(NXT_LOG_ALERT, c->socket.log, 0, "CyaSSL_new() failed");
        goto fail;
    }
...
static void nxt_cyassl_conn_handshake(nxt_thread_t *thr, void *obj, void *data)
{
    ...
    if (n == NXT_ERROR)
    {
        nxt_cyassl_conn_error(c, err, "CyaSSL_negotiate(%d) failed", c->socket.fd);
...
static void nxt_cyassl_conn_io_read(nxt_thread_t *thr, void *obj, void *data)
{
    ...
    if (b != NULL)
    {
        ...
        ret = CyaSSL_read(ssltls->session, b->mem.free, b->mem.end - b->mem.free);

        err = (ret <= 0) ? nxt_socket_errno : 0;

        nxt_log_debug(thr->log, "CyaSSL_read(%d, %p, %uz): %d", c->socket.fd, b->mem.free, b->mem.end - b->mem.free,
                      ret);
        ...
        {
            n = nxt_cyassl_conn_test_error(thr, c, ret, nxt_cyassl_conn_io_read);
            ...
            nxt_cyassl_conn_error(c, err, "CyaSSL_read(%d, %p, %uz) failed", c->socket.fd, b->mem.free,
...
static ssize_t nxt_cyassl_conn_io_send(nxt_event_conn_t *c, void *buf, size_t size)
{
    ...
    {
        nxt_cyassl_conn_error(c, err, "CyaSSL_write(%d, %p, %uz) failed", c->socket.fd, buf, size);
    }
...
static void nxt_cyassl_conn_io_shutdown(nxt_thread_t *thr, void *obj, void *data)
{
    ...
    nxt_event_conn_io_handle(thr, c->write_work_queue, c->write_state->close_handler, c, data);
}

static nxt_int_t nxt_cyassl_conn_test_error(nxt_thread_t *thr, nxt_event_conn_t *c, int ret, nxt_work_handler_t handler)
{
    nxt_work_queue_t *wq;
    nxt_cyassl_conn_t *ssltls;
...
static void nxt_cdecl nxt_cyassl_log_error(nxt_uint_t level, nxt_log_t *log, int err, const char *fmt, ...)
{
    u_char *p, *end;
    va_list args;
    u_char msg[NXT_MAX_ERROR_STR];
