                                  ...
                                  nxt_conf_value_t              *cv)
{
    ...
    if (nxt_inet6_probe(&addr)) {
        ...
        if (delim != NULL) {
            ...
            if (nxt_slow_path(
                    memcmp(&inet6->start, &inet6->end, sizeof(struct in6_addr))
                    > 0)) {
        ...
        if (delim != NULL) {
            ...
            if (nxt_slow_path(
                    !nxt_valid_ipv6_blocks(addr.start, addr.length))) {
                return NXT_ADDR_PATTERN_FORMAT_ERROR;
            }
    ...
    if (delim != NULL) {
        ...
        inet->end
            = nxt_inet_addr(delim + 1, addr.start + addr.length - (delim + 1));
        if (nxt_slow_path(inet->end == INADDR_NONE)) {
            return NXT_ADDR_PATTERN_FORMAT_ERROR;
        }

        if (nxt_slow_path(
                memcmp(&inet->start, &inet->end, sizeof(struct in_addr)) > 0)) {
            return NXT_ADDR_PATTERN_RANGE_OVERLAP_ERROR;
        }
    ...
    if (delim != NULL) {
        cidr_prefix
            = nxt_int_parse(delim + 1, addr.start + addr.length - (delim + 1));
