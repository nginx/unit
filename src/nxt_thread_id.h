...
nxt_inline nxt_tid_t nxt_thread_get_tid(void)
{
    ...
    err = pthread_getthrds_np(&pt, PTHRDSINFO_QUERY_TID, &ti, sizeof(struct __pthrdsinfo), NULL, size);
    ...
    nxt_main_log_alert("pthread_getthrds_np(PTHRDSINFO_QUERY_TID) failed %E", err);
    return 0;
}
