...
typedef void nxt_cdecl (*nxt_log_handler_t)(nxt_uint_t level, nxt_log_t *log, const char *fmt, ...);
...
NXT_EXPORT nxt_log_t *nxt_log_set_ctx(nxt_log_t *log, nxt_log_ctx_handler_t handler, void *ctx);

NXT_EXPORT void nxt_cdecl nxt_log_handler(nxt_uint_t level, nxt_log_t *log, const char *fmt, ...);

#define nxt_log_level_enough(log, level) ((log)->level >= (level))
...
#define nxt_assert(c)                                                                                                  \
    ...
    {                                                                                                                  \
        if (nxt_slow_path(!(c)))                                                                                       \
        {                                                                                                              \
            nxt_thread_log_alert("%s:%d assertion failed: %s", __FILE__, __LINE__, #c);                                \
            nxt_abort();                                                                                               \
        }                                                                                                              \
    } while (0)
...
#define nxt_debug_alloc(...) nxt_thread_log_debug(__VA_ARGS__)
...
#define nxt_main_log_alert(...) nxt_log_alert(&nxt_main_log, __VA_ARGS__)

#define nxt_main_log_error(level, ...) nxt_log_error(level, &nxt_main_log, __VA_ARGS__)

#define nxt_main_log_debug(...) nxt_log_debug(&nxt_main_log, __VA_ARGS__)
