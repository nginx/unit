...
typedef void nxt_cdecl (*nxt_log_handler_t)(nxt_uint_t level,
                                            nxt_log_t* log,
                                            const char* fmt,
                                            ...);
...
NXT_EXPORT nxt_log_t* nxt_log_set_ctx(nxt_log_t* log,
                                      nxt_log_ctx_handler_t handler,
                                      void* ctx);

NXT_EXPORT void nxt_cdecl nxt_log_handler(nxt_uint_t level,
                                          nxt_log_t* log,
                                          const char* fmt,
                                          ...);
...
#define nxt_assert(c)                                                        \
  do {                                                                       \
    if (nxt_slow_path(!(c))) {                                               \
      nxt_thread_log_alert("%s:%d assertion failed: %s", __FILE__, __LINE__, \
                           #c);                                              \
      nxt_abort();                                                           \
    }                                                                        \
  } while (0)
