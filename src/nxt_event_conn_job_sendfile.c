...
static void nxt_event_conn_job_sendfile_start(nxt_task_t *task, void *obj, void *data);
static void nxt_event_conn_job_sendfile_handler(nxt_task_t *task, void *obj, void *data);
static void nxt_event_conn_job_sendfile_return(nxt_task_t *task, void *obj, void *data);
static nxt_buf_t *nxt_event_conn_job_sendfile_completion(nxt_task_t *task, nxt_conn_t *c, nxt_buf_t *b);

void nxt_event_conn_job_sendfile(nxt_task_t *task, nxt_conn_t *c)
{
...
fast:
    ...
    nxt_work_set(&jbs->job.work, nxt_event_conn_job_sendfile_handler, jbs->job.task, jbs, c);
...
static void nxt_event_conn_job_sendfile_return(nxt_task_t *task, void *obj, void *data)
{
    ...
    if (c->socket.error == 0 && !nxt_event_conn_write_delayed(task->thread->engine, c, sent))
    {
        nxt_conn_timer(task->thread->engine, c, c->write_state, &c->write_timer);
    ...
    if (sent != 0)
    {
        nxt_work_queue_add(c->write_work_queue, c->write_state->ready_handler, task, c, c->socket.data);
        /*
    ...
    {
        nxt_work_queue_add(c->write_work_queue, c->write_state->error_handler, task, c, c->socket.data);
    }
}
...
static nxt_buf_t *nxt_event_conn_job_sendfile_completion(nxt_task_t *task, nxt_conn_t *c, nxt_buf_t *b)
{
    while (b != NULL)
    {
        ...
        nxt_work_queue_add(c->write_work_queue, b->completion_handler, task, b, b->parent);
