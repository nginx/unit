...
static void *
nxt_ruby_request_handler_gvl(void *req);
static int
...
nxt_ruby_start(nxt_task_t *task, nxt_process_data_t *data)
{
    ...
    res = rb_protect(nxt_ruby_init_basic, (VALUE) (uintptr_t) &rack_init,
                     &state);
    ...
    if (c->hooks.start != NULL) {
        path
            = rb_str_new((const char *) c->hooks.start, (long) c->hooks.length);
...
nxt_ruby_thread_func(VALUE arg)
{
    ...
    (void) rb_thread_call_without_gvl(nxt_ruby_unit_run, ctx, nxt_ruby_ubf,
                                      ctx);
