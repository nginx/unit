...
static int nxt_ruby_read_request (nxt_unit_request_info_t *req,
                                  VALUE hash_env);
...
nxt_inline void
nxt_ruby_add_sptr (VALUE hash_env, VALUE name, nxt_unit_sptr_t *sptr,
                   uint32_t len)
{
  char *str;
  ...
  if (TYPE (r_value) == T_ARRAY)
    {
      ...
      *rc = nxt_unit_response_add_field (
          headers_info->req, RSTRING_PTR (r_key), key_len, field, len);
  ...
  for (;;)
    {
      ...
      *rc = nxt_unit_response_add_field (
          headers_info->req, RSTRING_PTR (r_key), key_len, value, pos - value);
      if (nxt_slow_path (*rc != NXT_UNIT_OK))
        {
          goto fail;
        }
  ...
  if (value <= value_end)
    {
      *rc = nxt_unit_response_add_field (headers_info->req,
                                         RSTRING_PTR (r_key), key_len, value,
                                         value_end - value);
      if (nxt_slow_path (*rc != NXT_UNIT_OK))
        {
          goto fail;
...
static ssize_t
nxt_ruby_rack_file_read (nxt_unit_read_info_t *read_info, void *dst,
                         size_t size)
{
  ssize_t res;
  ...
  for (i = 0; i < c->threads - 1; i++)
    {
      ...
      rctx->env = rb_protect (nxt_ruby_rack_env_create, (VALUE)(uintptr_t)rctx,
                              &state);
