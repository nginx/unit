...
static nxt_int_t nxt_ruby_start(nxt_task_t *task, nxt_process_data_t *data);
...
static void nxt_ruby_join_threads(nxt_unit_ctx_t *ctx, nxt_ruby_app_conf_t *c);

static VALUE nxt_ruby_rack_app_run(VALUE arg);
static int nxt_ruby_read_request(nxt_unit_request_info_t *req, VALUE hash_env);
nxt_inline void nxt_ruby_add_sptr(VALUE hash_env, VALUE name, nxt_unit_sptr_t *sptr, uint32_t len);
static nxt_int_t nxt_ruby_rack_result_status(nxt_unit_request_info_t *req, VALUE result);
static int nxt_ruby_rack_result_headers(nxt_unit_request_info_t *req, VALUE result, nxt_int_t status);
...
static int nxt_ruby_rack_result_body(nxt_unit_request_info_t *req, VALUE result);
static int nxt_ruby_rack_result_body_file_write(nxt_unit_request_info_t *req, VALUE filepath);
static void *nxt_ruby_response_write_cb(void *read_info);
static VALUE nxt_ruby_rack_result_body_each(VALUE body, VALUE arg, int argc, const VALUE *argv, VALUE blockarg);
static void *nxt_ruby_response_write(void *body);

static void nxt_ruby_exception_log(nxt_unit_request_info_t *req, uint32_t level, const char *desc);
...
NXT_EXPORT nxt_app_module_t nxt_app_module = {
    sizeof(compat), compat,         nxt_string("ruby"), ruby_version, nxt_ruby_mounts, nxt_nitems(nxt_ruby_mounts),
    NULL,           nxt_ruby_start,
};
...
static int nxt_ruby_init_strings(void)
{
    VALUE v;
    ...
    while (pstr->string.start != NULL)
    {
        ...
        {
            nxt_unit_alert(NULL, "Ruby: failed to create const string '%.*s'", (int)pstr->string.length,
...
static void nxt_ruby_done_strings(void)
{
    nxt_ruby_string_t *pstr;
...
static VALUE nxt_ruby_hook_procs_load(VALUE path)
{
    ...
    rb_define_module_function(module, "on_worker_boot", &nxt_ruby_hook_register, 0);
    rb_define_module_function(module, "on_worker_shutdown", &nxt_ruby_hook_register, 0);
    rb_define_module_function(module, "on_thread_boot", &nxt_ruby_hook_register, 0);
    rb_define_module_function(module, "on_thread_shutdown", &nxt_ruby_hook_register, 0);

    file = rb_const_get(rb_cObject, rb_intern("File"));
    file_obj = rb_funcall(file, rb_intern("read"), 1, path);

    return rb_funcall(module, rb_intern("module_eval"), 3, file_obj, path, INT2NUM(1));
}
...
static nxt_int_t nxt_ruby_start(nxt_task_t *task, nxt_process_data_t *data)
{
    ...
    res = rb_protect(nxt_ruby_init_basic, (VALUE)(uintptr_t)&rack_init, &state);
    if (nxt_slow_path(res == Qnil || state != 0))
    {
        nxt_ruby_exception_log(NULL, NXT_LOG_ALERT, "Failed to init basic variables");
        return NXT_ERROR;
    }
    ...
    {
        path = rb_str_new((const char *)c->hooks.start, (long)c->hooks.length);

        rb_protect(nxt_ruby_hook_procs_load, path, &state);
        rb_str_free(path);
        if (nxt_slow_path(state != 0))
        {
            nxt_ruby_exception_log(NULL, NXT_LOG_ALERT, "Failed to setup hooks");
            return NXT_ERROR;
        }
    }
    ...
    if (nxt_ruby_hook_procs != Qnil)
    {
        ...
        if (nxt_slow_path(state != 0))
        {
            nxt_ruby_exception_log(NULL, NXT_LOG_ERR, "Failed to call on_worker_boot()");
            return NXT_ERROR;
        }
    }
    ...
    ruby_ctx.env = rb_protect(nxt_ruby_rack_env_create, (VALUE)(uintptr_t)&ruby_ctx, &state);
    if (nxt_slow_path(ruby_ctx.env == Qnil || state != 0))
    {
        nxt_ruby_exception_log(NULL, NXT_LOG_ALERT, "Failed to create 'environ' variable");
        goto fail;
    }
    ...
    if (nxt_ruby_hook_procs != Qnil)
    {
        ...
        if (nxt_slow_path(state != 0))
        {
            nxt_ruby_exception_log(NULL, NXT_LOG_ERR, "Failed to call on_thread_boot()");
        }
    }

    rc = (intptr_t)rb_thread_call_without_gvl2(nxt_ruby_unit_run, unit_ctx, nxt_ruby_ubf, unit_ctx);
    ...
    if (nxt_ruby_hook_procs != Qnil)
    {
        ...
        if (nxt_slow_path(state != 0))
        {
            nxt_ruby_exception_log(NULL, NXT_LOG_ERR, "Failed to call on_thread_shutdown()");
        }
    }
    ...
    if (nxt_ruby_hook_procs != Qnil)
    {
        ...
        if (nxt_slow_path(state != 0))
        {
            nxt_ruby_exception_log(NULL, NXT_LOG_ERR, "Failed to call on_worker_shutdown()");
        }
    }
...
static VALUE nxt_ruby_init_basic(VALUE arg)
{
    ...
    if (nxt_slow_path(state == 0))
    {
        nxt_alert(rack_init->task, "Ruby: Failed to find encoding index 'encdb'");

        return Qnil;
    }

    rb_funcall(rb_cObject, rb_intern("require"), 1, rb_str_new2("enc/trans/transdb"));
...
static VALUE nxt_ruby_rack_init(nxt_ruby_rack_init_t *rack_init)
{
    ...
    if (nxt_slow_path(state != 0))
    {
        nxt_ruby_exception_log(NULL, NXT_LOG_ALERT, "Failed to require 'rubygems' package");
        return Qnil;
    }
    ...
    if (state != 0)
    {
        ...
        {
            nxt_ruby_exception_log(NULL, NXT_LOG_ALERT, "Failed to require 'bundler/setup' package");
            return Qnil;
        }
    ...
    if (nxt_slow_path(state != 0))
    {
        nxt_ruby_exception_log(NULL, NXT_LOG_ALERT, "Failed to require 'rack' package");
        return Qnil;
    }

    rackup = rb_protect(nxt_ruby_rack_parse_script, (VALUE)(uintptr_t)rack_init, &state);

    if (nxt_slow_path(state != 0))
    {
        nxt_ruby_exception_log(NULL, NXT_LOG_ALERT, "Failed to parse rack script");
        return Qnil;
    }
...
static VALUE nxt_ruby_bundler_setup(VALUE arg)
{
    return rb_funcall(rb_cObject, rb_intern("require"), 1, rb_str_new2("bundler/setup"));
}
...
static VALUE nxt_ruby_rack_parse_script(VALUE ctx)
{
    ...
    script = rb_str_new((const char *)rack_init->script->start, (long)rack_init->script->length);
...
static VALUE nxt_ruby_rack_env_create(VALUE arg)
{
    ...
    rb_hash_aset(hash_env, rb_str_new2("SERVER_SOFTWARE"),
                 rb_str_new((const char *)nxt_server.start, (long)nxt_server.length));
    ...
    rb_hash_aset(hash_env, rb_str_new2("rack.multithread"), nxt_ruby_threads > 1 ? Qtrue : Qfalse);
...
static int nxt_ruby_init_io(nxt_ruby_ctx_t *rctx)
{
    ...
    rctx->io_input = rb_funcall(io_input, rb_intern("new"), 1, (VALUE)(uintptr_t)rctx);
    if (nxt_slow_path(rctx->io_input == Qnil))
    {
        nxt_unit_alert(NULL, "Ruby: Failed to create environment 'rack.input' var");
    ...
    rctx->io_error = rb_funcall(io_error, rb_intern("new"), 1, (VALUE)(uintptr_t)rctx);
    if (nxt_slow_path(rctx->io_error == Qnil))
    {
        nxt_unit_alert(NULL, "Ruby: Failed to create environment 'rack.error' var");
...
static void *nxt_ruby_request_handler_gvl(void *data)
{
    int state;
    VALUE res;
    nxt_ruby_ctx_t *rctx;
    ...
    if (nxt_slow_path(res == Qnil || state != 0))
    {
        nxt_ruby_exception_log(req, NXT_LOG_ERR, "Failed to run ruby script");
...
static VALUE nxt_ruby_rack_app_run(VALUE arg)
{
    ...
    {
        nxt_unit_req_error(req, "Ruby: Invalid response format from application");
    ...
    {
        nxt_unit_req_error(req, "Ruby: Invalid response status from application.");
...
static int nxt_ruby_read_request(nxt_unit_request_info_t *req, VALUE hash_env)
{
    ...
    nxt_ruby_add_sptr(hash_env, nxt_rb_request_method_str, &r->method, r->method_length);
    ...
    nxt_ruby_add_sptr(hash_env, nxt_rb_query_string_str, &r->query, r->query_length);
    nxt_ruby_add_sptr(hash_env, nxt_rb_server_protocol_str, &r->version, r->version_length);
    nxt_ruby_add_sptr(hash_env, nxt_rb_remote_addr_str, &r->remote, r->remote_length);
    nxt_ruby_add_sptr(hash_env, nxt_rb_server_addr_str, &r->local_addr, r->local_addr_length);
    nxt_ruby_add_sptr(hash_env, nxt_rb_server_name_str, &r->server_name, r->server_name_length);

    rb_hash_aset(hash_env, nxt_rb_server_port_str, nxt_rb_80_str);

    rb_hash_aset(hash_env, nxt_rb_rack_url_scheme_str, r->tls ? nxt_rb_https_str : nxt_rb_http_str);
    ...
    if (r->content_length_field != NXT_UNIT_NONE_FIELD)
    {
        ...
        nxt_ruby_add_sptr(hash_env, nxt_rb_content_length_str, &f->value, f->value_length);
    }
    ...
    if (r->content_type_field != NXT_UNIT_NONE_FIELD)
    {
        ...
        nxt_ruby_add_sptr(hash_env, nxt_rb_content_type_str, &f->value, f->value_length);
    }
...
static nxt_int_t nxt_ruby_rack_result_status(nxt_unit_request_info_t *req, VALUE result)
{
    ...
    {
        return nxt_int_parse((u_char *)RSTRING_PTR(status), RSTRING_LEN(status));
    }
...
static int nxt_ruby_rack_result_headers(nxt_unit_request_info_t *req, VALUE result, nxt_int_t status)
{
    ...
    {
        nxt_unit_req_error(req, "Ruby: Invalid response 'headers' format from "
                                "application");
    ...
    rb_hash_foreach(headers, nxt_ruby_hash_info, (VALUE)(uintptr_t)&headers_info);
    if (nxt_slow_path(headers_info.rc != NXT_UNIT_OK))
    {
        return headers_info.rc;
    }

    rc = nxt_unit_response_init(req, status, headers_info.fields, headers_info.size);
    if (nxt_slow_path(rc != NXT_UNIT_OK))
    {
        return rc;
    }

    rb_hash_foreach(headers, nxt_ruby_hash_add, (VALUE)(uintptr_t)&headers_info);
...
static int nxt_ruby_hash_info(VALUE r_key, VALUE r_value, VALUE arg)
{
    ...
    {
        nxt_unit_req_error(headers_info->req, "Ruby: Wrong header entry 'key' from application");
    ...
    {
        nxt_unit_req_error(headers_info->req, "Ruby: Wrong header entry 'value' from application");
    ...
    if (TYPE(r_value) == T_ARRAY)
    {
        ...
        for (i = 0; i < arr_len; i++)
        {
            ...
            {
                nxt_unit_req_error(headers_info->req, "Ruby: Wrong header entry in 'value' array "
                                                      "from application");
                goto fail;
            }
...
static int nxt_ruby_hash_add(VALUE r_key, VALUE r_value, VALUE arg)
{
    ...
    if (TYPE(r_value) == T_ARRAY)
    {
        ...
        *rc = nxt_unit_response_add_field(headers_info->req, RSTRING_PTR(r_key), key_len, field, len);
    ...
    for (;;)
    {
        ...
        *rc = nxt_unit_response_add_field(headers_info->req, RSTRING_PTR(r_key), key_len, value, pos - value);
        if (nxt_slow_path(*rc != NXT_UNIT_OK))
        {
            goto fail;
        }
    ...
    if (value <= value_end)
    {
        *rc = nxt_unit_response_add_field(headers_info->req, RSTRING_PTR(r_key), key_len, value, value_end - value);
        if (nxt_slow_path(*rc != NXT_UNIT_OK))
        {
            goto fail;
        }
...
static int nxt_ruby_rack_result_body(nxt_unit_request_info_t *req, VALUE result)
{
    ...
    if (rb_respond_to(body, rb_intern("to_path")))
    {
        ...
        {
            nxt_unit_req_error(req, "Ruby: Failed to get 'body' file path from "
                                    "application");
    ...
    {
        rb_block_call(body, rb_intern("each"), 0, 0, nxt_ruby_rack_result_body_each, (VALUE)(uintptr_t)req);
    }
    else
    {
        nxt_unit_req_error(req, "Ruby: Invalid response 'body' format "
                                "from application");
...
static ssize_t nxt_ruby_rack_file_read(nxt_unit_read_info_t *read_info, void *dst, size_t size)
{
    ssize_t res;
...
static int nxt_ruby_rack_result_body_file_write(nxt_unit_request_info_t *req, VALUE filepath)
{
    ...
    {
        nxt_unit_req_error(req, "Ruby: Failed to open content file \"%s\": %s (%d)", RSTRING_PTR(filepath),
    ...
    {
        nxt_unit_req_error(req, "Ruby: Content file fstat(\"%s\") failed: %s (%d)", RSTRING_PTR(filepath),
                           strerror(errno), errno);
    ...
    rc = (intptr_t)rb_thread_call_without_gvl(nxt_ruby_response_write_cb, &ri, nxt_ruby_ubf, req->ctx);
...
static void *nxt_ruby_response_write_cb(void *data)
{
    int rc;
    nxt_ruby_read_info_t *ri;
...
static VALUE nxt_ruby_rack_result_body_each(VALUE body, VALUE arg, int argc, const VALUE *argv, VALUE blockarg)
{
    ...
    (void)rb_thread_call_without_gvl(nxt_ruby_response_write, (void *)(uintptr_t)&wi, nxt_ruby_ubf, wi.req->ctx);

    return Qnil;
}

static void *nxt_ruby_response_write(void *data)
{
    int rc;
    nxt_ruby_write_info_t *wi;
    ...
    rc = nxt_unit_response_write(wi->req, RSTRING_PTR(wi->body), RSTRING_LEN(wi->body));
    if (nxt_slow_path(rc != NXT_UNIT_OK))
    {
        nxt_unit_req_error(wi->req, "Ruby: Failed to write 'body' from application");
    }
...
static void nxt_ruby_exception_log(nxt_unit_request_info_t *req, uint32_t level, const char *desc)
{
    ...
    if (RARRAY_LEN(ary) == 0)
    {
        nxt_unit_req_log(req, level, "Ruby: %s (%s)", RSTRING_PTR(msg), RSTRING_PTR(eclass));

        return;
    }

    nxt_unit_req_log(req, level, "Ruby: %s: %s (%s)", RSTRING_PTR(RARRAY_PTR(ary)[0]), RSTRING_PTR(msg),
                     RSTRING_PTR(eclass));

    for (i = 1; i < RARRAY_LEN(ary); i++)
    {
        nxt_unit_req_log(req, level, "from %s", RSTRING_PTR(RARRAY_PTR(ary)[i]));
    }
}
...
static VALUE nxt_ruby_thread_func(VALUE arg)
{
    ...
    (void)rb_thread_call_without_gvl(nxt_ruby_unit_run, ctx, nxt_ruby_ubf, ctx);
...
static int nxt_ruby_init_threads(nxt_ruby_app_conf_t *c)
{
    ...
    nxt_ruby_ctxs = nxt_unit_malloc(NULL, sizeof(nxt_ruby_ctx_t) * (c->threads - 1));
    ...
    for (i = 0; i < c->threads - 1; i++)
    {
        ...
        rctx->env = rb_protect(nxt_ruby_rack_env_create, (VALUE)(uintptr_t)rctx, &state);
