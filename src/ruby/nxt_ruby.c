...
static VALUE nxt_ruby_rack_result_body_each(VALUE body, VALUE arg, int argc,
                                            const VALUE *argv, VALUE blockarg);
static void *nxt_ruby_response_write(void *body);

static void nxt_ruby_exception_log(nxt_unit_request_info_t *req, uint32_t level,
                                   const char *desc);
...
NXT_EXPORT nxt_app_module_t nxt_app_module = {
    sizeof(compat), compat,          nxt_string("ruby"),
    ruby_version,   nxt_ruby_mounts, nxt_nitems(nxt_ruby_mounts),
    NULL,           nxt_ruby_start,
};
  ...
  while (pstr->string.start != NULL) {
    ...
    if (nxt_slow_path(v == Qnil)) {
      nxt_unit_alert(NULL, "Ruby: failed to create const string '%.*s'",
                     (int)pstr->string.length, (char *)pstr->string.start);
...
static VALUE nxt_ruby_hook_procs_load(VALUE path) {
  ...
  rb_define_module_function(module, "on_worker_boot", &nxt_ruby_hook_register,
                            0);
  rb_define_module_function(module, "on_worker_shutdown",
                            &nxt_ruby_hook_register, 0);
  rb_define_module_function(module, "on_thread_boot", &nxt_ruby_hook_register,
                            0);
...
static nxt_int_t nxt_ruby_start(nxt_task_t *task, nxt_process_data_t *data) {
  ...
  ruby_ctx.env =
      rb_protect(nxt_ruby_rack_env_create, (VALUE)(uintptr_t)&ruby_ctx, &state);
...
static VALUE nxt_ruby_rack_init(nxt_ruby_rack_init_t *rack_init) {
  ...
  rackup = rb_protect(nxt_ruby_rack_parse_script, (VALUE)(uintptr_t)rack_init,
                      &state);
...
static VALUE nxt_ruby_rack_env_create(VALUE arg) {
  ...
  rb_hash_aset(
      hash_env, rb_str_new2("SERVER_SOFTWARE"),
...
static int nxt_ruby_init_io(nxt_ruby_ctx_t *rctx) {
  ...
  rctx->io_input =
      rb_funcall(io_input, rb_intern("new"), 1, (VALUE)(uintptr_t)rctx);
  ...
  rctx->io_error =
      rb_funcall(io_error, rb_intern("new"), 1, (VALUE)(uintptr_t)rctx);
...
static int nxt_ruby_read_request(nxt_unit_request_info_t *req, VALUE hash_env) {
  ...
  if (r->content_length_field != NXT_UNIT_NONE_FIELD) {
    ...
    nxt_ruby_add_sptr(hash_env, nxt_rb_content_length_str, &f->value,
                      f->value_length);
  }
  ...
  if (r->content_type_field != NXT_UNIT_NONE_FIELD) {
    ...
    nxt_ruby_add_sptr(hash_env, nxt_rb_content_type_str, &f->value,
                      f->value_length);
  }
...
static int nxt_ruby_rack_result_headers(nxt_unit_request_info_t *req,
  ...
  rc = nxt_unit_response_init(req, status, headers_info.fields,
                              headers_info.size);
  if (nxt_slow_path(rc != NXT_UNIT_OK)) {
    return rc;
  }
...
static int nxt_ruby_hash_add(VALUE r_key, VALUE r_value, VALUE arg) {
  ...
  if (TYPE(r_value) == T_ARRAY) {
    ...
    *rc = nxt_unit_response_add_field(headers_info->req, RSTRING_PTR(r_key),
                                      key_len, field, len);
...
static int nxt_ruby_rack_result_body(nxt_unit_request_info_t *req,
  ...
  if (rb_respond_to(body, rb_intern("to_path"))) {
    ...
    rb_block_call(body, rb_intern("each"), 0, 0, nxt_ruby_rack_result_body_each,
                  (VALUE)(uintptr_t)req);
...
static ssize_t nxt_ruby_rack_file_read(nxt_unit_read_info_t *read_info,
...
static int nxt_ruby_rack_result_body_file_write(nxt_unit_request_info_t *req,
  ...
  if (nxt_slow_path(fd == -1)) {
    nxt_unit_req_error(req, "Ruby: Failed to open content file \"%s\": %s (%d)",
  ...
  if (nxt_slow_path(rc == -1)) {
    nxt_unit_req_error(req, "Ruby: Content file fstat(\"%s\") failed: %s (%d)",
                       RSTRING_PTR(filepath), strerror(errno), errno);
  ...
  rc = (intptr_t)rb_thread_call_without_gvl(nxt_ruby_response_write_cb, &ri,
                                            nxt_ruby_ubf, req->ctx);
...
static VALUE nxt_ruby_rack_result_body_each(VALUE body, VALUE arg, int argc,
  ...
  (void)rb_thread_call_without_gvl(nxt_ruby_response_write,
                                   (void *)(uintptr_t)&wi, nxt_ruby_ubf,
                                   wi.req->ctx);
...
static void nxt_ruby_exception_log(nxt_unit_request_info_t *req, uint32_t level,
  ...
  nxt_unit_req_log(req, level, "Ruby: %s: %s (%s)",
                   RSTRING_PTR(RARRAY_PTR(ary)[0]), RSTRING_PTR(msg),
                   RSTRING_PTR(eclass));
...
static void nxt_ruby_ubf(void *ctx) { nxt_unit_warn(ctx, "Ruby: UBF"); }
...
static int nxt_ruby_init_threads(nxt_ruby_app_conf_t *c) {
  ...
  nxt_ruby_ctxs =
      nxt_unit_malloc(NULL, sizeof(nxt_ruby_ctx_t) * (c->threads - 1));
  ...
  for (i = 0; i < c->threads - 1; i++) {
    ...
    rctx->env =
        rb_protect(nxt_ruby_rack_env_create, (VALUE)(uintptr_t)rctx, &state);
