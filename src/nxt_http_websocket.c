...
static void nxt_http_websocket_error_handler(nxt_task_t *task, void *obj, void *data);
...
static void nxt_http_websocket_client(nxt_task_t *task, void *obj, void *data)
{
    ...
    frame_size = nxt_websocket_frame_header_size(wsh) + nxt_websocket_frame_payload_len(wsh);
    ...
    while (b != NULL && frame_size > 0)
    {
        ...
        while (copy_size > 0)
        {
            ...
            buf->mem.free = nxt_cpymem(buf->mem.free, b->mem.pos, chunk_copy_size);
    ...
    res = nxt_port_socket_write(task, req_rpc_data->app_port, NXT_PORT_MSG_WEBSOCKET, -1, req_rpc_data->stream,
...
static void nxt_http_websocket_error_handler(nxt_task_t *task, void *obj, void *data)
{
    ...
    (void)nxt_port_socket_write(task, req_rpc_data->app_port, NXT_PORT_MSG_WEBSOCKET_LAST, -1, req_rpc_data->stream,
