...
static void nxt_port_rpc_remove_from_peers(nxt_task_t *task, nxt_port_t *port, nxt_port_rpc_reg_t *reg);
...
nxt_int_t nxt_port_rpc_init(void)
{
    ...
    p = nxt_mem_mmap(NULL, sizeof(*nxt_stream_ident), PROT_READ | PROT_WRITE, MAP_ANON | MAP_SHARED, -1, 0);
...
static nxt_int_t nxt_rpc_reg_test(nxt_lvlhsh_query_t *lhq, void *data)
{
    return NXT_OK;
}
...
nxt_inline void nxt_port_rpc_lhq_stream(nxt_lvlhsh_query_t *lhq, uint32_t *stream)
{
...
nxt_inline void nxt_port_rpc_lhq_peer(nxt_lvlhsh_query_t *lhq, nxt_pid_t *peer)
{
...
uint32_t nxt_port_rpc_register_handler(nxt_task_t *task, nxt_port_t *port, nxt_port_rpc_handler_t ready_handler,
                                       nxt_port_rpc_handler_t error_handler, nxt_pid_t peer, void *data)
{
    void *ex;
    nxt_port_rpc_reg_t *reg;

    ex = nxt_port_rpc_register_handler_ex(task, port, ready_handler, error_handler, 0);
...
void *nxt_port_rpc_register_handler_ex(nxt_task_t *task, nxt_port_t *port, nxt_port_rpc_handler_t ready_handler,
                                       nxt_port_rpc_handler_t error_handler, size_t ex_size)
{
    uint32_t stream;
    nxt_port_rpc_reg_t *reg;
...
void nxt_port_rpc_ex_set_peer(nxt_task_t *task, nxt_port_t *port, void *ex, nxt_pid_t peer)
{
    nxt_int_t ret;
    ...
    case NXT_OK:
        ...
        nxt_debug(task, "rpc: stream #%uD assigned uniq pid %PI (%p)", reg->stream, reg->peer, reg->link.next);
        break;
    ...
    case NXT_DECLINED:
        ...
        nxt_debug(task, "rpc: stream #%uD assigned duplicate pid %PI (%p)", reg->stream, reg->peer, reg->link.next);
        break;

    default:
        nxt_log_error(NXT_LOG_ERR, task->log,
                      "rpc: failed to add "
                      "peer for stream #%uD (%d)",
                      reg->stream, ret);
...
static void nxt_port_rpc_remove_from_peers(nxt_task_t *task, nxt_port_t *port, nxt_port_rpc_reg_t *reg)
{
    ...
    if (reg->link_first != 0)
    {
        ...
        if (reg->link.next == &reg->link)
        {
            ...
            nxt_debug(task,
                      "rpc: stream #%uD remove first and last pid %PI "
                      "registration (%p)",
                      stream, reg->peer, reg->link.next);
        ...
        {
            nxt_debug(task,
                      "rpc: stream #%uD remove first pid %PI "
                      "registration (%p)",
                      stream, reg->peer, reg->link.next);
...
void nxt_port_rpc_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
    ...
    nxt_debug(task, "rpc: stream #%uD %shandler, type %d", stream, (last ? "last " : ""), type);
...
void nxt_port_rpc_remove_peer(nxt_task_t *task, nxt_port_t *port, nxt_pid_t peer)
{
    uint8_t last;
    uint32_t stream;
    nxt_int_t ret;
    ...
    while (last == 0)
    {
        ...
        if (nxt_slow_path(ret != NXT_OK))
        {
            nxt_log_error(NXT_LOG_ERR, task->log, "rpc: stream #%uD failed to delete handler", stream);
