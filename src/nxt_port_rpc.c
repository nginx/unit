...
static void nxt_port_rpc_remove_from_peers(nxt_task_t *task, nxt_port_t *port,
...
void nxt_port_rpc_ex_set_peer(nxt_task_t *task, nxt_port_t *port, void *ex,
                              nxt_pid_t peer) {
  nxt_int_t ret;
  ...
  case NXT_OK:
    ...
    nxt_debug(task, "rpc: stream #%uD assigned uniq pid %PI (%p)", reg->stream,
              reg->peer, reg->link.next);
    break;
