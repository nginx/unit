...
nxt_regex_t *nxt_regex_compile(nxt_mp_t *mp, nxt_str_t *source, nxt_regex_err_t *err)
{
    int errcode;
    nxt_int_t ret;
    ...
    general_ctx = pcre2_general_context_create(nxt_pcre2_malloc, nxt_pcre2_free, mp);
    ...
    re->code = pcre2_compile((PCRE2_SPTR)source->start, source->length, 0, &errcode, &erroffset, compile_ctx);
    if (nxt_slow_path(re->code == NULL))
    {
        err->offset = erroffset;

        ret = pcre2_get_error_message(errcode, (PCRE2_UCHAR *)err->msg, ERR_BUF_SIZE);
        if (ret < 0)
        {
...
static void *nxt_pcre2_malloc(PCRE2_SIZE size, void *mp)
{
    return nxt_mp_get(mp, size);
}

static void nxt_pcre2_free(void *p, void *mp)
{
}

nxt_regex_match_t *nxt_regex_match_create(nxt_mp_t *mp, size_t size)
{
...
nxt_int_t nxt_regex_match(nxt_regex_t *re, u_char *subject, size_t length, nxt_regex_match_t *match)
{
    nxt_int_t ret;
    PCRE2_UCHAR errptr[ERR_BUF_SIZE];

    ret = pcre2_match(re->code, (PCRE2_SPTR)subject, length, 0, 0, match, NULL);
