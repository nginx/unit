...
static nxt_http_route_t* nxt_http_route_create(nxt_task_t* task,
                                               ...
                                               nxt_conf_value_t* cv);
static nxt_http_route_match_t* nxt_http_route_match_create(
    nxt_task_t* task,
    nxt_router_temp_conf_t* tmcf,
    nxt_conf_value_t* cv);
static nxt_http_route_table_t* nxt_http_route_table_create(
    nxt_task_t* task,
    nxt_mp_t* mp,
...
static nxt_http_route_ruleset_t* nxt_http_route_ruleset_create(
    nxt_task_t* task,
    nxt_mp_t* mp,
...
static nxt_http_route_rule_t* nxt_http_route_rule_name_create(
    ...
    nxt_bool_t case_sensitive,
    nxt_http_uri_encoding_t encoding);
static nxt_http_route_rule_t* nxt_http_route_rule_create(
    nxt_task_t* task,
    nxt_mp_t* mp,
    nxt_conf_value_t* cv,
    nxt_bool_t case_sensitive,
...
static nxt_int_t nxt_http_route_pattern_create(
    nxt_task_t* task,
    nxt_mp_t* mp,
    nxt_conf_value_t* cv,
    nxt_http_route_pattern_t* pattern,
...
static nxt_int_t nxt_http_route_decode_str(nxt_str_t* str,
                                           nxt_http_uri_encoding_t encoding);
static nxt_int_t nxt_http_route_pattern_slice(
    nxt_array_t* slices,
    nxt_str_t* test,
...
static nxt_http_action_t* nxt_http_pass_var(nxt_task_t* task,
                                            ...
                                            nxt_http_action_t* action);
static void nxt_http_pass_query(nxt_task_t* task,
                                nxt_http_request_t* r,
                                nxt_http_action_t* action);
static nxt_int_t nxt_http_pass_find(nxt_mp_t* mp,
                                    nxt_router_conf_t* rtcf,
                                    nxt_str_t* pass,
                                    nxt_http_action_t* action);
static nxt_int_t nxt_http_route_find(nxt_http_routes_t* routes,
                                     nxt_str_t* name,
                                     nxt_http_action_t* action);
...
static nxt_int_t nxt_http_route_pattern(nxt_http_request_t* r,
                                        ...
                                        u_char* start,
                                        size_t length);
static nxt_int_t nxt_http_route_memcmp(u_char* start,
                                       u_char* test,
                                       size_t length,
                                       nxt_bool_t case_sensitive);
...
static nxt_conf_map_t nxt_http_route_match_conf[] = {
    {nxt_string("scheme"), NXT_CONF_MAP_PTR,
...
static nxt_http_route_match_t* nxt_http_route_match_create(
  ...
  ret =
      nxt_conf_map_object(tmcf->mem_pool, match_conf, nxt_http_route_match_conf,
...
static nxt_conf_map_t nxt_http_route_action_conf[] = {
    {nxt_string("rewrite"), NXT_CONF_MAP_PTR,
     offsetof(nxt_http_action_conf_t, rewrite)},
    {nxt_string("response_headers"), NXT_CONF_MAP_PTR,
     offsetof(nxt_http_action_conf_t, set_headers)},
    {nxt_string("pass"), NXT_CONF_MAP_PTR,
     offsetof(nxt_http_action_conf_t, pass)},
    {nxt_string("return"), NXT_CONF_MAP_PTR,
     offsetof(nxt_http_action_conf_t, ret)},
    {nxt_string("location"), NXT_CONF_MAP_PTR,
     offsetof(nxt_http_action_conf_t, location)},
    {nxt_string("proxy"), NXT_CONF_MAP_PTR,
     offsetof(nxt_http_action_conf_t, proxy)},
    {nxt_string("share"), NXT_CONF_MAP_PTR,
     offsetof(nxt_http_action_conf_t, share)},
    {nxt_string("index"), NXT_CONF_MAP_PTR,
     offsetof(nxt_http_action_conf_t, index)},
    {nxt_string("chroot"), NXT_CONF_MAP_STR,
     offsetof(nxt_http_action_conf_t, chroot)},
    {nxt_string("follow_symlinks"), NXT_CONF_MAP_PTR,
     offsetof(nxt_http_action_conf_t, follow_symlinks)},
    {nxt_string("traverse_mounts"), NXT_CONF_MAP_PTR,
     offsetof(nxt_http_action_conf_t, traverse_mounts)},
    {nxt_string("types"), NXT_CONF_MAP_PTR,
     offsetof(nxt_http_action_conf_t, types)},
    {nxt_string("fallback"), NXT_CONF_MAP_PTR,
     offsetof(nxt_http_action_conf_t, fallback)},
};

nxt_int_t nxt_http_action_init(nxt_task_t* task,
                               nxt_router_temp_conf_t* tmcf,
                               nxt_conf_value_t* cv,
                               nxt_http_action_t* action) {
  nxt_mp_t* mp;
...
static nxt_http_route_rule_t* nxt_http_route_rule_name_create(
  ...
  rule = nxt_http_route_rule_create(task, mp, rule_cv, case_sensitive,
                                    NXT_HTTP_ROUTE_PATTERN_NOCASE, encoding);
  if (nxt_slow_path(rule == NULL)) {
    return NULL;
  }
...
nxt_http_route_rule_t* nxt_http_route_types_rule_create(
    nxt_task_t* task,
    nxt_mp_t* mp,
...
static nxt_int_t nxt_http_route_pattern_create(
  ...
  if (test.length > 0 && test.start[0] == '~') {
    ...
    if (nxt_slow_path(re == NULL)) {
      if (err.offset < test.length) {
        nxt_alert(task, "nxt_regex_compile(%V) failed: %s at offset %d", &test,
                  err.msg, (int)err.offset);
        return NXT_ERROR;
      }
  ...
  if (test.length != 0 && end[-1] != '*') {
    ...
    ret = nxt_http_route_pattern_slice(slices, &tmp, NXT_HTTP_ROUTE_PATTERN_END,
  ...
  while (p != end) {
    ...
    ret = nxt_http_route_pattern_slice(
        slices, &tmp, NXT_HTTP_ROUTE_PATTERN_SUBSTRING, encoding, pattern_case);
    if (nxt_slow_path(ret != NXT_OK)) {
      return ret;
    }
...
static nxt_http_action_t* nxt_http_pass_var(nxt_task_t* task,
  ...
  ret = nxt_tstr_query_init(&r->tstr_query, rtcf->tstr_state, &r->tstr_cache, r,
                            r->mem_pool);
  if (nxt_slow_path(ret != NXT_OK)) {
    goto fail;
  }
  ...
  action =
      nxt_mp_zget(r->mem_pool, sizeof(nxt_http_action_t) + sizeof(nxt_str_t));
...
nxt_int_t nxt_http_pass_segments(nxt_mp_t* mp,
                                 nxt_str_t* pass,
                                 nxt_str_t* segments,
                                 nxt_uint_t n) {
  u_char* p;
...
static nxt_int_t nxt_http_route_addr_pattern_match(
  ...
  switch (sa->u.sockaddr.sa_family) {
    ...
    case AF_INET6:
      ...
      switch (base->match_type) {
        ...
        case NXT_HTTP_ROUTE_ADDR_CIDR:
          for (i = 0; i < 16; i++) {
            match = ((sin6->sin6_addr.s6_addr[i] & p->addr.v6.end.s6_addr[i]) ==
