...
static nxt_int_t
nxt_http_route_test_argument(nxt_http_request_t *r, nxt_http_route_rule_t *rule,
                             nxt_array_t *array);
static nxt_int_t
...
static nxt_int_t
nxt_http_route_memcmp(u_char *start, u_char *test, size_t length,
                      nxt_bool_t case_sensitive);
                                  ...
                                  nxt_sockaddr_t                *sa)
{
    ...
    case AF_INET:
        ...
        match
            = (base->addr_family == AF_INET || base->addr_family == AF_UNSPEC);
        if (!match) {
            break;
        }
    ...
    case AF_INET6:
        ...
        case NXT_HTTP_ROUTE_ADDR_CIDR:
            for (i = 0; i < 16; i++) {
                match
                    = ((sin6->sin6_addr.s6_addr[i] & p->addr.v6.end.s6_addr[i])
