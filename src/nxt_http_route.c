                             ...
                             nxt_conf_value_t *cv)
{
  ...
  size
      = sizeof (nxt_http_route_match_t) + n * sizeof (nxt_http_route_test_t *);
  ...
  ret = nxt_conf_map_object (tmcf->mem_pool, match_conf,
                             nxt_http_route_match_conf,
                            ...
                            nxt_http_uri_encoding_t encoding)
{
  ...
  size
      = sizeof (nxt_http_route_rule_t) + n * sizeof (nxt_http_route_pattern_t);
  ...
  if (test.length != 0 && end[-1] != '*')
    {
      ...
      ret = nxt_http_route_pattern_slice (
          slices, &tmp, NXT_HTTP_ROUTE_PATTERN_END, encoding, pattern_case);
      if (nxt_slow_path (ret != NXT_OK))
        {
          return ret;
        }
...
static nxt_int_t
nxt_http_route_ruleset (nxt_http_request_t *r,
                        nxt_http_route_ruleset_t *ruleset)
{
  nxt_int_t ret;
                                   ...
                                   nxt_sockaddr_t *sa)
{
  ...
  switch (sa->u.sockaddr.sa_family)
    ...
    case AF_INET:
      ...
      switch (base->match_type)
        ...
        case NXT_HTTP_ROUTE_ADDR_CIDR:
          match
              = ((sin->sin_addr.s_addr & p->addr.v4.end) == p->addr.v4.start);
          break;
    ...
    case AF_INET6:
      ...
      match
          = (base->addr_family == AF_INET6 || base->addr_family == AF_UNSPEC);
      if (!match)
        {
          break;
        }
...
static nxt_int_t
nxt_http_route_test_cookie (nxt_http_request_t *r, nxt_http_route_rule_t *rule,
                            nxt_array_t *array)
{
  nxt_int_t ret;
  nxt_http_name_value_t *nv, *end;
...
static nxt_int_t
nxt_http_route_pattern (nxt_http_request_t *r,
                        nxt_http_route_pattern_t *pattern, u_char *start,
                        size_t length)
{
  u_char *p, *end, *test;
  size_t test_length;
