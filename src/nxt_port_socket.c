...
static uint8_t nxt_port_enqueue_buf(nxt_task_t* task,
                                    nxt_port_msg_t* pm,
                                    void* qbuf,
                                    nxt_buf_t* b);
static nxt_int_t nxt_port_msg_chk_insert(nxt_task_t* task,
                                         nxt_port_t* port,
...
static void nxt_port_queue_read_handler(nxt_task_t* task,
                                        void* obj,
                                        void* data);
static void nxt_port_read_msg_process(nxt_task_t* task,
                                      nxt_port_t* port,
...
nxt_int_t nxt_port_socket_init(nxt_task_t* task,
                               nxt_port_t* port,
                               size_t max_size) {
...
nxt_int_t nxt_port_socket_write2(nxt_task_t* task,
                                 nxt_port_t* port,
                                 nxt_uint_t type,
                                 nxt_fd_t fd,
                                 nxt_fd_t fd2,
  ...
  if (port->queue != NULL && type != _NXT_PORT_MSG_READ_QUEUE) {
    if (fd == -1 && nxt_port_can_enqueue_buf(b)) {
      ...
      nxt_debug(task, "port{%d,%d} %d: enqueue %d notify %d, %d",
                (int)port->pid, (int)port->id, port->socket.fd, (int)qmsg_size,
                notify, res);
      ...
      nxt_debug(task, "port{%d,%d} %d: enqueue 1 notify %d, %d", (int)port->pid,
                (int)port->id, port->socket.fd, notify, res);
...
static void nxt_port_fd_block_write(nxt_task_t* task,
                                    nxt_port_t* port,
                                    void* data) {
  nxt_fd_event_block_write(task->thread->engine, &port->socket);
}

static void nxt_port_fd_enable_write(nxt_task_t* task,
                                     nxt_port_t* port,
                                     void* data) {
...
static void nxt_port_write_handler(nxt_task_t* task, void* obj, void* data) {
  ...
  next_fragment:
    ...
    if (m == NXT_PORT_METHOD_MMAP) {
      sb.limit = (1ULL << 31) - 1;
      sb.nmax =
          nxt_min(NXT_IOBUF_MAX * 10 - 1, port->max_size / PORT_MMAP_MIN_SIZE);
    }
...
void nxt_port_read_enable(nxt_task_t* task, nxt_port_t* port) {
  ...
  port->socket.read_handler =
      port->queue != NULL ? nxt_port_queue_read_handler : nxt_port_read_handler;
  port->socket.error_handler = nxt_port_error_handler;
...
static void nxt_port_read_handler(nxt_task_t* task, void* obj, void* data) {
  ...
  for (;;) {
    ...
    if (n > 0) {
      ...
      ret =
          nxt_socket_msg_oob_get(&oob, msg.fd, nxt_recv_msg_cmsg_pid_ref(&msg));
...
static void nxt_port_queue_read_handler(nxt_task_t* task,
  ...
  for (;;) {
    if (port->from_socket == 0) {
      ...
      nxt_debug(task, "port{%d,%d} %d: dequeue %d", (int)port->pid,
                (int)port->id, port->socket.fd, (int)n);
      ...
      if ((smsg = port->socket_msg) != NULL && smsg->size != 0) {
        ...
        nxt_debug(task, "port{%d,%d} %d: use suspended message %d",
                  (int)port->pid, (int)port->id, port->socket.fd, (int)n);
    ...
    if (n >= (ssize_t)sizeof(nxt_port_msg_t)) {
      ...
      if (n == (ssize_t)sizeof(nxt_port_msg_t) &&
        ...
        nxt_debug(task, "port{%d,%d} %d: recv %d read_queue", (int)port->pid,
                  (int)port->id, port->socket.fd, (int)n);

        continue;
      }

      nxt_debug(task, "port{%d,%d} %d: recvmsg %d", (int)port->pid,
                (int)port->id, port->socket.fd, (int)n);

      if (n > 0) {
        if (port->from_socket == 0) {
          nxt_debug(task, "port{%d,%d} %d: suspend message %d", (int)port->pid,
                    (int)port->id, port->socket.fd, (int)n);
...
static nxt_int_t nxt_port_lvlhsh_frag_test(nxt_lvlhsh_query_t* lhq,
                                           void* data) {
...
static nxt_port_recv_msg_t* nxt_port_frag_start(nxt_task_t* task,
                                                nxt_port_t* port,
                                                nxt_port_recv_msg_t* msg) {
  nxt_int_t res;
...
static nxt_port_recv_msg_t* nxt_port_frag_find(nxt_task_t* task,
                                               nxt_port_t* port,
                                               nxt_port_recv_msg_t* msg) {
  nxt_int_t res;
  nxt_bool_t last;
...
static void nxt_port_read_msg_process(nxt_task_t* task,
  ...
  if (nxt_slow_path(msg->size < sizeof(nxt_port_msg_t))) {
    nxt_alert(task, "port %d: too small message:%uz", port->socket.fd,
              msg->size);
  ...
  if (msg->port_msg.mmap && orig_b != b) {
    ...
    if (msg->buf == b) {
      ...
      while (b != NULL) {
        ...
        nxt_work_queue_add(port->socket.read_work_queue, b->completion_handler,
                           task, b, b->parent);
