...
static nxt_int_t nxt_port_hash_test(nxt_lvlhsh_query_t *lhq, void *data)
{
    nxt_port_t *port;
    ...
    if (lhq->key.length == sizeof(nxt_pid_port_id_t) && pid_port_id->pid == port->pid &&
...
nxt_inline void nxt_port_hash_lhq(nxt_lvlhsh_query_t *lhq, nxt_pid_port_id_t *pid_port)
{
...
nxt_int_t nxt_port_hash_add(nxt_lvlhsh_t *port_hash, nxt_port_t *port)
{
    ...
    default:
        nxt_thread_log_error(NXT_LOG_WARN, "port #%d for pid %PI add failed", port->id, port->pid);
        break;
    }
...
nxt_int_t nxt_port_hash_remove(nxt_lvlhsh_t *port_hash, nxt_port_t *port)
{
    ...
    default:
        nxt_thread_log_error(NXT_LOG_WARN, "port #%d for pid %PI remove failed", port->id, port->pid);
        break;
    }
...
nxt_port_t *nxt_port_hash_find(nxt_lvlhsh_t *port_hash, nxt_pid_t pid, nxt_port_id_t port_id)
{
    nxt_pid_port_id_t pid_port;
    nxt_lvlhsh_query_t lhq;
