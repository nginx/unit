...
#define nxt_thread_extern_data(type, tsd) NXT_EXPORT extern __thread type tsd

#define nxt_thread_declare_data(type, tsd) __thread type tsd

#define nxt_thread_init_data(tsd)

#define nxt_thread_get_data(tsd) &tsd
...
#define nxt_thread_extern_data(type, tsd) NXT_EXPORT extern nxt_thread_specific_data_t tsd

#define nxt_thread_declare_data(type, tsd) nxt_thread_specific_data_t tsd = {{(nxt_atomic_int_t) - 1, sizeof(type)}}

NXT_EXPORT void nxt_thread_init_data(nxt_thread_specific_data_t tsd);

#define nxt_thread_get_data(tsd) pthread_getspecific((pthread_key_t)tsd->key)
...
NXT_EXPORT nxt_int_t nxt_thread_create(nxt_thread_handle_t *handle, nxt_thread_link_t *link);
...
#define nxt_thread_handle() pthread_self()
...
NXT_EXPORT nxt_err_t nxt_thread_cond_wait(nxt_thread_cond_t *cond, nxt_thread_mutex_t *mtx, nxt_nsec_t timeout);

#if (NXT_HAVE_PTHREAD_YIELD)
#define nxt_thread_yield() pthread_yield()

#elif (NXT_HAVE_PTHREAD_YIELD_NP)
#define nxt_thread_yield() pthread_yield_np()

#else
#define nxt_thread_yield() nxt_sched_yield()
