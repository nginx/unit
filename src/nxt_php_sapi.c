...
static nxt_int_t nxt_php_setup(nxt_task_t* task,
                               nxt_process_t* process,
                               nxt_common_app_conf_t* conf);
static nxt_int_t nxt_php_start(nxt_task_t* task, nxt_process_data_t* data);
static nxt_int_t nxt_php_set_target(nxt_task_t* task,
                                    nxt_php_target_t* target,
                                    nxt_conf_value_t* conf);
static nxt_int_t nxt_php_set_ini_path(nxt_task_t* task,
                                      nxt_str_t* path,
                                      char* workdir);
static void nxt_php_set_options(nxt_task_t* task,
                                nxt_conf_value_t* options,
                                int type);
static nxt_int_t nxt_php_alter_option(nxt_str_t* name,
                                      nxt_str_t* value,
                                      int type);
#ifdef NXT_PHP8
static void nxt_php_disable_functions(nxt_str_t* str);
#endif
static void nxt_php_disable(nxt_task_t* task,
                            const char* type,
                            nxt_str_t* value,
                            char** ptr,
...
static void nxt_zend_stream_init_fp(zend_file_handle* handle,
                                    FILE* fp,
                                    const char* filename);
#endif
...
static void nxt_php_set_sptr(nxt_unit_request_info_t* req,
                             const char* name,
                             nxt_unit_sptr_t* v,
                             uint32_t len,
                             zval* track_vars_array TSRMLS_DC);
nxt_inline void nxt_php_set_str(nxt_unit_request_info_t* req,
                                const char* name,
                                nxt_str_t* s,
                                zval* track_vars_array TSRMLS_DC);
static void nxt_php_set_cstr(nxt_unit_request_info_t* req,
                             const char* name,
                             const char* str,
                             uint32_t len,
                             zval* track_vars_array TSRMLS_DC);
...
#ifdef NXT_PHP7
#if (PHP_VERSION_ID < 70200)
ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(arginfo_fastcgi_finish_request,
                                        0,
                                        0,
                                        _IS_BOOL,
                                        NULL,
...
NXT_EXPORT nxt_app_module_t nxt_app_module = {
    sizeof(compat), compat, nxt_string("php"), PHP_VERSION,
    NULL,           0,      nxt_php_setup,     nxt_php_start,
};
  ...
  *p = '\0';
  ...
  if (value != NULL) {
    ...
    *p = '\0';
    ...
    if (!nxt_str_start(&target->script_filename, target->root.start,
                       target->root.length)) {
    ...
    target->script_name.length =
        target->script_filename.length - target->root.length;
    target->script_name.start =
        target->script_filename.start + target->root.length;
  ...
  if (options != NULL) {
    ...
    for (;;) {
      ...
      if (nxt_php_alter_option(&name, &value, type) != NXT_OK) {
        nxt_log(task, NXT_LOG_ERR, "setting PHP option \"%V: %V\" failed",
                &name, &value);
        continue;
      }
...
#else
        nxt_php_disable(task, "function", &value, &PG(disable_functions),
      ...
      if (nxt_str_eq(&name, "disable_classes", 15)) {
        nxt_php_disable(task, "class", &value, &PG(disable_classes),
                        zend_disable_class);
        continue;
      }
    }
...
static nxt_int_t nxt_php_alter_option(nxt_str_t* name,
  ...
  if (ini_entry->on_modify &&
      ini_entry->on_modify(ini_entry, zs, ini_entry->mh_arg1,
                           ...
                           ZEND_INI_STAGE_ACTIVATE) != SUCCESS) {
...
static nxt_int_t nxt_php_alter_option(nxt_str_t* name,
  ...
  if (ini_entry->on_modify &&
      ini_entry->on_modify(ini_entry, cstr, value->length, ini_entry->mh_arg1,
                           ini_entry->mh_arg2, ini_entry->mh_arg3,
        ...
        *p = '\0';
        ...
        if (disable(start, p - start TSRMLS_CC)
          ...
          nxt_log(task, NXT_LOG_ERR,
                  "PHP: failed to disable \"%s\": no such %s", start, type);
        }
      }
...
static nxt_int_t nxt_php_do_301(nxt_unit_request_info_t* req) {
  ...
  url = nxt_malloc(sizeof("https://") - 1 + r->server_name_length +
                   r->local_port_length + 1 + r->path_length + 1 +
                   r->query_length + 1 + 1);
  ...
  if (r->local_port_length > 0 && !(r->tls && strcmp(port, "443") == 0) &&
  ...
  *p = '\0';
  ...
  nxt_unit_response_add_field(req, "Location", nxt_length("Location"), url,
                              size);
...
static void nxt_php_dynamic_request(nxt_php_run_ctx_t* ctx,
  ...
  ctx->path_info.start =
      memmem(path.start, path.length, ".php/", strlen(".php/"));
  ...
  ctx->script_filename.length =
      ctx->root->length + path.length + script_name.length;
...
static void nxt_php_execute(nxt_php_run_ctx_t* ctx, nxt_unit_request_t* r) {
  ...
  SG(request_info).query_string =
      r->query.offset ? nxt_unit_sptr_get(&r->query) : NULL;
...
nxt_inline void nxt_php_vcwd_chdir(nxt_unit_request_info_t* req, u_char* dir) {
  if (nxt_slow_path(VCWD_CHDIR((char*)dir) != 0)) {
    nxt_unit_req_alert(req, "VCWD_CHDIR(%s) failed (%d: %s)", dir, errno,
                       strerror(errno));
  }
}
...
static int nxt_php_send_headers(sapi_headers_struct* sapi_headers TSRMLS_DC) {
  ...
  for (h = zend_llist_get_first_ex(&sapi_headers->headers, &zpos); h;
    ...
    nxt_unit_response_add_field(req, h->header, colon - h->header, value,
...
static void nxt_php_register_variables(zval* track_vars_array TSRMLS_DC) {
  ...
  php_register_variable_safe((char*)"SERVER_SOFTWARE", (char*)nxt_server.start,
...
#endif
{
  ...
  if (ctx != NULL) {
    ...
    nxt_unit_log(nxt_php_unit_ctx, NXT_UNIT_LOG_NOTICE, "php message: %s",
                 message);
  }
}
