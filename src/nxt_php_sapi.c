...
static nxt_int_t nxt_php_setup(nxt_task_t *task, nxt_process_t *process, nxt_common_app_conf_t *conf);
static nxt_int_t nxt_php_start(nxt_task_t *task, nxt_process_data_t *data);
static nxt_int_t nxt_php_set_target(nxt_task_t *task, nxt_php_target_t *target, nxt_conf_value_t *conf);
static nxt_int_t nxt_php_set_ini_path(nxt_task_t *task, nxt_str_t *path, char *workdir);
static void nxt_php_set_options(nxt_task_t *task, nxt_conf_value_t *options, int type);
static nxt_int_t nxt_php_alter_option(nxt_str_t *name, nxt_str_t *value, int type);
#ifdef NXT_PHP8
static void nxt_php_disable_functions(nxt_str_t *str);
#endif
static void nxt_php_disable(nxt_task_t *task, const char *type, nxt_str_t *value, char **ptr,
                            nxt_php_disable_t disable);
...
static void nxt_php_dynamic_request(nxt_php_run_ctx_t *ctx, nxt_unit_request_t *r);
#if (PHP_VERSION_ID < 70400)
static void nxt_zend_stream_init_fp(zend_file_handle *handle, FILE *fp, const char *filename);
#endif
...
static void *nxt_php_hash_str_find_ptr(const HashTable *ht, const nxt_str_t *str);
static char *nxt_php_read_cookies(TSRMLS_D);
static void nxt_php_set_sptr(nxt_unit_request_info_t *req, const char *name, nxt_unit_sptr_t *v, uint32_t len,
                             zval *track_vars_array TSRMLS_DC);
nxt_inline void nxt_php_set_str(nxt_unit_request_info_t *req, const char *name, nxt_str_t *s,
                                zval *track_vars_array TSRMLS_DC);
static void nxt_php_set_cstr(nxt_unit_request_info_t *req, const char *name, const char *str, uint32_t len,
                             zval *track_vars_array TSRMLS_DC);
...
#ifdef NXT_PHP7
#if (PHP_VERSION_ID < 70200)
ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(arginfo_fastcgi_finish_request, 0, 0, _IS_BOOL, NULL, 0)
#else
...
static const zend_function_entry nxt_php_ext_functions[] = {
    ZEND_FE(fastcgi_finish_request, arginfo_fastcgi_finish_request) ZEND_FE_END};

zif_handler nxt_php_chdir_handler;
zend_auto_global *nxt_php_server_ag;

static zend_module_entry nxt_php_unit_module = {STANDARD_MODULE_HEADER,
                                                "unit",
...
NXT_EXPORT nxt_app_module_t nxt_app_module = {
    sizeof(compat), compat, nxt_string("php"), PHP_VERSION, NULL, 0, nxt_php_setup, nxt_php_start,
};
...
#error PHP is built with thread safety and broken signals.
    ...
    if (c->options != NULL)
    {
        ...
        if (value != NULL)
        {
            ...
            ret = nxt_php_set_ini_path(task, &ini_path, conf->working_directory);
...
#ifdef NXT_PHP7
    nxt_php_server_ag = zend_hash_str_find_ptr(CG(auto_globals), "_SERVER", nxt_length("_SERVER"));
#else
    zend_hash_quick_find(CG(auto_globals), "_SERVER", sizeof("_SERVER"), zend_hash_func("_SERVER", sizeof("_SERVER")),
    ...
    *p = '\0';
    ...
    if (value != NULL)
    {
        ...
        *p = '\0';
        ...
        if (!nxt_str_start(&target->script_filename, target->root.start, target->root.length))
        {
            nxt_alert(task, "script is not under php root");
            return NXT_ERROR;
        }

        ret = nxt_php_dirname(&target->script_filename, &target->script_dirname);
        if (nxt_slow_path(ret != NXT_OK))
        {
            return NXT_ERROR;
        }

        target->script_name.length = target->script_filename.length - target->root.length;
        target->script_name.start = target->script_filename.start + target->root.length;
    }
    else
    {
...
static nxt_int_t nxt_php_set_ini_path(nxt_task_t *task, nxt_str_t *ini_path, char *workdir)
{
    size_t wdlen;
    u_char *p, *start;
...
static void nxt_php_set_options(nxt_task_t *task, nxt_conf_value_t *options, int type)
{
    ...
    if (options != NULL)
    {
        ...
        for (;;)
        {
            ...
            {
                nxt_log(task, NXT_LOG_ERR, "setting PHP option \"%V: %V\" failed", &name, &value);
                continue;
            }
            ...
            {
                nxt_php_disable(task, "class", &value, &PG(disable_classes), zend_disable_class);
                continue;
            }
        }
    }
...
static nxt_int_t nxt_php_alter_option(nxt_str_t *name, nxt_str_t *value, int type)
{
    ...
    if (ini_entry->on_modify && ini_entry->on_modify(ini_entry, zs, ini_entry->mh_arg1, ini_entry->mh_arg2,
...
static nxt_int_t nxt_php_alter_option(nxt_str_t *name, nxt_str_t *value, int type)
{
    ...
    if (ini_entry->on_modify &&
        ini_entry->on_modify(ini_entry, cstr, value->length, ini_entry->mh_arg1, ini_entry->mh_arg2, ini_entry->mh_arg3,
                             ZEND_INI_STAGE_ACTIVATE TSRMLS_CC) != SUCCESS)
    {
        nxt_free(cstr);
        return NXT_ERROR;
    }
...
static void nxt_php_disable(nxt_task_t *task, const char *type, nxt_str_t *value, char **ptr, nxt_php_disable_t disable)
{
    char c, *p, *start;
            ...
            if (p != start)
            {
                *p = '\0';
                ...
                if (disable(start, p - start TSRMLS_CC)
                    ...
                    != SUCCESS)
                {
                    nxt_log(task, NXT_LOG_ERR, "PHP: failed to disable \"%s\": no such %s", start, type);
                }
            }
...
static nxt_int_t nxt_php_do_301(nxt_unit_request_info_t *req)
{
    ...
    url = nxt_malloc(sizeof("https://") - 1 + r->server_name_length + r->local_port_length + 1 + r->path_length + 1 +
                     r->query_length + 1 + 1);
    ...
    p = nxt_cpymem(p, nxt_unit_sptr_get(&r->server_name), r->server_name_length);

    port = nxt_unit_sptr_get(&r->local_port);
    if (r->local_port_length > 0 && !(r->tls && strcmp(port, "443") == 0) && !(!r->tls && strcmp(port, "80") == 0))
    {
        *p++ = ':';
    ...
    *p = '\0';
    ...
    nxt_unit_response_init(req, NXT_HTTP_MOVED_PERMANENTLY, 1, nxt_length("Location") + size);
    nxt_unit_response_add_field(req, "Location", nxt_length("Location"), url, size);
...
static void nxt_php_dynamic_request(nxt_php_run_ctx_t *ctx, nxt_unit_request_t *r)
{
    ...
    ctx->path_info.start = memmem(path.start, path.length, ".php/", strlen(".php/"));
    ...
    else if (path.length < 4 || memcmp(path.start + (path.length - 4), ".php", 4) != 0)
    {
        char tpath[PATH_MAX];
        nxt_int_t ec;
        struct stat sb;
    ...
    ctx->script_filename.length = ctx->root->length + path.length + script_name.length;
...
nxt_inline void nxt_php_vcwd_chdir(nxt_unit_request_info_t *req, u_char *dir)
{
    ...
    {
        nxt_unit_req_alert(req, "VCWD_CHDIR(%s) failed (%d: %s)", dir, errno, strerror(errno));
    }
}
...
static int nxt_php_send_headers(sapi_headers_struct *sapi_headers TSRMLS_DC)
{
    ...
    for (h = zend_llist_get_first_ex(&sapi_headers->headers, &zpos); h;
         ...
         h = zend_llist_get_next_ex(&sapi_headers->headers, &zpos))
    {
        ...
        {
            nxt_unit_req_warn(req, "colon not found in header '%.*s'", (int)h->header_len, h->header);
            continue;
        }
        ...
        nxt_unit_response_add_field(req, h->header, colon - h->header, value, h->header_len - (value - h->header));
    }
...
static void nxt_php_register_variables(zval *track_vars_array TSRMLS_DC)
{
    ...
    php_register_variable_safe((char *)"SERVER_SOFTWARE", (char *)nxt_server.start, nxt_server.length,
                               track_vars_array TSRMLS_CC);

    nxt_php_set_sptr(req, "SERVER_PROTOCOL", &r->version, r->version_length, track_vars_array TSRMLS_CC);
    ...
    {
        nxt_php_set_sptr(req, "PHP_SELF", &r->path, r->path_length, track_vars_array TSRMLS_CC);

        nxt_php_set_str(req, "PATH_INFO", &ctx->path_info, track_vars_array TSRMLS_CC);
    }
    else
    {
        nxt_php_set_str(req, "PHP_SELF", &ctx->script_name, track_vars_array TSRMLS_CC);
    }
    ...
    nxt_php_set_str(req, "SCRIPT_NAME", &ctx->script_name, track_vars_array TSRMLS_CC);
    ...
    nxt_php_set_str(req, "SCRIPT_FILENAME", &ctx->script_filename, track_vars_array TSRMLS_CC);
    ...
    nxt_php_set_str(req, "DOCUMENT_ROOT", ctx->root, track_vars_array TSRMLS_CC);

    nxt_php_set_sptr(req, "REQUEST_METHOD", &r->method, r->method_length, track_vars_array TSRMLS_CC);
    nxt_php_set_sptr(req, "REQUEST_URI", &r->target, r->target_length, track_vars_array TSRMLS_CC);
    nxt_php_set_sptr(req, "QUERY_STRING", &r->query, r->query_length, track_vars_array TSRMLS_CC);

    nxt_php_set_sptr(req, "REMOTE_ADDR", &r->remote, r->remote_length, track_vars_array TSRMLS_CC);
    nxt_php_set_sptr(req, "SERVER_ADDR", &r->local_addr, r->local_addr_length, track_vars_array TSRMLS_CC);

    nxt_php_set_sptr(req, "SERVER_NAME", &r->server_name, r->server_name_length, track_vars_array TSRMLS_CC);
    ...
    for (f = r->fields; f < f_end; f++)
    {
        ...
        nxt_php_set_sptr(req, name, &f->value, f->value_length, track_vars_array TSRMLS_CC);
    }
    ...
    if (r->content_length_field != NXT_UNIT_NONE_FIELD)
    {
        ...
        nxt_php_set_sptr(req, "CONTENT_LENGTH", &f->value, f->value_length, track_vars_array TSRMLS_CC);
    }
    ...
    if (r->content_type_field != NXT_UNIT_NONE_FIELD)
    {
        ...
        nxt_php_set_sptr(req, "CONTENT_TYPE", &f->value, f->value_length, track_vars_array TSRMLS_CC);
    }
}
                             ...
                             zval *track_vars_array TSRMLS_DC)
{
    ...
    if (sapi_module.input_filter(PARSE_SERVER, (char *)name, &str, len, &new_len TSRMLS_CC))
    {
        php_register_variable_safe((char *)name, str, new_len, track_vars_array TSRMLS_CC);
    }
}
...
nxt_inline void nxt_php_set_str(nxt_unit_request_info_t *req, const char *name, nxt_str_t *s,
                                zval *track_vars_array TSRMLS_DC)
{
    nxt_php_set_cstr(req, name, (char *)s->start, s->length, track_vars_array TSRMLS_CC);
}
...
static void nxt_php_set_cstr(nxt_unit_request_info_t *req, const char *name, const char *cstr, uint32_t len,
                             zval *track_vars_array TSRMLS_DC)
{
    if (nxt_slow_path(cstr == NULL))
    {
    ...
    php_register_variable_safe((char *)name, (char *)cstr, len, track_vars_array TSRMLS_CC);
}
...
#endif
{
    ...
    if (ctx != NULL)
    {
        nxt_unit_req_log(ctx->req, NXT_UNIT_LOG_NOTICE, "php message: %s", message);
    }
    else
    {
        nxt_unit_log(nxt_php_unit_ctx, NXT_UNIT_LOG_NOTICE, "php message: %s", message);
    }
}
