...
#else
static int nxt_php_unbuffered_write (const char *str,
...
NXT_EXPORT nxt_app_module_t nxt_app_module = {
  sizeof (compat), compat,        nxt_string ("php"), PHP_VERSION, NULL, 0,
  nxt_php_setup,   nxt_php_start,
};
...
nxt_php_alter_option (nxt_str_t *name, nxt_str_t *value, int type)
{
  ...
  if (ini_entry->on_modify
      && ini_entry->on_modify (ini_entry, cstr, value->length,
                               ...
                               ini_entry->mh_arg3,
                               ZEND_INI_STAGE_ACTIVATE TSRMLS_CC)
             != SUCCESS)
    {
      nxt_free (cstr);
      return NXT_ERROR;
          ...
          if (p != start)
            {
              *p = '\0';
              ...
              if (disable (start, p - start TSRMLS_CC)
                ...
                {
                  nxt_log (task, NXT_LOG_ERR,
                           "PHP: failed to disable \"%s\": no such %s", start,
                           type);
                }
            }
...
nxt_inline void
nxt_php_set_str (nxt_unit_request_info_t *req, const char *name, nxt_str_t *s,
                 zval *track_vars_array TSRMLS_DC)
{
...
#endif
{
  ...
  if (ctx != NULL)
    {
      nxt_unit_req_log (ctx->req, NXT_UNIT_LOG_NOTICE, "php message: %s",
                        message);
    }
  else
    {
