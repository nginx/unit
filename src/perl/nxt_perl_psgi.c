...
static SV *nxt_perl_psgi_call_var_application(PerlInterpreter *my_perl, SV *env,
                                              SV *app,
                                              nxt_unit_request_info_t *req);
...
static int nxt_perl_psgi_io_init(PerlInterpreter *my_perl,
                                 nxt_perl_psgi_io_arg_t *arg, const char *mode,
                                 void *req);
...
static nxt_int_t nxt_perl_psgi_result_status(PerlInterpreter *my_perl,
                                             SV *result);
static int nxt_perl_psgi_result_head(PerlInterpreter *my_perl, SV *sv_head,
                                     nxt_unit_request_info_t *req,
                                     uint16_t status);
static int nxt_perl_psgi_result_body(PerlInterpreter *my_perl, SV *result,
                                     nxt_unit_request_info_t *req);
static int nxt_perl_psgi_result_body_ref(PerlInterpreter *my_perl, SV *sv_body,
                                         nxt_unit_request_info_t *req);
...
static ssize_t nxt_perl_psgi_io_read(nxt_unit_read_info_t *read_info, void *dst,
                                     size_t size);
static int nxt_perl_psgi_result_array(PerlInterpreter *my_perl, SV *result,
                                      nxt_unit_request_info_t *req);
...
nxt_inline void nxt_perl_psgi_cb_request_done(nxt_perl_psgi_ctx_t *pctx,
                                              int status) {
  if (pctx->req != NULL) {
...
XS(XS_NGINX__Unit__Sandbox_cb) {
  ...
  if (nxt_slow_path(SvOK(ST(0)) == 0 || SvROK(ST(0)) == 0 ||
                    SvTYPE(SvRV(ST(0))) != SVt_PVAV || pctx->req == NULL)) {
    nxt_perl_psgi_cb_request_done(CvXSUBANY(cv).any_ptr, NXT_UNIT_ERROR);
...
static void nxt_perl_psgi_xs_init(pTHX) {
  ...
  */
  ...
  nxt_perl_psgi_cb =
      newXS("NGINX::Unit::Sandbox::cb", XS_NGINX__Unit__Sandbox_cb, __FILE__);
}
...
static void nxt_perl_psgi_io_release(PerlInterpreter *my_perl,
                                     nxt_perl_psgi_io_arg_t *arg) {
  if (arg->io != NULL) {
    SvREFCNT_dec(arg->io);
    arg->io = NULL;
...
#define NL(S) (S), sizeof(S) - 1
  ...
  RC(nxt_perl_psgi_add_sptr(my_perl, hash_env, NL("REQUEST_METHOD"), &r->method,
                            r->method_length));
  RC(nxt_perl_psgi_add_sptr(my_perl, hash_env, NL("REQUEST_URI"), &r->target,
                            r->target_length));
  RC(nxt_perl_psgi_add_sptr(my_perl, hash_env, NL("PATH_INFO"), &r->path,
                            r->path_length));
  ...
  RC(nxt_perl_psgi_add_value(my_perl, hash_env, NL("psgi.multithread"),
                             nxt_perl_psgi_ctxs != NULL ? &PL_sv_yes
                                                        : &PL_sv_no));
  ...
  RC(nxt_perl_psgi_add_sptr(my_perl, hash_env, NL("QUERY_STRING"), &r->query,
                            r->query_length));
  RC(nxt_perl_psgi_add_sptr(my_perl, hash_env, NL("SERVER_PROTOCOL"),
                            &r->version, r->version_length));
  RC(nxt_perl_psgi_add_sptr(my_perl, hash_env, NL("REMOTE_ADDR"), &r->remote,
                            r->remote_length));
  ...
  for (i = 0; i < r->fields_count; i++) {
    ...
    RC(nxt_perl_psgi_add_sptr(my_perl, hash_env, nxt_unit_sptr_get(&f->name),
                              f->name_length, &f->value, f->value_length));
  }
  ...
  if (r->content_type_field != NXT_UNIT_NONE_FIELD) {
    ...
    RC(nxt_perl_psgi_add_sptr(my_perl, hash_env, NL("CONTENT_TYPE"), &f->value,
                              f->value_length));
  }
...
static int nxt_perl_psgi_result_body(PerlInterpreter *my_perl, SV *sv_body,
                                     nxt_unit_request_info_t *req) {
  SV **entry;
  AV *body_array;
  int rc;
...
static int nxt_perl_psgi_ready_handler(nxt_unit_ctx_t *ctx) {
  ...
  for (i = 0; i < c->threads - 1; i++) {
    ...
    if (nxt_fast_path(res == 0)) {
      ...
      nxt_unit_alert(ctx, "thread #%d create failed: %s (%d)", (int)(i + 1),
                     strerror(res), res);
...
static int nxt_perl_psgi_init_threads(nxt_perl_app_conf_t *c) {
  ...
  if (c->thread_stack_size > 0) {
    ...
    if (nxt_slow_path(rc != 0)) {
      nxt_unit_alert(NULL, "thread attr init failed: %s (%d)", strerror(rc),
                     rc);
  ...
  nxt_perl_psgi_ctxs =
      nxt_unit_malloc(NULL, sizeof(nxt_perl_psgi_ctx_t) * (c->threads - 1));
...
static void nxt_perl_psgi_join_threads(nxt_unit_ctx_t *ctx,
  ...
  for (i = 0; i < c->threads - 1; i++) {
    ...
    if (nxt_fast_path(res == 0)) {
      ...
      nxt_unit_alert(ctx, "thread #%d join failed: %s (%d)", (int)(i + 1),
                     strerror(res), res);
    }
  }
