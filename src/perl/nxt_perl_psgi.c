...
static SV *
nxt_perl_psgi_call_method (PerlInterpreter *my_perl, SV *obj,
...
static void
nxt_perl_psgi_io_release (PerlInterpreter *my_perl,
                          nxt_perl_psgi_io_arg_t *arg)
{
  if (arg->io != NULL)
    {
...
nxt_perl_psgi_add_str (PerlInterpreter *my_perl, HV *hash_env,
                       const char *name, uint32_t name_len, const char *str,
                       uint32_t len)
{
  SV **ha;
