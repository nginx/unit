...
static SSize_t nxt_perl_psgi_io_input_read(PerlInterpreter *my_perl, nxt_perl_psgi_io_arg_t *arg, void *vbuf,
                                           size_t length);
static SSize_t nxt_perl_psgi_io_input_write(PerlInterpreter *my_perl, nxt_perl_psgi_io_arg_t *arg, const void *vbuf,
                                            size_t length);

static SSize_t nxt_perl_psgi_io_error_read(PerlInterpreter *my_perl, nxt_perl_psgi_io_arg_t *arg, void *vbuf,
                                           size_t length);
static SSize_t nxt_perl_psgi_io_error_write(PerlInterpreter *my_perl, nxt_perl_psgi_io_arg_t *arg, const void *vbuf,
                                            size_t length);
...
static SV *nxt_perl_psgi_call_var_application(PerlInterpreter *my_perl, SV *env, SV *app, nxt_unit_request_info_t *req);
static SV *nxt_perl_psgi_call_method(PerlInterpreter *my_perl, SV *obj, const char *method,
                                     nxt_unit_request_info_t *req);

/* For currect load XS modules */
EXTERN_C void boot_DynaLoader(pTHX_ CV *cv);

static int nxt_perl_psgi_io_init(PerlInterpreter *my_perl, nxt_perl_psgi_io_arg_t *arg, const char *mode, void *req);

static int nxt_perl_psgi_ctx_init(const char *script, nxt_perl_psgi_ctx_t *pctx);

static SV *nxt_perl_psgi_env_create(PerlInterpreter *my_perl, nxt_unit_request_info_t *req);
nxt_inline int nxt_perl_psgi_add_sptr(PerlInterpreter *my_perl, HV *hash_env, const char *name, uint32_t name_len,
                                      nxt_unit_sptr_t *sptr, uint32_t len);
nxt_inline int nxt_perl_psgi_add_str(PerlInterpreter *my_perl, HV *hash_env, const char *name, uint32_t name_len,
                                     const char *str, uint32_t len);
nxt_inline int nxt_perl_psgi_add_value(PerlInterpreter *my_perl, HV *hash_env, const char *name, uint32_t name_len,
                                       void *value);

static char *nxt_perl_psgi_module_create(const char *script);

static nxt_int_t nxt_perl_psgi_result_status(PerlInterpreter *my_perl, SV *result);
static int nxt_perl_psgi_result_head(PerlInterpreter *my_perl, SV *sv_head, nxt_unit_request_info_t *req,
                                     uint16_t status);
static int nxt_perl_psgi_result_body(PerlInterpreter *my_perl, SV *result, nxt_unit_request_info_t *req);
static int nxt_perl_psgi_result_body_ref(PerlInterpreter *my_perl, SV *sv_body, nxt_unit_request_info_t *req);
static int nxt_perl_psgi_result_body_fh(PerlInterpreter *my_perl, SV *sv_body, nxt_unit_request_info_t *req);
static ssize_t nxt_perl_psgi_io_read(nxt_unit_read_info_t *read_info, void *dst, size_t size);
static int nxt_perl_psgi_result_array(PerlInterpreter *my_perl, SV *result, nxt_unit_request_info_t *req);
static void nxt_perl_psgi_result_cb(PerlInterpreter *my_perl, SV *result, nxt_unit_request_info_t *req);

static nxt_int_t nxt_perl_psgi_start(nxt_task_t *task, nxt_process_data_t *data);
...
static void nxt_perl_psgi_join_threads(nxt_unit_ctx_t *ctx, nxt_perl_app_conf_t *c);
...
NXT_EXPORT nxt_app_module_t nxt_app_module = {
    sizeof(nxt_perl_psgi_compat), nxt_perl_psgi_compat, nxt_string("perl"), PERL_VERSION_STRING, NULL, 0, NULL,
    nxt_perl_psgi_start,
};
...
nxt_inline void nxt_perl_psgi_cb_request_done(nxt_perl_psgi_ctx_t *pctx, int status)
{
    if (pctx->req != NULL)
    {
...
XS(XS_NGINX__Unit__Sandbox_cb)
{
    ...
    if (nxt_slow_path(SvOK(ST(0)) == 0 || SvROK(ST(0)) == 0 || SvTYPE(SvRV(ST(0))) != SVt_PVAV || pctx->req == NULL))
    {
        nxt_perl_psgi_cb_request_done(CvXSUBANY(cv).any_ptr, NXT_UNIT_ERROR);
    ...
    if (array_len < 2)
    {
        obj = sv_bless(newRV_noinc((SV *)newHV()), gv_stashpv("NGINX::Unit::Sandbox", GV_ADD));
        ST(0) = obj;
...
static void nxt_perl_psgi_xs_init(pTHX)
{
    ...
    */
    ...
    nxt_perl_psgi_write = newXS("NGINX::Unit::Sandbox::write", XS_NGINX__Unit__Sandbox_write, __FILE__);

    nxt_perl_psgi_close = newXS("NGINX::Unit::Sandbox::close", XS_NGINX__Unit__Sandbox_close, __FILE__);

    nxt_perl_psgi_cb = newXS("NGINX::Unit::Sandbox::cb", XS_NGINX__Unit__Sandbox_cb, __FILE__);
}
...
static char *nxt_perl_psgi_module_create(const char *script)
{
    ...
    static const nxt_str_t prefix = nxt_string("package NGINX::Unit::Sandbox;"
                                               "{my $app = do \"");
...
static void nxt_perl_psgi_io_release(PerlInterpreter *my_perl, nxt_perl_psgi_io_arg_t *arg)
{
    if (arg->io != NULL)
    {
        SvREFCNT_dec(arg->io);
...
static int nxt_perl_psgi_ctx_init(const char *script, nxt_perl_psgi_ctx_t *pctx)
{
    ...
    {
        nxt_unit_alert(NULL, "PSGI: Failed to allocate memory for Perl interpreter");
...
#define NL(S) (S), sizeof(S) - 1
    ...
    RC(nxt_perl_psgi_add_str(my_perl, hash_env, NL("SERVER_SOFTWARE"), (char *)nxt_server.start, nxt_server.length));

    RC(nxt_perl_psgi_add_sptr(my_perl, hash_env, NL("REQUEST_METHOD"), &r->method, r->method_length));
    RC(nxt_perl_psgi_add_sptr(my_perl, hash_env, NL("REQUEST_URI"), &r->target, r->target_length));
    RC(nxt_perl_psgi_add_sptr(my_perl, hash_env, NL("PATH_INFO"), &r->path, r->path_length));
    ...
    RC(nxt_perl_psgi_add_value(my_perl, hash_env, NL("psgi.version"), newRV_noinc((SV *)array_version)));

    RC(nxt_perl_psgi_add_value(my_perl, hash_env, NL("psgi.url_scheme"),
                               r->tls ? newSVpv("https", 5) : newSVpv("http", 4)));

    RC(nxt_perl_psgi_io_init(my_perl, &pctx->arg_input, "r", req));
    RC(nxt_perl_psgi_add_value(my_perl, hash_env, NL("psgi.input"), SvREFCNT_inc(pctx->arg_input.io)));

    RC(nxt_perl_psgi_io_init(my_perl, &pctx->arg_error, "w", req));
    RC(nxt_perl_psgi_add_value(my_perl, hash_env, NL("psgi.errors"), SvREFCNT_inc(pctx->arg_error.io)));

    RC(nxt_perl_psgi_add_value(my_perl, hash_env, NL("psgi.multithread"),
                               nxt_perl_psgi_ctxs != NULL ? &PL_sv_yes : &PL_sv_no));
    RC(nxt_perl_psgi_add_value(my_perl, hash_env, NL("psgi.multiprocess"), &PL_sv_yes));
    RC(nxt_perl_psgi_add_value(my_perl, hash_env, NL("psgi.run_once"), &PL_sv_no));
    RC(nxt_perl_psgi_add_value(my_perl, hash_env, NL("psgi.nonblocking"), &PL_sv_no));
    RC(nxt_perl_psgi_add_value(my_perl, hash_env, NL("psgi.streaming"), &PL_sv_yes));

    RC(nxt_perl_psgi_add_sptr(my_perl, hash_env, NL("QUERY_STRING"), &r->query, r->query_length));
    RC(nxt_perl_psgi_add_sptr(my_perl, hash_env, NL("SERVER_PROTOCOL"), &r->version, r->version_length));
    RC(nxt_perl_psgi_add_sptr(my_perl, hash_env, NL("REMOTE_ADDR"), &r->remote, r->remote_length));
    RC(nxt_perl_psgi_add_sptr(my_perl, hash_env, NL("SERVER_ADDR"), &r->local_addr, r->local_addr_length));

    RC(nxt_perl_psgi_add_sptr(my_perl, hash_env, NL("SERVER_NAME"), &r->server_name, r->server_name_length));
    ...
    for (i = 0; i < r->fields_count; i++)
    {
        ...
        RC(nxt_perl_psgi_add_sptr(my_perl, hash_env, nxt_unit_sptr_get(&f->name), f->name_length, &f->value,
                                  f->value_length));
    }
    ...
    if (r->content_length_field != NXT_UNIT_NONE_FIELD)
    {
        ...
        RC(nxt_perl_psgi_add_sptr(my_perl, hash_env, NL("CONTENT_LENGTH"), &f->value, f->value_length));
    }
    ...
    if (r->content_type_field != NXT_UNIT_NONE_FIELD)
    {
        ...
        RC(nxt_perl_psgi_add_sptr(my_perl, hash_env, NL("CONTENT_TYPE"), &f->value, f->value_length));
    }
...
nxt_inline int nxt_perl_psgi_add_sptr(PerlInterpreter *my_perl, HV *hash_env, const char *name, uint32_t name_len,
                                      nxt_unit_sptr_t *sptr, uint32_t len)
{
    return nxt_perl_psgi_add_str(my_perl, hash_env, name, name_len, nxt_unit_sptr_get(sptr), len);
}
                                     ...
                                     const char *str, uint32_t len)
{
    ...
    ha = hv_store(hash_env, name, (I32)name_len, newSVpv(str, (STRLEN)len), 0);
                                     ...
                                     uint16_t status)
{
    ...
    if (nxt_slow_path(SvROK(sv_head) == 0 || SvTYPE(SvRV(sv_head)) != SVt_PVAV))
    {
...
static int nxt_perl_psgi_result_body(PerlInterpreter *my_perl, SV *sv_body, nxt_unit_request_info_t *req)
{
    SV **entry;
    AV *body_array;
    int rc;
    ...
    if (nxt_slow_path(SvROK(sv_body) == 0 || SvTYPE(SvRV(sv_body)) != SVt_PVAV))
    {
        nxt_unit_req_error(req, "PSGI: An unsupported format was received from "
...
static int nxt_perl_psgi_result_array(PerlInterpreter *my_perl, SV *result, nxt_unit_request_info_t *req)
{
    ...
    {
        nxt_unit_req_error(req, "PSGI: Invalid result format from Perl Application");
    ...
    if (nxt_slow_path(status < 0))
    {
        nxt_unit_req_error(req, "PSGI: An unexpected status was received "
                                "from Perl Application");
    ...
    {
        nxt_unit_req_error(req, "PSGI: Failed to get body from "
                                "Perl ARRAY variable");
...
static void nxt_perl_psgi_request_handler(nxt_unit_request_info_t *req)
{
    ...
    {
        nxt_unit_req_error(req, "PSGI: Failed to create 'env' for Perl Application");
    ...
    result = nxt_perl_psgi_call_var_application(my_perl, env, pctx->app, req);
...
static int nxt_perl_psgi_ready_handler(nxt_unit_ctx_t *ctx)
{
    ...
    for (i = 0; i < c->threads - 1; i++)
    {
        ...
        res = pthread_create(&pctx->thread, nxt_perl_psgi_thread_attr, nxt_perl_psgi_thread_func, pctx);
        ...
        {
            nxt_unit_alert(ctx, "thread #%d create failed: %s (%d)", (int)(i + 1), strerror(res), res);
...
static void *nxt_perl_psgi_thread_func(void *data)
{
    nxt_unit_ctx_t *ctx;
    nxt_perl_psgi_ctx_t *pctx;
...
static int nxt_perl_psgi_init_threads(nxt_perl_app_conf_t *c)
{
    ...
    if (c->thread_stack_size > 0)
    {
        ...
        {
            nxt_unit_alert(NULL, "thread attr init failed: %s (%d)", strerror(rc), rc);
        ...
        if (nxt_slow_path(rc != 0))
        {
            nxt_unit_alert(NULL, "thread attr set stack size failed: %s (%d)", strerror(rc), rc);
    ...
    nxt_perl_psgi_ctxs = nxt_unit_malloc(NULL, sizeof(nxt_perl_psgi_ctx_t) * (c->threads - 1));
    if (nxt_slow_path(nxt_perl_psgi_ctxs == NULL))
    {
        return NXT_UNIT_ERROR;
    }

    memset(nxt_perl_psgi_ctxs, 0, sizeof(nxt_perl_psgi_ctx_t) * (c->threads - 1));
...
static void nxt_perl_psgi_join_threads(nxt_unit_ctx_t *ctx, nxt_perl_app_conf_t *c)
{
    ...
    for (i = 0; i < c->threads - 1; i++)
    {
        ...
        {
            nxt_unit_alert(ctx, "thread #%d join failed: %s (%d)", (int)(i + 1), strerror(res), res);
        }
    }
