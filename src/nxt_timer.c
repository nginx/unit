...
static intptr_t nxt_timer_rbtree_compare(nxt_rbtree_node_t *node1, nxt_rbtree_node_t *node2);
static void nxt_timer_change(nxt_event_engine_t *engine, nxt_timer_t *timer, nxt_timer_operation_t change,
                             nxt_msec_t time);
...
void nxt_timer_add(nxt_event_engine_t *engine, nxt_timer_t *timer, nxt_msec_t timeout)
{
    int32_t diff;
    uint32_t time;
    ...
    nxt_debug(timer->task, "timer add: %M±%d %M:%M", timer->time, timer->bias, timeout, time);
    ...
    if (nxt_timer_is_in_tree(timer))
    {
        ...
        {
            nxt_debug(timer->task, "timer previous: %M±%d", time, timer->bias);
...
nxt_bool_t nxt_timer_delete(nxt_event_engine_t *engine, nxt_timer_t *timer)
{
    nxt_debug(timer->task, "timer delete: %M±%d", timer->time, timer->bias);
                             ...
                             nxt_msec_t time)
{
    ...
    nxt_debug(timer->task, "timer change: %M±%d:%d", time, timer->bias, change);
...
static void nxt_timer_changes_commit(nxt_event_engine_t *engine)
{
    ...
    while (ch < end)
    {
        ...
        case NXT_TIMER_DELETE:
            nxt_debug(timer->task, "timer rbtree delete: %M±%d", timer->time, timer->bias);
    ...
    while (add < add_end)
    {
        ...
        nxt_debug(timer->task, "timer rbtree insert: %M±%d", timer->time, timer->bias);
...
nxt_msec_t nxt_timer_find(nxt_event_engine_t *engine)
{
    ...
    for (node = nxt_rbtree_min(tree); nxt_rbtree_is_there_successor(tree, node); node = next)
    {
        ...
        if (timer->enabled)
        {
            ...
            nxt_debug(timer->task, "timer found minimum: %M±%d:%M", time, timer->bias, timers->now);
...
void nxt_timer_expire(nxt_event_engine_t *engine, nxt_msec_t now)
{
    ...
    nxt_debug(&engine->task, "timer expire minimum: %M:%M", timers->minimum, now);
    ...
    for (node = nxt_rbtree_min(tree); nxt_rbtree_is_there_successor(tree, node); node = next)
    {
        ...
        nxt_debug(timer->task, "timer expire delete: %M±%d", timer->time, timer->bias);
        ...
        if (timer->enabled)
        {
            ...
            nxt_work_queue_add(timer->work_queue, nxt_timer_handler, timer->task, timer, NULL);
        }
    }
}
