...
static PyObject *nxt_py_asgi_websocket_accept(nxt_py_asgi_websocket_t *ws, PyObject *dict);
static PyObject *nxt_py_asgi_websocket_close(nxt_py_asgi_websocket_t *ws, PyObject *dict);
static PyObject *nxt_py_asgi_websocket_send_frame(nxt_py_asgi_websocket_t *ws, PyObject *dict);
static void nxt_py_asgi_websocket_receive_done(nxt_py_asgi_websocket_t *ws, PyObject *msg);
static void nxt_py_asgi_websocket_receive_fail(nxt_py_asgi_websocket_t *ws, PyObject *exc);
static void nxt_py_asgi_websocket_suspend_frame(nxt_unit_websocket_frame_t *f);
static PyObject *nxt_py_asgi_websocket_pop_msg(nxt_py_asgi_websocket_t *ws, nxt_unit_websocket_frame_t *frame);
static uint64_t nxt_py_asgi_websocket_pending_len(nxt_py_asgi_websocket_t *ws);
static nxt_unit_websocket_frame_t *nxt_py_asgi_websocket_pop_frame(nxt_py_asgi_websocket_t *ws);
static PyObject *nxt_py_asgi_websocket_disconnect_msg(nxt_py_asgi_websocket_t *ws);
static PyObject *nxt_py_asgi_websocket_done(PyObject *self, PyObject *future);

static PyMethodDef nxt_py_asgi_websocket_methods[] = {{"receive", nxt_py_asgi_websocket_receive, METH_NOARGS, 0},
...
int nxt_py_asgi_websocket_init(void)
{
    ...
    {
        nxt_unit_alert(NULL, "Python failed to initialize the \"asgi_websocket\" type object");
...
static PyObject *nxt_py_asgi_websocket_receive(PyObject *self, PyObject *none)
{
    ...
    if (nxt_slow_path(future == NULL))
    {
        ...
        return PyErr_Format(PyExc_RuntimeError, "failed to create Future object");
    }
...
static PyObject *nxt_py_asgi_websocket_send(PyObject *self, PyObject *dict)
{
    ...
    nxt_unit_req_debug(ws->req, "asgi_websocket_send type is '%.*s'", (int)type_len, type_str);

    if (type_len == (Py_ssize_t)websocket_accept.length && memcmp(type_str, websocket_accept.start, type_len) == 0)
    {
        return nxt_py_asgi_websocket_accept(ws, dict);
    }

    if (type_len == (Py_ssize_t)websocket_close.length && memcmp(type_str, websocket_close.start, type_len) == 0)
    {
        return nxt_py_asgi_websocket_close(ws, dict);
    }

    if (type_len == (Py_ssize_t)websocket_send.length && memcmp(type_str, websocket_send.start, type_len) == 0)
    {
...
static PyObject *nxt_py_asgi_websocket_accept(nxt_py_asgi_websocket_t *ws, PyObject *dict)
{
    ...
    {
    case NXT_WS_INIT:
        return PyErr_Format(PyExc_RuntimeError, "WebSocket connect not received");
    case NXT_WS_CONNECT:
        break;
    ...
    if (headers != NULL)
    {
        res = nxt_py_asgi_enum_headers(headers, nxt_py_asgi_calc_size, &calc_size_ctx);
    ...
    rc = nxt_unit_response_init(ws->req, 101, calc_size_ctx.fields_count, calc_size_ctx.fields_size);
    ...
    if (headers != NULL)
    {
        res = nxt_py_asgi_enum_headers(headers, nxt_py_asgi_add_field, &add_field_ctx);
        if (nxt_slow_path(res == NULL))
        {
            return NULL;
        }
    ...
    {
        rc = nxt_unit_response_add_field(ws->req, (const char *)ws_protocol.start, ws_protocol.length, subprotocol_str,
                                         subprotocol_len);
        if (nxt_slow_path(rc != NXT_UNIT_OK))
        {
...
static PyObject *nxt_py_asgi_websocket_close(nxt_py_asgi_websocket_t *ws, PyObject *dict)
{
    ...
    if (nxt_unit_response_is_websocket(ws->req))
    {
        ...
        status_code = (code != NULL) ? htons(PyLong_AsLong(code)) : htons(NXT_WEBSOCKET_CR_NORMAL);

        rc = nxt_unit_websocket_send(ws->req, NXT_WEBSOCKET_OP_CLOSE, 1, &status_code, 2);
        if (nxt_slow_path(rc != NXT_UNIT_OK))
        {
...
static PyObject *nxt_py_asgi_websocket_send_frame(nxt_py_asgi_websocket_t *ws, PyObject *dict)
{
    ...
    {
        return PyErr_Format(PyExc_ValueError, "Exactly one of 'bytes' or 'text' must be non-None");
    }
...
void nxt_py_asgi_websocket_handler(nxt_unit_websocket_frame_t *frame)
{
    uint8_t opcode;
    uint16_t status_code;
    uint64_t rest;
    ...
    if (nxt_slow_path(opcode != NXT_WEBSOCKET_OP_CONT && opcode != NXT_WEBSOCKET_OP_TEXT &&
                      opcode != NXT_WEBSOCKET_OP_BINARY && opcode != NXT_WEBSOCKET_OP_CLOSE))
    {
        nxt_unit_websocket_done(frame);

        nxt_unit_req_debug(ws->req, "asgi_websocket_handler: ignore frame with opcode %d", opcode);
    ...
    {
        if (nxt_slow_path(opcode == NXT_WEBSOCKET_OP_TEXT || opcode == NXT_WEBSOCKET_OP_BINARY))
        {
            ...
            PyErr_SetString(PyExc_AssertionError, "Invalid state: pending frames with active receiver. "
                                                  "CONT frame expected.");
...
bad_state:
    ...
    exc = PyObject_CallFunctionObjArgs(PyExc_RuntimeError, nxt_py_bad_state_str, NULL);
...
too_big:
    ...
    (void)nxt_unit_websocket_send(ws->req, NXT_WEBSOCKET_OP_CLOSE, 1, &status_code, 2);
    ...
    exc = PyObject_CallFunctionObjArgs(PyExc_RuntimeError, nxt_py_message_too_big_str, NULL);
    if (nxt_slow_path(exc == NULL))
    {
        nxt_unit_req_alert(ws->req, "RuntimeError create failed");
        nxt_python_print_exception();
...
static void nxt_py_asgi_websocket_receive_fail(nxt_py_asgi_websocket_t *ws, PyObject *exc)
{
    ...
    res = PyObject_CallMethodObjArgs(future, nxt_py_set_exception_str, exc, NULL);
    if (nxt_slow_path(res == NULL))
    {
...
static void nxt_py_asgi_websocket_suspend_frame(nxt_unit_websocket_frame_t *frame)
{
    int rc;
    nxt_py_asgi_websocket_t *ws;
    ...
    nxt_unit_req_debug(frame->req,
                       ...
                       "%d, %" PRIu64 ", %d",
                       frame->header->opcode, frame->payload_len, frame->header->fin);
    ...
    if (nxt_slow_path(rc == NXT_UNIT_ERROR))
    {
        ...
        PyErr_SetString(PyExc_RuntimeError, "Failed to retain frame for suspension.");
    ...
    {
        nxt_unit_req_alert(ws->req, "Failed to allocate buffer to suspend frame.");

        nxt_unit_websocket_done(frame);

        PyErr_SetString(PyExc_RuntimeError, "Failed to allocate buffer to suspend frame.");
...
static PyObject *nxt_py_asgi_websocket_pop_msg(nxt_py_asgi_websocket_t *ws, nxt_unit_websocket_frame_t *frame)
{
    ...
    if (nxt_queue_is_empty(&ws->pending_frames) || (frame != NULL && frame->header->opcode == NXT_WEBSOCKET_OP_CLOSE))
    {
    ...
    {
        nxt_unit_req_alert(ws->req, "Invalid state: attempt to process CONT frame.");

        nxt_unit_websocket_done(frame);

        return PyErr_Format(PyExc_AssertionError, "Invalid state: attempt to process CONT frame.");
    }
    ...
    case NXT_WEBSOCKET_OP_TEXT:
        ...
        {
            nxt_unit_req_alert(ws->req, "Failed to allocate buffer for payload (%d).", (int)payload_len);

            nxt_unit_websocket_done(frame);

            return PyErr_Format(PyExc_RuntimeError, "Failed to allocate buffer for payload (%d).", (int)payload_len);
        }
    ...
    case NXT_WEBSOCKET_OP_BINARY:
        ...
        {
            nxt_unit_req_alert(ws->req, "Failed to create Bytes for payload (%d).", (int)payload_len);
            nxt_python_print_exception();

            nxt_unit_websocket_done(frame);

            return PyErr_Format(PyExc_RuntimeError, "Failed to create Bytes for payload.");
        }
    ...
    case NXT_WEBSOCKET_OP_CLOSE:
        ...
        if (nxt_slow_path(data == NULL))
        {
            nxt_unit_req_alert(ws->req, "Failed to create Long from code %d.", (int)code);
            nxt_python_print_exception();

            return PyErr_Format(PyExc_RuntimeError, "Failed to create Long from code %d.", (int)code);
        }
    ...
    default:
        ...
        return PyErr_Format(PyExc_AssertionError, "Unexpected opcode %d", opcode);
    }
    ...
    if (buf != NULL)
    {
        ...
        if (!fin)
        {
            ...
            if (fin_frame != NULL)
            {
                buf += nxt_unit_websocket_read(fin_frame, buf, fin_frame->payload_len);
        ...
        if (opcode == NXT_WEBSOCKET_OP_TEXT)
        {
            ...
            if (nxt_slow_path(data == NULL))
            {
                nxt_unit_req_alert(ws->req, "Failed to create Unicode for payload (%d).", (int)payload_len);
                nxt_python_print_exception();

                return PyErr_Format(PyExc_RuntimeError, "Failed to create Unicode.");
            }
        }
    }
    ...
    if (nxt_slow_path(PyDict_SetItem(msg, data_key, data) == -1))
    {
        ...
        return PyErr_Format(PyExc_RuntimeError, "Python failed to set 'msg.data' item");
    }
...
static uint64_t nxt_py_asgi_websocket_pending_len(nxt_py_asgi_websocket_t *ws)
{
    ...
    nxt_queue_each(p, &ws->pending_frames, nxt_py_asgi_penging_frame_t, link)
    {
        ...
        {
            nxt_unit_req_debug(ws->req, "asgi_websocket_pending_len: %d", (int)res);
            return res;
        }
    }
    nxt_queue_loop;

    nxt_unit_req_debug(ws->req, "asgi_websocket_pending_len: %d (all)", (int)res);
    return res;
}
...
static PyObject *nxt_py_asgi_websocket_disconnect_msg(nxt_py_asgi_websocket_t *ws)
{
    ...
    if (nxt_slow_path(PyDict_SetItem(msg, nxt_py_code_str, code) == -1))
    {
        ...
        return PyErr_Format(PyExc_RuntimeError, "Python failed to set 'msg.code' item");
    }
...
static PyObject *nxt_py_asgi_websocket_done(PyObject *self, PyObject *future)
{
    ...
    {
        status_code =
            (rc == NXT_UNIT_OK) ? htons(NXT_WEBSOCKET_CR_NORMAL) : htons(NXT_WEBSOCKET_CR_INTERNAL_SERVER_ERROR);
