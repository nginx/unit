...
static PyObject* nxt_py_asgi_websocket_pop_msg(
...
int nxt_py_asgi_websocket_init(void) {
  if (nxt_slow_path(PyType_Ready(&nxt_py_asgi_websocket_type) != 0)) {
    nxt_unit_alert(
        NULL, "Python failed to initialize the \"asgi_websocket\" type object");
...
static PyObject* nxt_py_asgi_websocket_accept(nxt_py_asgi_websocket_t* ws,
  ...
  if (subprotocol_len > 0) {
    rc = nxt_unit_response_add_field(ws->req, (const char*)ws_protocol.start,
                                     ws_protocol.length, subprotocol_str,
                                     subprotocol_len);
    if (nxt_slow_path(rc != NXT_UNIT_OK)) {
...
void nxt_py_asgi_websocket_handler(nxt_unit_websocket_frame_t* frame) {
  ...
  if (nxt_slow_path(opcode != NXT_WEBSOCKET_OP_CONT &&
    ...
    nxt_unit_req_debug(
        ws->req, "asgi_websocket_handler: ignore frame with opcode %d", opcode);
...
static void nxt_py_asgi_websocket_suspend_frame(
    nxt_unit_websocket_frame_t* frame) {
  int rc;
  nxt_py_asgi_websocket_t* ws;
  ...
  switch (opcode) {
    ...
    case NXT_WEBSOCKET_OP_BINARY:
      ...
      if (nxt_slow_path(data == NULL)) {
        nxt_unit_req_alert(ws->req, "Failed to create Bytes for payload (%d).",
                           (int)payload_len);
        nxt_python_print_exception();
    ...
    case NXT_WEBSOCKET_OP_CLOSE:
      ...
      if (nxt_slow_path(data == NULL)) {
        ...
        return PyErr_Format(PyExc_RuntimeError,
                            "Failed to create Long from code %d.", (int)code);
      }
