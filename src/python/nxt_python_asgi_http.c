...
static PyObject *nxt_py_asgi_http_response_start(nxt_py_asgi_http_t *http, PyObject *dict);
static PyObject *nxt_py_asgi_http_response_body(nxt_py_asgi_http_t *http, PyObject *dict);
static void nxt_py_asgi_http_emit_disconnect(nxt_py_asgi_http_t *http);
static void nxt_py_asgi_http_set_result(nxt_py_asgi_http_t *http, PyObject *future, PyObject *msg);
static PyObject *nxt_py_asgi_http_done(PyObject *self, PyObject *future);

static PyMethodDef nxt_py_asgi_http_methods[] = {{"receive", nxt_py_asgi_http_receive, METH_NOARGS, 0},
...
int nxt_py_asgi_http_init(void)
{
    ...
    {
        nxt_unit_alert(NULL, "Python failed to initialize the 'http' type object");
...
static PyObject *nxt_py_asgi_http_read_msg(nxt_py_asgi_http_t *http)
{
    ...
    if (size > 0)
    {
        ...
        if (nxt_slow_path(body == NULL))
        {
            ...
            return PyErr_Format(PyExc_RuntimeError, "failed to create Bytes object");
        }
...
#define SET_ITEM(dict, key, value)                                                                                     \
    ...
    {                                                                                                                  \
        ...
        PyErr_SetString(PyExc_RuntimeError, "Python failed to set '" #dict "." #key "' item");                         \
        goto fail;                                                                                                     \
    }
...
static PyObject *nxt_py_asgi_http_send(PyObject *self, PyObject *dict)
{
    ...
    nxt_unit_req_debug(http->req, "asgi_http_send type is '%.*s'", (int)type_len, type_str);
...
static PyObject *nxt_py_asgi_http_response_start(nxt_py_asgi_http_t *http, PyObject *dict)
{
    ...
    rc =
        nxt_unit_response_init(http->req, PyLong_AsLong(status), calc_size_ctx.fields_count, calc_size_ctx.fields_size);
...
static PyObject *nxt_py_asgi_http_response_body(nxt_py_asgi_http_t *http, PyObject *dict)
{
    ...
    {
        return PyErr_Format(PyExc_RuntimeError, "Unexpected ASGI message 'http.response.body' "
    ...
    if (body != NULL)
    {
        ...
        {
            return PyErr_Format(PyExc_TypeError, "'body' is not a byte string or bytearray");
        }

        nxt_unit_req_debug(http->req, "asgi_http_response_body: %d, %d", (int)body_len, (more_body == Py_True));

        if (nxt_slow_path(http->bytes_sent + body_len > http->content_length))
        {
            return PyErr_Format(PyExc_RuntimeError, "Response content longer than Content-Length");
        }
        ...
        while (body_len > 0)
        {
            ...
            if (nxt_slow_path(sent == 0))
            {
                ...
                if (nxt_slow_path(future == NULL))
                {
                    nxt_unit_req_alert(http->req, "Python failed to create Future object");
                    nxt_python_print_exception();
    ...
    {
        nxt_unit_req_debug(http->req, "asgi_http_response_body: 0, %d", (more_body == Py_True));
...
fail:
    ...
    exc = PyObject_CallFunctionObjArgs(PyExc_RuntimeError, nxt_py_failed_to_send_body_str, NULL);
...
static PyObject *nxt_py_asgi_http_done(PyObject *self, PyObject *future)
{
    ...
    if (nxt_slow_path(res == NULL))
    {
        nxt_unit_req_error(http->req, "Python failed to call 'future.result()'");
        nxt_python_print_exception();
