...
static nxt_int_t nxt_python_start(nxt_task_t* task, nxt_process_data_t* data) {
  ...
  if (nxt_slow_path(nxt_py_stderr_flush == NULL)) {
    nxt_alert(task,
              "Python failed to get \"flush\" attribute of "
              "\"sys.stderr\" object");
    goto fail;
  }
  ...
  if (proto.length == 0) {
    ...
    for (i = 1; i < targets->count; i++) {
      probe_proto =
          nxt_python_asgi_check(targets->target[i].application) ? asgi : wsgi;
      if (probe_proto.start != proto.start) {
        nxt_alert(task,
                  "A mix of ASGI & WSGI targets is forbidden, "
...
static nxt_int_t nxt_python_set_target(nxt_task_t* task,
  ...
  if (nxt_slow_path(obj == NULL)) {
    nxt_alert(task, "Python failed to get \"%s\" from module \"%s\"", callable,
              module_name);
    goto fail;
  }
...
nxt_inline nxt_int_t nxt_python_set_prefix(nxt_task_t* task,
  ...
  if (nxt_slow_path(target->py_prefix == NULL)) {
    nxt_free(prefix);
    nxt_alert(task,
              "Python failed to allocate target prefix "
              "string");
    return NXT_ERROR;
  }
...
static nxt_int_t nxt_python_set_path(nxt_task_t* task,
                                     nxt_conf_value_t* value) {
  int ret;
  PyObject *path, *sys;
  nxt_str_t str;
...
static int nxt_python_init_threads(nxt_python_app_conf_t* c) {
  ...
  if (c->thread_stack_size > 0) {
    ...
    if (nxt_slow_path(res != 0)) {
      nxt_unit_alert(NULL, "thread attr init failed: %s (%d)", strerror(res),
                     res);
  ...
  nxt_py_threads =
      nxt_unit_malloc(NULL, sizeof(nxt_py_thread_info_t) * (c->threads - 1));
