...
static nxt_int_t nxt_python_start(nxt_task_t *task, nxt_process_data_t *data);
static nxt_int_t nxt_python_set_target(nxt_task_t *task, nxt_python_target_t *target, nxt_conf_value_t *conf);
nxt_inline nxt_int_t nxt_python_set_prefix(nxt_task_t *task, nxt_python_target_t *target, nxt_conf_value_t *value);
...
static void nxt_python_join_threads(nxt_unit_ctx_t *ctx, nxt_python_app_conf_t *c);
...
NXT_EXPORT nxt_app_module_t nxt_app_module = {
    sizeof(compat),   compat, nxt_string("python"), PY_VERSION, nxt_python_mounts, nxt_nitems(nxt_python_mounts), NULL,
    nxt_python_start,
};
...
static nxt_int_t nxt_python_start(nxt_task_t *task, nxt_process_data_t *data)
{
    ...
    if (proto.length == 0)
    {
        proto = nxt_python_asgi_check(targets->target[0].application) ? asgi : wsgi;

        for (i = 1; i < targets->count; i++)
        {
            probe_proto = nxt_python_asgi_check(targets->target[i].application) ? asgi : wsgi;
...
static nxt_int_t nxt_python_set_target(nxt_task_t *task, nxt_python_target_t *target, nxt_conf_value_t *conf)
{
    ...
    if (nxt_slow_path(obj == NULL))
    {
        nxt_alert(task, "Python failed to get \"%s\" from module \"%s\"", callable, module_name);
        goto fail;
    }
    ...
    {
        nxt_alert(task, "\"%s\" in module \"%s\" is not a callable object", callable, module_name);
        goto fail;
    }
...
nxt_inline nxt_int_t nxt_python_set_prefix(nxt_task_t *task, nxt_python_target_t *target, nxt_conf_value_t *value)
{
    ...
    target->py_prefix = PyString_FromStringAndSize((char *)str.start, str.length);
...
static nxt_int_t nxt_python_set_path(nxt_task_t *task, nxt_conf_value_t *value)
{
    ...
    while (n != 0)
    {
        ...
        if (nxt_slow_path(path == NULL))
        {
            nxt_alert(task, "Python failed to create string object \"%V\"", &str);
            return NXT_ERROR;
        }
        ...
        {
            nxt_alert(task, "Python failed to insert \"%V\" into \"sys.path\"", &str);
            return NXT_ERROR;
        }
    }
...
static int nxt_python_init_threads(nxt_python_app_conf_t *c)
{
    ...
    if (c->thread_stack_size > 0)
    {
        ...
        {
            nxt_unit_alert(NULL, "thread attr init failed: %s (%d)", strerror(res), res);
        ...
        if (nxt_slow_path(res != 0))
        {
            nxt_unit_alert(NULL, "thread attr set stack size failed: %s (%d)", strerror(res), res);
    ...
    nxt_py_threads = nxt_unit_malloc(NULL, sizeof(nxt_py_thread_info_t) * (c->threads - 1));
...
static int nxt_python_ready_handler(nxt_unit_ctx_t *ctx)
{
    ...
    for (i = 0; i < c->threads - 1; i++)
    {
        ...
        res = pthread_create(&ti->thread, nxt_py_thread_attr, nxt_python_thread_func, ti);
...
static void *nxt_python_thread_func(void *data)
{
    ...
    nxt_unit_debug(ti->ctx, "worker thread #%d start", (int)(ti - nxt_py_threads + 1));
...
fail:
    ...
    nxt_unit_debug(NULL, "worker thread #%d end", (int)(ti - nxt_py_threads + 1));
...
int nxt_python_init_strings(nxt_python_string_t *pstr)
{
    ...
    {
        obj = PyString_FromStringAndSize((char *)pstr->string.start, pstr->string.length);
        if (nxt_slow_path(obj == NULL))
        {
            return NXT_UNIT_ERROR;
        }
