...
typedef struct
{
    PyObject_HEAD nxt_py_asgi_ctx_data_t *ctx_data;
    int disabled;
...
static PyObject *nxt_py_asgi_lifespan_target_startup(nxt_py_asgi_ctx_data_t *ctx_data, nxt_python_target_t *target);
static int nxt_py_asgi_lifespan_target_shutdown(nxt_py_asgi_lifespan_t *lifespan);
...
static PyObject *nxt_py_asgi_lifespan_send_startup(nxt_py_asgi_lifespan_t *lifespan, int v, PyObject *dict);
static PyObject *nxt_py_asgi_lifespan_send_(nxt_py_asgi_lifespan_t *lifespan, int v, int *sent, PyObject **future);
static PyObject *nxt_py_asgi_lifespan_send_shutdown(nxt_py_asgi_lifespan_t *lifespan, int v, PyObject *dict);
...
static PyMethodDef nxt_py_asgi_lifespan_methods[] = {{"receive", nxt_py_asgi_lifespan_receive, METH_NOARGS, 0},
...
static PyObject *nxt_py_asgi_lifespan_target_startup(nxt_py_asgi_ctx_data_t *ctx_data, nxt_python_target_t *target)
{
    ...
    {
        nxt_unit_alert(NULL, "Python failed to initialize the 'asgi_lifespan' type object");
        return NULL;
    }
    ...
    {
        nxt_unit_req_error(NULL, "Python failed to create 'state' dict");
        goto release_future;
    }

    if (nxt_slow_path(PyDict_SetItem(scope, nxt_py_state_str, lifespan->state) == -1))
    {
        nxt_unit_req_error(NULL, "Python failed to set 'scope.state' item");
    ...
    else
    {
        ...
        {
            nxt_unit_log(NULL, NXT_UNIT_LOG_INFO, "ASGI Lifespan processing exception");
            nxt_python_print_exception();
        ...
        {
            nxt_unit_req_error(NULL, "Legacy ASGI application returns not a callable");
    ...
    res = PyObject_CallMethodObjArgs(py_task, nxt_py_add_done_callback_str, done, NULL);
    if (nxt_slow_path(res == NULL))
    {
    ...
    res = PyObject_CallFunctionObjArgs(ctx_data->loop_run_until_complete, lifespan->startup_future, NULL);
    if (nxt_slow_path(res == NULL))
    {
...
static int nxt_py_asgi_lifespan_target_shutdown(nxt_py_asgi_lifespan_t *lifespan)
{
    ...
    lifespan->shutdown_future = PyObject_CallObject(ctx_data->loop_create_future, NULL);
    ...
    res = PyObject_CallFunctionObjArgs(ctx_data->loop_run_until_complete, lifespan->shutdown_future, NULL);
    if (nxt_slow_path(res == NULL))
    {
        nxt_unit_alert(NULL, "Python failed to call loop.run_until_complete");
        nxt_python_print_exception();
...
static PyObject *nxt_py_asgi_lifespan_send(PyObject *self, PyObject *dict)
{
    ...
    static const nxt_str_t startup_complete = nxt_string("lifespan.startup.complete");
    static const nxt_str_t startup_failed = nxt_string("lifespan.startup.failed");
    static const nxt_str_t shutdown_complete = nxt_string("lifespan.shutdown.complete");
    static const nxt_str_t shutdown_failed = nxt_string("lifespan.shutdown.failed");
    ...
    {
        nxt_unit_error(NULL, "asgi_lifespan_send: 'type' is not a unicode string");
    ...
    if (type_len == (Py_ssize_t)startup_complete.length && memcmp(type_str, startup_complete.start, type_len) == 0)
    {
        return nxt_py_asgi_lifespan_send_startup(lifespan, 0, NULL);
    }

    if (type_len == (Py_ssize_t)startup_failed.length && memcmp(type_str, startup_failed.start, type_len) == 0)
    {
        msg = PyDict_GetItem(dict, nxt_py_message_str);
        return nxt_py_asgi_lifespan_send_startup(lifespan, 1, msg);
    }

    if (type_len == (Py_ssize_t)shutdown_complete.length && memcmp(type_str, shutdown_complete.start, type_len) == 0)
    {
        return nxt_py_asgi_lifespan_send_shutdown(lifespan, 0, NULL);
    }

    if (type_len == (Py_ssize_t)shutdown_failed.length && memcmp(type_str, shutdown_failed.start, type_len) == 0)
    {
        msg = PyDict_GetItem(dict, nxt_py_message_str);
...
static PyObject *nxt_py_asgi_lifespan_send_startup(nxt_py_asgi_lifespan_t *lifespan, int v, PyObject *message)
{
    ...
    return nxt_py_asgi_lifespan_send_(lifespan, v, &lifespan->startup_sent, &lifespan->startup_future);
}

static PyObject *nxt_py_asgi_lifespan_send_(nxt_py_asgi_lifespan_t *lifespan, int v, int *sent, PyObject **pfuture)
{
    PyObject *future, *res;
    ...
    *sent = 1 + v;
    ...
    if (*pfuture != NULL)
    {
        ...
        *pfuture = NULL;
        ...
        res = PyObject_CallMethodObjArgs(future, nxt_py_set_result_str, Py_None, NULL);
        if (nxt_slow_path(res == NULL))
        {
...
static PyObject *nxt_py_asgi_lifespan_disable(nxt_py_asgi_lifespan_t *lifespan)
{
    ...
    return PyErr_Format(PyExc_AssertionError, "Got invalid state transition on lifespan protocol");
}

static PyObject *nxt_py_asgi_lifespan_send_shutdown(nxt_py_asgi_lifespan_t *lifespan, int v, PyObject *message)
{
    return nxt_py_asgi_lifespan_send_(lifespan, v, &lifespan->shutdown_sent, &lifespan->shutdown_future);
}
