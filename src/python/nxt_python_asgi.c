...
static PyObject *nxt_python_asgi_get_event_loop(PyObject *asyncio, const char *event_loop_func);
...
static void nxt_py_asgi_remove_reader(nxt_unit_ctx_t *ctx, nxt_unit_port_t *port);
...
static PyObject *nxt_py_asgi_create_http_scope(nxt_unit_request_info_t *req, nxt_python_target_t *app_target);
static PyObject *nxt_py_asgi_create_address(nxt_unit_sptr_t *sptr, uint8_t len, uint16_t port);
static PyObject *nxt_py_asgi_create_ip_address(nxt_unit_sptr_t *sptr, uint8_t len, uint16_t port);
...
static void nxt_py_asgi_remove_port(nxt_unit_t *lib, nxt_unit_ctx_t *ctx, nxt_unit_port_t *port);
...
static PyMethodDef nxt_py_port_read_method = {"unit_port_read", nxt_py_asgi_port_read, METH_VARARGS, ""};
...
int nxt_python_asgi_check(PyObject *obj)
{
    ...
    nxt_unit_debug(NULL,
                   ...
                   "%d argument(s)",
                   (code->co_flags & CO_COROUTINE) != 0 ? "" : "not ", code->co_argcount);
...
int nxt_python_asgi_init(nxt_unit_init_t *init, nxt_python_proto_t *proto)
{
    ...
    {
        nxt_unit_alert(NULL, "Python failed to initialize the 'port_read' function");
        return NXT_UNIT_ERROR;
    }
...
static PyObject *nxt_python_asgi_get_event_loop(PyObject *asyncio, const char *event_loop_func)
{
    ...
    event_loop = PyDict_GetItemString(PyModule_GetDict(asyncio), event_loop_func);
    if (nxt_slow_path(event_loop == NULL))
    {
        nxt_unit_alert(NULL, "Python failed to get '%s' from module 'asyncio'", event_loop_func);
        return NULL;
    }

    if (nxt_slow_path(PyCallable_Check(event_loop) == 0))
    {
        nxt_unit_alert(NULL, "'asyncio.%s' is not a callable object", event_loop_func);
        return NULL;
    }
    ...
    if (nxt_slow_path(loop == NULL))
    {
        ...
        {
            nxt_unit_alert(NULL, "Python failed to call 'asyncio.%s'", event_loop_func);
        }
...
static int nxt_python_asgi_ctx_data_alloc(void **pdata, int main)
{
    ...
    for (i = 0; i < nxt_nitems(handlers); i++)
    {
        ...
        {
            nxt_unit_alert(NULL, "Python failed to get 'loop.%s'", handlers[i].key);
            goto fail;
        }
        ...
        *handlers[i].handler = obj;
        ...
        {
            nxt_unit_alert(NULL, "'loop.%s' is not a callable object", handlers[i].key);
            goto fail;
        }
    }
...
static int nxt_python_asgi_run(nxt_unit_ctx_t *ctx)
{
    ...
    res = PyObject_CallFunctionObjArgs(ctx_data->loop_run_until_complete, ctx_data->quit_future, NULL);
...
static void nxt_py_asgi_request_handler(nxt_unit_request_info_t *req)
{
    ...
    if (nxt_slow_path(PyDict_SetItem(scope, nxt_py_state_str, newstate) == -1))
    {
        Py_DECREF(newstate);
        goto release_scope;
    }
    ...
    if (!target->asgi_legacy)
    {
        ...
        res = PyObject_CallFunctionObjArgs(target->application, scope, receive, send, NULL);
    }
    else
    {
        ...
        {
            nxt_unit_req_error(req, "Legacy ASGI application returns not a callable");
            nxt_unit_request_done(req, NXT_UNIT_ERROR);
    ...
    res = PyObject_CallMethodObjArgs(task, nxt_py_add_done_callback_str, done, NULL);
    if (nxt_slow_path(res == NULL))
    {
        nxt_unit_req_error(req, "Python failed to call 'task.add_done_callback'");
        nxt_python_print_exception();
        nxt_unit_request_done(req, NXT_UNIT_ERROR);
...
#define SET_ITEM(dict, key, value)                                                                                     \
    if (nxt_slow_path(PyDict_SetItem(dict, nxt_py_##key##_str, value) == -1))                                          \
    {                                                                                                                  \
        nxt_unit_req_alert(req, "Python failed to set '" #dict "." #key "' item");                                     \
        goto fail;                                                                                                     \
    }
    ...
    if (prefix.length > 0 &&
        ((path_length > prefix.length && p[prefix.length] == '/') || path_length == prefix.length) &&
    ...
    v = PyUnicode_DecodeUTF8(nxt_unit_sptr_get(&r->path), r->path_length, "replace");
    if (nxt_slow_path(v == NULL))
    {
    ...
    for (i = 0; i < r->fields_count; i++)
    {
        ...
        if (f->hash == NXT_UNIT_HASH_WS_PROTOCOL && f->name_length == ws_protocol.length && f->value_length > 0 &&
...
static PyObject *nxt_py_asgi_create_header(nxt_unit_field_t *f)
{
    ...
    v = PyBytes_FromStringAndSize(nxt_unit_sptr_get(&f->value), f->value_length);
    if (nxt_slow_path(v == NULL))
    {
        Py_DECREF(header);
...
static int nxt_py_asgi_add_port(nxt_unit_ctx_t *ctx, nxt_unit_port_t *port)
{
    ...
    {
        nxt_unit_alert(ctx, "ioctl(%d, FIONBIO, 0) failed: %s (%d)", port->in_fd, strerror(errno), errno);
...
static int nxt_py_asgi_add_reader(nxt_unit_ctx_t *ctx, nxt_unit_port_t *port)
{
    ...
    res = PyObject_CallFunctionObjArgs(ctx_data->loop_add_reader, fd, nxt_py_port_read, py_ctx, py_port, NULL);
    if (nxt_slow_path(res == NULL))
    {
...
static void nxt_py_asgi_quit(nxt_unit_ctx_t *ctx)
{
    ...
    {
        res = PyObject_CallFunctionObjArgs(ctx_data->quit_future_set_result, p, NULL);
        if (nxt_slow_path(res == NULL))
        {
...
static PyObject *nxt_py_asgi_port_read(PyObject *self, PyObject *args)
{
    ...
    if (n != 2)
    {
        nxt_unit_alert(NULL, "nxt_py_asgi_port_read: invalid number of arguments %d", (int)n);
    ...
    {
        return PyErr_Format(PyExc_TypeError, "the first argument is not a long");
    }
    ...
    {
        return PyErr_Format(PyExc_TypeError, "the second argument is not a long");
    }
    ...
    {
        return PyErr_Format(PyExc_RuntimeError, "error processing port %d message", port->id.id);
    }
    ...
    if (rc == NXT_UNIT_OK)
    {
        ...
        res = PyObject_CallFunctionObjArgs(ctx_data->loop_call_soon, nxt_py_port_read, arg0, arg1, NULL);
        if (nxt_slow_path(res == NULL))
        {
...
PyObject *nxt_py_asgi_enum_headers(PyObject *headers, nxt_py_asgi_enum_header_cb cb, void *data)
{
    int i;
    ...
    for (i = 0; /* void */; i++)
    {
        ...
        if (nxt_slow_path(h_iter == NULL))
        {
            ...
            return PyErr_Format(PyExc_TypeError, "'headers' item #%d is not an iterable", i);
        }
        ...
        if (nxt_slow_path(name == NULL || !PyBytes_Check(name)))
        {
            ...
            return PyErr_Format(PyExc_TypeError, "'headers' item #%d 'name' is not a byte string", i);
        }
        ...
        if (nxt_slow_path(val == NULL || !PyBytes_Check(val)))
        {
            ...
            return PyErr_Format(PyExc_TypeError, "'headers' item #%d 'value' is not a byte string", i);
        }
...
PyObject *nxt_py_asgi_calc_size(void *data, int i, PyObject *name, PyObject *val)
{
...
PyObject *nxt_py_asgi_add_field(void *data, int i, PyObject *name, PyObject *val)
{
    int rc;
    ...
    if (req->response->fields[i].hash == NXT_UNIT_HASH_CONTENT_LENGTH)
    {
        ...
        if (nxt_slow_path(content_length < 0))
        {
            ...
            return PyErr_Format(PyExc_ValueError, "Failed to parse Content-Length: '%.*s'", (int)val_len, val_str);
        }
...
PyObject *nxt_py_asgi_set_result_soon(nxt_unit_request_info_t *req, nxt_py_asgi_ctx_data_t *ctx_data, PyObject *future,
                                      PyObject *result)
{
    ...
    res = PyObject_CallFunctionObjArgs(ctx_data->loop_call_soon, set_result, result, NULL);
    if (nxt_slow_path(res == NULL))
    {
...
PyObject *nxt_py_asgi_new_msg(nxt_unit_request_info_t *req, PyObject *type)
{
    ...
    if (nxt_slow_path(msg == NULL))
    {
        ...
        return PyErr_Format(PyExc_RuntimeError, "failed to create message dict");
    }
...
PyObject *nxt_py_asgi_new_scope(nxt_unit_request_info_t *req, PyObject *type, PyObject *spec_version)
{
    PyObject *scope, *asgi;
    ...
    if (nxt_slow_path(scope == NULL))
    {
        ...
        return PyErr_Format(PyExc_RuntimeError, "failed to create 'scope' dict");
    }
    ...
    if (nxt_slow_path(PyDict_SetItem(scope, nxt_py_type_str, type) == -1))
    {
        ...
        return PyErr_Format(PyExc_RuntimeError, "failed to set 'scope.type' item");
    }
    ...
    if (nxt_slow_path(PyDict_SetItem(scope, nxt_py_asgi_str, asgi) == -1))
    {
        ...
        return PyErr_Format(PyExc_RuntimeError, "failed to set 'scope.asgi' item");
    }

    if (nxt_slow_path(PyDict_SetItem(asgi, nxt_py_version_str, nxt_py_3_0_str) == -1))
    {
        ...
        return PyErr_Format(PyExc_RuntimeError, "failed to set 'asgi.version' item");
    }

    if (nxt_slow_path(PyDict_SetItem(asgi, nxt_py_spec_version_str, spec_version) == -1))
    {
        nxt_unit_req_alert(req, "Python failed to set 'asgi.spec_version' item");

        Py_DECREF(asgi);
        Py_DECREF(scope);

        return PyErr_Format(PyExc_RuntimeError, "failed to set 'asgi.spec_version' item");
    }
...
void nxt_py_asgi_drain_wait(nxt_unit_request_info_t *req, nxt_queue_link_t *link)
{
    nxt_py_asgi_ctx_data_t *ctx_data;
