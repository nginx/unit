...
static PyObject* nxt_py_asgi_create_address(nxt_unit_sptr_t* sptr,
                                            uint8_t len,
                                            uint16_t port);
...
static void nxt_py_asgi_remove_port(nxt_unit_t* lib,
                                    nxt_unit_ctx_t* ctx,
                                    nxt_unit_port_t* port);
...
int nxt_python_asgi_init(nxt_unit_init_t* init, nxt_python_proto_t* proto) {
  ...
  for (i = 0; i < nxt_py_targets->count; i++) {
    ...
    if ((code->co_flags & CO_COROUTINE) == 0) {
      nxt_unit_debug(NULL,
                     "asgi: callable is not a coroutine function "
...
static PyObject* nxt_py_asgi_create_http_scope(
...
static int nxt_py_asgi_add_port(nxt_unit_ctx_t* ctx, nxt_unit_port_t* port) {
  ...
  if (nxt_slow_path(ioctl(port->in_fd, FIONBIO, &nb) == -1)) {
    nxt_unit_alert(ctx, "ioctl(%d, FIONBIO, 0) failed: %s (%d)", port->in_fd,
...
static void nxt_py_asgi_quit(nxt_unit_ctx_t* ctx) {
  ...
  if (nxt_slow_path(p == NULL)) {
    ...
    res =
        PyObject_CallFunctionObjArgs(ctx_data->quit_future_set_result, p, NULL);
    if (nxt_slow_path(res == NULL)) {
...
static PyObject* nxt_py_asgi_port_read(PyObject* self, PyObject* args) {
  ...
  if (n != 2) {
    nxt_unit_alert(
        NULL, "nxt_py_asgi_port_read: invalid number of arguments %d", (int)n);
  ...
  if (nxt_slow_path(rc == NXT_UNIT_ERROR)) {
    return PyErr_Format(PyExc_RuntimeError, "error processing port %d message",
                        port->id.id);
  }
...
PyObject* nxt_py_asgi_calc_size(void* data,
                                int i,
                                PyObject* name,
                                PyObject* val) {
...
PyObject* nxt_py_asgi_add_field(void* data,
                                int i,
                                PyObject* name,
                                PyObject* val) {
  int rc;
...
PyObject* nxt_py_asgi_new_scope(nxt_unit_request_info_t* req,
                                PyObject* type,
  ...
  if (nxt_slow_path(
          PyDict_SetItem(asgi, nxt_py_spec_version_str, spec_version) == -1)) {
