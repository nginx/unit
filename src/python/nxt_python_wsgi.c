...
static PyObject *nxt_python_get_environ(nxt_python_ctx_t *pctx, nxt_python_target_t *app_target);
static int nxt_python_add_sptr(nxt_python_ctx_t *pctx, PyObject *name, nxt_unit_sptr_t *sptr, uint32_t size);
static int nxt_python_add_char(nxt_python_ctx_t *pctx, PyObject *name, char *src, uint32_t size);
static int nxt_python_add_py_string(nxt_python_ctx_t *pctx, PyObject *name, PyObject *value);
static int nxt_python_add_field(nxt_python_ctx_t *pctx, nxt_unit_field_t *field, int n, uint32_t vl);
static PyObject *nxt_python_field_name(const char *name, uint8_t len);
static PyObject *nxt_python_field_value(nxt_unit_field_t *f, int n, uint32_t vl);
static int nxt_python_add_obj(nxt_python_ctx_t *pctx, PyObject *name, PyObject *value);

static PyObject *nxt_py_start_resp(PyObject *self, PyObject *args);
static int nxt_python_response_add_field(nxt_python_ctx_t *pctx, PyObject *name, PyObject *value, int i);
static int nxt_python_str_buf(PyObject *str, char **buf, uint32_t *len, PyObject **bytes);
...
static PyObject *nxt_py_input_readline(nxt_python_ctx_t *pctx, PyObject *args);
static PyObject *nxt_py_input_getline(nxt_python_ctx_t *pctx, size_t size);
static PyObject *nxt_py_input_readlines(nxt_python_ctx_t *self, PyObject *args);
...
static PyMethodDef nxt_py_start_resp_method[] = {{"unit_start_response", nxt_py_start_resp, METH_VARARGS, ""}};

static PyMethodDef nxt_py_write_method[] = {{"unit_write", nxt_py_write, METH_O, ""}};

static PyMethodDef nxt_py_input_methods[] = {{"read", (PyCFunction)nxt_py_input_read, METH_VARARGS, 0},
...
int nxt_python_wsgi_init(nxt_unit_init_t *init, nxt_python_proto_t *proto)
{
    ...
    if (nxt_slow_path(nxt_python_init_strings(nxt_python_strings) != NXT_UNIT_OK))
    {
...
static int nxt_python_wsgi_ctx_data_alloc(void **pdata, int main)
{
    ...
    {
        nxt_unit_alert(NULL, "Python failed to create the \"wsgi.input\" object");
        return NXT_UNIT_ERROR;
    }
    ...
    {
        nxt_unit_alert(NULL, "Python failed to initialize the \"start_response\" function");
        goto fail;
    }
    ...
    {
        nxt_unit_alert(NULL, "Python failed to initialize the \"write\" function");
        goto fail;
    }
...
static void nxt_python_request_handler(nxt_unit_request_info_t *req)
{
    ...
    else
    {
        ...
        {
            nxt_unit_req_error(req, "the application returned not an iterable object");
            nxt_python_print_exception();
...
static PyObject *nxt_python_create_environ(nxt_python_app_conf_t *c)
{
    ...
    if (nxt_slow_path(environ == NULL))
    {
        nxt_unit_alert(NULL, "Python failed to create the \"environ\" dictionary");
        return NULL;
    }

    obj = PyString_FromStringAndSize((char *)nxt_server.start, nxt_server.length);
    if (nxt_slow_path(obj == NULL))
    {
        nxt_unit_alert(NULL, "Python failed to create the \"SERVER_SOFTWARE\" environ value");
        goto fail;
    }

    if (nxt_slow_path(PyDict_SetItemString(environ, "SERVER_SOFTWARE", obj) != 0))
    {
        nxt_unit_alert(NULL, "Python failed to set the \"SERVER_SOFTWARE\" environ value");
        goto fail;
    }
    ...
    if (nxt_slow_path(obj == NULL))
    {
        nxt_unit_alert(NULL, "Python failed to build the \"wsgi.version\" environ value");
        goto fail;
    }

    if (nxt_slow_path(PyDict_SetItemString(environ, "wsgi.version", obj) != 0))
    {
        nxt_unit_alert(NULL, "Python failed to set the \"wsgi.version\" environ value");
        goto fail;
    }
    ...
    if (nxt_slow_path(PyDict_SetItemString(environ, "wsgi.multithread", c->threads > 1 ? Py_True : Py_False) != 0))
    {
        nxt_unit_alert(NULL, "Python failed to set the \"wsgi.multithread\" environ value");
        goto fail;
    }

    if (nxt_slow_path(PyDict_SetItemString(environ, "wsgi.multiprocess", Py_True) != 0))
    {
        nxt_unit_alert(NULL, "Python failed to set the \"wsgi.multiprocess\" environ value");
        goto fail;
    }

    if (nxt_slow_path(PyDict_SetItemString(environ, "wsgi.run_once", Py_False) != 0))
    {
        nxt_unit_alert(NULL, "Python failed to set the \"wsgi.run_once\" environ value");
        goto fail;
    }

    if (nxt_slow_path(PyType_Ready(&nxt_py_input_type) != 0))
    {
        nxt_unit_alert(NULL, "Python failed to initialize the \"wsgi.input\" type object");
        goto fail;
    }
    ...
    {
        nxt_unit_alert(NULL, "Python failed to set the \"wsgi.errors\" environ value");
        goto fail;
    }
...
static PyObject *nxt_python_copy_environ(nxt_unit_request_info_t *req)
{
    ...
    if (nxt_slow_path(environ == NULL))
    {
        nxt_unit_req_alert(req, "Python failed to copy the \"environ\" dictionary");
        nxt_python_print_exception();
    }
...
#define RC(S)                                                                                                          \
    ...
    RC(nxt_python_add_sptr(pctx, nxt_py_request_method_str, &r->method, r->method_length));
    RC(nxt_python_add_sptr(pctx, nxt_py_request_uri_str, &r->target, r->target_length));
    RC(nxt_python_add_sptr(pctx, nxt_py_query_string_str, &r->query, r->query_length));

    prefix = app_target->prefix;
    path_length = r->path_length;
    path = nxt_unit_sptr_get(&r->path);
    if (prefix.length > 0 &&
        ((path_length > prefix.length && path[prefix.length] == '/') || path_length == prefix.length) &&
        memcmp(prefix.start, path, prefix.length) == 0)
    {
        RC(nxt_python_add_py_string(pctx, nxt_py_script_name_str, app_target->py_prefix));
    ...
    RC(nxt_python_add_sptr(pctx, nxt_py_remote_addr_str, &r->remote, r->remote_length));
    RC(nxt_python_add_sptr(pctx, nxt_py_server_addr_str, &r->local_addr, r->local_addr_length));

    if (r->tls)
    {
        RC(nxt_python_add_obj(pctx, nxt_py_wsgi_uri_scheme_str, nxt_py_https_str));
    }
    else
    {
    ...
    RC(nxt_python_add_sptr(pctx, nxt_py_server_protocol_str, &r->version, r->version_length));

    RC(nxt_python_add_sptr(pctx, nxt_py_server_name_str, &r->server_name, r->server_name_length));
    ...
    for (i = 0; i < r->fields_count;)
    {
        ...
        for (j = i + 1; j < r->fields_count; j++)
        {
            ...
            if (f2->hash != f->hash || nxt_unit_sptr_get(&f2->name) != nxt_unit_sptr_get(&f->name))
            {
                break;
            }
    ...
    if (r->content_length_field != NXT_UNIT_NONE_FIELD)
    {
        ...
        RC(nxt_python_add_sptr(pctx, nxt_py_content_length_str, &f->value, f->value_length));
    }
    ...
    if (r->content_type_field != NXT_UNIT_NONE_FIELD)
    {
        ...
        RC(nxt_python_add_sptr(pctx, nxt_py_content_type_str, &f->value, f->value_length));
    }

#undef RC

    if (nxt_slow_path(PyDict_SetItem(pctx->environ, nxt_py_wsgi_input_str, (PyObject *)pctx) != 0))
    {
        nxt_unit_req_error(pctx->req, "Python failed to set the \"wsgi.input\" environ value");
        goto fail;
    }
...
static int nxt_python_add_char(nxt_python_ctx_t *pctx, PyObject *name, char *src, uint32_t size)
{
    ...
    {
        nxt_unit_req_error(pctx->req, "Python failed to create value string \"%.*s\"", (int)size, src);
        nxt_python_print_exception();
...
static int nxt_python_add_py_string(nxt_python_ctx_t *pctx, PyObject *name, PyObject *value)
{
    ...
    {
        nxt_unit_req_error(pctx->req, "Python failed to set the \"%s\" environ value", PyUnicode_AsUTF8(name));
...
static int nxt_python_add_field(nxt_python_ctx_t *pctx, nxt_unit_field_t *field, int n, uint32_t vl)
{
    ...
    {
        nxt_unit_req_error(pctx->req, "Python failed to create name string \"%.*s\"", (int)field->name_length, src);
        nxt_python_print_exception();
    ...
    if (nxt_slow_path(value == NULL))
    {
        nxt_unit_req_error(pctx->req, "Python failed to create value string \"%.*s\"", (int)field->value_length,
...
static PyObject *nxt_py_start_resp(PyObject *self, PyObject *args)
{
    ...
    {
        return PyErr_Format(PyExc_RuntimeError, "start_response() is called "
    ...
    {
        return PyErr_Format(PyExc_TypeError, "failed to write first argument (not a string?)");
    }

    headers = PyTuple_GET_ITEM(args, 1);
    if (!PyList_Check(headers))
    {
        return PyErr_Format(PyExc_TypeError, "the second argument is not a response headers list");
    }
    ...
    for (i = 0; i < fields_count; i++)
    {
        ...
        {
            return PyErr_Format(PyExc_TypeError, "the response headers must be a list of tuples");
        }

        if (PyTuple_GET_SIZE(tuple) != 2)
        {
            return PyErr_Format(PyExc_TypeError, "each header must be a tuple of two items");
        }
        ...
        {
            return PyErr_Format(PyExc_TypeError, "header #%d name is not a string", (int)i);
        }
        ...
        {
            return PyErr_Format(PyExc_TypeError, "header #%d value is not a string", (int)i);
        }
    }
    ...
    if (nxt_slow_path(rc != NXT_UNIT_OK))
    {
        return PyErr_Format(PyExc_RuntimeError, "failed to allocate response object");
    }
    ...
    for (i = 0; i < fields_count; i++)
    {
        ...
        rc = nxt_python_response_add_field(pctx, PyTuple_GET_ITEM(tuple, 0), PyTuple_GET_ITEM(tuple, 1), i);
        if (nxt_slow_path(rc != NXT_UNIT_OK))
        {
            return PyErr_Format(PyExc_RuntimeError, "failed to add header #%d", (int)i);
        }
    }
    ...
    if (pctx->content_length == 0)
    {
        ...
        if (nxt_slow_path(rc != NXT_UNIT_OK))
        {
            return PyErr_Format(PyExc_RuntimeError, "failed to send response headers");
        }
    }
...
static int nxt_python_response_add_field(nxt_python_ctx_t *pctx, PyObject *name, PyObject *value, int i)
{
    ...
    rc = nxt_unit_response_add_field(pctx->req, name_str, name_length, value_str, value_length);
    if (nxt_slow_path(rc != NXT_UNIT_OK))
    {
        goto fail;
    }
...
static PyObject *nxt_py_write(PyObject *self, PyObject *str)
{
    int rc;

    if (nxt_fast_path(!PyBytes_Check(str)))
    {
        return PyErr_Format(PyExc_TypeError, "the argument is not a %s", NXT_PYTHON_BYTES_TYPE);
    }

    rc = nxt_python_write((nxt_python_ctx_t *)self, str);
    if (nxt_slow_path(rc != NXT_UNIT_OK))
    {
        return PyErr_Format(PyExc_RuntimeError, "failed to write response value");
    }

    Py_RETURN_NONE;
}

static void nxt_py_input_dealloc(nxt_python_ctx_t *self)
{
    PyObject_Del(self);
}
...
static PyObject *nxt_py_input_read(nxt_python_ctx_t *pctx, PyObject *args)
{
    ...
    if (nxt_slow_path(pctx->req == NULL))
    {
        return PyErr_Format(PyExc_RuntimeError, "wsgi.input.read() is called "
                                                "outside of WSGI request processing");
    }
    ...
    if (n > 0)
    {
        ...
        if (nxt_slow_path(size < 0))
        {
            ...
            if (size != -1)
            {
                return PyErr_Format(PyExc_ValueError, "the read body size cannot be zero or less");
            }
        }
...
static PyObject *nxt_py_input_readline(nxt_python_ctx_t *pctx, PyObject *args)
{
    ...
    if (nxt_slow_path(pctx->req == NULL))
    {
        return PyErr_Format(PyExc_RuntimeError, "wsgi.input.readline() is called "
                                                "outside of WSGI request processing");
    }
    ...
    if (n > 0)
    {
        ...
        if (ssize != -1)
        {
            return PyErr_Format(PyExc_ValueError, "the read line size cannot be zero or less");
        }
...
static PyObject *nxt_py_input_readlines(nxt_python_ctx_t *pctx, PyObject *args)
{
    PyObject *res;

    if (nxt_slow_path(pctx->req == NULL))
    {
        return PyErr_Format(PyExc_RuntimeError, "wsgi.input.readlines() is called "
                                                "outside of WSGI request processing");
    }
...
static PyObject *nxt_py_input_next(PyObject *self)
{
    ...
    if (nxt_slow_path(pctx->req == NULL))
    {
        return PyErr_Format(PyExc_RuntimeError, "wsgi.input.next() is called "
                                                "outside of WSGI request processing");
    }
...
static int nxt_python_write(nxt_python_ctx_t *pctx, PyObject *bytes)
{
    ...
    {
        nxt_unit_req_error(pctx->req, "content length %" PRIu64 " exceeded", pctx->content_length);
