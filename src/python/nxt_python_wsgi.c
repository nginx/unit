...
static int nxt_python_add_sptr(nxt_python_ctx_t* pctx,
                               PyObject* name,
                               nxt_unit_sptr_t* sptr,
                               uint32_t size);
static int nxt_python_add_char(nxt_python_ctx_t* pctx,
                               PyObject* name,
                               char* src,
                               uint32_t size);
static int nxt_python_add_py_string(nxt_python_ctx_t* pctx,
                                    PyObject* name,
                                    PyObject* value);
...
static PyObject* nxt_python_field_value(nxt_unit_field_t* f,
                                        int n,
                                        uint32_t vl);
static int nxt_python_add_obj(nxt_python_ctx_t* pctx,
                              PyObject* name,
                              PyObject* value);
...
static int nxt_python_response_add_field(nxt_python_ctx_t* pctx,
                                         ...
                                         PyObject* value,
                                         int i);
static int nxt_python_str_buf(PyObject* str,
                              char** buf,
                              uint32_t* len,
                              PyObject** bytes);
...
static int nxt_python_wsgi_ctx_data_alloc(void** pdata, int main) {
  ...
  if (nxt_slow_path(pctx->start_resp == NULL)) {
    nxt_unit_alert(
        NULL, "Python failed to initialize the \"start_response\" function");
    goto fail;
  }
  ...
  if (PyBytes_Check(response)) {
    ...
    if (close != NULL) {
      ...
      if (nxt_slow_path(result == NULL)) {
        nxt_unit_req_error(req,
                           "Python failed to call the close() "
...
static PyObject* nxt_python_create_environ(nxt_python_app_conf_t* c) {
  ...
  if (nxt_slow_path(obj == NULL)) {
    nxt_unit_alert(
        NULL, "Python failed to create the \"SERVER_SOFTWARE\" environ value");
    goto fail;
  }
  ...
  if (nxt_slow_path(PyDict_SetItemString(environ, "SERVER_SOFTWARE", obj) !=
                    0)) {
    nxt_unit_alert(
        NULL, "Python failed to set the \"SERVER_SOFTWARE\" environ value");
    goto fail;
  }
  ...
  if (nxt_slow_path(PyDict_SetItemString(environ, "wsgi.multithread",
                    ...
                    0)) {
    nxt_unit_alert(
        NULL, "Python failed to set the \"wsgi.multithread\" environ value");
    goto fail;
  }

  if (nxt_slow_path(
          PyDict_SetItemString(environ, "wsgi.multiprocess", Py_True) != 0)) {
    nxt_unit_alert(
        NULL, "Python failed to set the \"wsgi.multiprocess\" environ value");
    goto fail;
  }

  if (nxt_slow_path(PyDict_SetItemString(environ, "wsgi.run_once", Py_False) !=
                    0)) {
    nxt_unit_alert(NULL,
  ...
  if (nxt_slow_path(PyType_Ready(&nxt_py_input_type) != 0)) {
    nxt_unit_alert(
        NULL, "Python failed to initialize the \"wsgi.input\" type object");
    goto fail;
  }
...
static int nxt_python_add_field(nxt_python_ctx_t* pctx,
  ...
  if (nxt_slow_path(value == NULL)) {
    nxt_unit_req_error(
        pctx->req, "Python failed to create value string \"%.*s\"",
        (int)field->value_length, (char*)nxt_unit_sptr_get(&field->value));
    nxt_python_print_exception();
...
static PyObject* nxt_py_start_resp(PyObject* self, PyObject* args) {
  ...
  for (i = 0; i < fields_count; i++) {
    ...
    if (PyBytes_Check(string)) {
      ...
      return PyErr_Format(PyExc_TypeError, "header #%d name is not a string",
                          (int)i);
    }
    ...
    if (PyBytes_Check(string)) {
      ...
      return PyErr_Format(PyExc_TypeError, "header #%d value is not a string",
                          (int)i);
    }
  }
  ...
  for (i = 0; i < fields_count; i++) {
    ...
    if (nxt_slow_path(rc != NXT_UNIT_OK)) {
      return PyErr_Format(PyExc_RuntimeError, "failed to add header #%d",
                          (int)i);
    }
  }
...
static int nxt_python_response_add_field(nxt_python_ctx_t* pctx,
  ...
  rc = nxt_unit_response_add_field(pctx->req, name_str, name_length, value_str,
                                   value_length);
  if (nxt_slow_path(rc != NXT_UNIT_OK)) {
    goto fail;
  }
...
static PyObject* nxt_py_input_readlines(nxt_python_ctx_t* pctx,
                                        PyObject* args) {
  PyObject* res;
