...
int eventfd(u_int initval, int flags) { return -1; }

int signalfd(int fd, const sigset_t *mask, int flags) { return -1; }
...
int port_dissociate(int port, int source, uintptr_t object) { return -1; }

int port_send(int port, int events, void *user) { return -1; }
...
pollset_t pollset_create(int maxfd) { return -1; }

int pollset_destroy(pollset_t ps) { return -1; }

int pollset_query(pollset_t ps, struct pollfd *pollfd_query) { return -1; }
