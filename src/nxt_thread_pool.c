...
nxt_thread_pool_t *nxt_thread_pool_create(nxt_uint_t max_threads, nxt_nsec_t timeout, nxt_thread_pool_init_t init,
...
nxt_int_t nxt_thread_pool_post(nxt_thread_pool_t *tp, nxt_work_t *work)
{
...
static void nxt_thread_pool_loop(void *ctx)
{
    ...
    for (;;)
    {
        ...
        handler = nxt_locked_work_queue_pop(&tp->work_queue, &task, &obj, &data);
...
void nxt_thread_pool_destroy(nxt_thread_pool_t *tp)
{
    nxt_thread_t *thr;
    ...
    if (!tp->ready)
    {
        nxt_work_queue_add(&thr->engine->fast_work_queue, tp->exit, &tp->engine->task, tp, NULL);
        return;
    }
...
static void nxt_thread_pool_exit(nxt_task_t *task, void *obj, void *data)
{
    ...
    if (threads > 1)
    {
        nxt_work_set(&tp->work, nxt_thread_pool_exit, &tp->task, tp, (void *)(uintptr_t)thread->handle);
    ...
    else
    {
        ...
        nxt_work_set(&tp->work, tp->exit, &tp->engine->task, tp, (void *)(uintptr_t)thread->handle);
