...
static void nxt_http_request_proto_info(nxt_task_t* task,
                                        nxt_http_request_t* r);
static void nxt_http_request_mem_buf_completion(nxt_task_t* task,
                                                void* obj,
                                                void* data);
static void nxt_http_request_done(nxt_task_t* task, void* obj, void* data);

static u_char* nxt_http_date_cache_handler(u_char* buf,
                                           nxt_realtime_t* now,
                                           struct tm* tm,
                                           size_t size,
                                           const char* format);
...
static nxt_http_name_value_t* nxt_http_argument(nxt_array_t* array,
                                                ...
                                                u_char* start,
                                                const u_char* end);
static nxt_int_t nxt_http_cookie_parse(nxt_array_t* cookies,
                                       u_char* start,
                                       const u_char* end);
...
static nxt_int_t nxt_http_validate_host(nxt_str_t* host, nxt_mp_t* mp) {
  ...
  enum { sw_usual, sw_literal, sw_rest } state;
...
static nxt_int_t nxt_http_request_forward(nxt_task_t* task,
  ...
  if (client_ip->header != NULL) {
    client_ip_fields =
        nxt_array_create(r->mem_pool, 1, sizeof(nxt_http_field_t*));
  ...
  nxt_list_each(f, r->fields) {
    if (client_ip_fields != NULL && f->hash == client_ip->header_hash &&
        f->value_length > 0 && f->name_length == client_ip->header->length &&
    ...
    if (protocol->header != NULL && protocol_field == NULL &&
        f->hash == protocol->header_hash && f->value_length > 0 &&
...
int nxt_http_cond_value(nxt_task_t* task,
  ...
  if (cond->expr != NULL) {
    ...
    if (str.length == 0 || nxt_str_eq(&str, "0", 1) ||
        nxt_str_eq(&str, "false", 5) || nxt_str_eq(&str, "null", 4) ||
