...
static nxt_int_t nxt_http_request_forward(nxt_task_t *task, nxt_http_request_t *r, nxt_http_forward_t *forward);
static void nxt_http_request_forward_client_ip(nxt_http_request_t *r, nxt_http_forward_t *forward, nxt_array_t *fields);
static nxt_sockaddr_t *nxt_http_request_client_ip_sockaddr(nxt_http_request_t *r, u_char *start, size_t len);
static void nxt_http_request_forward_protocol(nxt_http_request_t *r, nxt_http_field_t *field);
static void nxt_http_request_ready(nxt_task_t *task, void *obj, void *data);
static void nxt_http_request_proto_info(nxt_task_t *task, nxt_http_request_t *r);
static void nxt_http_request_mem_buf_completion(nxt_task_t *task, void *obj, void *data);
static void nxt_http_request_done(nxt_task_t *task, void *obj, void *data);

static u_char *nxt_http_date_cache_handler(u_char *buf, nxt_realtime_t *now, struct tm *tm, size_t size,
                                           const char *format);

static nxt_http_name_value_t *nxt_http_argument(nxt_array_t *array, u_char *name, size_t name_length, uint32_t hash,
                                                u_char *start, const u_char *end);
static nxt_int_t nxt_http_cookie_parse(nxt_array_t *cookies, u_char *start, const u_char *end);
static nxt_http_name_value_t *nxt_http_cookie(nxt_array_t *array, u_char *name, size_t name_length, u_char *start,
                                              const u_char *end);

#define NXT_HTTP_COOKIE_HASH                                                                                           \
    (nxt_http_field_hash_end(nxt_http_field_hash_char(                                                                 \
         nxt_http_field_hash_char(                                                                                     \
             nxt_http_field_hash_char(                                                                                 \
                 nxt_http_field_hash_char(                                                                             \
                     nxt_http_field_hash_char(nxt_http_field_hash_char(NXT_HTTP_FIELD_HASH_INIT, 'c'), 'o'), 'o'),     \
                 'k'),                                                                                                 \
             'i'),                                                                                                     \
         'e')) &                                                                                                       \
     0xFFFF)
...
nxt_time_string_t nxt_http_date_cache = {
    (nxt_atomic_uint_t)-1, nxt_http_date_cache_handler, NULL,
    NXT_HTTP_DATE_LEN,     NXT_THREAD_TIME_GMT,         NXT_THREAD_TIME_SEC,
};
...
static const nxt_http_request_state_t nxt_http_request_init_state nxt_aligned(64) = {
...
static nxt_int_t nxt_http_request_forward(nxt_task_t *task, nxt_http_request_t *r, nxt_http_forward_t *forward)
{
    ...
    {
        client_ip_fields = nxt_array_create(r->mem_pool, 1, sizeof(nxt_http_field_t *));
    ...
    {
        if (client_ip_fields != NULL && f->hash == client_ip->header_hash && f->value_length > 0 &&
            f->name_length == client_ip->header->length &&
            nxt_memcasecmp(f->name, client_ip->header->start, client_ip->header->length) == 0)
        {
        ...
        if (protocol->header != NULL && protocol_field == NULL && f->hash == protocol->header_hash &&
            f->value_length > 0 && f->name_length == protocol->header->length &&
            nxt_memcasecmp(f->name, protocol->header->start, protocol->header->length) == 0)
        {
            protocol_field = f;
        }
    }
...
static const nxt_http_request_state_t nxt_http_request_body_state nxt_aligned(64) = {
    .ready_handler = nxt_http_request_ready,
    .error_handler = nxt_http_request_close_handler,
};

static nxt_int_t nxt_http_request_chunked_transform(nxt_http_request_t *r)
{
    size_t size;
    u_char *p, *end;
    nxt_http_field_t *f;
...
void nxt_http_request_header_send(nxt_task_t *task, nxt_http_request_t *r, nxt_work_handler_t body_handler, void *data)
{
    ...
    if (r->resp.content_length_n != -1 && (r->resp.content_length == NULL || r->resp.content_length->skip))
    {
...
void nxt_http_request_close_handler(nxt_task_t *task, void *obj, void *data)
{
    ...
    if (r->body != NULL && nxt_buf_is_file(r->body) && r->body->file->fd != -1)
    {
...
nxt_array_t *nxt_http_cookies_parse(nxt_http_request_t *r)
{
    ...
    nxt_list_each(f, r->fields)
    {
        ...
        if (f->hash != NXT_HTTP_COOKIE_HASH || f->name_length != 6 ||
            nxt_strncasecmp(f->name, (u_char *)"Cookie", 6) != 0)
        {
            continue;
        }

        ret = nxt_http_cookie_parse(cookies, f->value, f->value + f->value_length);
        if (ret != NXT_OK)
        {
            return NULL;
        }
...
int nxt_http_cond_value(nxt_task_t *task, nxt_http_request_t *r, nxt_tstr_cond_t *cond)
{
    ...
    if (cond->expr != NULL)
    {
        ...
        if (str.length == 0 || nxt_str_eq(&str, "0", 1) || nxt_str_eq(&str, "false", 5) ||
            nxt_str_eq(&str, "null", 4) || nxt_str_eq(&str, "undefined", 9))
        {
            expr = 0;
        }
    }
