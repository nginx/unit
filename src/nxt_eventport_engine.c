...
static nxt_int_t nxt_eventport_create(nxt_event_engine_t *engine, nxt_uint_t mchanges, nxt_uint_t mevents);
static void nxt_eventport_free(nxt_event_engine_t *engine);
static void nxt_eventport_enable(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_eventport_disable(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static nxt_bool_t nxt_eventport_close(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_eventport_enable_read(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_eventport_enable_write(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_eventport_enable_event(nxt_event_engine_t *engine, nxt_fd_event_t *ev, nxt_uint_t events);
static void nxt_eventport_disable_read(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_eventport_disable_write(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_eventport_disable_event(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static nxt_int_t nxt_eventport_commit_changes(nxt_event_engine_t *engine);
static void nxt_eventport_error_handler(nxt_task_t *task, void *obj, void *data);
static void nxt_eventport_block_read(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_eventport_block_write(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_eventport_oneshot_read(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_eventport_oneshot_write(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_eventport_enable_accept(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static nxt_int_t nxt_eventport_enable_post(nxt_event_engine_t *engine, nxt_work_handler_t handler);
static void nxt_eventport_signal(nxt_event_engine_t *engine, nxt_uint_t signo);
static void nxt_eventport_poll(nxt_event_engine_t *engine, nxt_msec_t timeout);
...
static void nxt_eventport_free(nxt_event_engine_t *engine)
{
    ...
    {
        nxt_alert(&engine->task, "eventport close(%d) failed %E", port, nxt_errno);
    }
...
static void nxt_eventport_enable_read(nxt_event_engine_t *engine, nxt_fd_event_t *ev)
{
    ...
    {
        events = (ev->write == NXT_EVENT_INACTIVE) ? POLLIN : (POLLIN | POLLOUT);
...
static void nxt_eventport_enable_write(nxt_event_engine_t *engine, nxt_fd_event_t *ev)
{
    ...
    {
        events = (ev->read == NXT_EVENT_INACTIVE) ? POLLOUT : (POLLIN | POLLOUT);
        nxt_eventport_enable_event(engine, ev, events);
    }
...
static void nxt_eventport_enable_event(nxt_event_engine_t *engine, nxt_fd_event_t *ev, nxt_uint_t events)
{
    ...
    nxt_debug(ev->task, "port %d set event: fd:%d ev:%04XD u:%p", engine->u.eventport.fd, ev->fd, events, ev);
...
static void nxt_eventport_disable_event(nxt_event_engine_t *engine, nxt_fd_event_t *ev)
{
    ...
    nxt_debug(ev->task, "port %d disable event : fd:%d", engine->u.eventport.fd, ev->fd);
...
static nxt_int_t nxt_eventport_commit_changes(nxt_event_engine_t *engine)
{
    ...
    nxt_debug(&engine->task, "eventport %d changes:%ui", port, engine->u.eventport.nchanges);
    ...
    do
    {
        ...
        if (change->events != 0)
        {
            nxt_debug(ev->task, "port_associate(%d): fd:%d ev:%04XD u:%p", port, ev->fd, change->events, ev);

            ret = port_associate(port, PORT_SOURCE_FD, ev->fd, change->events, ev);
            ...
            nxt_alert(ev->task, "port_associate(%d, %d, %d, %04XD) failed %E", port, PORT_SOURCE_FD, ev->fd,
                      change->events, nxt_errno);
        }
        else
        {
            ...
            nxt_alert(ev->task, "port_dissociate(%d, %d, %d) failed %E", port, PORT_SOURCE_FD, ev->fd, nxt_errno);
        }

        nxt_work_queue_add(&engine->fast_work_queue, nxt_eventport_error_handler, ev->task, ev, ev->data);
...
static void nxt_eventport_poll(nxt_event_engine_t *engine, nxt_msec_t timeout)
{
    ...
    nxt_debug(&engine->task, "port_getn(%d) timeout: %M", engine->u.eventport.fd, timeout);
    ...
    nevents = 1;
    n = port_getn(engine->u.eventport.fd, engine->u.eventport.events, engine->u.eventport.mevents, &nevents, tp);
    ...
    if (n == -1)
    {
        if (err == NXT_ETIME || err == NXT_EINTR)
        {
            if (nevents != 0)
            {
                nxt_alert(&engine->task, "port_getn(%d) failed %E, events:%ud", engine->u.eventport.fd, err, nevents);
            }
        }
        ...
        if (err != NXT_ETIME)
        {
            ...
            nxt_log(&engine->task, level, "port_getn(%d) failed %E", engine->u.eventport.fd, err);
    ...
    nxt_debug(&engine->task, "port_getn(%d) events: %d", engine->u.eventport.fd, nevents);
    ...
    for (i = 0; i < nevents; i++)
    {
        ...
        case PORT_SOURCE_FD:
            ...
            nxt_debug(ev->task, "eventport: fd:%d ev:%04Xd u:%p rd:%d wr:%d", event->portev_object, events, ev,
                      ev->read, ev->write);

            if (nxt_slow_path(events & (POLLERR | POLLHUP | POLLNVAL)) != 0)
            {
                nxt_alert(ev->task, "port_getn(%d) error fd:%d events:%04Xud", engine->u.eventport.fd, ev->fd, events);
        ...
        case PORT_SOURCE_USER:
            nxt_debug(&engine->task, "eventport: user ev:%d u:%p", event->portev_events, event->portev_user);

            signo = event->portev_events;

            handler = (signo == 0) ? engine->u.eventport.post_handler : engine->u.eventport.signal_handler;

            nxt_work_queue_add(&engine->fast_work_queue, handler, &engine->task, (void *)(uintptr_t)signo, NULL);
        ...
        default:
            nxt_alert(&engine->task,
                      ...
                      engine->u.eventport.fd, event->portev_events, event->portev_source, event->portev_object,
                      event->portev_user);
        }
    }
}
