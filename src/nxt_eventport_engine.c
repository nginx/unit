...
static nxt_int_t nxt_eventport_commit_changes(nxt_event_engine_t *engine) {
  ...
  do {
    ...
    if (change->events != 0) {
      ...
      nxt_alert(ev->task, "port_associate(%d, %d, %d, %04XD) failed %E", port,
    ...
    nxt_work_queue_add(&engine->fast_work_queue, nxt_eventport_error_handler,
                       ev->task, ev, ev->data);
...
static void nxt_eventport_poll(nxt_event_engine_t *engine, nxt_msec_t timeout) {
  ...
  nxt_debug(&engine->task, "port_getn(%d) timeout: %M", engine->u.eventport.fd,
            timeout);
  ...
  for (i = 0; i < nevents; i++) {
    ...
    case PORT_SOURCE_FD:
      ...
      if (events & POLLIN) {
        ...
        if (ev->read != NXT_EVENT_BLOCKED) {
          nxt_work_queue_add(ev->read_work_queue, ev->read_handler, ev->task,
                             ev, ev->data);
        }
    ...
    default:
      nxt_alert(&engine->task,
                ...
                event->portev_source, event->portev_object, event->portev_user);
    }
  }
}
