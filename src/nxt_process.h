...
typedef void (*nxt_isolation_cleanup_t)(nxt_task_t *task, nxt_process_t *process);
typedef void (*nxt_cgroup_cleanup_t)(nxt_task_t *task, const nxt_process_t *process);
...
typedef nxt_int_t (*nxt_process_prefork_t)(nxt_task_t *task, nxt_process_t *process, nxt_mp_t *mp);
typedef nxt_int_t (*nxt_process_postfork_t)(nxt_task_t *task, nxt_process_t *process, nxt_mp_t *mp);
typedef nxt_int_t (*nxt_process_setup_t)(nxt_task_t *task, nxt_process_t *process);
typedef nxt_int_t (*nxt_process_start_t)(nxt_task_t *task, nxt_process_data_t *data);
...
NXT_EXPORT nxt_pid_t nxt_process_execute(nxt_task_t *task, char *name, char **argv, char **envp);
NXT_EXPORT nxt_int_t nxt_process_daemon(nxt_task_t *task);
NXT_EXPORT void nxt_nanosleep(nxt_nsec_t ns);

NXT_EXPORT void nxt_process_arguments(nxt_task_t *task, char **orig_argv, char ***orig_envp);

#define nxt_process_init(process) (nxt_pointer_to(process, sizeof(nxt_process_t)))

#define nxt_process_port_remove(port) nxt_queue_remove(&port->link)

#define nxt_process_port_first(process) nxt_queue_link_data(nxt_queue_first(&process->ports), nxt_port_t, link)

NXT_EXPORT void nxt_process_port_add(nxt_task_t *task, nxt_process_t *process, nxt_port_t *port);

#define nxt_process_port_each(process, port) nxt_queue_each(port, &process->ports, nxt_port_t, link)

#define nxt_process_port_loop nxt_queue_loop
...
#define nxt_process_title(task, fmt, ...) setproctitle(fmt, __VA_ARGS__)
...
#define nxt_sched_yield() sched_yield()
...
#define nxt_abort() (void)raise(SIGABRT)
