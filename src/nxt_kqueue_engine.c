...
static nxt_int_t nxt_kqueue_create(nxt_event_engine_t *engine, nxt_uint_t mchanges, nxt_uint_t mevents);
...
static nxt_bool_t nxt_kqueue_close(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_kqueue_enable_read(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_kqueue_enable_write(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_kqueue_disable_read(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_kqueue_disable_write(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_kqueue_block_read(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_kqueue_block_write(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_kqueue_oneshot_read(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_kqueue_oneshot_write(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_kqueue_enable_accept(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_kqueue_enable_file(nxt_event_engine_t *engine, nxt_file_event_t *ev);
static void nxt_kqueue_close_file(nxt_event_engine_t *engine, nxt_file_event_t *ev);
static void nxt_kqueue_fd_set(nxt_event_engine_t *engine, nxt_fd_event_t *ev, nxt_int_t filter, nxt_uint_t flags);
...
static void nxt_kqueue_fd_error_handler(nxt_task_t *task, void *obj, void *data);
static void nxt_kqueue_file_error_handler(nxt_task_t *task, void *obj, void *data);
static nxt_int_t nxt_kqueue_add_signal(nxt_event_engine_t *engine, const nxt_sig_event_t *sigev);
#if (NXT_HAVE_EVFILT_USER)
static nxt_int_t nxt_kqueue_enable_post(nxt_event_engine_t *engine, nxt_work_handler_t handler);
...
static void nxt_kqueue_conn_io_connect(nxt_task_t *task, void *obj, void *data);
static void nxt_kqueue_conn_connected(nxt_task_t *task, void *obj, void *data);
static void nxt_kqueue_listen_handler(nxt_task_t *task, void *obj, void *data);
static void nxt_kqueue_conn_io_accept(nxt_task_t *task, void *obj, void *data);
static void nxt_kqueue_conn_io_read(nxt_task_t *task, void *obj, void *data);
...
static void nxt_kqueue_free(nxt_event_engine_t *engine)
{
    ...
    if (fd != -1 && engine->u.kqueue.pid == nxt_pid)
    {
        ...
        if (close(fd) != 0)
        {
            nxt_alert(&engine->task, "kqueue close(%d) failed %E", fd, nxt_errno);
        }
    }
...
static void nxt_kqueue_enable_read(nxt_event_engine_t *engine, nxt_fd_event_t *ev)
{
    if (ev->read == NXT_EVENT_INACTIVE)
    {
        nxt_kqueue_fd_set(engine, ev, EVFILT_READ, EV_ADD | EV_ENABLE | EV_CLEAR);
    }
...
static void nxt_kqueue_enable_write(nxt_event_engine_t *engine, nxt_fd_event_t *ev)
{
    if (ev->write == NXT_EVENT_INACTIVE)
    {
        nxt_kqueue_fd_set(engine, ev, EVFILT_WRITE, EV_ADD | EV_ENABLE | EV_CLEAR);
    }
...
static void nxt_kqueue_oneshot_read(nxt_event_engine_t *engine, nxt_fd_event_t *ev)
{
    ...
    nxt_kqueue_fd_set(engine, ev, EVFILT_WRITE, EV_ADD | EV_ENABLE | NXT_KEVENT_ONESHOT);
}
...
static void nxt_kqueue_enable_file(nxt_event_engine_t *engine, nxt_file_event_t *ev)
{
    ...
    const nxt_uint_t fflags = NOTE_DELETE | NOTE_WRITE | NOTE_EXTEND | NOTE_ATTRIB | NOTE_RENAME | NOTE_REVOKE;

    nxt_debug(&engine->task, "kevent(%d) set: id:%d ft:%i fl:%04Xd, ff:%04XuD", engine->u.kqueue.fd, ev->file->fd,
              EVFILT_VNODE, flags, fflags);
...
static void nxt_kqueue_fd_set(nxt_event_engine_t *engine, nxt_fd_event_t *ev, nxt_int_t filter, nxt_uint_t flags)
{
    ...
    nxt_debug(ev->task, "kevent(%d) set event: id:%d ft:%i fl:%04Xui", engine->u.kqueue.fd, ev->fd, filter, flags);
...
static struct kevent *nxt_kqueue_get_kevent(nxt_event_engine_t *engine)
{
    ...
    if (nxt_slow_path(nchanges >= engine->u.kqueue.mchanges))
    {
        ...
        nxt_debug(&engine->task, "kevent(%d) changes:%d", engine->u.kqueue.fd, nchanges);

        ret = kevent(engine->u.kqueue.fd, engine->u.kqueue.changes, nchanges, NULL, 0, NULL);

        if (nxt_slow_path(ret != 0))
        {
            nxt_alert(&engine->task, "kevent(%d) failed %E", engine->u.kqueue.fd, nxt_errno);
...
static void nxt_kqueue_error(nxt_event_engine_t *engine)
{
    ...
    for (kev = engine->u.kqueue.changes; kev < end; kev++)
    {
        ...
        case EVFILT_VNODE:
            ...
            nxt_work_queue_add(wq, nxt_kqueue_file_error_handler, fev->task, fev, fev->data);
            break;
        }
    }
}
...
static void nxt_kqueue_fd_error_handler(nxt_task_t *task, void *obj, void *data)
{
    ...
    if (ev->kq_eof && ev->kq_errno != 0)
    {
        ...
        nxt_log(task, nxt_socket_error_level(ev->kq_errno), "kevent() reported error on descriptor %d %E", ev->fd,
                ev->kq_errno);
    }
...
static nxt_int_t nxt_kqueue_add_signal(nxt_event_engine_t *engine, const nxt_sig_event_t *sigev)
{
    ...
    nxt_debug(&engine->task, "kevent(%d) signo:%d (%s)", engine->u.kqueue.fd, signo, sigev->name);
...
static void nxt_kqueue_poll(nxt_event_engine_t *engine, nxt_msec_t timeout)
{
    ...
    nxt_debug(&engine->task, "kevent(%d) changes:%d timeout:%M", engine->u.kqueue.fd, engine->u.kqueue.nchanges,
              timeout);

    nevents = kevent(engine->u.kqueue.fd, engine->u.kqueue.changes, engine->u.kqueue.nchanges, engine->u.kqueue.events,
    ...
    if (nevents == -1)
    {
        ...
        nxt_log(&engine->task, level, "kevent(%d) failed %E", engine->u.kqueue.fd, err);
    ...
    for (i = 0; i < nevents; i++)
    {
        ...
        {
            nxt_alert(&engine->task, "kevent(%d) error %E on ident:%d filter:%d", engine->u.kqueue.fd, kev->data,
...
#if (NXT_DEBUG)
            nxt_alert(&engine->task, "unexpected kevent(%d) filter %d on ident %d", engine->u.kqueue.fd, kev->filter,
                      kev->ident);
#endif
...
static void nxt_kqueue_listen_handler(nxt_task_t *task, void *obj, void *data)
{
    ...
    nxt_debug(task, "kevent fd:%d avail:%D", lev->socket.fd, lev->socket.kq_available);
...
static void nxt_kqueue_conn_io_read(nxt_task_t *task, void *obj, void *data)
{
    ...
    if (c->socket.kq_available == 0 && c->socket.kq_eof)
    {
        ...
        c->socket.closed = 1;
        nxt_work_queue_add(c->read_work_queue, c->read_state->close_handler, task, c, data);
        return;
    }
...
static ssize_t nxt_kqueue_conn_io_recvbuf(nxt_conn_t *c, nxt_buf_t *b)
{
    ...
    if (n > 0)
    {
        ...
        nxt_debug(c->socket.task, "kevent fd:%d avail:%D eof:%d", c->socket.fd, c->socket.kq_available,
                  c->socket.kq_eof);

        c->socket.read_ready = (c->socket.kq_available != 0 || c->socket.kq_eof);
    }
