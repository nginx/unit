...
static void nxt_kqueue_close_file(nxt_event_engine_t* engine,
                                  nxt_file_event_t* ev);
static void nxt_kqueue_fd_set(nxt_event_engine_t* engine,
                              nxt_fd_event_t* ev,
                              nxt_int_t filter,
                              nxt_uint_t flags);
...
static void nxt_kqueue_fd_error_handler(nxt_task_t* task,
                                        void* obj,
                                        void* data);
static void nxt_kqueue_file_error_handler(nxt_task_t* task,
                                          void* obj,
                                          void* data);
...
static struct kevent* nxt_kqueue_get_kevent(nxt_event_engine_t* engine) {
  ...
  if (nxt_slow_path(nchanges >= engine->u.kqueue.mchanges)) {
    ...
    ret = kevent(engine->u.kqueue.fd, engine->u.kqueue.changes, nchanges, NULL,
                 0, NULL);
  ...
  for (kev = engine->u.kqueue.changes; kev < end; kev++) {
    switch (kev->filter) {
      ...
      case EVFILT_VNODE:
        ...
        nxt_work_queue_add(wq, nxt_kqueue_file_error_handler, fev->task, fev,
                           fev->data);
        break;
    }
  }
...
static void nxt_kqueue_poll(nxt_event_engine_t* engine, nxt_msec_t timeout) {
  ...
  if (nevents == -1) {
    ...
    nxt_log(&engine->task, level, "kevent(%d) failed %E", engine->u.kqueue.fd,
            err);
  ...
  for (i = 0; i < nevents; i++) {
    ...
    if (nxt_slow_path(kev->flags & EV_ERROR)) {
      nxt_alert(&engine->task, "kevent(%d) error %E on ident:%d filter:%d",
...
#if (NXT_DEBUG)
        nxt_alert(&engine->task, "unexpected kevent(%d) filter %d on ident %d",
...
static void nxt_kqueue_conn_io_connect(nxt_task_t* task,
                                       void* obj,
                                       void* data) {
  nxt_conn_t* c;
...
static void nxt_kqueue_conn_io_read(nxt_task_t* task, void* obj, void* data) {
  ...
  if (c->socket.kq_available == 0 && c->socket.kq_eof) {
    ...
    c->socket.closed = 1;
    nxt_work_queue_add(c->read_work_queue, c->read_state->close_handler, task,
                       c, data);
    return;
  }
