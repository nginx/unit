...
#define nxt_port_handler_idx(name) (offsetof(nxt_port_handlers_t, name) / sizeof(nxt_port_handler_t))

#define nxt_msg_last(handler) (handler | NXT_PORT_MSG_LAST)
...
typedef enum
{
    ...
    NXT_PORT_MSG_MAX = sizeof(nxt_port_handlers_t) / sizeof(nxt_port_handler_t),
    ...
    NXT_PORT_MSG_MMAP = nxt_msg_last(_NXT_PORT_MSG_MMAP) | NXT_PORT_MSG_SYNC,
...
typedef void (*nxt_port_post_handler_t)(nxt_task_t *task, nxt_port_t *port, void *data);
...
nxt_inline nxt_int_t nxt_port_socket_write(nxt_task_t *task, nxt_port_t *port, nxt_uint_t type, nxt_fd_t fd,
                                           uint32_t stream, nxt_port_id_t reply_port, nxt_buf_t *b)
{
    return nxt_port_socket_write2(task, port, type, fd, -1, stream, reply_port, b);
}
