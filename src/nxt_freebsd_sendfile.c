...
static int
nxt_sys_sendfile(int fd, int s, off_t offset, size_t nbytes,
                                   ...
                                   size_t limit)
{
    ...
    sent = 0;
    n    = nxt_sys_sendfile(fb->file->fd, c->socket.fd, fb->file_pos, file_size,
                            ht, &sent, 0);
    ...
    if (n == -1) {
        ...
        default:
            c->socket.error = err;
            nxt_log(
                c->socket.task, nxt_socket_error_level(err),
