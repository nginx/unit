...
ssize_t nxt_freebsd_event_conn_io_sendfile(nxt_event_conn_t *c, nxt_buf_t *b, size_t limit);

static int nxt_sys_sendfile(int fd, int s, off_t offset, size_t nbytes, struct sf_hdtr *hdtr, off_t *sbytes, int flags)
{
    return -1;
}
...
ssize_t nxt_freebsd_event_conn_io_sendfile(nxt_event_conn_t *c, nxt_buf_t *b, size_t limit)
{
    ...
    nxt_debug(c->socket.task, "sendfile(%FD, %d, @%O, %uz) hd:%ui tr:%ui", fb->file->fd, c->socket.fd, fb->file_pos,
              file_size, nhd, ntr);

    sent = 0;
    n = nxt_sys_sendfile(fb->file->fd, c->socket.fd, fb->file_pos, file_size, ht, &sent, 0);
    ...
    if (n == -1)
    {
        ...
        default:
            ...
            nxt_log(c->socket.task, nxt_socket_error_level(err),
                    "sendfile(%FD, %d, %O, %uz) failed %E \"%FN\" hd:%ui tr:%ui", fb->file->fd, c->socket.fd,
                    fb->file_pos, file_size, err, fb->file->name, nhd, ntr);
