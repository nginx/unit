...
#define nxt_atomic_cmp_set(lock, cmp, set) __sync_bool_compare_and_swap(lock, cmp, set)

#define nxt_atomic_xchg(lock, set) __sync_lock_test_and_set(lock, set)

#define nxt_atomic_fetch_add(value, add) __sync_fetch_and_add(value, add)

#define nxt_atomic_try_lock(lock) nxt_atomic_cmp_set(lock, 0, 1)

#define nxt_atomic_release(lock) __sync_lock_release(lock)

#define nxt_atomic_or_fetch(ptr, val) __sync_or_and_fetch(ptr, val)

#define nxt_atomic_and_fetch(ptr, val) __sync_and_and_fetch(ptr, val)

#if (__i386__ || __i386 || __amd64__ || __amd64)
#define nxt_cpu_pause() __asm__("pause")

#elif (__aarch64__ || __arm64__)
#define nxt_cpu_pause() __asm__("isb")
