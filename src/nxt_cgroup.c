...
static int nxt_mk_cgpath_relative(nxt_task_t *task, const char *dir, char *cgpath);
static nxt_int_t nxt_mk_cgpath(nxt_task_t *task, const char *dir, char *cgpath);
...
nxt_int_t nxt_cgroup_proc_add(nxt_task_t *task, nxt_process_t *process)
{
    ...
    if (task->thread->runtime->type != NXT_PROCESS_MAIN || nxt_process_type(process) != NXT_PROCESS_PROTOTYPE ||
