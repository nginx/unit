...
static nxt_bool_t nxt_sendbuf_copy(nxt_buf_mem_t *bm, nxt_buf_t *b, size_t *copied);
static nxt_buf_t *nxt_sendbuf_coalesce_completion(nxt_task_t *task, nxt_work_queue_t *wq, nxt_buf_t *start);
    ...
    for (b = sb->buf; b != NULL && total < sb->limit; b = b->next)
    {
        ...
        if (nxt_buf_is_mem(b))
        {
            ...
            if (size != 0)
            {
                ...
                nxt_debug(task, "sendbuf: %ui, %p, %uz", n, iov[n].iov_base, iov[n].iov_len);
    ...
    for (b = sb->buf; b != NULL && total < sb->limit; b = b->next)
    {
        ...
        if (nxt_buf_is_mem(b))
        {
            ...
            if (size != 0)
            {
                ...
                nxt_debug(task, "sendbuf: %ui, %p, %uz", n, sb->iobuf[n].iov_base, sb->iobuf[n].iov_len);
