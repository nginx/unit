...
static nxt_int_t nxt_select_create(nxt_event_engine_t *engine, nxt_uint_t mchanges, nxt_uint_t mevents);
...
static nxt_bool_t nxt_select_close(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_select_enable_read(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_select_enable_write(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_select_error_handler(nxt_task_t *task, void *obj, void *data);
static void nxt_select_disable_read(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_select_disable_write(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_select_block_read(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_select_block_write(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_select_oneshot_read(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
static void nxt_select_oneshot_write(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
...
static void nxt_select_enable_read(nxt_event_engine_t *engine, nxt_fd_event_t *ev)
{
    ...
    {
        nxt_work_queue_add(&engine->fast_work_queue, nxt_select_error_handler, ev->task, ev, ev->data);
        return;
    }
...
static void nxt_select_poll(nxt_event_engine_t *engine, nxt_msec_t timeout)
{
    ...
    nevents = select(nfds, &engine->u.select.work_read_fd_set, &engine->u.select.work_write_fd_set, NULL, tp);
    ...
    for (fd = 0; fd < (nxt_uint_t)nfds && nevents != 0; fd++)
    {
        ...
        if (FD_ISSET(fd, &engine->u.select.work_read_fd_set))
        {
            ...
            nxt_debug(ev->task, "select() fd:%ui read rd:%d wr:%d", fd, ev->read, ev->write);
        ...
        if (FD_ISSET(fd, &engine->u.select.work_write_fd_set))
        {
            ...
            nxt_debug(ev->task, "select() fd:%ui write rd:%d wr:%d", fd, ev->read, ev->write);
