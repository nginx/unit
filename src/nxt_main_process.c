...
static void nxt_main_process_sigterm_handler(nxt_task_t* task,
                                             void* obj,
                                             void* data);
static void nxt_main_process_sigquit_handler(nxt_task_t* task,
                                             void* obj,
                                             void* data);
static void nxt_main_process_sigusr1_handler(nxt_task_t* task,
                                             void* obj,
                                             void* data);
static void nxt_main_process_sigchld_handler(nxt_task_t* task,
                                             void* obj,
                                             void* data);
static void nxt_main_process_signal_handler(nxt_task_t* task,
                                            void* obj,
                                            void* data);
...
static void nxt_main_port_conf_store_handler(nxt_task_t* task,
                                             nxt_port_recv_msg_t* msg);
static nxt_int_t nxt_main_file_store(nxt_task_t* task,
                                     const char* tmp_name,
                                     const char* name,
                                     u_char* buf,
                                     size_t size);
...
nxt_int_t nxt_main_process_start(nxt_thread_t* thr,
                                 nxt_task_t* task,
                                 nxt_runtime_t* rt) {
...
static void nxt_main_new_port_handler(nxt_task_t* task,
                                      nxt_port_recv_msg_t* msg) {
  void* mem;
  nxt_port_t* port;
...
static void nxt_main_start_process_handler(nxt_task_t* task,
                                           nxt_port_recv_msg_t* msg) {
  u_char *start, *p, ch;
  size_t type_len;
  nxt_int_t ret;
  ...
  *init = nxt_proto_process;
  ...
  process->name = nxt_mp_alloc(
      process->mem_pool, app_conf->name.length + sizeof("\"\" prototype") + 1);
...
static void nxt_main_process_created_handler(nxt_task_t* task,
                                             nxt_port_recv_msg_t* msg) {
  nxt_port_t* port;
  nxt_process_t* process;
  nxt_runtime_t* rt;
  ...
  if (nxt_is_clone_flag_set(process->isolation.clone.flags, NEWUSER)) {
    if (nxt_slow_path(
            nxt_clone_credential_map(task, process->pid, process->user_cred,
                                     &process->isolation.clone) != NXT_OK)) {
...
static nxt_int_t nxt_main_process_port_create(nxt_task_t* task,
  ...
  port =
      nxt_runtime_process_port_create(task, rt, nxt_pid, 0, NXT_PROCESS_MAIN);
  if (nxt_slow_path(port == NULL)) {
    return NXT_ERROR;
  }
...
static void nxt_main_process_sigusr1_handler(nxt_task_t* task,
  ...
  if (nxt_fast_path(port != NULL)) {
    (void)nxt_port_socket_write(task, port, NXT_PORT_MSG_ACCESS_LOG, -1, 0, 0,
                                NULL);
  }
    ...
    if (process != NULL) {
      ...
      if (!nxt_queue_is_empty(&process->children)) {
        ...
        nxt_queue_each(child, &children, nxt_process_t, link) {
          ...
          (void)nxt_port_socket_write(task, port, NXT_PORT_MSG_QUIT, -1, 0, 0,
                                      NULL);
        }
        nxt_queue_loop;
      }
...
static void nxt_main_process_signal_handler(nxt_task_t* task,
                                            void* obj,
                                            void* data) {
  nxt_trace(task, "signal signo:%d (%s) received, ignored", (int)(uintptr_t)obj,
            data);
}
...
static nxt_int_t nxt_main_listening_socket(nxt_sockaddr_t* sa,
  ...
  if (nxt_slow_path(s == -1)) {
    ...
    ls->end = nxt_sprintf(ls->start, ls->end, "socket(\\\"%*s\\\") failed %E",
  ...
  if (setsockopt(s, SOL_SOCKET, SO_REUSEADDR, &enable, length) != 0) {
    ls->end = nxt_sprintf(
        ls->start, ls->end, "setsockopt(\\\"%*s\\\", SO_REUSEADDR) failed %E",
  ...
  if (sa->u.sockaddr.sa_family == AF_INET6) {
    if (setsockopt(s, IPPROTO_IPV6, IPV6_V6ONLY, &enable, length) != 0) {
      ls->end = nxt_sprintf(
          ls->start, ls->end, "setsockopt(\\\"%*s\\\", IPV6_V6ONLY) failed %E",
          (size_t)sa->length, nxt_sockaddr_start(sa), nxt_errno);
      goto fail;
    }
  }
  ...
  if (sa->u.sockaddr.sa_family == AF_UNIX &&
    ...
    if (chmod(filename, 0666) != 0) {
      ls->end = nxt_sprintf(ls->start, ls->end, "chmod(\\\"%s\\\") failed %E",
                            filename, nxt_errno);
      goto fail;
    }
...
static void nxt_main_port_modules_handler(nxt_task_t* task,
  ...
  for (index = 0; /* void */; index++) {
    ...
    for (jindex = 0; /* */; jindex++) {
      ...
      ret = nxt_conf_map_object(rt->mem_pool, value, nxt_app_lang_mounts_map,
    ...
    nxt_debug(task, "lang %d %s \"%s\" (%d mounts)", lang->type, lang->version,
              lang->file, lang->mounts->nelts);
  }
...
static void nxt_main_port_access_log_handler(nxt_task_t* task,
  ...
  if (nxt_fast_path(port != NULL)) {
    (void)nxt_port_socket_write(task, port, type, file.fd, msg->port_msg.stream,
                                0, NULL);
  }
}
