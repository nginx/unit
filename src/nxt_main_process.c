...
static nxt_int_t nxt_main_process_port_create(nxt_task_t *task, nxt_runtime_t *rt);
static void nxt_main_process_title(nxt_task_t *task);
static void nxt_main_process_sigterm_handler(nxt_task_t *task, void *obj, void *data);
static void nxt_main_process_sigquit_handler(nxt_task_t *task, void *obj, void *data);
static void nxt_main_process_sigusr1_handler(nxt_task_t *task, void *obj, void *data);
static void nxt_main_process_sigchld_handler(nxt_task_t *task, void *obj, void *data);
static void nxt_main_process_signal_handler(nxt_task_t *task, void *obj, void *data);
static void nxt_main_process_cleanup(nxt_task_t *task, nxt_process_t *process);
static void nxt_main_port_socket_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg);
static void nxt_main_port_socket_unlink_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg);
static nxt_int_t nxt_main_listening_socket(nxt_sockaddr_t *sa, nxt_listening_socket_t *ls);
static void nxt_main_port_modules_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg);
static int nxt_cdecl nxt_app_lang_compare(const void *v1, const void *v2);
static void nxt_main_process_whoami_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg);
static void nxt_main_port_conf_store_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg);
static nxt_int_t nxt_main_file_store(nxt_task_t *task, const char *tmp_name, const char *name, u_char *buf,
                                     size_t size);
static void nxt_main_port_access_log_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg);
...
nxt_int_t nxt_main_process_start(nxt_thread_t *thr, nxt_task_t *task, nxt_runtime_t *rt)
{
...
static void nxt_main_data_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
    nxt_debug(task, "main data: %*s", nxt_buf_mem_used_size(&msg->buf->mem), msg->buf->mem.pos);
}

static void nxt_main_new_port_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
    void *mem;
    nxt_port_t *port;
    ...
    {
        mem = nxt_mem_mmap(NULL, sizeof(nxt_port_queue_t), PROT_READ | PROT_WRITE, MAP_SHARED, msg->fd[1], 0);
...
static void nxt_main_start_process_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
    u_char *start, *p, ch;
    size_t type_len;
    nxt_int_t ret;
    ...
    {
        nxt_alert(task, "process %PI cannot start processes", nxt_recv_msg_cmsg_pid(msg));
    ...
    *init = nxt_proto_process;
    ...
    nxt_debug(task, "main start prototype: %*s", b->mem.free - b->mem.pos, b->mem.pos);
    ...
    process->name = nxt_mp_alloc(process->mem_pool, app_conf->name.length + sizeof("\"\" prototype") + 1);
    ...
    *p = '\0';
    ...
    ret = nxt_conf_map_object(process->mem_pool, conf, nxt_common_app_conf, nxt_nitems(nxt_common_app_conf), app_conf);
    ...
    ret = nxt_conf_map_object(process->mem_pool, conf, nxt_app_maps[idx].map, nxt_app_maps[idx].size, app_conf);
    ...
    {
        ret = nxt_conf_map_object(process->mem_pool, app_conf->limits, nxt_common_app_limits_conf,
...
static void nxt_main_process_created_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
    nxt_port_t *port;
    nxt_process_t *process;
    nxt_runtime_t *rt;
    ...
    {
        if (nxt_slow_path(nxt_clone_credential_map(task, process->pid, process->user_cred, &process->isolation.clone) !=
                          NXT_OK))
        {
            (void)nxt_port_socket_write(task, port, NXT_PORT_MSG_RPC_ERROR, -1, msg->port_msg.stream, 0, NULL);
            return;
        }
    }
    ...
    (void)nxt_port_socket_write(task, port, NXT_PORT_MSG_RPC_READY_LAST, -1, msg->port_msg.stream, 0, NULL);
}
...
static void nxt_main_process_whoami_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
    ...
    if (nxt_slow_path(msg->buf == NULL || nxt_buf_used_size(msg->buf) != sizeof(nxt_pid_t)))
    {
    ...
    nxt_debug(task, "whoami: from %PI, parent %PI, fd %d", pid, ppid, msg->fd[0]);

    if (msg->fd[0] != -1)
    {
        port = nxt_runtime_process_port_create(task, rt, pid, 0, NXT_PROCESS_APP);
        if (nxt_slow_path(port == NULL))
        {
            goto fail;
        }
    ...
    buf = nxt_buf_mem_alloc(task->thread->engine->mem_pool, sizeof(nxt_pid_t), 0);
    ...
    (void)nxt_port_socket_write(task, port, NXT_PORT_MSG_RPC_READY_LAST, -1, msg->port_msg.stream, 0, buf);
...
static nxt_int_t nxt_main_process_port_create(nxt_task_t *task, nxt_runtime_t *rt)
{
    ...
    port = nxt_runtime_process_port_create(task, rt, nxt_pid, 0, NXT_PROCESS_MAIN);
    if (nxt_slow_path(port == NULL))
    {
        return NXT_ERROR;
    }
...
static void nxt_main_process_title(nxt_task_t *task)
{
    ...
    p = nxt_sprintf(title, end, "unit: main v" NXT_VERSION " [%s", nxt_process_argv[0]);
...
static void nxt_main_process_sigterm_handler(nxt_task_t *task, void *obj, void *data)
{
    nxt_debug(task, "sigterm handler signo:%d (%s)", (int)(uintptr_t)obj, data);
...
static void nxt_main_process_sigquit_handler(nxt_task_t *task, void *obj, void *data)
{
    nxt_debug(task, "sigquit handler signo:%d (%s)", (int)(uintptr_t)obj, data);
...
static void nxt_main_process_sigusr1_handler(nxt_task_t *task, void *obj, void *data)
{
    ...
    nxt_log(task, NXT_LOG_NOTICE, "signal %d (%s) received, %s", (int)(uintptr_t)obj, data, "log files rotation");
    ...
    if (nxt_fast_path(port != NULL))
    {
        (void)nxt_port_socket_write(task, port, NXT_PORT_MSG_ACCESS_LOG, -1, 0, 0, NULL);
    }
    ...
    nxt_list_each(file, rt->log_files)
    {
        ...
        ret = nxt_file_open(task, new_file, O_WRONLY | O_APPEND, O_CREAT, NXT_FILE_OWNER_ACCESS);
...
static void nxt_main_process_sigchld_handler(nxt_task_t *task, void *obj, void *data)
{
    ...
    nxt_debug(task, "sigchld handler signo:%d (%s)", (int)(uintptr_t)obj, data);
    ...
    for (;;)
    {
        ...
        {
#ifdef WCOREDUMP
            nxt_alert(task, "process %PI exited on signal %d%s", pid, WTERMSIG(status),
                      WCOREDUMP(status) ? " (core dumped)" : "");
#else
            nxt_alert(task, "process %PI exited on signal %d", pid, WTERMSIG(status));
#endif
        }
        else
        {
            nxt_trace(task, "process %PI exited with code %d", pid, WEXITSTATUS(status));
        }
...
static void nxt_main_process_signal_handler(nxt_task_t *task, void *obj, void *data)
{
    nxt_trace(task, "signal signo:%d (%s) received, ignored", (int)(uintptr_t)obj, data);
}
...
static void nxt_main_port_socket_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
    ...
    {
        nxt_alert(task, "process %PI cannot create listener sockets", msg->port_msg.pid);
    ...
    nxt_debug(task, "listening socket \"%*s\"", (size_t)sa->length, nxt_sockaddr_start(sa));

    ret = nxt_main_listening_socket(sa, &ls);

    if (ret == NXT_OK)
    {
        nxt_debug(task, "socket(\"%*s\"): %d", (size_t)sa->length, nxt_sockaddr_start(sa), ls.socket);
    ...
    else
    {
        ...
        out = nxt_buf_mem_ts_alloc(task, task->thread->engine->mem_pool, size + 1);
    ...
    nxt_port_socket_write(task, port, type, ls.socket, msg->port_msg.stream, 0, out);
}
...
static nxt_int_t nxt_main_listening_socket(nxt_sockaddr_t *sa, nxt_listening_socket_t *ls)
{
    ...
    if (nxt_slow_path(s == -1))
    {
        ...
        ls->end = nxt_sprintf(ls->start, ls->end, "socket(\\\"%*s\\\") failed %E", (size_t)sa->length,
    ...
    {
        ls->end = nxt_sprintf(ls->start, ls->end, "setsockopt(\\\"%*s\\\", SO_REUSEADDR) failed %E", (size_t)sa->length,
                              nxt_sockaddr_start(sa), nxt_errno);
        goto fail;
    }
    ...
    if (sa->u.sockaddr.sa_family == AF_INET6)
    {
        ...
        {
            ls->end = nxt_sprintf(ls->start, ls->end, "setsockopt(\\\"%*s\\\", IPV6_V6ONLY) failed %E",
    ...
    if (bind(s, &sa->u.sockaddr, sa->socklen) != 0)
    {
        ...
        ls->end = nxt_sprintf(ls->start, ls->end, "bind(\\\"%*s\\\") failed %E", (size_t)sa->length,
                              nxt_sockaddr_start(sa), err);
        goto fail;
    }
    ...
    if (sa->u.sockaddr.sa_family == AF_UNIX && sa->u.sockaddr_un.sun_path[0] != '\0')
    {
        ...
        {
            ls->end = nxt_sprintf(ls->start, ls->end, "chmod(\\\"%s\\\") failed %E", filename, nxt_errno);
            goto fail;
        }
...
static void nxt_main_port_modules_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
    ...
    nxt_debug(task, "application languages: \"%*s\"", b->mem.free - b->mem.pos, b->mem.pos);
    ...
    for (index = 0; /* void */; index++)
    {
        ...
        ret = nxt_conf_map_object(rt->mem_pool, value, nxt_app_lang_module_map, nxt_nitems(nxt_app_lang_module_map),
                                  lang);
        ...
        lang->mounts = nxt_array_create(rt->mem_pool, nmounts, sizeof(nxt_fs_mount_t));
        ...
        for (jindex = 0; /* */; jindex++)
        {
            ...
            ret = nxt_conf_map_object(rt->mem_pool, value, nxt_app_lang_mounts_map, nxt_nitems(nxt_app_lang_mounts_map),
                                      mnt);
        ...
        nxt_debug(task, "lang %d %s \"%s\" (%d mounts)", lang->type, lang->version, lang->file, lang->mounts->nelts);
    }

    qsort(rt->languages->elts, rt->languages->nelts, sizeof(nxt_app_lang_module_t), nxt_app_lang_compare);
...
static void nxt_main_port_conf_store_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
    ...
    {
        nxt_alert(task, "conf_store_handler: unexpected buffer size (%d)", (int)nxt_buf_mem_used_size(&msg->buf->mem));
        goto error;
    }
...
static nxt_int_t nxt_main_file_store(nxt_task_t *task, const char *tmp_name, const char *name, u_char *buf, size_t size)
{
    ssize_t n;
    nxt_int_t ret;
    nxt_file_t file;
    ...
    ret = nxt_file_open(task, &file, NXT_FILE_WRONLY, NXT_FILE_TRUNCATE, NXT_FILE_OWNER_ACCESS);
    if (nxt_slow_path(ret != NXT_OK))
    {
        return NXT_ERROR;
    }
...
static void nxt_main_port_access_log_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
    ...
    type = (ret == NXT_OK) ? NXT_PORT_MSG_RPC_READY_LAST | NXT_PORT_MSG_CLOSE_FD : NXT_PORT_MSG_RPC_ERROR;

    port = nxt_runtime_port_find(task->thread->runtime, msg->port_msg.pid, msg->port_msg.reply_port);

    if (nxt_fast_path(port != NULL))
    {
        (void)nxt_port_socket_write(task, port, type, file.fd, msg->port_msg.stream, 0, NULL);
    }
}
