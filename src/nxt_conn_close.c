...
static void nxt_conn_close_timer_handler(nxt_task_t* task,
                                         void* obj,
                                         void* data);
static void nxt_conn_close_error_ignore(nxt_task_t* task,
                                        void* obj,
                                        void* data);
...
void nxt_conn_close(nxt_event_engine_t* engine, nxt_conn_t* c) {
  ...
  nxt_debug(c->socket.task, "conn close fd:%d, to:%d", c->socket.fd,
            c->socket.timedout);
...
static void nxt_conn_shutdown_handler(nxt_task_t* task, void* obj, void* data) {
  ...
  nxt_work_queue_add(&engine->close_work_queue, nxt_conn_close_handler, task, c,
                     engine);
}
  ...
  if (events_pending == 0) {
    ...
    if (timers_pending == 0) {
      nxt_work_queue_add(&engine->fast_work_queue,
                         c->write_state->ready_handler, task, c,
                         c->socket.data);
      return;
    }
  }
