...
static void nxt_conn_close_timer_handler(nxt_task_t *task, void *obj, void *data);
static void nxt_conn_close_error_ignore(nxt_task_t *task, void *obj, void *data);

void nxt_conn_close(nxt_event_engine_t *engine, nxt_conn_t *c)
{
    int ret;
    nxt_work_queue_t *wq;
    ...
    nxt_debug(c->socket.task, "conn close fd:%d, to:%d", c->socket.fd, c->socket.timedout);
    ...
    if (c->socket.timedout)
    {
         ...
         */
        ret = setsockopt(c->socket.fd, SOL_SOCKET, SO_LINGER, &linger_off, sizeof(struct linger));

        if (nxt_slow_path(ret != 0))
        {
            nxt_alert(c->socket.task, "setsockopt(%d, SO_LINGER) failed %E", c->socket.fd, nxt_socket_errno);
        }
    }
...
static void nxt_conn_shutdown_handler(nxt_task_t *task, void *obj, void *data)
{
    ...
    nxt_work_queue_add(&engine->close_work_queue, nxt_conn_close_handler, task, c, engine);
}
...
static void nxt_conn_close_handler(nxt_task_t *task, void *obj, void *data)
{
    ...
    if (events_pending == 0)
    {
        ...
        if (timers_pending == 0)
        {
            nxt_work_queue_add(&engine->fast_work_queue, c->write_state->ready_handler, task, c, c->socket.data);
            return;
        }
    }
