...
static nxt_int_t nxt_utf8_overlong(nxt_thread_t *thr, u_char *overlong, size_t len)
{
    u_char *p, utf8[4];
    size_t size;
    uint32_t u, d;
    ...
    if (d != 0xFFFFFFFF)
    {
        ...
        if (len != size || memcmp(overlong, utf8, size) != 0)
        {
            ...
            nxt_log_alert(thr->log, "nxt_utf8_decode(%05uxD, %uz) failed: %05uxD, %uz", u, len, d, size);
...
nxt_int_t nxt_utf8_test(nxt_thread_t *thr)
{
    ...
    for (u = 0; u < 0x110000; u++)
    {
        ...
        if (u != d)
        {
            nxt_log_alert(thr->log, "nxt_utf8_decode(%05uxD) failed: %05uxD", u, d);
            return NXT_ERROR;
        }
    }
    ...
    for (i = 0; i < sizeof(invalid); i += 5)
    {
        ...
        if (d != 0xFFFFFFFF)
        {
            ...
            nxt_log_alert(thr->log, "nxt_utf8_decode(%05uxD, %uz) failed: %05uxD", u, len, d);
            return NXT_ERROR;
        }
    }
    ...
    n = nxt_utf8_casecmp((u_char *)"ABC АБВ ΑΒΓ", (u_char *)"abc абв αβγ", nxt_length("ABC АБВ ΑΒΓ"),
