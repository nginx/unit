...
static nxt_int_t nxt_http_parse_test_bench(nxt_thread_t *thr,
                                           nxt_str_t *request,
                                           nxt_lvlhsh_t *hash, const char *name,
                                           nxt_uint_t n);
static nxt_int_t
...
static nxt_http_parse_test_case_t nxt_http_test_cases[] = {
    ...
    {nxt_string("GET /?# HTTP/1.0\r\n\r\n"),
     NXT_DONE,
     &nxt_http_parse_test_request_line,
     {.request_line = {nxt_string("GET"), nxt_string("/?#"), nxt_string(""),
                       "HTTP/1.0", 1, 0, 0}}},
    {nxt_string("GET // HTTP/1.0\r\n\r\n"),
     NXT_DONE,
     &nxt_http_parse_test_request_line,
    ...
    {nxt_string("GET /%20 HTTP/1.0\r\n\r\n"),
     NXT_DONE,
     &nxt_http_parse_test_request_line,
     {.request_line = {nxt_string("GET"), nxt_string("/%20"), nxt_null_string,
                       "HTTP/1.0", 0, 1, 0}}},
    {nxt_string("GET / a HTTP/1.0\r\n\r\n"),
     NXT_DONE,
     &nxt_http_parse_test_request_line,
     {.request_line = {nxt_string("GET"), nxt_string("/ a"), nxt_null_string,
                       "HTTP/1.0", 0, 0, 1}}},
...
nxt_int_t nxt_http_parse_test(nxt_thread_t *thr) {
  ...
  for (i = 0; i < nxt_nitems(nxt_http_test_cases); i++) {
    ...
    if (test->handler != NULL &&
        test->handler(&rp, &test->data, &test->request, thr->log) != NXT_OK) {
      return NXT_ERROR;
    }
  ...
  colls = nxt_http_fields_hash_collisions(
      &hash, nxt_http_test_bench_fields, nxt_nitems(nxt_http_test_bench_fields),
      0);
...
static nxt_int_t nxt_http_parse_test_bench(nxt_thread_t *thr,
  ...
  nxt_log_error(NXT_LOG_NOTICE, thr->log, "http parse %s request bench: %0.3fs",
