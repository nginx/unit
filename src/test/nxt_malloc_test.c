...
static nxt_malloc_size_t *nxt_malloc_run_test(nxt_thread_t *thr, nxt_malloc_size_t *last, size_t size, nxt_uint_t times)
{
...
nxt_int_t nxt_malloc_test(nxt_thread_t *thr)
{
    ...
    {
        nxt_log_error(NXT_LOG_NOTICE, thr->log, "malloc sizes: %uz-%uz alignment:%uz tight:%ui", s[-1].size + 1,
                      s->size, s->alignment, s->tight);
    }
