...
static nxt_int_t nxt_lvlhsh_test_add(nxt_lvlhsh_t* lh,
...
static nxt_int_t nxt_lvlhsh_test_get(nxt_lvlhsh_t* lh,
                                     const nxt_lvlhsh_proto_t* proto,
                                     uintptr_t key) {
  nxt_lvlhsh_query_t lhq;
...
static nxt_int_t nxt_lvlhsh_test_delete(nxt_lvlhsh_t* lh,
                                        const nxt_lvlhsh_proto_t* proto,
                                        void* pool,
                                        uintptr_t key) {
  nxt_int_t ret;
...
nxt_int_t nxt_lvlhsh_test(nxt_thread_t* thr,
                          nxt_uint_t n,
                          nxt_bool_t use_pool) {
  void* value;
  uint32_t key;
  ...
  if (use_pool) {
    ...
    nxt_log_error(NXT_LOG_NOTICE, thr->log, "lvlhsh test started: %uD malloc",
                  n);
    proto = &malloc_proto;
    mp = NULL;
  }
  ...
  for (i = 0; i < n; i++) {
    ...
    if (nxt_lvlhsh_test_add(&lh, proto, mp, key) != NXT_OK) {
      nxt_log_error(NXT_LOG_NOTICE, thr->log, "lvlhsh add test failed at %ui",
                    i);
      return NXT_ERROR;
    }
  }
