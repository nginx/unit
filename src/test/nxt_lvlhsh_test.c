...
static nxt_int_t nxt_lvlhsh_test_key_test(nxt_lvlhsh_query_t *lhq, void *data)
{
...
static nxt_int_t nxt_lvlhsh_test_add(nxt_lvlhsh_t *lh, const nxt_lvlhsh_proto_t *proto, void *pool, uintptr_t key)
{
...
static nxt_int_t nxt_lvlhsh_test_get(nxt_lvlhsh_t *lh, const nxt_lvlhsh_proto_t *proto, uintptr_t key)
{
    nxt_lvlhsh_query_t lhq;
...
static nxt_int_t nxt_lvlhsh_test_delete(nxt_lvlhsh_t *lh, const nxt_lvlhsh_proto_t *proto, void *pool, uintptr_t key)
{
    nxt_int_t ret;
    nxt_lvlhsh_query_t lhq;
...
nxt_int_t nxt_lvlhsh_test(nxt_thread_t *thr, nxt_uint_t n, nxt_bool_t use_pool)
{
    void *value;
    uint32_t key;
    nxt_mp_t *mp;
    ...
    if (use_pool)
    {
        ...
        nxt_log_error(NXT_LOG_NOTICE, thr->log, "lvlhsh test started: %uD pool", n);
        proto = &pool_proto;
    }
    else
    {
        nxt_log_error(NXT_LOG_NOTICE, thr->log, "lvlhsh test started: %uD malloc", n);
        proto = &malloc_proto;
        mp = NULL;
    }
    ...
    for (i = 0; i < n; i++)
    {
        ...
        {
            nxt_log_error(NXT_LOG_NOTICE, thr->log, "lvlhsh add test failed at %ui", i);
            return NXT_ERROR;
        }
    }
    ...
    if (i != n)
    {
        nxt_log_error(NXT_LOG_NOTICE, thr->log, "lvlhsh each test failed at %ui of %ui", i, n);
        return NXT_ERROR;
    }
    ...
    if (i != n)
    {
        nxt_log_error(NXT_LOG_NOTICE, thr->log, "lvlhsh peek test failed at %ui of %ui", i, n);
        return NXT_ERROR;
    }

    if (!nxt_lvlhsh_is_empty(&lh))
    {
        nxt_log_error(NXT_LOG_NOTICE, thr->log, "lvlhsh is not empty after deletion");
        return NXT_ERROR;
    }
    ...
    if (i != n)
    {
        nxt_log_error(NXT_LOG_NOTICE, thr->log, "lvlhsh retrieve test failed at %ui of %ui", i, n);
        return NXT_ERROR;
    }

    if (!nxt_lvlhsh_is_empty(&lh))
    {
        nxt_log_error(NXT_LOG_NOTICE, thr->log, "lvlhsh is not empty after retrieving");
        return NXT_ERROR;
    }
    ...
    nxt_log_error(NXT_LOG_NOTICE, thr->log, "lvlhsh test passed: %0.3fs", (end - start) / 1000000000.0);
