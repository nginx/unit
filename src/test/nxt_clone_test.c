...
nxt_int_t nxt_clone_test_mappings(nxt_task_t* task,
                                  nxt_mp_t* mp,
                                  nxt_clone_creds_ctx_t* ctx,
                                  nxt_clone_creds_testcase_t* tc);
void nxt_cdecl nxt_clone_test_log_handler(nxt_uint_t level,
                                          nxt_log_t* log,
                                          const char* fmt,
                                          ...);
  ...
  *p++ = '\0';
  ...
  if (!nxt_str_eq(&tc->errmsg, msg, (nxt_uint_t)(p - msg - 1))) {
    nxt_log_error(NXT_LOG_ERR, &nxt_main_log,
                  "error log mismatch: got [%s] but wants [%V]", msg,
                  &tc->errmsg);
    return;
  }
}
...
nxt_int_t nxt_clone_creds_test(nxt_thread_t* thr) {
  ...
  nxt_log_t nxt_clone_creds_log = {NXT_LOG_INFO, 0, nxt_clone_test_log_handler,
                                   NULL, &ctx};
...
static nxt_int_t nxt_clone_test_parse_map(nxt_task_t* task,
  ...
  map->map =
      nxt_mp_alloc(rt->mem_pool, map->size * sizeof(nxt_clone_map_entry_t));
