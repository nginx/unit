...
static int ws_chat_root(nxt_unit_request_info_t *req) {
  ...
  rc = nxt_unit_response_init(
      req, 200 /* Status code. */, 2 /* Number of response headers. */,
      nxt_length(CONTENT_TYPE) + nxt_length(TEXT_HTML) +
          nxt_length(CONTENT_LENGTH) + ws_chat_index_content_length_size +
          ws_chat_index_html_size);
  if (nxt_slow_path(rc != NXT_UNIT_OK)) {
    return rc;
  }

  rc = nxt_unit_response_add_field(req, CONTENT_TYPE, nxt_length(CONTENT_TYPE),
  ...
  rc = nxt_unit_response_add_field(
      req, CONTENT_LENGTH, nxt_length(CONTENT_LENGTH),
      ws_chat_index_content_length, ws_chat_index_content_length_size);
  if (nxt_slow_path(rc != NXT_UNIT_OK)) {
    return rc;
  }
...
static void ws_chat_websocket_handler(nxt_unit_websocket_frame_t *ws) {
  ...
  buf_size += nxt_unit_websocket_read(ws, buf + buf_size,
                                      nxt_min(sizeof(buf), ws->content_length));
...
static void ws_chat_close_handler(nxt_unit_request_info_t *req) {
  ...
  data = req->data;
  buf_size =
      snprintf(buf, sizeof(buf), "Guest #%d has disconnected.", data->id);
...
int main(void) {
  ...
  ws_chat_index_content_length_size = snprintf(
      ws_chat_index_content_length, sizeof(ws_chat_index_content_length), "%d",
      ws_chat_index_html_size);
...
static const char ws_chat_index_html[] =
    ...
    "<div>\n"
    "    <p><input type=\"text\" placeholder=\"type and press enter to chat\" "
    "id=\"chat\" /></p>\n"
    "    <div id=\"container\">\n"
    "        <div id=\"console\"/>\n"
    "    </div>\n"
