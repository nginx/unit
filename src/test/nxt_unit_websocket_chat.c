  ...
  if (strcmp (target, "/chat") == 0)
    {
      ...
      buf_size
          = snprintf (buf, sizeof (buf), "Guest #%d has joined.", data->id);
...
ws_chat_root (nxt_unit_request_info_t *req)
{
  ...
  rc = nxt_unit_response_add_field (req, CONTENT_TYPE,
                                    nxt_length (CONTENT_TYPE), TEXT_HTML,
...
ws_chat_websocket_handler (nxt_unit_websocket_frame_t *ws)
{
  ...
  buf_size += nxt_unit_websocket_read (
      ws, buf + buf_size, nxt_min (sizeof (buf), ws->content_length));
...
main (void)
{
  ...
  ws_chat_index_content_length_size = snprintf (
      ws_chat_index_content_length, sizeof (ws_chat_index_content_length),
...
static const char ws_chat_index_html[]
    = "<html>\n"
      ...
      "support Javascript! Websockets rely on Javascript being enabled. "
      "Please enable\n"
      "    Javascript and reload this page!</h2></noscript>\n"
      "<div>\n"
      "    <p><input type=\"text\" placeholder=\"type and press enter to "
      "chat\" id=\"chat\" /></p>\n"
      "    <div id=\"container\">\n"
      "        <div id=\"console\"/>\n"
      "    </div>\n"
