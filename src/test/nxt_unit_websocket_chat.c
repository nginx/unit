...
static void ws_chat_request_handler(nxt_unit_request_info_t *req)
{
...
static int ws_chat_root(nxt_unit_request_info_t *req)
{
    ...
    rc = nxt_unit_response_init(req, 200 /* Status code. */, 2 /* Number of response headers. */,
                                nxt_length(CONTENT_TYPE) + nxt_length(TEXT_HTML) + nxt_length(CONTENT_LENGTH) +
                                    ws_chat_index_content_length_size + ws_chat_index_html_size);
    if (nxt_slow_path(rc != NXT_UNIT_OK))
    {
        return rc;
    }

    rc = nxt_unit_response_add_field(req, CONTENT_TYPE, nxt_length(CONTENT_TYPE), TEXT_HTML, nxt_length(TEXT_HTML));
    if (nxt_slow_path(rc != NXT_UNIT_OK))
    {
        return rc;
    }

    rc = nxt_unit_response_add_field(req, CONTENT_LENGTH, nxt_length(CONTENT_LENGTH), ws_chat_index_content_length,
    ...
    rc = nxt_unit_response_add_content(req, ws_chat_index_html, ws_chat_index_html_size);
    if (nxt_slow_path(rc != NXT_UNIT_OK))
    {
        return rc;
    }
...
static void ws_chat_websocket_handler(nxt_unit_websocket_frame_t *ws)
{
    int buf_size;
    static char buf[1024];
    ws_chat_request_data_t *data;
    ...
    buf_size += nxt_unit_websocket_read(ws, buf + buf_size, nxt_min(sizeof(buf), ws->content_length));
...
static void ws_chat_close_handler(nxt_unit_request_info_t *req)
{
    int buf_size;
    static char buf[1024];
    ws_chat_request_data_t *data;
    ...
    data = req->data;
    buf_size = snprintf(buf, sizeof(buf), "Guest #%d has disconnected.", data->id);
...
int main(void)
{
    ...
    ws_chat_index_content_length_size =
        snprintf(ws_chat_index_content_length, sizeof(ws_chat_index_content_length), "%d", ws_chat_index_html_size);
...
static const char ws_chat_index_html[] =
    ...
    "</head>\n"
    "<body>\n"
    "<noscript><h2 style=\"color: #ff0000\">Seems your browser doesn't support Javascript! Websockets rely on "
