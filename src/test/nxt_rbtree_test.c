...
static intptr_t nxt_rbtree_test_comparison(nxt_rbtree_node_t *node1, nxt_rbtree_node_t *node2);
...
nxt_int_t nxt_rbtree_test(nxt_thread_t *thr, nxt_uint_t n)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        {
            nxt_log_alert(thr->log, "rbtree test failed: %08XD not found", items[i].key);
            goto fail;
        }
    }
    ...
    while (nxt_rbtree_is_there_successor(&tree, node))
    {
        ...
        if (keys[i] != item->key)
        {
            nxt_log_alert(thr->log, "rbtree test failed: %i: %08XD %08XD", i, keys[i], item->key);
            goto fail;
        }
    ...
    nxt_log_error(NXT_LOG_NOTICE, thr->log, "rbtree test passed %0.3fs", (end - start) / 1000000000.0);
...
nxt_int_t nxt_rbtree_mb_start(nxt_thread_t *thr)
{
    ...
    nxt_log_error(NXT_LOG_NOTICE, thr->log, "rbtree mb cached insert: %L cycles", end - start);
...
void nxt_rbtree_mb_insert(nxt_thread_t *thr)
{
    ...
    nxt_log_error(NXT_LOG_NOTICE, thr->log, "rbtree mb insert: %L cycles", end - start);
}
...
void nxt_rbtree_mb_delete(nxt_thread_t *thr)
{
    ...
    nxt_log_error(NXT_LOG_NOTICE, thr->log, "rbtree mb delete: %L cycles", end - start);
