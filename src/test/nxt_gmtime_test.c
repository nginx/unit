...
nxt_int_t nxt_gmtime_test(nxt_thread_t *thr)
{
    ...
    for (s = 0; s < NXT_GMTIME_MAX; s += 86400)
    {
        ...
        if (tm0.tm_mday != tm1->tm_mday || tm0.tm_mon != tm1->tm_mon || tm0.tm_year != tm1->tm_year ||
            tm0.tm_yday != tm1->tm_yday || tm0.tm_wday != tm1->tm_wday)
        {
            nxt_log_alert(thr->log, "gmtime test failed: %T @ %02d.%02d.%d", s, tm1->tm_mday, tm1->tm_mon + 1,
                          tm1->tm_year + 1900);
            return NXT_ERROR;
        }
    }
    ...
    nxt_log_error(NXT_LOG_NOTICE, thr->log, "nxt_gmtime(): %0.1fns", (end - start) / 10000000.0);
    ...
    nxt_log_error(NXT_LOG_NOTICE, thr->log, "gmtime(): %0.1fns", (end - start) / 10000000.0);
