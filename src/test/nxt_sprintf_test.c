...
static const nxt_sprintf_double_test_t double_test[] = {
    {"%3.5f", "1234.56700", 1234.567}, {"%3.0f", "1234", 1234.567},
    {"%f", "1234.567", 1234.567},      {"%f", "0.1", 0.1},
    {"%f", "0.000001", 0.000001},      {"%f", "4503599627370495", 4503599627370495.0},
};

static nxt_int_t nxt_sprintf_test_double(u_char *buf, u_char *end, const char *fmt, const char *test, double n)
{
    u_char *p;
...
nxt_int_t nxt_sprintf_test(nxt_thread_t *thr)
{
    ...
    for (i = 0; i < nxt_nitems(double_test); i++)
    {
        ...
        nxt_log_alert(thr->log, "nxt_sprintf(\"%s\") failed: \"%s\" vs \"%s\"", double_test[i].format,
                      double_test[i].test, buf);
