...
static double mean(const double *times, int n)
{
    int i;
    double sum;
...
static double cov(const double *times, double mean, int n)
{
    int i;
    double variance;
...
static void enq_deq_worker(void *p)
{
    nxt_cq_t *q;
    uintptr_t i, v;
    nxt_worker_info_t *wi;
...
static void wenq_worker(void *p)
{
    nxt_cq_t *q;
    uintptr_t i, v;
    nxt_worker_info_t *wi;
...
static void wdeq_worker(void *p)
{
    nxt_cq_t *q;
    uintptr_t i, v;
    nxt_worker_info_t *wi;
...
static nxt_int_t worker_create(nxt_worker_info_t *wi, int id, nxt_thread_start_t start)
{
    wi->id = id;
...
static void worker_wait(nxt_worker_info_t *wi)
{
#if NXT_QTEST_USE_THREAD
...
int nxt_cdecl main(int argc, char **argv)
{
    ...
    nprocs = nprocs_enq + nprocs_deq + nprocs_wenq + nprocs_wdeq + nprocs_enq_deq + nprocs_cas + nprocs_faa;
    ...
    pgq = mmap(NULL, sizeof(nxt_cq_t), PROT_READ | PROT_WRITE, MAP_ANON | MAP_SHARED, -1, 0);
    if (pgq == MAP_FAILED)
    {
        return 2;
    }
    ...
    wi = mmap(NULL, nprocs * sizeof(nxt_worker_info_t), PROT_READ | PROT_WRITE, MAP_ANON | MAP_SHARED, -1, 0);
    if (wi == MAP_FAILED)
    {
        return 3;
    }
