...
nxt_int_t nxt_mp_test(nxt_thread_t *thr, nxt_uint_t runs, nxt_uint_t nblocks, size_t max_size)
{
    void **blocks;
    size_t total;
    uint32_t value, size;
    ...
    nxt_log_error(NXT_LOG_NOTICE, thr->log, "mem pool test started, max:%uz", max_size);
    ...
    valid = nxt_mp_test_sizes(cluster_size, page_alignment, page_size, min_chunk_size);
    if (!valid)
    {
        return NXT_ERROR;
    }
    ...
    for (i = 0; i < runs; i++)
    {
        ...
        for (n = 0; n < nblocks; n++)
        {
            ...
            if (blocks[n] == NULL)
            {
                nxt_log_error(NXT_LOG_NOTICE, thr->log, "mem pool test failed: %uz", total);
                return NXT_ERROR;
            }
        }
