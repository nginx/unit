...
{
    nxt_atomic_t slot;
    u_char *(*handler)(u_char *buf, nxt_realtime_t *now, struct tm *tm, size_t size, const char *format);
    const char *format;
    size_t size;
...
NXT_EXPORT u_char *nxt_thread_time_string(nxt_thread_t *thr, nxt_time_string_t *ts, u_char *buf);
...
#define nxt_thread_time_debug_update(thr) nxt_thread_time_update(thr)
