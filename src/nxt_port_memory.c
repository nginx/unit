...
static void nxt_port_broadcast_shm_ack(nxt_task_t *task, nxt_port_t *port, void *data);

nxt_inline void nxt_port_mmap_handler_use(nxt_port_mmap_handler_t *mmap_handler, int i)
{
    int c;
...
static nxt_port_mmap_t *nxt_port_mmap_at(nxt_port_mmaps_t *port_mmaps, uint32_t i)
{
    uint32_t cap;
    ...
    if (cap != port_mmaps->cap)
    {
        ...
        port_mmaps->elts = nxt_realloc(port_mmaps->elts, cap * sizeof(nxt_port_mmap_t));
        if (nxt_slow_path(port_mmaps->elts == NULL))
        {
            return NULL;
        }

        nxt_memzero(port_mmaps->elts + port_mmaps->cap, sizeof(nxt_port_mmap_t) * (cap - port_mmaps->cap));
...
#define nxt_port_mmap_free_junk(p, size) memset((p), 0xA5, size)

static void nxt_port_mmap_buf_completion(nxt_task_t *task, void *obj, void *data)
{
    u_char *p;
    nxt_mp_t *mp;
    nxt_buf_t *b, *next;
...
complete_buf:
    ...
    {
        nxt_debug(task,
                  "mmap buf completion: mmap for other process pair "
                  "%PI->%PI",
                  hdr->src_pid, hdr->dst_pid);
    ...
    nxt_debug(task,
              "mmap buf completion: %p [%p,%uz] (sent=%d), "
              "%PI->%PI,%d,%d",
              b, b->mem.start, b->mem.end - b->mem.start, b->is_port_mmap_sent, hdr->src_pid, hdr->dst_pid, hdr->id, c);
    ...
    if (hdr->dst_pid == nxt_pid && nxt_atomic_cmp_set(&hdr->oosm, 1, 0))
    {
...
nxt_port_mmap_handler_t *nxt_port_incoming_port_mmap(nxt_task_t *task, nxt_process_t *process, nxt_fd_t fd)
{
    void *mem;
    ...
    nxt_debug(task, "got new mmap fd #%FD from process %PI", fd, process->pid);
    ...
    mem = nxt_mem_mmap(NULL, mmap_stat.st_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    ...
    if (nxt_slow_path(hdr->src_pid != process->pid || hdr->dst_pid != nxt_pid))
    {
        nxt_log(task, NXT_LOG_WARN,
                "unexpected pid in mmap header detected: "
                "%PI != %PI or %PI != %PI",
                hdr->src_pid, process->pid, hdr->dst_pid, nxt_pid);
...
static nxt_port_mmap_handler_t *nxt_port_new_port_mmap(nxt_task_t *task, nxt_port_mmaps_t *mmaps, nxt_bool_t tracking,
                                                       nxt_int_t n)
{
    void *mem;
    nxt_fd_t fd;
    ...
    mem = nxt_mem_mmap(NULL, PORT_MMAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    ...
    nxt_log(task, NXT_LOG_DEBUG, "new mmap #%D created for %PI -> ...", hdr->id, nxt_pid);
...
nxt_int_t nxt_shm_open(nxt_task_t *task, size_t size)
{
    ...
    p = nxt_sprintf(name, name + sizeof(name), NXT_SHM_PREFIX "unit.%PI.%uxD", nxt_pid,
    ...
    *p = '\0';
    ...
    {
        nxt_log(task, NXT_LOG_WARN, "shm_unlink(%s) failed %E", name, nxt_errno);
    }
...
static nxt_port_mmap_handler_t *nxt_port_mmap_get(nxt_task_t *task, nxt_port_mmaps_t *mmaps, nxt_chunk_id_t *c,
                                                  nxt_int_t n, nxt_bool_t tracking)
{
    ...
    for (port_mmap = mmaps->elts; port_mmap < end_port_mmap; port_mmap++)
    {
...
static nxt_port_mmap_handler_t *nxt_port_get_port_incoming_mmap(nxt_task_t *task, nxt_pid_t spid, uint32_t id)
{
    nxt_process_t *process;
    nxt_port_mmap_handler_t *mmap_handler;
...
nxt_buf_t *nxt_port_mmap_get_buf(nxt_task_t *task, nxt_port_mmaps_t *mmaps, size_t size)
{
    nxt_mp_t *mp;
    nxt_buf_t *b;
    nxt_int_t nchunks;
    ...
    nxt_debug(task, "outgoing mmap buf allocation: %p [%p,%uz] %PI->%PI,%d,%d", b, b->mem.start,
              b->mem.end - b->mem.start, hdr->src_pid, hdr->dst_pid, hdr->id, c);

    return b;
}

nxt_int_t nxt_port_mmap_increase_buf(nxt_task_t *task, nxt_buf_t *b, size_t size, size_t min_size)
{
    ...
    if (nchunks != 0 && min_size > free_size + PORT_MMAP_CHUNK_SIZE * (c - start))
    {
        c--;
        while (c >= start)
        {
...
static nxt_buf_t *nxt_port_mmap_get_incoming_buf(nxt_task_t *task, nxt_port_t *port, nxt_pid_t spid,
                                                 nxt_port_mmap_msg_t *mmap_msg)
{
    ...
    mmap_handler = nxt_port_get_port_incoming_mmap(task, spid, mmap_msg->mmap_id);
    if (nxt_slow_path(mmap_handler == NULL))
    {
        return NULL;
    }
    ...
    nxt_debug(task, "incoming mmap buf allocation: %p [%p,%uz] %PI->%PI,%d,%d", b, b->mem.start,
              b->mem.end - b->mem.start, hdr->src_pid, hdr->dst_pid, hdr->id, mmap_msg->chunk_id);

    return b;
}

void nxt_port_mmap_write(nxt_task_t *task, nxt_port_t *port, nxt_port_send_msg_t *msg, nxt_sendbuf_coalesce_t *sb,
                         void *mmsg_buf)
{
    size_t bsize;
    nxt_buf_t *bmem;
    ...
    nxt_debug(task,
              "prepare %z bytes message for transfer to process %PI "
              "via shared memory",
              sb->size, port->pid);
    ...
    for (i = 0; i < sb->niov; i++, mmap_msg++)
    {
        ...
        {
            nxt_log_error(NXT_LOG_ERR, task->log, "failed to find buf for iobuf[%d]", i);
            return;
        ...
        nxt_debug(task, "mmap_msg={%D, %D, %D} to %PI", mmap_msg->mmap_id, mmap_msg->chunk_id, mmap_msg->size,
                  port->pid);
    }
...
void nxt_port_mmap_read(nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
    nxt_buf_t *b, **pb;
    ...
    for (b = msg->buf; b != NULL; b = b->next)
    {
        ...
        while (mmap_msg < end)
        {
            nxt_debug(task, "mmap_msg={%D, %D, %D} from %PI", mmap_msg->mmap_id, mmap_msg->chunk_id, mmap_msg->size,
                      msg->port_msg.pid);

            *pb = nxt_port_mmap_get_incoming_buf(task, msg->port, msg->port_msg.pid, mmap_msg);
...
nxt_port_method_t nxt_port_mmap_get_method(nxt_task_t *task, nxt_port_t *port, nxt_buf_t *b)
{
    nxt_port_method_t m;
...
static void nxt_port_broadcast_shm_ack(nxt_task_t *task, nxt_port_t *port, void *data)
{
    ...
    {
        (void)nxt_port_socket_write(task, port, NXT_PORT_MSG_SHM_ACK, -1, 0, 0, NULL);
    }
    nxt_queue_loop;
}
