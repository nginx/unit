...
static void nxt_port_broadcast_shm_ack(nxt_task_t* task,
                                       nxt_port_t* port,
                                       void* data);

nxt_inline void nxt_port_mmap_handler_use(nxt_port_mmap_handler_t* mmap_handler,
                                          int i) {
  int c;
...
static nxt_port_mmap_t* nxt_port_mmap_at(nxt_port_mmaps_t* port_mmaps,
                                         uint32_t i) {
  uint32_t cap;
  ...
  if (cap != port_mmaps->cap) {
    port_mmaps->elts =
        nxt_realloc(port_mmaps->elts, cap * sizeof(nxt_port_mmap_t));
...
void nxt_port_mmaps_destroy(nxt_port_mmaps_t* port_mmaps,
                            nxt_bool_t free_elts) {
  uint32_t i;
...
static void nxt_port_mmap_buf_completion(nxt_task_t* task,
                                         void* obj,
                                         void* data) {
  u_char* p;
  nxt_mp_t* mp;
...
complete_buf:
  ...
  nxt_debug(task,
            ...
            "%PI->%PI,%d,%d",
            b, b->mem.start, b->mem.end - b->mem.start, b->is_port_mmap_sent,
...
nxt_port_mmap_handler_t* nxt_port_incoming_port_mmap(nxt_task_t* task,
                                                     nxt_process_t* process,
                                                     nxt_fd_t fd) {
  void* mem;
  ...
  mem = nxt_mem_mmap(NULL, mmap_stat.st_size, PROT_READ | PROT_WRITE,
                     MAP_SHARED, fd, 0);
...
static nxt_port_mmap_handler_t* nxt_port_new_port_mmap(nxt_task_t* task,
  ...
  mem = nxt_mem_mmap(NULL, PORT_MMAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED,
                     fd, 0);
  ...
  nxt_log(task, NXT_LOG_DEBUG, "new mmap #%D created for %PI -> ...", hdr->id,
          nxt_pid);
...
static nxt_port_mmap_handler_t* nxt_port_mmap_get(nxt_task_t* task,
                                                  nxt_port_mmaps_t* mmaps,
                                                  nxt_chunk_id_t* c,
                                                  nxt_int_t n,
                                                  nxt_bool_t tracking) {
...
nxt_buf_t* nxt_port_mmap_get_buf(nxt_task_t* task,
                                 nxt_port_mmaps_t* mmaps,
                                 size_t size) {
  nxt_mp_t* mp;
  nxt_buf_t* b;
  ...
  nxt_debug(task, "outgoing mmap buf allocation: %p [%p,%uz] %PI->%PI,%d,%d", b,
            b->mem.start, b->mem.end - b->mem.start, hdr->src_pid, hdr->dst_pid,
            hdr->id, c);

  return b;
}

nxt_int_t nxt_port_mmap_increase_buf(nxt_task_t* task,
                                     nxt_buf_t* b,
                                     size_t size,
                                     size_t min_size) {
...
static nxt_buf_t* nxt_port_mmap_get_incoming_buf(
    nxt_task_t* task,
    nxt_port_t* port,
    nxt_pid_t spid,
  ...
  nxt_debug(task, "incoming mmap buf allocation: %p [%p,%uz] %PI->%PI,%d,%d", b,
            b->mem.start, b->mem.end - b->mem.start, hdr->src_pid, hdr->dst_pid,
            hdr->id, mmap_msg->chunk_id);

  return b;
}

void nxt_port_mmap_write(nxt_task_t* task,
                         nxt_port_t* port,
  ...
  for (i = 0; i < sb->niov; i++, mmap_msg++) {
    ...
    if (nxt_slow_path(bmem == NULL)) {
      nxt_log_error(NXT_LOG_ERR, task->log, "failed to find buf for iobuf[%d]",
                    i);
      return;
    ...
    nxt_debug(task, "mmap_msg={%D, %D, %D} to %PI", mmap_msg->mmap_id,
              mmap_msg->chunk_id, mmap_msg->size, port->pid);
  }
...
void nxt_port_mmap_read(nxt_task_t* task, nxt_port_recv_msg_t* msg) {
  ...
  for (b = msg->buf; b != NULL; b = b->next) {
    ...
    while (mmap_msg < end) {
      nxt_debug(task, "mmap_msg={%D, %D, %D} from %PI", mmap_msg->mmap_id,
                mmap_msg->chunk_id, mmap_msg->size, msg->port_msg.pid);

      *pb = nxt_port_mmap_get_incoming_buf(task, msg->port, msg->port_msg.pid,
                                           mmap_msg);
...
nxt_port_method_t nxt_port_mmap_get_method(nxt_task_t* task,
                                           nxt_port_t* port,
                                           nxt_buf_t* b) {
  nxt_port_method_t m;
...
static void nxt_port_broadcast_shm_ack(nxt_task_t* task,
  ...
  nxt_queue_each(port, &process->ports, nxt_port_t, link) {
    (void)nxt_port_socket_write(task, port, NXT_PORT_MSG_SHM_ACK, -1, 0, 0,
                                NULL);
  }
  nxt_queue_loop;
}
