...
typedef struct
{
     ...
     */
    nxt_int_t (*create)(nxt_event_engine_t *engine, nxt_uint_t mchanges, nxt_uint_t mevents);
     ...
     */
    void (*enable)(nxt_event_engine_t *engine, nxt_fd_event_t *ev);

    /* Disable file descriptor event notifications. */
    void (*disable)(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
     ...
     */
    void (*delete)(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
     ...
     */
    nxt_bool_t (*close)(nxt_event_engine_t *engine, nxt_fd_event_t *ev);
     ...
     */
    void (*enable_read)(nxt_event_engine_t *engine, nxt_fd_event_t *ev);

    /*
     * Add a file descriptor to an event set and enable the most effective
     * write event notification method provided by underlying event facility.
     */
    void (*enable_write)(nxt_event_engine_t *engine, nxt_fd_event_t *ev);

    /* Disable file descriptor read event notifications. */
    void (*disable_read)(nxt_event_engine_t *engine, nxt_fd_event_t *ev);

    /* Disable file descriptor write event notifications. */
    void (*disable_write)(nxt_event_engine_t *engine, nxt_fd_event_t *ev);

    /* Block file descriptor read event notifications. */
    void (*block_read)(nxt_event_engine_t *engine, nxt_fd_event_t *ev);

    /* Block file descriptor write event notifications. */
    void (*block_write)(nxt_event_engine_t *engine, nxt_fd_event_t *ev);

    /*
     * Add a file descriptor to an event set and enable an oneshot
     * read event notification method.
     */
    void (*oneshot_read)(nxt_event_engine_t *engine, nxt_fd_event_t *ev);

    /*
     * Add a file descriptor to an event set and enable an oneshot
     * write event notification method.
     */
    void (*oneshot_write)(nxt_event_engine_t *engine, nxt_fd_event_t *ev);

    /*
     * Add a listening socket descriptor to an event set and enable
     * a level-triggered read event notification method.
     */
    void (*enable_accept)(nxt_event_engine_t *engine, nxt_fd_event_t *ev);

    /*
     * Add a file to an event set and enable a file change notification
     * events.
     */
    void (*enable_file)(nxt_event_engine_t *engine, nxt_file_event_t *ev);

    /*
     * Delete a file from an event set before closing the file descriptor.
     */
    void (*close_file)(nxt_event_engine_t *engine, nxt_file_event_t *ev);

    /*
     * Enable post event notifications and set a post handler to handle
     * the zero signal.
     */
    nxt_int_t (*enable_post)(nxt_event_engine_t *engine, nxt_work_handler_t handler);
     ...
     */
    void (*signal)(nxt_event_engine_t *engine, nxt_uint_t signo);

    /* Poll an event set for new event notifications. */
    void (*poll)(nxt_event_engine_t *engine, nxt_msec_t timeout);
...
nxt_int_t nxt_fd_event_hash_add(nxt_lvlhsh_t *lvlhsh, nxt_fd_t fd, nxt_fd_event_t *ev);
void *nxt_fd_event_hash_get(nxt_task_t *task, nxt_lvlhsh_t *lvlhsh, nxt_fd_t fd);
void nxt_fd_event_hash_delete(nxt_task_t *task, nxt_lvlhsh_t *lvlhsh, nxt_fd_t fd, nxt_bool_t ignore);
void nxt_fd_event_hash_destroy(nxt_lvlhsh_t *lvlhsh);

#define nxt_fd_event_disable(engine, ev) (engine)->event.disable(engine, ev)

#define nxt_fd_event_delete(engine, ev) (engine)->event.delete(engine, ev)

#define nxt_fd_event_close(engine, ev) (engine)->event.close(engine, ev)

#define nxt_fd_event_enable_read(engine, ev) (engine)->event.enable_read(engine, ev)

#define nxt_fd_event_enable_write(engine, ev) (engine)->event.enable_write(engine, ev)

#define nxt_fd_event_disable_read(engine, ev) (engine)->event.disable_read(engine, ev)

#define nxt_fd_event_disable_write(engine, ev) (engine)->event.disable_write(engine, ev)
...
#define nxt_fd_event_oneshot_read(engine, ev) (engine)->event.oneshot_read(engine, ev)

#define nxt_fd_event_oneshot_write(engine, ev) (engine)->event.oneshot_write(engine, ev)

#define nxt_fd_event_enable_accept(engine, ev) (engine)->event.enable_accept(engine, ev)
...
NXT_EXPORT nxt_event_engine_t *nxt_event_engine_create(nxt_task_t *task, const nxt_event_interface_t *interface,
                                                       const nxt_sig_event_t *signals, nxt_uint_t flags,
                                                       nxt_uint_t batch);
NXT_EXPORT nxt_int_t nxt_event_engine_change(nxt_event_engine_t *engine, const nxt_event_interface_t *interface,
                                             nxt_uint_t batch);
...
NXT_EXPORT void nxt_event_engine_post(nxt_event_engine_t *engine, nxt_work_t *work);
NXT_EXPORT void nxt_event_engine_signal(nxt_event_engine_t *engine, nxt_uint_t signo);
...
nxt_inline nxt_event_engine_t *nxt_thread_event_engine(void)
{
    nxt_thread_t *thr;
