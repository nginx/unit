...
nxt_wasmtime_execute_request(const nxt_wasm_ctx_t *ctx)
{
    ...
    if (error != NULL || trap != NULL) {
        nxt_wasmtime_err_msg(
            error, trap, "failed to call function [->wasm_request_handler]");
        return -1;
    }
...
nxt_wasmtime_set_function_imports(nxt_wasm_ctx_t *ctx)
{
    ...
    for (imf = import_functions; imf->func_name != NULL; imf++) {
        ...
        wasmtime_linker_define_func(rt_ctx->linker, "env", 3, imf->func_name,
                                    strlen(imf->func_name), func_ty, imf->func,
                                    ctx, NULL);
...
nxt_wasmtime_init(nxt_wasm_ctx_t *ctx)
{
    ...
    error = wasmtime_module_new(rt_ctx->engine, (uint8_t *) wasm.data,
                                wasm.size, &rt_ctx->module);
    if (!rt_ctx->module) {
