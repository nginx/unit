...
static void nxt_wasmtime_err_msg(wasmtime_error_t *error, wasm_trap_t *trap, const char *fmt, ...)
{
    va_list args;
...
static wasm_trap_t *nxt_wasm_get_init_mem_size(void *env, wasmtime_caller_t *caller, const wasmtime_val_t *args,
...
static wasm_trap_t *nxt_wasm_response_end(void *env, wasmtime_caller_t *caller, const wasmtime_val_t *args,
                                          size_t nargs, wasmtime_val_t *results, size_t nresults)
{
...
static wasm_trap_t *nxt_wasm_send_response(void *env, wasmtime_caller_t *caller, const wasmtime_val_t *args,
                                           size_t nargs, wasmtime_val_t *results, size_t nresults)
{
...
static wasm_trap_t *nxt_wasm_send_headers(void *env, wasmtime_caller_t *caller, const wasmtime_val_t *args,
                                          size_t nargs, wasmtime_val_t *results, size_t nresults)
{
...
static wasm_trap_t *nxt_wasm_set_resp_status(void *env, wasmtime_caller_t *caller, const wasmtime_val_t *args,
                                             size_t nargs, wasmtime_val_t *results, size_t nresults)
{
...
static void nxt_wasmtime_execute_hook(const nxt_wasm_ctx_t *ctx, nxt_wasm_fh_t hook)
{
    ...
    if (error != NULL || trap != NULL)
    {
        nxt_wasmtime_err_msg(error, trap, "failed to call hook function [%s]", name);
    }
}

static int nxt_wasmtime_execute_request(const nxt_wasm_ctx_t *ctx)
{
    int i = 0;
    wasm_trap_t *trap = NULL;
    ...
    if (error != NULL || trap != NULL)
    {
        nxt_wasmtime_err_msg(error, trap, "failed to call function [->wasm_request_handler]");
        return -1;
    }
...
static void nxt_wasmtime_set_function_imports(nxt_wasm_ctx_t *ctx)
{
    nxt_wasmtime_ctx_t *rt_ctx = &nxt_wasmtime_ctx;
    ...
    } import_functions[] = {{.func_name = "nxt_wasm_get_init_mem_size",
    ...
    for (imf = import_functions; imf->func_name != NULL; imf++)
    {
        ...
        wasmtime_linker_define_func(rt_ctx->linker, "env", 3, imf->func_name, strlen(imf->func_name), func_ty,
                                    imf->func, ctx, NULL);
        wasm_functype_delete(func_ty);
    }
}

static int nxt_wasmtime_get_function_exports(nxt_wasm_ctx_t *ctx)
{
    int i;
    nxt_wasmtime_ctx_t *rt_ctx = &nxt_wasmtime_ctx;
    ...
    for (i = 0; i < NXT_WASM_FH_NR; i++)
    {
        ...
        ok = wasmtime_linker_get(rt_ctx->linker, rt_ctx->ctx, "", 0, ctx->fh[i].func_name, strlen(ctx->fh[i].func_name),
                                 &item);
        if (!ok)
        {
            nxt_wasmtime_err_msg(NULL, NULL, "couldn't get (%s) export from module", ctx->fh[i].func_name);
            return -1;
        }
...
static int nxt_wasmtime_wasi_init(const nxt_wasm_ctx_t *ctx)
{
    char **dir;
...
static int nxt_wasmtime_init_memory(nxt_wasm_ctx_t *ctx)
{
    int i = 0;
    bool ok;
    wasm_trap_t *trap = NULL;
    ...
    if (error != NULL || trap != NULL)
    {
        nxt_wasmtime_err_msg(error, trap, "failed to call function [->wasm_malloc_handler]");
        return -1;
    }

    ok = wasmtime_linker_get(rt_ctx->linker, rt_ctx->ctx, "", 0, "memory", strlen("memory"), &item);
    if (!ok)
    {
...
static int nxt_wasmtime_init(nxt_wasm_ctx_t *ctx)
{
    int err;
    FILE *fp;
    size_t file_size;
    ...
    error = wasmtime_module_new(rt_ctx->engine, (uint8_t *)wasm.data, wasm.size, &rt_ctx->module);
    if (!rt_ctx->module)
    {
    ...
    error = wasmtime_linker_module(rt_ctx->linker, rt_ctx->ctx, "", 0, rt_ctx->module);
    if (error != NULL)
    {
...
static void nxt_wasmtime_destroy(const nxt_wasm_ctx_t *ctx)
{
    int i = 0;
    wasmtime_val_t args[1] = {};
    nxt_wasmtime_ctx_t *rt_ctx = &nxt_wasmtime_ctx;
