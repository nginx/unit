...
static void nxt_wasmtime_err_msg(wasmtime_error_t* error,
                                 wasm_trap_t* trap,
                                 const char* fmt,
                                 ...) {
  va_list args;
...
static wasm_trap_t* nxt_wasm_get_init_mem_size(void* env,
...
static wasm_trap_t* nxt_wasm_response_end(void* env,
                                          wasmtime_caller_t* caller,
                                          const wasmtime_val_t* args,
                                          size_t nargs,
                                          wasmtime_val_t* results,
...
static wasm_trap_t* nxt_wasm_send_response(void* env,
                                           wasmtime_caller_t* caller,
                                           const wasmtime_val_t* args,
                                           size_t nargs,
                                           wasmtime_val_t* results,
...
static wasm_trap_t* nxt_wasm_send_headers(void* env,
                                          wasmtime_caller_t* caller,
                                          const wasmtime_val_t* args,
                                          size_t nargs,
                                          wasmtime_val_t* results,
...
static wasm_trap_t* nxt_wasm_set_resp_status(void* env,
                                             wasmtime_caller_t* caller,
                                             const wasmtime_val_t* args,
                                             size_t nargs,
                                             wasmtime_val_t* results,
...
static void nxt_wasmtime_execute_hook(const nxt_wasm_ctx_t* ctx,
                                      nxt_wasm_fh_t hook) {
...
static void nxt_wasmtime_set_function_imports(nxt_wasm_ctx_t* ctx) {
  ...
  for (imf = import_functions; imf->func_name != NULL; imf++) {
    ...
    wasmtime_linker_define_func(rt_ctx->linker, "env", 3, imf->func_name,
                                strlen(imf->func_name), func_ty, imf->func, ctx,
                                NULL);
...
static int nxt_wasmtime_get_function_exports(nxt_wasm_ctx_t* ctx) {
  ...
  for (i = 0; i < NXT_WASM_FH_NR; i++) {
    ...
    ok = wasmtime_linker_get(rt_ctx->linker, rt_ctx->ctx, "", 0,
                             ctx->fh[i].func_name, strlen(ctx->fh[i].func_name),
                             &item);
    if (!ok) {
      nxt_wasmtime_err_msg(NULL, NULL, "couldn't get (%s) export from module",
                           ctx->fh[i].func_name);
      return -1;
    }
...
static int nxt_wasmtime_init(nxt_wasm_ctx_t* ctx) {
  ...
  if (!fp) {
    nxt_wasmtime_err_msg(NULL, NULL, "error opening file (%s)",
                         ctx->module_path);
    return -1;
  }
