...
enum { NXT_WASM_HTTP_OK = 200, NXT_WASM_HTTP_ERROR = 500 };
...
void nxt_wasm_do_send_headers(nxt_wasm_ctx_t *ctx, uint32_t offset) {
  ...
  for (i = 0; i < rh->nfields; i++) {
    ...
    nxt_unit_response_add_field(ctx->req, name, rh->fields[i].name_len, val,
                                rh->fields[i].value_len);
  }
...
#undef SET_REQ_MEMBER
  ...
  do {
    read_bytes =
        nxt_min(content_len - content_sent, NXT_WASM_MEM_SIZE - offset);
    bytes_read = nxt_unit_request_read(req, (uint8_t *)wr + offset, read_bytes);
...
static nxt_int_t nxt_wasm_setup(nxt_task_t *task, nxt_process_t *process,
                                nxt_common_app_conf_t *conf) {
  int n, i, err;
