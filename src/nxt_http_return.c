...
static nxt_http_action_t *nxt_http_return(nxt_task_t *task, nxt_http_request_t *r, nxt_http_action_t *action);
static nxt_int_t nxt_http_return_encode(nxt_mp_t *mp, nxt_str_t *encoded, const nxt_str_t *location);
static void nxt_http_return_send(nxt_task_t *task, nxt_http_request_t *r, nxt_http_return_ctx_t *ctx);
...
nxt_http_action_t *nxt_http_return(nxt_task_t *task, nxt_http_request_t *r, nxt_http_action_t *action)
{
    nxt_int_t ret;
    ...
    if (conf->status >= NXT_HTTP_BAD_REQUEST && conf->status <= NXT_HTTP_SERVER_ERROR_MAX)
    {
    ...
    else
    {
        ...
        ret = nxt_tstr_query(task, r->tstr_query, conf->location, &ctx->location);
        if (nxt_slow_path(ret != NXT_OK))
        {
            goto fail;
        }
...
static void nxt_http_return_send(nxt_task_t *task, nxt_http_request_t *r, nxt_http_return_ctx_t *ctx)
{
    ...
    if (ctx != NULL)
    {
        ...
        {
            ret = nxt_http_return_encode(r->mem_pool, &ctx->encoded, &ctx->location);
...
static const nxt_http_request_state_t nxt_http_return_send_state nxt_aligned(64) = {
