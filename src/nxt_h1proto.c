...
static void nxt_h1p_conn_request_header_parse(nxt_task_t* task,
                                              void* obj,
                                              void* data);
static nxt_int_t nxt_h1p_header_process(nxt_task_t* task,
                                        nxt_h1proto_t* h1p,
...
static nxt_int_t nxt_h1p_connection(void* ctx,
                                    nxt_http_field_t* field,
                                    uintptr_t data);
static nxt_int_t nxt_h1p_upgrade(void* ctx,
                                 nxt_http_field_t* field,
                                 uintptr_t data);
static nxt_int_t nxt_h1p_websocket_key(void* ctx,
                                       nxt_http_field_t* field,
                                       uintptr_t data);
static nxt_int_t nxt_h1p_websocket_version(void* ctx,
                                           nxt_http_field_t* field,
                                           uintptr_t data);
static nxt_int_t nxt_h1p_transfer_encoding(void* ctx,
                                           nxt_http_field_t* field,
                                           uintptr_t data);
static void nxt_h1p_request_body_read(nxt_task_t* task, nxt_http_request_t* r);
static void nxt_h1p_conn_request_body_read(nxt_task_t* task,
                                           void* obj,
                                           void* data);
...
static void nxt_h1p_request_header_send(nxt_task_t* task,
                                        ...
                                        void* data);
static void nxt_h1p_request_send(nxt_task_t* task,
                                 nxt_http_request_t* r,
                                 nxt_buf_t* out);
static nxt_buf_t* nxt_h1p_chunk_create(nxt_task_t* task,
                                       nxt_http_request_t* r,
                                       nxt_buf_t* out);
static nxt_off_t nxt_h1p_request_body_bytes_sent(nxt_task_t* task,
                                                 nxt_http_proto_t proto);
static void nxt_h1p_request_discard(nxt_task_t* task,
                                    nxt_http_request_t* r,
                                    nxt_buf_t* last);
static void nxt_h1p_conn_request_error(nxt_task_t* task, void* obj, void* data);
static void nxt_h1p_conn_request_timeout(nxt_task_t* task,
                                         void* obj,
                                         void* data);
static void nxt_h1p_conn_request_send_timeout(nxt_task_t* task,
                                              void* obj,
                                              void* data);
nxt_inline void nxt_h1p_request_error(nxt_task_t* task,
                                      nxt_h1proto_t* h1p,
                                      nxt_http_request_t* r);
static void nxt_h1p_request_close(nxt_task_t* task,
                                  nxt_http_proto_t proto,
...
static void nxt_h1p_keepalive(nxt_task_t* task,
                              nxt_h1proto_t* h1p,
                              nxt_conn_t* c);
...
static void nxt_h1p_idle_response_error(nxt_task_t* task,
                                        void* obj,
                                        void* data);
static void nxt_h1p_idle_response_timeout(nxt_task_t* task,
                                          void* obj,
                                          void* data);
...
static void nxt_h1p_peer_header_read_done(nxt_task_t* task,
                                          void* obj,
                                          void* data);
...
static void nxt_h1p_peer_body_process(nxt_task_t* task,
...
nxt_int_t nxt_h1p_init(nxt_task_t* task) {
  ...
  ret = nxt_http_fields_hash(&nxt_h1p_fields_hash, nxt_h1p_fields,
                             nxt_nitems(nxt_h1p_fields));

  if (nxt_fast_path(ret == NXT_OK)) {
    ret = nxt_http_fields_hash(&nxt_h1p_peer_fields_hash, nxt_h1p_peer_fields,
...
static void nxt_h1p_conn_request_header_parse(nxt_task_t* task,
  ...
  switch (ret) {
    case NXT_DONE:
      ...
      r->request_line.length =
          h1p->parser.request_line_end - r->request_line.start;
...
static nxt_int_t nxt_h1p_header_process(nxt_task_t* task,
  ...
  if (h1p->connection_upgrade && h1p->upgrade_websocket) {
    ...
    if (nxt_slow_path(h1p->parser.method.length != 3 || m[0] != 'G' ||
                      m[1] != 'E' || m[2] != 'T')) {
...
static nxt_int_t nxt_h1p_websocket_version(void* ctx,
  ...
  if (field->value_length == 2 && field->value[0] == '1' &&
      field->value[1] == '3') {
...
static void nxt_h1p_request_body_read(nxt_task_t* task, nxt_http_request_t* r) {
  ...
  nxt_debug(task, "h1p request body read %O te:%d", r->content_length_n,
  ...
  if (body_length > body_buffer_size) {
    ...
    b = nxt_buf_file_alloc(
        r->mem_pool,
        body_buffer_size + sizeof(nxt_file_t) + tmp_name.length + 1, 0);
    if (nxt_slow_path(b == NULL)) {
      status = NXT_HTTP_INTERNAL_SERVER_ERROR;
      goto error;
    }
...
static void nxt_h1p_request_local_addr(nxt_task_t* task,
                                       nxt_http_request_t* r) {
...
static void nxt_h1p_request_header_send(nxt_task_t* task,
    ...
    if (r->resp.content_length == NULL || r->resp.content_length->skip) {
      if (http11) {
        if (n != NXT_HTTP_NOT_MODIFIED && n != NXT_HTTP_NO_CONTENT &&
            body_handler != NULL && !h1p->websocket) {
          h1p->chunked = 1;
  ...
  if (body_handler != NULL) {
     ...
     */
    nxt_work_queue_add(&task->thread->engine->fast_work_queue, body_handler,
                       task, r, data);
...
void nxt_h1p_complete_buffers(nxt_task_t* task,
                              nxt_h1proto_t* h1p,
                              nxt_bool_t all) {
  size_t size;
  nxt_buf_t *b, *in, *next;
...
static nxt_buf_t* nxt_h1p_chunk_create(nxt_task_t* task,
  ...
  header->next = out;
  header->mem.free =
      nxt_sprintf(header->mem.free, header->mem.end, "\r\n%xO\r\n", size);
  return header;
}
...
static void nxt_h1p_conn_request_error(nxt_task_t* task,
                                       void* obj,
                                       void* data) {
  nxt_h1proto_t* h1p;
  nxt_http_request_t* r;
...
static void nxt_h1p_idle_response(nxt_task_t* task, nxt_conn_t* c) {
  ...
  size = nxt_length(NXT_H1P_IDLE_TIMEOUT) + nxt_http_date_cache.size +
         nxt_length("\r\n\r\n");
...
static void nxt_h1p_idle_response_sent(nxt_task_t* task,
                                       void* obj,
                                       void* data) {
  nxt_conn_t* c;
