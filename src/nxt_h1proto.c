...
static void nxt_h1p_conn_request_init (nxt_task_t *task, void *obj,
                                       void *data);
...
static nxt_int_t nxt_h1p_header_buffer_test (nxt_task_t *task,
                                             nxt_h1proto_t *h1p, nxt_conn_t *c,
...
static nxt_int_t nxt_h1p_transfer_encoding (void *ctx, nxt_http_field_t *field,
                                            uintptr_t data);
static void nxt_h1p_request_body_read (nxt_task_t *task,
                                       nxt_http_request_t *r);
...
static void nxt_h1p_request_discard (nxt_task_t *task, nxt_http_request_t *r,
                                     nxt_buf_t *last);
static void nxt_h1p_conn_request_error (nxt_task_t *task, void *obj,
                                        void *data);
...
static void nxt_h1p_idle_response_sent (nxt_task_t *task, void *obj,
                                        void *data);
...
static void nxt_h1p_peer_body_process (nxt_task_t *task, nxt_http_peer_t *peer,
                                       nxt_buf_t *out);
...
static void nxt_h1p_peer_send_timeout (nxt_task_t *task, void *obj,
                                       void *data);
static void nxt_h1p_peer_read_timeout (nxt_task_t *task, void *obj,
                                       void *data);
...
static nxt_int_t
nxt_h1p_header_buffer_test (nxt_task_t *task, nxt_h1proto_t *h1p,
                            nxt_conn_t *c, nxt_socket_conf_t *skcf)
{
  size_t size, used;
  nxt_buf_t *in, *b;
  ...
  if (body_length > body_buffer_size)
    {
      ...
      nxt_debug (task, "create body tmp file \"%V\", %d", &tmp_name,
                 b->file->fd);
...
static const nxt_conn_state_t
    nxt_h1p_peer_header_body_send_state nxt_aligned (64)
    = {
        .ready_handler = nxt_h1p_peer_header_sent,
        .error_handler = nxt_h1p_peer_error,
...
static const nxt_conn_state_t
    nxt_h1p_peer_header_read_timer_state nxt_aligned (64)
    = {
        .ready_handler = nxt_h1p_peer_header_read_done,
        .close_handler = nxt_h1p_peer_closed,
        .error_handler = nxt_h1p_peer_error,
...
nxt_h1p_peer_header_read_done (nxt_task_t *task, void *obj, void *data)
{
  ...
  switch (ret)
    ...
    case NXT_DONE:
      ...
      ret = nxt_http_fields_process (peer->fields, &nxt_h1p_peer_fields_hash,
                                     r);
      if (nxt_slow_path (ret != NXT_OK))
        {
          peer->status = NXT_HTTP_INTERNAL_SERVER_ERROR;
