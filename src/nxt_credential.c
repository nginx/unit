...
static nxt_int_t nxt_credential_groups_get(nxt_task_t* task,
                                           nxt_mp_t* mp,
                                           nxt_credential_t* uc);

nxt_int_t nxt_credential_get(nxt_task_t* task,
                             nxt_mp_t* mp,
                             nxt_credential_t* uc,
                             const char* group) {
  struct group* grp;
  ...
  if (group != NULL && group[0] != '\0') {
    ...
    if (nxt_slow_path(grp == NULL)) {
      if (nxt_errno == 0) {
        nxt_alert(task, "getgrnam(\"%s\") failed, group \"%s\" not found",
                  group, group);
      } else {
  ...
  nxt_debug(task, "about to get \"%s\" groups (uid:%d, base gid:%d)", uc->user,
            uc->uid, uc->base_gid);
...
static nxt_int_t nxt_credential_groups_get(nxt_task_t* task,
  ...
  if (ngroups > NXT_NGROUPS) {
    ...
    if (nxt_slow_path(getgrouplist(uc->user, uc->base_gid, uc->gids, &ngroups) <
                      0)) {
      nxt_alert(task, "getgrouplist(\"%s\", %d) failed %E", uc->user,
                uc->base_gid, nxt_errno);
...
static nxt_int_t nxt_credential_groups_get(nxt_task_t* task,
  ...
  if (initgroups(uc->user, uc->base_gid) != 0) {
    if (nxt_errno == NXT_EPERM) {
      nxt_log(task, NXT_LOG_NOTICE, "initgroups(%s, %d) failed %E, ignored",
      ...
      nxt_alert(task, "initgroups(%s, %d) failed %E", uc->user, uc->base_gid,
                nxt_errno);
      goto restore;
    }
  }
...
nxt_int_t nxt_credential_setgids(nxt_task_t* task, nxt_credential_t* uc) {
  ...
  nxt_debug(task, "user cred set gids: base gid:%d, ngroups: %d", uc->base_gid,
            uc->ngroups);
