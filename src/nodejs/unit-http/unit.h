...
class Unit : public nxt_napi
{
  ...
  private:
    ...
    static void remove_port(nxt_unit_t *unit, nxt_unit_ctx_t *ctx, nxt_unit_port_t *port);
    ...
    napi_value create_socket(napi_value server_obj, nxt_unit_request_info_t *req);

    napi_value create_request(napi_value server_obj, napi_value socket, nxt_unit_request_info_t *req);

    napi_value create_response(napi_value server_obj, napi_value request, nxt_unit_request_info_t *req);

    napi_value create_websocket_frame(napi_value server_obj, nxt_unit_websocket_frame_t *ws);

    static napi_value request_read(napi_env env, napi_callback_info info);

    static napi_value response_send_headers(napi_env env, napi_callback_info info);
    ...
    static napi_value websocket_send_frame(napi_env env, napi_callback_info info);
    ...
    void append_header(nxt_unit_field_t *f, napi_value headers, napi_value raw_headers, uint32_t idx);
