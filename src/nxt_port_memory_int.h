...
#define FREE_MASK(nchunk) (1ULL << ((nchunk) % FREE_BITS))
...
nxt_inline nxt_bool_t nxt_port_mmap_get_free_chunk(nxt_free_map_t *m, nxt_chunk_id_t *c);

#define nxt_port_mmap_get_chunk_busy(m, c) ((m[FREE_IDX(c)] & FREE_MASK(c)) == 0)

nxt_inline void nxt_port_mmap_set_chunk_busy(nxt_free_map_t *m, nxt_chunk_id_t c);

nxt_inline nxt_bool_t nxt_port_mmap_chk_set_chunk_busy(nxt_free_map_t *m, nxt_chunk_id_t c);

nxt_inline void nxt_port_mmap_set_chunk_free(nxt_free_map_t *m, nxt_chunk_id_t c);

nxt_inline nxt_chunk_id_t nxt_port_mmap_chunk_id(nxt_port_mmap_header_t *hdr, const u_char *p)
{
    u_char *mm_start;
...
nxt_inline u_char *nxt_port_mmap_chunk_start(nxt_port_mmap_header_t *hdr, nxt_chunk_id_t c)
{
    u_char *mm_start;
