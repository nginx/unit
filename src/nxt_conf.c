...
static u_char *nxt_conf_json_parse_object(nxt_mp_t *mp, nxt_conf_value_t *value,
                                          u_char *start, u_char *end,
                                          nxt_conf_json_error_t *error);
static nxt_int_t nxt_conf_object_hash_add(nxt_mp_t *mp, nxt_lvlhsh_t *lvlhsh,
...
nxt_str_t *nxt_conf_get_string_dup(const nxt_conf_value_t *value, nxt_mp_t *mp,
                                   nxt_str_t *str) {
  nxt_str_t s;
...
nxt_int_t nxt_conf_set_string_dup(nxt_conf_value_t *value, nxt_mp_t *mp,
                                  const nxt_str_t *str) {
  nxt_str_t tmp, *ptr;
...
void nxt_conf_set_member_string(nxt_conf_value_t *object, const nxt_str_t *name,
...
void nxt_conf_set_member_integer(nxt_conf_value_t *object,
                                 const nxt_str_t *name, int64_t value,
                                 uint32_t index) {
  u_char *p, *end;
  nxt_conf_object_member_t *member;
...
nxt_int_t nxt_conf_map_object(nxt_mp_t *mp, const nxt_conf_value_t *value,
                              const nxt_conf_map_t *map, nxt_uint_t n,
                              void *data) {
  double num;
  nxt_str_t str, *s;
  nxt_uint_t i;
...
static u_char *nxt_conf_json_parse_array(nxt_mp_t *mp, nxt_conf_value_t *value,
  ...
  nxt_list_each(value, list) { *element++ = *value; }
  nxt_list_loop;
...
static u_char *nxt_conf_json_parse_string(nxt_mp_t *mp, nxt_conf_value_t *value,
  ...
  for (p = start; nxt_fast_path(p != end); p++) {
    ...
    case sw_encoded4:
      ...
      if (nxt_fast_path((ch >= '0' && ch <= '9') || (ch >= 'A' && ch <= 'F') ||
                        (ch >= 'a' && ch <= 'f'))) {
