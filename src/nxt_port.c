...
static void nxt_port_remove_pid(nxt_task_t *task, nxt_port_recv_msg_t *msg, nxt_pid_t pid);
static void nxt_port_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg);

static nxt_atomic_uint_t nxt_port_last_id = 1;

static void nxt_port_mp_cleanup(nxt_task_t *task, void *obj, void *data)
{
    nxt_mp_t *mp;
    nxt_port_t *port;
...
nxt_port_t *nxt_port_new(nxt_task_t *task, nxt_port_id_t id, nxt_pid_t pid, nxt_process_type_t type)
{
    nxt_mp_t *mp;
    nxt_port_t *port;
...
void nxt_port_close(nxt_task_t *task, nxt_port_t *port)
{
    ...
    nxt_debug(task, "port %p %d:%d close, type %d", port, port->pid, port->id, port->type);
    ...
    if (port->queue != NULL)
    {
        size = (port->id == (nxt_port_id_t)-1) ? sizeof(nxt_app_queue_t) : sizeof(nxt_port_queue_t);
...
static void nxt_port_release(nxt_task_t *task, nxt_port_t *port)
{
    nxt_debug(task, "port %p %d:%d release, type %d", port, port->pid, port->id, port->type);
...
void nxt_port_enable(nxt_task_t *task, nxt_port_t *port, const nxt_port_handlers_t *handlers)
{
    port->pid = nxt_pid;
...
static void nxt_port_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
    ...
    if (nxt_fast_path(msg->port_msg.type < NXT_PORT_MSG_MAX))
    {
        ...
        nxt_debug(task, "port %d: message type:%uD fds:%d,%d", msg->port->socket.fd, msg->port_msg.type, msg->fd[0],
                  msg->fd[1]);
    ...
    nxt_alert(task, "port %d: unknown message type:%uD", msg->port->socket.fd, msg->port_msg.type);
}
...
nxt_inline void nxt_port_send_new_port(nxt_task_t *task, nxt_runtime_t *rt, nxt_port_t *new_port, uint32_t stream)
{
    nxt_port_t *port;
    nxt_process_t *process;

    nxt_debug(task, "new port %d for process %PI", new_port->pair[1], new_port->pid);
...
nxt_int_t nxt_port_send_port(nxt_task_t *task, nxt_port_t *port, nxt_port_t *new_port, uint32_t stream)
{
    nxt_buf_t *b;
    nxt_port_msg_new_port_t *msg;

    b = nxt_buf_mem_ts_alloc(task, task->thread->engine->mem_pool, sizeof(nxt_port_data_t));
    if (nxt_slow_path(b == NULL))
    {
        return NXT_ERROR;
    }

    nxt_debug(task, "send port %FD to process %PI", new_port->pair[1], port->pid);
    ...
    return nxt_port_socket_write2(task, port, NXT_PORT_MSG_NEW_PORT, new_port->pair[1], new_port->queue_fd, stream, 0,
                                  b);
}
...
void nxt_port_new_port_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
    ...
    nxt_debug(task, "new port %d received for process %PI:%d", msg->fd[0], new_port_msg->pid, new_port_msg->id);

    port = nxt_runtime_port_find(rt, new_port_msg->pid, new_port_msg->id);
    if (port != NULL)
    {
        nxt_debug(task, "port %PI:%d already exists", new_port_msg->pid, new_port_msg->id);
    ...
    port = nxt_runtime_process_port_create(task, rt, new_port_msg->pid, new_port_msg->id, new_port_msg->type);
...
void nxt_port_process_ready_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
    ...
    if (msg->fd[0] != -1)
    {
        ...
        port->queue = nxt_mem_mmap(NULL, sizeof(nxt_port_queue_t), PROT_READ | PROT_WRITE, MAP_SHARED, msg->fd[0], 0);
    }
...
void nxt_port_mmap_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
    ...
    if (nxt_slow_path(process == NULL))
    {
        nxt_log(task, NXT_LOG_WARN, "failed to get process #%PI", msg->port_msg.pid);
...
void nxt_port_change_log_file(nxt_task_t *task, nxt_runtime_t *rt, nxt_uint_t slot, nxt_fd_t fd)
{
    nxt_buf_t *b;
    nxt_port_t *port;
    nxt_process_t *process;
    ...
    nxt_runtime_process_each(rt, process)
    {
        ...
        b = nxt_buf_mem_alloc(task->thread->engine->mem_pool, sizeof(nxt_uint_t), 0);
        if (nxt_slow_path(b == NULL))
        {
            continue;
        }
        ...
        (void)nxt_port_socket_write(task, port, NXT_PORT_MSG_CHANGE_FILE, fd, 0, 0, b);
    }
    nxt_runtime_process_loop;
}

void nxt_port_change_log_file_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
    nxt_buf_t *b;
    nxt_uint_t slot;
    nxt_file_t *log_file;
...
void nxt_port_remove_notify_others(nxt_task_t *task, nxt_process_t *process)
{
    ...
    nxt_runtime_process_each(rt, p)
    {
        ...
        if (p->pid == nxt_pid || p->pid == pid || nxt_queue_is_empty(&p->ports))
        {
            continue;
        }
        ...
        buf = nxt_buf_mem_ts_alloc(task, task->thread->engine->mem_pool, sizeof(pid));
        ...
        nxt_port_socket_write(task, port, NXT_PORT_MSG_REMOVE_PID, -1, process->stream, 0, buf);
    }
    nxt_runtime_process_loop;
}
...
static void nxt_port_post_handler(nxt_task_t *task, void *obj, void *data)
{
    nxt_port_t *port;
    nxt_port_work_t *pw;
    nxt_port_post_handler_t handler;
...
nxt_int_t nxt_port_post(nxt_task_t *task, nxt_port_t *port, nxt_port_post_handler_t handler, void *data)
{
    nxt_port_work_t *pw;
...
static void nxt_port_release_handler(nxt_task_t *task, nxt_port_t *port, void *data)
{
    /* no op */
}
