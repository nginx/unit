...
static void nxt_port_remove_pid(nxt_task_t* task,
...
nxt_port_t* nxt_port_new(nxt_task_t* task,
                         nxt_port_id_t id,
                         nxt_pid_t pid,
...
void nxt_port_enable(nxt_task_t* task,
                     nxt_port_t* port,
...
static void nxt_port_handler(nxt_task_t* task, nxt_port_recv_msg_t* msg) {
  ...
  if (nxt_fast_path(msg->port_msg.type < NXT_PORT_MSG_MAX)) {
    nxt_debug(task, "port %d: message type:%uD fds:%d,%d", msg->port->socket.fd,
              msg->port_msg.type, msg->fd[0], msg->fd[1]);
...
nxt_inline void nxt_port_send_new_port(nxt_task_t* task,
                                       nxt_runtime_t* rt,
                                       nxt_port_t* new_port,
                                       uint32_t stream) {
  nxt_port_t* port;
...
nxt_int_t nxt_port_send_port(nxt_task_t* task,
                             nxt_port_t* port,
                             nxt_port_t* new_port,
                             uint32_t stream) {
  nxt_buf_t* b;
...
void nxt_port_new_port_handler(nxt_task_t* task, nxt_port_recv_msg_t* msg) {
  ...
  port = nxt_runtime_process_port_create(task, rt, new_port_msg->pid,
                                         new_port_msg->id, new_port_msg->type);
...
void nxt_port_process_ready_handler(nxt_task_t* task,
                                    nxt_port_recv_msg_t* msg) {
  nxt_port_t* port;
  nxt_process_t* process;
  nxt_runtime_t* rt;
  ...
  if (msg->fd[0] != -1) {
    ...
    port->queue =
        nxt_mem_mmap(NULL, sizeof(nxt_port_queue_t), PROT_READ | PROT_WRITE,
                     MAP_SHARED, msg->fd[0], 0);
  }
...
void nxt_port_change_log_file(nxt_task_t* task,
                              nxt_runtime_t* rt,
                              nxt_uint_t slot,
                              nxt_fd_t fd) {
  nxt_buf_t* b;
  ...
  nxt_runtime_process_each(rt, process) {
    ...
    b = nxt_buf_mem_alloc(task->thread->engine->mem_pool, sizeof(nxt_uint_t),
                          0);
    if (nxt_slow_path(b == NULL)) {
      continue;
    }
    ...
    (void)nxt_port_socket_write(task, port, NXT_PORT_MSG_CHANGE_FILE, fd, 0, 0,
                                b);
  }
  nxt_runtime_process_loop;
}
...
void nxt_port_remove_notify_others(nxt_task_t* task, nxt_process_t* process) {
  ...
  nxt_runtime_process_each(rt, p) {
    ...
    buf =
        nxt_buf_mem_ts_alloc(task, task->thread->engine->mem_pool, sizeof(pid));
...
nxt_int_t nxt_port_post(nxt_task_t* task,
                        nxt_port_t* port,
                        nxt_port_post_handler_t handler,
                        void* data) {
  nxt_port_work_t* pw;
...
static void nxt_port_release_handler(nxt_task_t* task,
                                     nxt_port_t* port,
                                     void* data) {
  /* no op */
}
