...
nxt_int_t nxt_fs_mount(nxt_task_t *task, nxt_fs_mount_t *mnt)
{
    ...
    rc = mount((const char *)mnt->src, (const char *)mnt->dst, fsname, flags, mnt->data);

    if (nxt_slow_path(rc < 0))
    {
        nxt_alert(task, "mount(\"%s\", \"%s\", \"%s\", %ul, \"%s\") %E", mnt->src, mnt->dst, fsname, flags, mnt->data,
                  nxt_errno);
...
void nxt_fs_unmount(const u_char *path)
{
    ...
    {
        nxt_thread_log_error(NXT_LOG_WARN, "umount2(%s, MNT_DETACH) %E", path, nxt_errno);
    }
}
