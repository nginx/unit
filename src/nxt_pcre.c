...
static void *nxt_pcre_malloc(size_t size)
{
    ...
    {
        nxt_thread_log_alert("pcre_malloc(%uz) called without memory pool", size);
        return NULL;
    }
...
nxt_int_t nxt_regex_match(nxt_regex_t *re, u_char *subject, size_t length, nxt_regex_match_t *match)
{
    ...
    ret = pcre_exec(re->code, re->extra, (const char *)subject, length, 0, 0, match->ovec, match->ovecsize);
    if (nxt_slow_path(ret < PCRE_ERROR_NOMATCH))
    {
        nxt_thread_log_error(NXT_LOG_ERR, "pcre_exec() failed: %d on \"%*s\" using \"%V\"", ret, length, subject,
                             &re->pattern);
