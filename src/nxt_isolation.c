...
static nxt_int_t nxt_isolation_set(nxt_task_t *task, nxt_conf_value_t *isolation, nxt_process_t *process);

#if (NXT_HAVE_CGROUP)
static nxt_int_t nxt_isolation_set_cgroup(nxt_task_t *task, nxt_conf_value_t *isolation, nxt_process_t *process);
#endif

#if (NXT_HAVE_LINUX_NS)
static nxt_int_t nxt_isolation_set_namespaces(nxt_task_t *task, nxt_conf_value_t *isolation, nxt_process_t *process);
static nxt_int_t nxt_isolation_clone_flags(nxt_task_t *task, nxt_conf_value_t *namespaces, nxt_clone_t *clone);
#endif

#if (NXT_HAVE_CLONE_NEWUSER)
static nxt_int_t nxt_isolation_set_creds(nxt_task_t *task, nxt_conf_value_t *isolation, nxt_process_t *process);
static nxt_int_t nxt_isolation_credential_map(nxt_task_t *task, nxt_mp_t *mem_pool, nxt_conf_value_t *map_array,
                                              nxt_clone_credential_map_t *map);
static nxt_int_t nxt_isolation_vldt_creds(nxt_task_t *task, nxt_process_t *process);
#endif

#if (NXT_HAVE_ISOLATION_ROOTFS)
static nxt_int_t nxt_isolation_set_rootfs(nxt_task_t *task, nxt_conf_value_t *isolation, nxt_process_t *process);
static nxt_int_t nxt_isolation_set_automount(nxt_task_t *task, nxt_conf_value_t *isolation, nxt_process_t *process);
static nxt_int_t nxt_isolation_set_mounts(nxt_task_t *task, nxt_process_t *process, nxt_str_t *app_type);
static nxt_int_t nxt_isolation_set_lang_mounts(nxt_task_t *task, nxt_process_t *process, nxt_array_t *syspaths);
static int nxt_cdecl nxt_isolation_mount_compare(const void *v1, const void *v2);
...
static nxt_int_t nxt_isolation_make_private_mount(nxt_task_t *task, const char *rootfs);
...
static nxt_int_t nxt_isolation_set_new_privs(nxt_task_t *task, nxt_conf_value_t *isolation, nxt_process_t *process);
#endif

nxt_int_t nxt_isolation_main_prefork(nxt_task_t *task, nxt_process_t *process, nxt_mp_t *mp)
{
    nxt_int_t cap_setid;
    nxt_int_t ret;
    nxt_runtime_t *rt;
    ...
    if (cap_setid)
    {
        ret = nxt_process_creds_set(task, process, &app_conf->user, &app_conf->group);
    ...
    {
        if (!nxt_str_eq(&app_conf->user, (u_char *)rt->user_cred.user, nxt_strlen(rt->user_cred.user)))
        {
            nxt_alert(task,
                      "cannot set user \"%V\" for app \"%V\": "
                      "missing capabilities",
                      &app_conf->user, &app_conf->name);

            return NXT_ERROR;
        }

        if (app_conf->group.length > 0 && !nxt_str_eq(&app_conf->group, (u_char *)rt->group, nxt_strlen(rt->group)))
        {
            nxt_alert(task,
                      "cannot set group \"%V\" for app \"%V\": "
                      "missing capabilities",
                      &app_conf->group, &app_conf->name);
...
{
#if (NXT_HAVE_CGROUP)
    if (nxt_slow_path(nxt_isolation_set_cgroup(task, isolation, process) != NXT_OK))
    {
        return NXT_ERROR;
    }
#endif
...
#if (NXT_HAVE_LINUX_NS)
    if (nxt_slow_path(nxt_isolation_set_namespaces(task, isolation, process) != NXT_OK))
    {
        return NXT_ERROR;
    }
#endif
...
#if (NXT_HAVE_CLONE_NEWUSER)
    if (nxt_slow_path(nxt_isolation_set_creds(task, isolation, process) != NXT_OK))
    {
        return NXT_ERROR;
    }
#endif
...
#if (NXT_HAVE_ISOLATION_ROOTFS)
    if (nxt_slow_path(nxt_isolation_set_rootfs(task, isolation, process) != NXT_OK))
    {
        return NXT_ERROR;
    }

    if (nxt_slow_path(nxt_isolation_set_automount(task, isolation, process) != NXT_OK))
    {
        return NXT_ERROR;
    }
#endif
...
#if (NXT_HAVE_PR_SET_NO_NEW_PRIVS)
    if (nxt_slow_path(nxt_isolation_set_new_privs(task, isolation, process) != NXT_OK))
    {
        return NXT_ERROR;
    }
#endif
...
static nxt_int_t nxt_isolation_set_cgroup(nxt_task_t *task, nxt_conf_value_t *isolation, nxt_process_t *process)
{
    ...
    process->isolation.cgroup.path = nxt_mp_alloc(process->mem_pool, str.length + 1);
...
static nxt_int_t nxt_isolation_set_creds(nxt_task_t *task, nxt_conf_value_t *isolation, nxt_process_t *process)
{
    ...
    if (array != NULL)
    {
        ret = nxt_isolation_credential_map(task, process->mem_pool, array, &clone->uidmap);
    ...
    if (array != NULL)
    {
        ret = nxt_isolation_credential_map(task, process->mem_pool, array, &clone->gidmap);
...
static nxt_int_t nxt_isolation_credential_map(nxt_task_t *task, nxt_mp_t *mp, nxt_conf_value_t *map_array,
                                              nxt_clone_credential_map_t *map)
{
    nxt_int_t ret;
    nxt_uint_t i;
    ...
    for (i = 0; i < map->size; i++)
    {
        ...
        ret =
            nxt_conf_map_object(mp, obj, nxt_clone_map_entry_conf, nxt_nitems(nxt_clone_map_entry_conf), map->map + i);
        if (nxt_slow_path(ret != NXT_OK))
        {
...
static nxt_int_t nxt_isolation_set_rootfs(nxt_task_t *task, nxt_conf_value_t *isolation, nxt_process_t *process)
{
    ...
    if (obj != NULL)
    {
        ...
        {
            nxt_log(task, NXT_LOG_ERR,
                    "rootfs requires an absolute path other "
                    "than \"/\" but given \"%V\"",
                    &str);
        ...
        process->isolation.rootfs = nxt_mp_alloc(process->mem_pool, str.length + 1);
...
static nxt_int_t nxt_isolation_set_lang_mounts(nxt_task_t *task, nxt_process_t *process, nxt_array_t *lang_mounts)
{
    u_char *p;
    ...
    if (process->isolation.automount.tmpfs)
    {
        ...
        mnt->type = NXT_FS_TMP;
        mnt->flags = (NXT_FS_FLAGS_NOSUID | NXT_FS_FLAGS_NODEV | NXT_FS_FLAGS_NOEXEC);
    ...
    qsort(mounts->elts, mounts->nelts, sizeof(nxt_fs_mount_t), nxt_isolation_mount_compare);
...
void nxt_isolation_unmount_all(nxt_task_t *task, nxt_process_t *process)
{
    size_t n;
    nxt_array_t *mounts;
    nxt_runtime_t *rt;
...
nxt_int_t nxt_isolation_prepare_rootfs(nxt_task_t *task, nxt_process_t *process)
{
    size_t i, n;
    nxt_int_t ret;
    struct stat st;
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (nxt_slow_path(mnt[i].type == NXT_FS_BIND && stat((const char *)mnt[i].src, &st) != 0))
        {
...
static nxt_int_t nxt_isolation_pivot_root(nxt_task_t *task, const char *path)
{
    /*
...
found:
    ...
    if (shared[index])
    {
        ...
        {
            nxt_alert(task, "mount(\"\", \"%s\", MS_PRIVATE) %E", parent_mnt, nxt_errno);
