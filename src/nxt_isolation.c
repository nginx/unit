...
nxt_int_t nxt_isolation_main_prefork(nxt_task_t *task, nxt_process_t *process,
  ...
  if (cap_setid) {
    ret =
        nxt_process_creds_set(task, process, &app_conf->user, &app_conf->group);
...
static nxt_int_t nxt_isolation_set_cgroup(nxt_task_t *task,
  ...
  process->isolation.cgroup.path =
      nxt_mp_alloc(process->mem_pool, str.length + 1);
...
static nxt_int_t nxt_isolation_credential_map(nxt_task_t *task, nxt_mp_t *mp,
                                              nxt_conf_value_t *map_array,
                                              nxt_clone_credential_map_t *map) {
  nxt_int_t ret;
  nxt_uint_t i;
  ...
  for (i = 0; i < map->size; i++) {
    ...
    ret =
        nxt_conf_map_object(mp, obj, nxt_clone_map_entry_conf,
                            nxt_nitems(nxt_clone_map_entry_conf), map->map + i);
    if (nxt_slow_path(ret != NXT_OK)) {
...
static nxt_int_t nxt_isolation_set_lang_mounts(nxt_task_t *task,
  ...
  if (process->isolation.automount.tmpfs) {
    ...
    mnt->type = NXT_FS_TMP;
    mnt->flags =
        (NXT_FS_FLAGS_NOSUID | NXT_FS_FLAGS_NODEV | NXT_FS_FLAGS_NOEXEC);
