...
ssize_t
nxt_hpux_event_conn_io_sendfile(nxt_event_conn_t *c, nxt_buf_t *b,
                                size_t limit);

static ssize_t
nxt_sys_sendfile(int s, int fd, off_t offset, size_t nbytes,
...
sbsize_t
sendfile(int s, int fd, off_t offset, bsize_t nbytes, const struct iovec *hdtrl,
         int flags);
...
nxt_hpux_event_conn_io_sendfile(nxt_event_conn_t *c, nxt_buf_t *b, size_t limit)
{
    ...
    n   = nxt_sys_sendfile(c->socket.fd, fb->file->fd, fb->file_pos, file_size,
                           hdtrl, 0);
    ...
    if (n == -1) {
        ...
        default:
            ...
            nxt_log(c->socket.task, nxt_socket_error_level(err),
                    ...
                    c->socket.fd, fb->file_pos, file_size, &fb->file->name, nhd,
                    ntr);
