...
static nxt_conf_value_t* nxt_cert_name_details(nxt_mp_t* mp,
                                               X509* x509,
                                               nxt_bool_t issuer);
    ...
    */
    ...
    if (x509 != NULL) {
      if (cert->count == nalloc) {
        ...
        new_cert =
            nxt_realloc(cert, sizeof(nxt_cert_t) + nalloc * sizeof(X509*));
        if (new_cert == NULL) {
          X509_free(x509);
          goto fail;
        }
...
static nxt_conf_value_t* nxt_cert_name_details(nxt_mp_t* mp,
  ...
  alt_names = X509_get_ext_d2i(
      x509, issuer ? NID_issuer_alt_name : NID_subject_alt_name, NULL, NULL);
...
void nxt_cert_store_get(nxt_task_t* task,
                        nxt_str_t* name,
                        nxt_mp_t* mp,
  ...
  *b->mem.free++ = '\0';
  ...
  stream =
      nxt_port_rpc_register_handler(task, recv_port, handler, handler, -1, ctx);
...
void nxt_cert_store_delete(nxt_task_t* task, nxt_str_t* name, nxt_mp_t* mp) {
  ...
  if (nxt_fast_path(b != NULL)) {
    ...
    *b->mem.free++ = '\0';
    ...
    (void)nxt_port_socket_write(task, main_port, NXT_PORT_MSG_CERT_DELETE, -1,
                                0, 0, b);
  }
}
