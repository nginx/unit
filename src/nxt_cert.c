...
static nxt_conf_value_t *nxt_cert_name_details(nxt_mp_t *mp, X509 *x509, nxt_bool_t issuer);
...
static nxt_cert_t *nxt_cert_bio(nxt_task_t *task, BIO *bio)
{
    ...
    for (;;)
    {
        ...
        if (ret == 0)
        {
            ...
            {
                nxt_openssl_log_error(task, NXT_LOG_ALERT, "PEM_read_bio() failed");
                goto fail;
            }
        ...
        */
        ...
        if (nxt_strcmp(type, PEM_STRING_PKCS8INF) == 0)
        {
            ...
            if (p8inf == NULL)
            {
                nxt_openssl_log_error(task, NXT_LOG_ALERT, "d2i_PKCS8_PRIV_KEY_INFO() failed");
                goto done;
            }
        ...
        if (suffix != 0)
        {
            ...
            if (ameth == NULL)
            {
                nxt_openssl_log_error(task, NXT_LOG_ALERT, "EVP_PKEY_asn1_find_str() failed");
                goto done;
            }
        ...
        if (nxt_strcmp(type, PEM_STRING_X509) == 0 || nxt_strcmp(type, PEM_STRING_X509_OLD) == 0)
        {
            data_copy = data;
        ...
        if (nxt_strcmp(type, PEM_STRING_X509_TRUSTED) == 0)
        {
            ...
            if (x509 == NULL)
            {
                nxt_openssl_log_error(task, NXT_LOG_ALERT, "d2i_X509_AUX() failed");
            }
        ...
        if (x509 != NULL)
        {
            ...
            if (cert->count == nalloc)
            {
                ...
                new_cert = nxt_realloc(cert, sizeof(nxt_cert_t) + nalloc * sizeof(X509 *));
                if (new_cert == NULL)
                {
                    X509_free(x509);
                    goto fail;
...
static nxt_int_t nxt_cert_info_hash_test(nxt_lvlhsh_query_t *lhq, void *data)
{
...
static nxt_conf_value_t *nxt_cert_details(nxt_mp_t *mp, nxt_cert_t *cert)
{
    ...
    if (cert->key != NULL)
    {
        ...
        {
        case EVP_PKEY_RSA:
            end = nxt_sprintf(buf, buf + sizeof(buf), "RSA (%d bits)", EVP_PKEY_bits(key));
        ...
        case EVP_PKEY_DH:
            end = nxt_sprintf(buf, buf + sizeof(buf), "DH (%d bits)", EVP_PKEY_bits(key));
    ...
    for (i = 0; i < cert->count; i++)
    {
        ...
        if (nxt_fast_path(ret == 1))
        {
            ...
            ret = nxt_conf_set_member_string_dup(value, mp, &since_str, &str, 0);
        }
        else
        {
            ret = NXT_ERROR;
        ...
        {
            str.length = BIO_get_mem_data(bio, &str.start);
            ret = nxt_conf_set_member_string_dup(value, mp, &until_str, &str, 1);
        }
        else
        {
            ret = NXT_ERROR;
...
static nxt_conf_value_t *nxt_cert_name_details(nxt_mp_t *mp, X509 *x509, nxt_bool_t issuer)
{
    ...
    x509_name = issuer ? X509_get_issuer_name(x509) : X509_get_subject_name(x509);
    ...
    alt_names = X509_get_ext_d2i(x509, issuer ? NID_issuer_alt_name : NID_subject_alt_name, NULL, NULL);
    ...
    for (n = 0, i = 0; n != nxt_nitems(nids) && i != count; n++)
    {
        ...
        len = X509_NAME_get_text_by_NID(x509_name, nids[n].nid, (char *)buf, sizeof(buf));
        ...
        ret = nxt_conf_set_member_string_dup(object, mp, &nids[n].name, &str, i++);
        if (nxt_slow_path(ret != NXT_OK))
        {
            return NULL;
        }
...
nxt_array_t *nxt_cert_store_load(nxt_task_t *task, nxt_mp_t *mp)
{
    DIR *dir;
    size_t size, alloc;
    u_char *buf, *p;
    ...
    {
        nxt_alert(task, "opendir(\"%s\") failed %E", rt->certs.start, nxt_errno);
        goto fail;
    }
    ...
    for (;;)
    {
        ...
        ret = nxt_file_open(task, &file, NXT_FILE_RDONLY, NXT_FILE_OPEN, NXT_FILE_OWNER_ACCESS);
...
void nxt_cert_store_get(nxt_task_t *task, nxt_str_t *name, nxt_mp_t *mp, nxt_port_rpc_handler_t handler, void *ctx)
{
    uint32_t stream;
    nxt_int_t ret;
    nxt_buf_t *b;
    ...
    *b->mem.free++ = '\0';
    ...
    stream = nxt_port_rpc_register_handler(task, recv_port, handler, handler, -1, ctx);
    if (nxt_slow_path(stream == 0))
    {
        goto fail;
    }

    ret = nxt_port_socket_write(task, main_port, NXT_PORT_MSG_CERT_GET, -1, stream, recv_port->id, b);
...
void nxt_cert_store_get_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
    ...
    if (nxt_slow_path(port == NULL))
    {
        nxt_alert(task, "process port not found (pid %PI, reply_port %d)", msg->port_msg.pid, msg->port_msg.reply_port);
        return;
    }

    if (nxt_slow_path(port->type != NXT_PROCESS_CONTROLLER && port->type != NXT_PROCESS_ROUTER))
    {
        nxt_alert(task, "process %PI cannot store certificates", msg->port_msg.pid);
        return;
    }
    ...
    ret = nxt_file_open(task, &file, NXT_FILE_RDWR, NXT_FILE_CREATE_OR_OPEN, NXT_FILE_OWNER_ACCESS);
...
void nxt_cert_store_delete(nxt_task_t *task, nxt_str_t *name, nxt_mp_t *mp)
{
    ...
    if (nxt_fast_path(b != NULL))
    {
        ...
        *b->mem.free++ = '\0';
        ...
        (void)nxt_port_socket_write(task, main_port, NXT_PORT_MSG_CERT_DELETE, -1, 0, 0, b);
    }
}
...
void nxt_cert_store_delete_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
    ...
    {
        nxt_alert(task, "process %PI cannot delete certificates", nxt_recv_msg_cmsg_pid(msg));
        return;
    }
