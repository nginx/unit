...
static nxt_script_t *nxt_script_get(nxt_task_t *task, nxt_str_t *name, nxt_fd_t fd);
...
nxt_script_t *nxt_script_new(nxt_task_t *task, nxt_str_t *name, u_char *data, size_t size, u_char *error)
{
    u_char *start;
    njs_vm_t *vm;
    njs_str_t mod_name;
...
static nxt_int_t nxt_script_info_hash_test(nxt_lvlhsh_query_t *lhq, void *data)
{
...
static nxt_conf_value_t *nxt_script_details(nxt_mp_t *mp, nxt_script_t *script)
{
    nxt_conf_value_t *value;
...
nxt_array_t *nxt_script_store_load(nxt_task_t *task, nxt_mp_t *mp)
{
    DIR *dir;
    size_t size, alloc;
    u_char *buf, *p;
    ...
    {
        nxt_alert(task, "opendir(\"%s\") failed %E", rt->scripts.start, nxt_errno);
        goto fail;
    }
    ...
    for (;;)
    {
        ...
        nxt_debug(task, "readdir(\"%s\"): \"%s\"", rt->scripts.start, de->d_name);
...
void nxt_script_store_get(nxt_task_t *task, nxt_str_t *name, nxt_mp_t *mp, nxt_port_rpc_handler_t handler, void *ctx)
{
    uint32_t stream;
    nxt_int_t ret;
    nxt_buf_t *b;
    ...
    *b->mem.free++ = '\0';
    ...
    ret = nxt_port_socket_write(task, main_port, NXT_PORT_MSG_SCRIPT_GET, -1, stream, recv_port->id, b);
...
void nxt_script_store_delete(nxt_task_t *task, nxt_str_t *name, nxt_mp_t *mp)
{
    ...
    if (nxt_fast_path(b != NULL))
    {
        ...
        *b->mem.free++ = '\0';
        ...
        (void)nxt_port_socket_write(task, main_port, NXT_PORT_MSG_SCRIPT_DELETE, -1, 0, 0, b);
    }
}

void nxt_script_store_delete_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
    u_char *p;
    nxt_str_t name;
    nxt_port_t *ctl_port;
    ...
    {
        nxt_alert(task, "process %PI cannot delete scripts", nxt_recv_msg_cmsg_pid(msg));
        return;
    }
