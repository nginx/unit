...
#define NXT_TIMER {NXT_RBTREE_NODE_INIT, 0, NXT_TIMER_NO_CHANGE, 0, 0, 0, NULL, NULL, NULL, NULL}
...
#define nxt_timer_data(obj, type, timer) nxt_container_of(obj, type, timer)
...
#define nxt_timer_is_in_tree(timer) ((timer)->node.parent != NULL)

#define nxt_timer_in_tree_set(timer)
/* Noop, because rbtree insertion sets a node's parent link. */

#define nxt_timer_in_tree_clear(timer) (timer)->node.parent = NULL
...
NXT_EXPORT void nxt_timer_add(nxt_event_engine_t *engine, nxt_timer_t *timer, nxt_msec_t timeout);
NXT_EXPORT nxt_bool_t nxt_timer_delete(nxt_event_engine_t *engine, nxt_timer_t *timer);

nxt_inline void nxt_timer_disable(nxt_event_engine_t *engine, nxt_timer_t *timer)
{
