...
nxt_sockaddr_t* nxt_sockaddr_alloc(nxt_mp_t* mp,
                                   socklen_t socklen,
...
nxt_sockaddr_t* nxt_sockaddr_create(nxt_mp_t* mp,
...
nxt_sockaddr_t* nxt_getsockname(nxt_task_t* task,
                                nxt_mp_t* mp,
                                nxt_socket_t s) {
  int ret;
  size_t length;
...
void nxt_sockaddr_text(nxt_sockaddr_t* sa) {
  ...
  switch (sa->u.sockaddr.sa_family) {
    case AF_INET:
      ...
      p = nxt_sprintf(start, end, "%ud.%ud.%ud.%ud", octet[0], octet[1],
                      octet[2], octet[3]);
...
nxt_int_t nxt_inet6_addr(struct in6_addr* in6_addr,
                         u_char* buf,
                         size_t length) {
