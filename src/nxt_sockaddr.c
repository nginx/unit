...
nxt_sockaddr_t *nxt_sockaddr_alloc(nxt_mp_t *mp, socklen_t socklen,
...
nxt_sockaddr_t *nxt_sockaddr_create(nxt_mp_t *mp, struct sockaddr *sockaddr,
...
nxt_sockaddr_t *nxt_getsockname(nxt_task_t *task, nxt_mp_t *mp,
                                nxt_socket_t s) {
  int ret;
  size_t length;
  socklen_t socklen;
...
void nxt_sockaddr_text(nxt_sockaddr_t *sa) {
  ...
  case AF_INET:
    ...
    p = nxt_sprintf(start, end, "%ud.%ud.%ud.%ud", octet[0], octet[1], octet[2],
                    octet[3]);
...
nxt_bool_t nxt_sockaddr_cmp(nxt_sockaddr_t *sa1, nxt_sockaddr_t *sa2) {
  ...
  case AF_INET6:
    ...
    if (memcmp(&sa1->u.sockaddr_in6.sin6_addr, &sa2->u.sockaddr_in6.sin6_addr,
               16) != 0) {
      return 0;
    }
  ...
  case AF_UNIX: {
    ...
    if (memcmp(&sa1->u.sockaddr_un.sun_path, &sa2->u.sockaddr_un.sun_path,
               length) != 0) {
      return 0;
    }
...
static u_char *nxt_inet6_ntop(u_char *addr, u_char *buf, u_char *end) {
  ...
  if (ipv6_bytes == 12) {
    p = nxt_sprintf(p, end, "%ud.%ud.%ud.%ud", addr[12], addr[13], addr[14],
                    addr[15]);
  }
...
nxt_sockaddr_t *nxt_sockaddr_parse(nxt_mp_t *mp, nxt_str_t *addr) {
  ...
  if (sa != NULL && sa->u.sockaddr.sa_family != AF_UNIX &&
    ...
    nxt_thread_log_error(NXT_LOG_ERR, "The address \"%V\" must specify a port.",
                         addr);
    return NULL;
  }
...
static nxt_sockaddr_t *nxt_sockaddr_inet_parse(nxt_mp_t *mp, nxt_str_t *addr) {
  ...
  sa =
      nxt_sockaddr_alloc(mp, sizeof(struct sockaddr_in), NXT_INET_ADDR_STR_LEN);
  if (nxt_slow_path(sa == NULL)) {
    return NULL;
  }
...
nxt_int_t nxt_inet6_addr(struct in6_addr *in6_addr, u_char *buf,
                         size_t length) {
