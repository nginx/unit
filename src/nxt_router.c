...
static nxt_int_t nxt_router_prefork(nxt_task_t* task,
...
static nxt_app_t* nxt_router_apps_hash_get(nxt_router_conf_t* rtcf,
                                           nxt_str_t* name);
static void nxt_router_apps_hash_use(nxt_task_t* task,
                                     nxt_router_conf_t* rtcf,
                                     int i);
...
static nxt_int_t nxt_router_listen_socket_find(nxt_router_temp_conf_t* tmcf,
                                               nxt_socket_conf_t* nskcf,
                                               nxt_sockaddr_t* sa);

static nxt_int_t nxt_router_engines_create(
    nxt_task_t* task,
    nxt_router_t* router,
    nxt_router_temp_conf_t* tmcf,
...
static nxt_int_t nxt_router_threads_create(nxt_task_t* task,
                                           nxt_runtime_t* rt,
                                           nxt_router_temp_conf_t* tmcf);
static nxt_int_t nxt_router_thread_create(nxt_task_t* task,
                                          nxt_runtime_t* rt,
                                          nxt_event_engine_t* engine);
static void nxt_router_apps_sort(nxt_task_t* task,
                                 nxt_router_t* router,
                                 nxt_router_temp_conf_t* tmcf);
...
static void nxt_router_listen_socket_create(nxt_task_t* task,
                                            void* obj,
                                            void* data);
static void nxt_router_listen_socket_update(nxt_task_t* task,
                                            void* obj,
                                            void* data);
static void nxt_router_listen_socket_delete(nxt_task_t* task,
                                            void* obj,
                                            void* data);
static void nxt_router_worker_thread_quit(nxt_task_t* task,
                                          void* obj,
                                          void* data);
static void nxt_router_listen_socket_close(nxt_task_t* task,
                                           void* obj,
                                           void* data);
static void nxt_router_thread_exit_handler(nxt_task_t* task,
                                           void* obj,
                                           void* data);
static void nxt_router_req_headers_ack_handler(
    nxt_task_t* task,
    nxt_port_recv_msg_t* msg,
...
static void nxt_router_app_port_release(nxt_task_t* task,
                                        nxt_app_t* app,
                                        nxt_port_t* port,
                                        nxt_apr_action_t action);
static void nxt_router_app_port_get(nxt_task_t* task,
                                    nxt_app_t* app,
                                    nxt_request_rpc_data_t* req_rpc_data);
static void nxt_router_http_request_error(nxt_task_t* task,
                                          void* obj,
                                          void* data);
static void nxt_router_http_request_done(nxt_task_t* task,
                                         void* obj,
                                         void* data);

static void nxt_router_app_prepare_request(
    nxt_task_t* task,
    nxt_request_rpc_data_t* req_rpc_data);
...
static void nxt_router_adjust_idle_timer(nxt_task_t* task,
                                         void* obj,
                                         void* data);
static void nxt_router_app_idle_timeout(nxt_task_t* task,
                                        void* obj,
                                        void* data);
static void nxt_router_app_joint_release_handler(nxt_task_t* task,
                                                 void* obj,
                                                 void* data);
...
static void nxt_router_http_request_release_post(nxt_task_t* task,
                                                 nxt_http_request_t* r);
static void nxt_router_http_request_release(nxt_task_t* task,
                                            void* obj,
                                            void* data);
...
static const nxt_str_t* nxt_app_msg_prefix[] = {
    [NXT_APP_EXTERNAL] = &empty_prefix, [NXT_APP_PYTHON] = &empty_prefix,
    [NXT_APP_PHP] = &http_prefix,       [NXT_APP_PERL] = &http_prefix,
    [NXT_APP_RUBY] = &http_prefix,      [NXT_APP_JAVA] = &empty_prefix,
...
static void nxt_router_greet_controller(nxt_task_t* task,
                                        nxt_port_t* controller_port) {
  nxt_port_socket_write(task, controller_port, NXT_PORT_MSG_PROCESS_READY, -1,
                        0, 0, NULL);
}
...
static void nxt_router_start_app_process_handler(nxt_task_t* task,
  ...
  if (dport != NULL) {
    ...
    if (app->proto_port_requests > 0) {
      nxt_debug(task, "app '%V' %p wait for prototype process", &app->name,
                app);
  ...
  app_joint_rpc = nxt_port_rpc_register_handler_ex(
      task, port, nxt_router_app_port_ready, nxt_router_app_port_error,
  ...
  ret = nxt_port_socket_write2(task, dport, NXT_PORT_MSG_START_PROCESS, port_fd,
                               queue_fd, stream, port->id, b);
  if (nxt_slow_path(ret != NXT_OK)) {
...
static nxt_int_t nxt_router_start_app_process(nxt_task_t* task,
                                              nxt_app_t* app) {
  nxt_int_t res;
...
nxt_router_msg_cancel(nxt_task_t* task, nxt_request_rpc_data_t* req_rpc_data) {
  ...
  if (app_port != NULL && app_port->id == NXT_SHARED_PORT_ID) {
    cancelled = nxt_app_queue_cancel(app_port->queue, msg_info->tracking_cookie,
                                     req_rpc_data->stream);
...
nxt_inline void nxt_request_rpc_data_unlink(
    nxt_task_t* task,
    nxt_request_rpc_data_t* req_rpc_data) {
  nxt_app_t* app;
  nxt_bool_t unlinked;
...
static void nxt_router_app_restart_handler(nxt_task_t* task,
  ...
  reply_port = nxt_runtime_port_find(task->thread->runtime, msg->port_msg.pid,
                                     msg->port_msg.reply_port);
  ...
  if (nxt_fast_path(app != NULL)) {
    shared_port =
        nxt_port_new(task, NXT_SHARED_PORT_ID, nxt_pid, NXT_PROCESS_APP);
    ...
    if (proto_port != NULL) {
      (void)nxt_port_socket_write(task, proto_port, NXT_PORT_MSG_QUIT, -1, 0, 0,
                                  NULL);
  ...
  nxt_port_socket_write(task, reply_port, reply, -1, msg->port_msg.stream, 0,
                        NULL);
}
...
static void nxt_router_app_process_remove_pid(nxt_task_t* task,
                                              nxt_port_t* port,
                                              void* data) {
  union {
...
static nxt_router_temp_conf_t* nxt_router_temp_conf(nxt_task_t* task) {
  ...
  tmcf->engines =
      nxt_array_create(tmcf->mem_pool, 4, sizeof(nxt_router_engine_conf_t));
...
void nxt_router_conf_error(nxt_task_t* task, nxt_router_temp_conf_t* tmcf) {
  ...
  for (qlk = nxt_queue_first(&creating_sockets);
       qlk != nxt_queue_tail(&creating_sockets); qlk = nxt_queue_next(qlk)) {
    skcf = nxt_queue_link_data(qlk, nxt_socket_conf_t, link);
    ...
    for (;;) {
      ...
      if (apcf.limits_value != NULL) {
        ...
        ret = nxt_conf_map_object(
            mp, apcf.limits_value, nxt_router_app_limits_conf,
      ...
      if (apcf.processes_value != NULL &&
        ...
        ret = nxt_conf_map_object(
            mp, apcf.processes_value, nxt_router_app_processes_conf,
      ...
      if (apcf.targets_value != NULL) {
        ...
        for (i = 0; i < n; i++) {
          (void)nxt_conf_next_object_member(apcf.targets_value, &target,
                                            &next_target);
      ...
      app->max_pending_processes =
          apcf.spare_processes ? apcf.spare_processes : 1;
  ...
  if (mtypes_conf != NULL) {
    ...
    for (;;) {
      ...
      if (nxt_conf_type(ext_conf) == NXT_CONF_STRING) {
        ...
        ret = nxt_http_static_mtypes_hash_add(mp, &rtcf->mtypes_hash, &exten,
                                              type);
        if (nxt_slow_path(ret != NXT_OK)) {
          return NXT_ERROR;
        }
...
static nxt_http_forward_t* nxt_router_conf_forward(nxt_task_t* task,
  ...
  if (client_ip_conf != NULL) {
    ret =
        nxt_router_conf_forward_header(mp, client_ip_conf, &forward->client_ip);
    if (nxt_slow_path(ret != NXT_OK)) {
      return NULL;
    }
  }
...
static nxt_int_t nxt_router_app_queue_init(nxt_task_t* task, nxt_port_t* port) {
  ...
  mem = nxt_mem_mmap(NULL, sizeof(nxt_app_queue_t), PROT_READ | PROT_WRITE,
                     MAP_SHARED, fd, 0);
...
static nxt_int_t nxt_router_port_queue_init(nxt_task_t* task,
  ...
  mem = nxt_mem_mmap(NULL, sizeof(nxt_port_queue_t), PROT_READ | PROT_WRITE,
                     MAP_SHARED, fd, 0);
  if (nxt_slow_path(mem == MAP_FAILED)) {
    nxt_fd_close(fd);
...
static nxt_int_t nxt_router_apps_hash_test(nxt_lvlhsh_query_t* lhq,
                                           void* data) {
  nxt_app_t* app;
...
nxt_int_t nxt_router_application_init(nxt_router_conf_t* rtcf,
                                      nxt_str_t* name,
                                      nxt_str_t* target,
...
static nxt_socket_conf_t* nxt_router_socket_conf(nxt_task_t* task,
  ...
  nxt_debug(task, "router listener: \"%*s\"", (size_t)sa->length,
...
static nxt_int_t nxt_router_listen_socket_find(nxt_router_temp_conf_t* tmcf,
  ...
  for (qlk = nxt_queue_first(&router->sockets);
       qlk != nxt_queue_tail(&router->sockets); qlk = nxt_queue_next(qlk)) {
    skcf = nxt_queue_link_data(qlk, nxt_socket_conf_t, link);
...
static void nxt_router_listen_socket_rpc_create(nxt_task_t* task,
  ...
  stream = nxt_port_rpc_register_handler(
      task, router_port, nxt_router_listen_socket_ready,
      nxt_router_listen_socket_error, main_port->pid, rpc);
  if (nxt_slow_path(stream == 0)) {
    goto fail;
  }

  ret = nxt_port_socket_write(task, main_port, NXT_PORT_MSG_SOCKET, -1, stream,
                              router_port->id, b);
...
static void nxt_router_app_rpc_create(nxt_task_t* task,
  ...
  rpc = nxt_port_rpc_register_handler_ex(
      task, router_port, nxt_router_app_prefork_ready,
  ...
  ret = nxt_port_socket_write2(task, dport, NXT_PORT_MSG_START_PROCESS, port_fd,
                               queue_fd, stream, router_port->id, b);
  if (nxt_slow_path(ret != NXT_OK)) {
    nxt_port_rpc_cancel(task, router_port, stream);
    goto fail;
  }
...
static void nxt_router_app_prefork_ready(nxt_task_t* task,
  ...
  nxt_debug(task, "app '%V' move new port %PI:%d to spare_ports", &app->name,
            port->pid, port->id);
  ...
  nxt_work_queue_add(&engine->fast_work_queue, nxt_router_conf_apply, task,
                     rpc->temp_conf, NULL);
}
...
static nxt_int_t nxt_router_engines_create(
  ...
  for (qlk = nxt_queue_first(&router->engines);
       qlk != nxt_queue_tail(&router->engines); qlk = nxt_queue_next(qlk)) {
...
static nxt_int_t nxt_router_engine_joints_create(nxt_router_temp_conf_t* tmcf,
  ...
  for (qlk = nxt_queue_first(sockets); qlk != nxt_queue_tail(sockets);
...
static void nxt_router_thread_start(void* data) {
  ...
  port =
      nxt_port_new(task, nxt_port_get_next_id(), nxt_pid, NXT_PROCESS_ROUTER);
  if (nxt_slow_path(port == NULL)) {
    return;
  }
...
nxt_inline nxt_listen_event_t* nxt_router_listen_event(
  ...
  for (qlk = nxt_queue_first(listen_connections);
       qlk != nxt_queue_tail(listen_connections); qlk = nxt_queue_next(qlk)) {
...
static void nxt_router_listen_socket_update(nxt_task_t* task,
  ...
  lev =
      nxt_router_listen_event(&engine->listen_connections, joint->socket_conf);
...
static void nxt_router_listen_socket_release(nxt_task_t* task,
  ...
  (void)nxt_port_socket_write(task, main_port, NXT_PORT_MSG_SOCKET_UNLINK, -1,
                              0, 0, b);
...
void nxt_router_listen_event_release(nxt_task_t* task,
                                     nxt_listen_event_t* lev,
                                     nxt_socket_conf_joint_t* joint) {
  nxt_event_engine_t* engine;
...
void nxt_router_conf_release(nxt_task_t* task, nxt_socket_conf_joint_t* joint) {
  ...
  nxt_debug(task, "conf skcf %p: %D, rtcf %p: %D", skcf, skcf->count, rtcf,
            rtcf->count);
...
static void nxt_router_response_ready_handler(nxt_task_t* task,
                                              nxt_port_recv_msg_t* msg,
                                              void* data) {
  size_t b_size, count;
  nxt_int_t ret;
  ...
  if (r->header_sent) {
    ...
    for (f = resp->fields; f < resp->fields + resp->fields_count; f++) {
      ...
      nxt_debug(task, "header%s: %*s: %*s", (field->skip ? " skipped" : ""),
...
static void nxt_router_req_headers_ack_handler(
  ...
  nxt_debug(task, "stream #%uD: got ack from %PI:%d", req_rpc_data->stream,
  ...
  if (nxt_queue_chk_remove(&main_app_port->idle_link)) {
    ...
    nxt_debug(task, "app '%V' move port %PI:%d out of %s (ack)", &app->name,
              main_app_port->pid, main_app_port->id,
  ...
  if (req_rpc_data->msg_info.body_fd != -1 || b != NULL) {
    ...
    res = nxt_port_socket_write(
        task, app_port, NXT_PORT_MSG_REQ_BODY, req_rpc_data->msg_info.body_fd,
...
static void nxt_http_request_send_body(nxt_task_t* task,
                                       void* obj,
                                       void* data) {
  nxt_buf_t* out;
  nxt_http_request_t* r;
...
static void nxt_router_response_error_handler(nxt_task_t* task,
                                              nxt_port_recv_msg_t* msg,
                                              void* data) {
  nxt_request_rpc_data_t* req_rpc_data;
...
static void nxt_router_app_port_ready(nxt_task_t* task,
  ...
  nxt_debug(task, "app '%V' new port ready, pid %PI, %d/%d", &app->name,
            port->pid, app->processes, app->pending_processes);
...
nxt_inline nxt_port_t* nxt_router_app_get_port_for_quit(nxt_task_t* task,
                                                        nxt_app_t* app) {
  nxt_port_t* port;
...
static void nxt_router_app_port_release(nxt_task_t* task,
  ...
  nxt_debug(task, "app '%V' release port %PI:%d: %d %d", &app->name, port->pid,
            port->id, (int)inc_use, (int)got_response);
  ...
  if (main_app_port->pair[1] != -1 && main_app_port->active_requests == 0 &&
    ...
    if (app->idle_processes < app->spare_processes) {
      ...
      nxt_debug(task, "app '%V' move port %PI:%d to spare_ports", &app->name,
                main_app_port->pid, main_app_port->id);
    } else {
      ...
      nxt_debug(task, "app '%V' move port %PI:%d to idle_ports", &app->name,
                main_app_port->pid, main_app_port->id);
    }
...
void nxt_router_app_port_close(nxt_task_t* task, nxt_port_t* port) {
  ...
  if (port->id != 0) {
    ...
    nxt_debug(task, "app '%V' port (%PI, %d) closed", &app->name, port->pid,
              port->id);
...
static void nxt_router_adjust_idle_timer(nxt_task_t* task,
  ...
  nxt_debug(task, "app '%V' idle_processes %d, spare_processes %d", &app->name,
  ...
  while (app->idle_processes > app->spare_processes) {
    ...
    nxt_debug(task, "app '%V' pid %PI, start %M, timeout %M, threshold %M",
              &app->name, port->pid, port->idle_start, timeout, threshold);
    ...
    nxt_debug(task, "app '%V' send QUIT to idle port %PI", &app->name,
              port->pid);
...
void nxt_router_process_http_request(nxt_task_t* task,
                                     nxt_http_request_t* r,
                                     nxt_http_action_t* action) {
  nxt_event_engine_t* engine;
  nxt_http_app_conf_t* conf;
  ...
  req_rpc_data = nxt_port_rpc_register_handler_ex(
      task, engine->port, nxt_router_response_ready_handler,
...
static void nxt_router_app_prepare_request(
  ...
  nxt_debug(task, "about to send %O bytes buffer to app process port %d",
            nxt_buf_used_size(buf), port->socket.fd);
  ...
  res = nxt_app_queue_send(port->queue, &msg, sizeof(msg), req_rpc_data->stream,
                           &notify, &req_rpc_data->msg_info.tracking_cookie);
  if (nxt_fast_path(res == NXT_OK)) {
    if (notify != 0) {
      (void)nxt_port_socket_write(task, port, NXT_PORT_MSG_READ_QUEUE, -1,
                                  req_rpc_data->stream, reply_port->id, NULL);
...
static nxt_http_field_t* nxt_fields_part_first(nxt_list_part_t* part,
                                               nxt_fields_iter_t* i) {
  if (part == NULL) {
    return NULL;
  }
...
static nxt_http_field_t* nxt_fields_first(nxt_list_t* fields,
                                          nxt_fields_iter_t* i) {
...
static nxt_buf_t* nxt_router_prepare_msg(nxt_task_t* task,
  ...
  req_size = sizeof(nxt_unit_request_t) + r->method->length + 1 +
             r->version.length + 1 + r->remote->address_length + 1 +
             r->local->address_length + 1 + nxt_sockaddr_port_length(r->local) +
             1 + r->server_name.length + 1 + r->target.length + 1 +
  ...
  nxt_list_each(field, r->fields) {
    ...
    req_size +=
        field->name_length + prefix->length + 1 + field->value_length + 1;
  }
  nxt_list_loop;
  ...
  *p++ = '\0';
  ...
  for (field = nxt_fields_first(r->fields, &iter); field != NULL;
    ...
    nxt_debug(task, "add field 0x%04Xd, %d, %d, %p : %d %p", (int)field->hash,
              (int)field->skip, (int)field->name_length, field->name,
    ...
    *p++ = '\0';
    ...
    if (prefix->length != 0) {
      ...
      for (dup = nxt_fields_next(&dup_iter); dup != NULL;
           dup = nxt_fields_next(&dup_iter)) {
        if (dup->name_length != field->name_length || dup->skip ||
            dup->hash != field->hash ||
    ...
    while (size > 0) {
      if (buf == NULL) {
        ...
        *tail = buf;
        ...
        if (free_size < size &&
            nxt_port_mmap_increase_buf(task, buf, size, 1) == NXT_OK) {
          free_size = nxt_buf_mem_free_size(&buf->mem);
        }
      }
...
static void nxt_router_oosm_handler(nxt_task_t* task,
                                    nxt_port_recv_msg_t* msg) {
  size_t mi;
  uint32_t i;
  nxt_bool_t ack;
...
static void nxt_router_get_mmap_handler(nxt_task_t* task,
  ...
  nxt_debug(task, "get mmap %PI:%d found", msg->port_msg.pid,
            (int)get_mmap_msg->id);
...
static void nxt_router_get_port_handler(nxt_task_t* task,
  ...
  reply_port =
      nxt_runtime_port_find(rt, msg->port_msg.pid, msg->port_msg.reply_port);
  if (nxt_slow_path(reply_port == NULL)) {
