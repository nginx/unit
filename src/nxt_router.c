...
nxt_router_engine_conf_delete (nxt_router_temp_conf_t *tmcf,
                               nxt_router_engine_conf_t *recf);
static nxt_int_t nxt_router_engine_joints_create (
...
nxt_router_msg_cancel (nxt_task_t *task, nxt_request_rpc_data_t *req_rpc_data)
{
  ...
  if (app_port != NULL && app_port->id == NXT_SHARED_PORT_ID)
    {
      cancelled = nxt_app_queue_cancel (
          app_port->queue, msg_info->tracking_cookie, req_rpc_data->stream);
  ...
  if (nxt_fast_path (app != NULL))
    {
      ...
      if (proto_port != NULL)
        {
          (void)nxt_port_socket_write (task, proto_port, NXT_PORT_MSG_QUIT, -1,
                                       0, 0, NULL);
  ...
  if (applications != NULL)
    {
      ...
      for (;;)
        {
          application
              = nxt_conf_next_object_member (applications, &name, &next);
          if (application == NULL)
            {
              break;
            }
          ...
          app->conf.start
              = nxt_pointer_to (app, sizeof (nxt_app_t) + name.length);
      ...
      for (;;)
        {
          ...
          if (websocket != NULL)
            {
              ret = nxt_conf_map_object (
                  mp, websocket, nxt_router_websocket_conf,
          ...
          if (certificate != NULL)
            {
              ...
              tls_init->conf_cmds
                  = nxt_conf_get_path (listener, &conf_commands_path);

              tls_init->tickets_conf
                  = nxt_conf_get_path (listener, &conf_tickets);
              ...
              for (i = 0; i < n; i++)
                {
                  value
                      = nxt_conf_get_array_element_or_itself (certificate, i);
          ...
          if (lscf.pass.length != 0)
            ...
            {
              skcf->action
                  = nxt_http_pass_application (task, rtcf, &lscf.application);
            }
...
nxt_router_conf_forward (nxt_task_t *task, nxt_mp_t *mp,
                         nxt_conf_value_t *conf)
{
  nxt_int_t ret;
...
static void
nxt_router_app_rpc_create (nxt_task_t *task, nxt_router_temp_conf_t *tmcf,
                           nxt_app_t *app)
{
  size_t size;
  uint32_t stream;
                                   ...
                                   void *data)
{
  ...
  // nxt_router_msg_cancel(task, &req_rpc_data->msg_info,
  // req_rpc_data->stream);
...
static void
nxt_router_app_port_release (nxt_task_t *task, nxt_app_t *app,
                             nxt_port_t *port, nxt_apr_action_t action)
{
  ...
  nxt_debug (task, "app '%V' release port %PI:%d: %d %d", &app->name,
             port->pid, port->id, (int)inc_use, (int)got_response);
...
nxt_router_adjust_idle_timer (nxt_task_t *task, void *obj, void *data)
{
  ...
  nxt_debug (task, "app '%V' idle_processes %d, spare_processes %d",
             &app->name, (int)app->idle_processes, (int)app->spare_processes);
      ...
      if (notify != 0)
        {
          (void)nxt_port_socket_write (task, port, NXT_PORT_MSG_READ_QUEUE, -1,
                                       req_rpc_data->stream, reply_port->id,
                                       NULL);
        }
      else
        {
                        ...
                        nxt_app_t *app, const nxt_str_t *prefix)
{
  ...
  req_size = sizeof (nxt_unit_request_t) + r->method->length + 1
             ...
             + nxt_sockaddr_port_length (r->local) + 1 + r->server_name.length
             + 1 + r->target.length + 1
  ...
  *p++ = '\0';
  ...
  p = nxt_cpymem (p, nxt_sockaddr_address (r->local),
                  r->local->address_length);
  *p++ = '\0';
  ...
  *p++ = '\0';
  ...
  if (r->args->start != NULL)
    {
      query_pos
          = nxt_pointer_to (target_pos, r->args->start - r->target.start);
  ...
  for (field = nxt_fields_first (r->fields, &iter); field != NULL;
       field = nxt_fields_next (&iter))
    {
      ...
      nxt_debug (task, "add field 0x%04Xd, %d, %d, %p : %d %p",
                 (int)field->hash, (int)field->skip, (int)field->name_length,
                 field->name, (int)field->value_length, field->value);
...
nxt_router_oosm_handler (nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
  ...
  process
      = nxt_runtime_process_find (task->thread->runtime, msg->port_msg.pid);
          ...
          if (m[mi] != 0)
            {
              ...
              nxt_debug (task, "oosm: already free #%uD %uz = 0x%08xA", i, mi,
                         m[mi]);
