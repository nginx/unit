...
nxt_router_app_prepare_request(nxt_task_t             *task,
                               nxt_request_rpc_data_t *req_rpc_data);
static nxt_buf_t *
nxt_router_prepare_msg(nxt_task_t *task, nxt_http_request_t *r, nxt_app_t *app,
                                  ...
                                  void *data)
{
    ...
    if (r->header_sent) {
        ...
        resp = (void *) b->mem.pos;
        count
            = (b_size - sizeof(nxt_unit_response_t)) / sizeof(nxt_unit_field_t);
...
nxt_router_adjust_idle_timer(nxt_task_t *task, void *obj, void *data)
{
    ...
    nxt_debug(task, "app '%V' idle_processes %d, spare_processes %d",
              &app->name, (int) app->idle_processes,
                               ...
                               nxt_request_rpc_data_t *req_rpc_data)
{
    ...
    if (nxt_fast_path(res == NXT_OK)) {
        if (notify != 0) {
            (void) nxt_port_socket_write(task, port, NXT_PORT_MSG_READ_QUEUE,
        ...
        if (prefix->length != 0) {
            ...
            for (dup = nxt_fields_next(&dup_iter); dup != NULL;
                ...
                if (dup->name_length != field->name_length || dup->skip
                    || dup->hash != field->hash
                    || nxt_memcasecmp(dup->name, field->name,
                                      dup->name_length)) {
                    continue;
                }
