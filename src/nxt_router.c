...
static nxt_int_t nxt_router_prefork(nxt_task_t *task, nxt_process_t *process, nxt_mp_t *mp);
static nxt_int_t nxt_router_start(nxt_task_t *task, nxt_process_data_t *data);
static void nxt_router_greet_controller(nxt_task_t *task, nxt_port_t *controller_port);

static nxt_int_t nxt_router_start_app_process(nxt_task_t *task, nxt_app_t *app);

static void nxt_router_new_port_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg);
static void nxt_router_conf_data_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg);
static void nxt_router_app_restart_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg);
static void nxt_router_status_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg);
static void nxt_router_remove_pid_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg);

static nxt_router_temp_conf_t *nxt_router_temp_conf(nxt_task_t *task);
static void nxt_router_conf_ready(nxt_task_t *task, nxt_router_temp_conf_t *tmcf);
static void nxt_router_conf_send(nxt_task_t *task, nxt_router_temp_conf_t *tmcf, nxt_port_msg_type_t type);

static nxt_int_t nxt_router_conf_create(nxt_task_t *task, nxt_router_temp_conf_t *tmcf, u_char *start, u_char *end);
static nxt_int_t nxt_router_conf_process_static(nxt_task_t *task, nxt_router_conf_t *rtcf, nxt_conf_value_t *conf);
static nxt_http_forward_t *nxt_router_conf_forward(nxt_task_t *task, nxt_mp_t *mp, nxt_conf_value_t *conf);
static nxt_int_t nxt_router_conf_forward_header(nxt_mp_t *mp, nxt_conf_value_t *conf, nxt_http_forward_header_t *fh);
...
static nxt_int_t nxt_router_apps_hash_add(nxt_router_conf_t *rtcf, nxt_app_t *app);
static nxt_app_t *nxt_router_apps_hash_get(nxt_router_conf_t *rtcf, nxt_str_t *name);
static void nxt_router_apps_hash_use(nxt_task_t *task, nxt_router_conf_t *rtcf, int i);

static nxt_int_t nxt_router_app_queue_init(nxt_task_t *task, nxt_port_t *port);
static nxt_int_t nxt_router_port_queue_init(nxt_task_t *task, nxt_port_t *port);
static nxt_int_t nxt_router_port_queue_map(nxt_task_t *task, nxt_port_t *port, nxt_fd_t fd);
static void nxt_router_listen_socket_rpc_create(nxt_task_t *task, nxt_router_temp_conf_t *tmcf,
                                                nxt_socket_conf_t *skcf);
static void nxt_router_listen_socket_ready(nxt_task_t *task, nxt_port_recv_msg_t *msg, void *data);
static void nxt_router_listen_socket_error(nxt_task_t *task, nxt_port_recv_msg_t *msg, void *data);
#if (NXT_TLS)
static void nxt_router_tls_rpc_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg, void *data);
static nxt_int_t nxt_router_conf_tls_insert(nxt_router_temp_conf_t *tmcf, nxt_conf_value_t *value,
                                            nxt_socket_conf_t *skcf, nxt_tls_init_t *tls_init, nxt_bool_t last);
#endif
#if (NXT_HAVE_NJS)
static void nxt_router_js_module_rpc_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg, void *data);
static nxt_int_t nxt_router_js_module_insert(nxt_router_temp_conf_t *tmcf, nxt_conf_value_t *value);
#endif
static void nxt_router_app_rpc_create(nxt_task_t *task, nxt_router_temp_conf_t *tmcf, nxt_app_t *app);
static void nxt_router_app_prefork_ready(nxt_task_t *task, nxt_port_recv_msg_t *msg, void *data);
static void nxt_router_app_prefork_error(nxt_task_t *task, nxt_port_recv_msg_t *msg, void *data);
static nxt_socket_conf_t *nxt_router_socket_conf(nxt_task_t *task, nxt_router_temp_conf_t *tmcf, nxt_str_t *name,
                                                 int backlog);
static nxt_int_t nxt_router_listen_socket_find(nxt_router_temp_conf_t *tmcf, nxt_socket_conf_t *nskcf,
                                               nxt_sockaddr_t *sa);

static nxt_int_t nxt_router_engines_create(nxt_task_t *task, nxt_router_t *router, nxt_router_temp_conf_t *tmcf,
                                           const nxt_event_interface_t *interface);
static nxt_int_t nxt_router_engine_conf_create(nxt_router_temp_conf_t *tmcf, nxt_router_engine_conf_t *recf);
static nxt_int_t nxt_router_engine_conf_update(nxt_router_temp_conf_t *tmcf, nxt_router_engine_conf_t *recf);
static nxt_int_t nxt_router_engine_conf_delete(nxt_router_temp_conf_t *tmcf, nxt_router_engine_conf_t *recf);
static nxt_int_t nxt_router_engine_joints_create(nxt_router_temp_conf_t *tmcf, nxt_router_engine_conf_t *recf,
                                                 nxt_queue_t *sockets, nxt_work_handler_t handler);
static nxt_int_t nxt_router_engine_quit(nxt_router_temp_conf_t *tmcf, nxt_router_engine_conf_t *recf);
static nxt_int_t nxt_router_engine_joints_delete(nxt_router_temp_conf_t *tmcf, nxt_router_engine_conf_t *recf,
                                                 nxt_queue_t *sockets);

static nxt_int_t nxt_router_threads_create(nxt_task_t *task, nxt_runtime_t *rt, nxt_router_temp_conf_t *tmcf);
static nxt_int_t nxt_router_thread_create(nxt_task_t *task, nxt_runtime_t *rt, nxt_event_engine_t *engine);
static void nxt_router_apps_sort(nxt_task_t *task, nxt_router_t *router, nxt_router_temp_conf_t *tmcf);

static void nxt_router_engines_post(nxt_router_t *router, nxt_router_temp_conf_t *tmcf);
static void nxt_router_engine_post(nxt_event_engine_t *engine, nxt_work_t *jobs);

static void nxt_router_thread_start(void *data);
static void nxt_router_rt_add_port(nxt_task_t *task, void *obj, void *data);
static void nxt_router_listen_socket_create(nxt_task_t *task, void *obj, void *data);
static void nxt_router_listen_socket_update(nxt_task_t *task, void *obj, void *data);
static void nxt_router_listen_socket_delete(nxt_task_t *task, void *obj, void *data);
static void nxt_router_worker_thread_quit(nxt_task_t *task, void *obj, void *data);
static void nxt_router_listen_socket_close(nxt_task_t *task, void *obj, void *data);
static void nxt_router_thread_exit_handler(nxt_task_t *task, void *obj, void *data);
static void nxt_router_req_headers_ack_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg,
                                               nxt_request_rpc_data_t *req_rpc_data);
static void nxt_router_listen_socket_release(nxt_task_t *task, nxt_socket_conf_t *skcf);

static void nxt_router_app_port_ready(nxt_task_t *task, nxt_port_recv_msg_t *msg, void *data);
static void nxt_router_app_port_error(nxt_task_t *task, nxt_port_recv_msg_t *msg, void *data);
...
static void nxt_router_app_port_release(nxt_task_t *task, nxt_app_t *app, nxt_port_t *port, nxt_apr_action_t action);
static void nxt_router_app_port_get(nxt_task_t *task, nxt_app_t *app, nxt_request_rpc_data_t *req_rpc_data);
static void nxt_router_http_request_error(nxt_task_t *task, void *obj, void *data);
static void nxt_router_http_request_done(nxt_task_t *task, void *obj, void *data);

static void nxt_router_app_prepare_request(nxt_task_t *task, nxt_request_rpc_data_t *req_rpc_data);
static nxt_buf_t *nxt_router_prepare_msg(nxt_task_t *task, nxt_http_request_t *r, nxt_app_t *app,
                                         const nxt_str_t *prefix);

static void nxt_router_app_timeout(nxt_task_t *task, void *obj, void *data);
static void nxt_router_adjust_idle_timer(nxt_task_t *task, void *obj, void *data);
static void nxt_router_app_idle_timeout(nxt_task_t *task, void *obj, void *data);
static void nxt_router_app_joint_release_handler(nxt_task_t *task, void *obj, void *data);
...
static void nxt_router_app_joint_use(nxt_task_t *task, nxt_app_joint_t *app_joint, int i);

static void nxt_router_http_request_release_post(nxt_task_t *task, nxt_http_request_t *r);
static void nxt_router_http_request_release(nxt_task_t *task, void *obj, void *data);
static void nxt_router_oosm_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg);
static void nxt_router_get_port_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg);
static void nxt_router_get_mmap_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg);
...
static const nxt_str_t *nxt_app_msg_prefix[] = {
    [NXT_APP_EXTERNAL] = &empty_prefix, [NXT_APP_PYTHON] = &empty_prefix,  [NXT_APP_PHP] = &http_prefix,
    [NXT_APP_PERL] = &http_prefix,      [NXT_APP_RUBY] = &http_prefix,     [NXT_APP_JAVA] = &empty_prefix,
    [NXT_APP_WASM] = &empty_prefix,     [NXT_APP_WASM_WC] = &empty_prefix,
};
...
static void nxt_router_greet_controller(nxt_task_t *task, nxt_port_t *controller_port)
{
    nxt_port_socket_write(task, controller_port, NXT_PORT_MSG_PROCESS_READY, -1, 0, 0, NULL);
}

static void nxt_router_start_app_process_handler(nxt_task_t *task, nxt_port_t *port, void *data)
{
    size_t size;
    uint32_t stream;
    ...
    else
    {
        ...
        {
            nxt_debug(task, "app '%V' %p wait for prototype process", &app->name, app);
    ...
    app_joint_rpc = nxt_port_rpc_register_handler_ex(task, port, nxt_router_app_port_ready, nxt_router_app_port_error,
    ...
    ret = nxt_port_socket_write2(task, dport, NXT_PORT_MSG_START_PROCESS, port_fd, queue_fd, stream, port->id, b);
    if (nxt_slow_path(ret != NXT_OK))
    {
...
static nxt_int_t nxt_router_start_app_process(nxt_task_t *task, nxt_app_t *app)
{
    ...
    res = nxt_port_post(task, router_port, nxt_router_start_app_process_handler, app);
...
nxt_inline nxt_bool_t nxt_router_msg_cancel(nxt_task_t *task, nxt_request_rpc_data_t *req_rpc_data)
{
    nxt_buf_t *b, *next;
    ...
    {
        cancelled = nxt_app_queue_cancel(app_port->queue, msg_info->tracking_cookie, req_rpc_data->stream);

        if (cancelled)
        {
            nxt_debug(task, "stream #%uD: cancelled by router", req_rpc_data->stream);
        }
    }
    else
    {
...
nxt_inline void nxt_request_rpc_data_unlink(nxt_task_t *task, nxt_request_rpc_data_t *req_rpc_data)
{
    ...
    {
        nxt_router_app_port_release(task, app, req_rpc_data->app_port, req_rpc_data->apr_action);
    ...
    if (req_rpc_data->rpc_cancel)
    {
        ...
        nxt_port_rpc_cancel(task, task->thread->engine->port, req_rpc_data->stream);
    }
}
...
static void nxt_router_conf_data_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
    ...
    port = nxt_runtime_port_find(task->thread->runtime, msg->port_msg.pid, msg->port_msg.reply_port);
    ...
    {
        nxt_alert(task, "conf_data_handler: unexpected buffer size (%d)", (int)nxt_buf_mem_used_size(&msg->buf->mem));
        goto fail;
    }
...
fail:
    ...
    nxt_port_socket_write(task, port, NXT_PORT_MSG_RPC_ERROR, -1, msg->port_msg.stream, 0, NULL);
...
static void nxt_router_app_restart_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
    ...
    reply_port = nxt_runtime_port_find(task->thread->runtime, msg->port_msg.pid, msg->port_msg.reply_port);
    ...
    if (nxt_fast_path(app != NULL))
    {
        shared_port = nxt_port_new(task, NXT_SHARED_PORT_ID, nxt_pid, NXT_PROCESS_APP);
        ...
        if (proto_port != NULL)
        {
            nxt_debug(task, "send QUIT to prototype '%V' pid %PI", &app->name, proto_port->pid);
        ...
        if (proto_port != NULL)
        {
            (void)nxt_port_socket_write(task, proto_port, NXT_PORT_MSG_QUIT, -1, 0, 0, NULL);
    ...
    nxt_port_socket_write(task, reply_port, reply, -1, msg->port_msg.stream, 0, NULL);
}
...
static void nxt_router_app_process_remove_pid(nxt_task_t *task, nxt_port_t *port, void *data)
{
    union {
...
static void nxt_router_remove_pid_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
    ...
    nxt_queue_each(engine, &nxt_router->engines, nxt_event_engine_t, link0)
    {
        ...
        {
            nxt_port_post(task, engine->port, nxt_router_app_process_remove_pid, msg->u.data);
        }
    }
    nxt_queue_loop;
...
static nxt_router_temp_conf_t *nxt_router_temp_conf(nxt_task_t *task)
{
    ...
    tmcf->engines = nxt_array_create(tmcf->mem_pool, 4, sizeof(nxt_router_engine_conf_t));
...
nxt_inline nxt_bool_t nxt_router_app_can_start(nxt_app_t *app)
{
...
{
    return (app->active_requests > app->port_hash_count + app->pending_processes) ||
           (app->spare_processes > app->idle_processes + app->pending_processes);
}
...
void nxt_router_conf_apply(nxt_task_t *task, void *obj, void *data)
{
    ...
    if (qlk != nxt_queue_head(&tmcf->tls))
    {
        ...
        nxt_cert_store_get(task, &tls->name, tmcf->mem_pool, nxt_router_tls_rpc_handler, tls);
        return;
    }
#endif
    ...
    if (qlk != nxt_queue_head(&tmcf->js_modules))
    {
        ...
        nxt_script_store_get(task, &js_module->name, tmcf->mem_pool, nxt_router_js_module_rpc_handler, js_module);
        return;
    }
#endif
...
static void nxt_router_conf_wait(nxt_task_t *task, void *obj, void *data)
{
    nxt_joint_job_t *job;
...
void nxt_router_conf_error(nxt_task_t *task, nxt_router_temp_conf_t *tmcf)
{
    ...
    for (qlk = nxt_queue_first(&creating_sockets); qlk != nxt_queue_tail(&creating_sockets); qlk = nxt_queue_next(qlk))
    {
        skcf = nxt_queue_link_data(qlk, nxt_socket_conf_t, link);
...
static nxt_int_t nxt_router_conf_create(nxt_task_t *task, nxt_router_temp_conf_t *tmcf, u_char *start, u_char *end)
{
    ...
    static const nxt_str_t conf_commands_path = nxt_string("/tls/conf_commands");
    static const nxt_str_t conf_cache_path = nxt_string("/tls/session/cache_size");
    static const nxt_str_t conf_timeout_path = nxt_string("/tls/session/timeout");
    ...
    static const nxt_str_t websocket_path = nxt_string("/settings/http/websocket");
    ...
    if (settings != NULL)
    {
        ret = nxt_conf_map_object(mp, settings, nxt_router_conf, nxt_nitems(nxt_router_conf), rtcf);
        if (ret != NXT_OK)
        {
    ...
    if (applications != NULL)
    {
        ...
        for (;;)
        {
            ...
            ret = nxt_conf_map_object(mp, application, nxt_router_app_conf, nxt_nitems(nxt_router_app_conf), &apcf);
            if (ret != NXT_OK)
            {
            ...
            if (apcf.limits_value != NULL)
            {
                ...
                ret = nxt_conf_map_object(mp, apcf.limits_value, nxt_router_app_limits_conf,
                                          nxt_nitems(nxt_router_app_limits_conf), &apcf);
                if (ret != NXT_OK)
                {
            ...
            if (apcf.processes_value != NULL && nxt_conf_type(apcf.processes_value) == NXT_CONF_OBJECT)
            {
                ret = nxt_conf_map_object(mp, apcf.processes_value, nxt_router_app_processes_conf,
                                          nxt_nitems(nxt_router_app_processes_conf), &apcf);
                if (ret != NXT_OK)
                {
            ...
            if (apcf.targets_value != NULL)
            {
                ...
                for (i = 0; i < n; i++)
                {
                    (void)nxt_conf_next_object_member(apcf.targets_value, &target, &next_target);
            ...
            app->max_pending_processes = apcf.spare_processes ? apcf.spare_processes : 1;
            ...
            port = nxt_port_new(task, NXT_SHARED_PORT_ID, nxt_pid, NXT_PROCESS_APP);
            if (nxt_slow_path(port == NULL))
            {
                return NXT_ERROR;
            }
    ...
    if (listeners != NULL)
    {
        ...
        for (;;)
        {
            ...
            ret = nxt_conf_map_object(mp, listener, nxt_router_listener_conf, nxt_nitems(nxt_router_listener_conf),
                                      &lscf);
            if (ret != NXT_OK)
            {
            ...
            if (http != NULL)
            {
                ret = nxt_conf_map_object(mp, http, nxt_router_http_conf, nxt_nitems(nxt_router_http_conf), skcf);
                if (ret != NXT_OK)
                {
            ...
            if (websocket != NULL)
            {
                ret = nxt_conf_map_object(mp, websocket, nxt_router_websocket_conf,
                                          nxt_nitems(nxt_router_websocket_conf), &skcf->websocket_conf);
                if (ret != NXT_OK)
                {
            ...
            if (certificate != NULL)
            {
                ...
                tls_init->conf_cmds = nxt_conf_get_path(listener, &conf_commands_path);

                tls_init->tickets_conf = nxt_conf_get_path(listener, &conf_tickets);

                n = nxt_conf_array_elements_count_or_1(certificate);

                for (i = 0; i < n; i++)
                {
                    value = nxt_conf_get_array_element_or_itself(certificate, i);
                    nxt_assert(value != NULL);

                    ret = nxt_router_conf_tls_insert(tmcf, value, skcf, tls_init, i == 0);
                    if (nxt_slow_path(ret != NXT_OK))
                    {
                        goto fail;
                    }
            ...
            {
                skcf->action = nxt_http_pass_application(task, rtcf, &lscf.application);
            }
...
static void nxt_router_js_module_rpc_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg, void *data)
{
    ...
    nxt_work_queue_add(&task->thread->engine->fast_work_queue, nxt_router_conf_apply, task, tmcf, NULL);
    return;
    ...
    if (mtypes_conf != NULL)
    {
        ...
        for (;;)
        {
            ...
            if (nxt_conf_type(ext_conf) == NXT_CONF_STRING)
            {
                ...
                ret = nxt_http_static_mtypes_hash_add(mp, &rtcf->mtypes_hash, &exten, type);
                if (nxt_slow_path(ret != NXT_OK))
                {
                    return NXT_ERROR;
                }
...
static nxt_http_forward_t *nxt_router_conf_forward(nxt_task_t *task, nxt_mp_t *mp, nxt_conf_value_t *conf)
{
    ...
    if (source_conf == NULL || (protocol_conf == NULL && client_ip_conf == NULL))
    {
        return NULL;
    }
    ...
    {
        ret = nxt_router_conf_forward_header(mp, client_ip_conf, &forward->client_ip);
        if (nxt_slow_path(ret != NXT_OK))
        {
            return NULL;
        }
    ...
    {
        ret = nxt_router_conf_forward_header(mp, protocol_conf, &forward->protocol);
        if (nxt_slow_path(ret != NXT_OK))
        {
            return NULL;
        }
...
static nxt_int_t nxt_router_app_queue_init(nxt_task_t *task, nxt_port_t *port)
{
    ...
    mem = nxt_mem_mmap(NULL, sizeof(nxt_app_queue_t), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
...
static nxt_int_t nxt_router_port_queue_init(nxt_task_t *task, nxt_port_t *port)
{
    ...
    mem = nxt_mem_mmap(NULL, sizeof(nxt_port_queue_t), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    if (nxt_slow_path(mem == MAP_FAILED))
    {
        nxt_fd_close(fd);
...
nxt_int_t nxt_router_application_init(nxt_router_conf_t *rtcf, nxt_str_t *name, nxt_str_t *target,
                                                 ...
                                                 int backlog)
{
    ...
    nxt_debug(task, "router listener: \"%*s\"", (size_t)sa->length, nxt_sockaddr_start(sa));
                                               ...
                                               nxt_sockaddr_t *sa)
{
    ...
    for (qlk = nxt_queue_first(&router->sockets); qlk != nxt_queue_tail(&router->sockets); qlk = nxt_queue_next(qlk))
    {
        skcf = nxt_queue_link_data(qlk, nxt_socket_conf_t, link);
...
static void nxt_router_listen_socket_rpc_create(nxt_task_t *task, nxt_router_temp_conf_t *tmcf, nxt_socket_conf_t *skcf)
{
    size_t size;
    uint32_t stream;
    nxt_int_t ret;
    ...
    stream = nxt_port_rpc_register_handler(task, router_port, nxt_router_listen_socket_ready,
                                           nxt_router_listen_socket_error, main_port->pid, rpc);
    if (nxt_slow_path(stream == 0))
    {
        goto fail;
    }

    ret = nxt_port_socket_write(task, main_port, NXT_PORT_MSG_SOCKET, -1, stream, router_port->id, b);
...
static void nxt_router_listen_socket_ready(nxt_task_t *task, nxt_port_recv_msg_t *msg, void *data)
{
    ...
    nxt_work_queue_add(&task->thread->engine->fast_work_queue, nxt_router_conf_apply, task, rpc->temp_conf, NULL);
...
static void nxt_router_app_rpc_create(nxt_task_t *task, nxt_router_temp_conf_t *tmcf, nxt_app_t *app)
{
    ...
    rpc = nxt_port_rpc_register_handler_ex(task, router_port, nxt_router_app_prefork_ready,
                                           nxt_router_app_prefork_error, sizeof(nxt_app_rpc_t));
    ...
    ret =
        nxt_port_socket_write2(task, dport, NXT_PORT_MSG_START_PROCESS, port_fd, queue_fd, stream, router_port->id, b);
    if (nxt_slow_path(ret != NXT_OK))
    {
        nxt_port_rpc_cancel(task, router_port, stream);
        goto fail;
...
static void nxt_router_app_prefork_ready(nxt_task_t *task, nxt_port_recv_msg_t *msg, void *data)
{
    ...
    nxt_debug(task, "app '%V' move new port %PI:%d to spare_ports", &app->name, port->pid, port->id);
    ...
    nxt_work_queue_add(&engine->fast_work_queue, nxt_router_conf_apply, task, rpc->temp_conf, NULL);
}
...
static void nxt_router_app_prefork_error(nxt_task_t *task, nxt_port_recv_msg_t *msg, void *data)
{
    ...
    if (rpc->proto)
    {
        nxt_log(task, NXT_LOG_WARN, "failed to start prototype \"%V\"", &app->name);
    }
    else
    {
        nxt_log(task, NXT_LOG_WARN, "failed to start application \"%V\"", &app->name);
                                           ...
                                           const nxt_event_interface_t *interface)
{
    ...
    tmcf->engines = nxt_array_create(tmcf->mem_pool, threads, sizeof(nxt_router_engine_conf_t));
    if (nxt_slow_path(tmcf->engines == NULL))
    {
        return NXT_ERROR;
    }
    ...
    for (qlk = nxt_queue_first(&router->engines); qlk != nxt_queue_tail(&router->engines); qlk = nxt_queue_next(qlk))
    {
...
static nxt_int_t nxt_router_engine_conf_create(nxt_router_temp_conf_t *tmcf, nxt_router_engine_conf_t *recf)
{
    ...
    ret = nxt_router_engine_joints_create(tmcf, recf, &creating_sockets, nxt_router_listen_socket_create);
    if (nxt_slow_path(ret != NXT_OK))
    {
        return ret;
    }

    ret = nxt_router_engine_joints_create(tmcf, recf, &updating_sockets, nxt_router_listen_socket_create);
    if (nxt_slow_path(ret != NXT_OK))
    {
        return ret;
    }
...
static nxt_int_t nxt_router_engine_conf_update(nxt_router_temp_conf_t *tmcf, nxt_router_engine_conf_t *recf)
{
    ...
    ret = nxt_router_engine_joints_create(tmcf, recf, &updating_sockets, nxt_router_listen_socket_update);
    if (nxt_slow_path(ret != NXT_OK))
    {
        return ret;
    }
                                                 ...
                                                 nxt_queue_t *sockets, nxt_work_handler_t handler)
{
    ...
    for (qlk = nxt_queue_first(sockets); qlk != nxt_queue_tail(sockets); qlk = nxt_queue_next(qlk))
    {
        ...
        joint = nxt_mp_alloc(tmcf->router_conf->mem_pool, sizeof(nxt_socket_conf_joint_t));
...
static void nxt_router_thread_start(void *data)
{
    ...
    port = nxt_port_new(task, nxt_port_get_next_id(), nxt_pid, NXT_PROCESS_ROUTER);
    if (nxt_slow_path(port == NULL))
    {
        return;
    }
...
nxt_inline nxt_listen_event_t *nxt_router_listen_event(nxt_queue_t *listen_connections, nxt_socket_conf_t *skcf)
{
    nxt_socket_t fd;
    nxt_queue_link_t *qlk;
    nxt_listen_event_t *lev;
    ...
    for (qlk = nxt_queue_first(listen_connections); qlk != nxt_queue_tail(listen_connections);
...
static void nxt_router_listen_socket_update(nxt_task_t *task, void *obj, void *data)
{
    ...
    lev = nxt_router_listen_event(&engine->listen_connections, joint->socket_conf);
...
static void nxt_router_listen_socket_delete(nxt_task_t *task, void *obj, void *data)
{
    ...
    nxt_debug(task, "engine %p: listen socket delete: %d", engine, lev->socket.fd);
...
static void nxt_router_listen_socket_close(nxt_task_t *task, void *obj, void *data)
{
    ...
    nxt_debug(task, "engine %p: listen socket close: %d", task->thread->engine, lev->socket.fd);
...
static void nxt_router_listen_socket_release(nxt_task_t *task, nxt_socket_conf_t *skcf)
{
    ...
    nxt_debug(task, "engine %p: listen socket release: ls->count %D", task->thread->engine, ls->count);
    ...
    (void)nxt_port_socket_write(task, main_port, NXT_PORT_MSG_SOCKET_UNLINK, -1, 0, 0, b);
...
void nxt_router_listen_event_release(nxt_task_t *task, nxt_listen_event_t *lev, nxt_socket_conf_joint_t *joint)
{
    nxt_event_engine_t *engine;
...
void nxt_router_conf_release(nxt_task_t *task, nxt_socket_conf_joint_t *joint)
{
    ...
    nxt_debug(task, "conf skcf %p: %D, rtcf %p: %D", skcf, skcf->count, rtcf, rtcf->count);
...
static void nxt_router_response_ready_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg, void *data)
{
    size_t b_size, count;
    nxt_int_t ret;
    nxt_app_t *app;
    ...
    else
    {
        ...
        resp = (void *)b->mem.pos;
        count = (b_size - sizeof(nxt_unit_response_t)) / sizeof(nxt_unit_field_t);

        if (nxt_slow_path(count < resp->fields_count))
        {
            nxt_alert(task, "response buffer too small for fields count: %D", resp->fields_count);
            goto fail;
        }
        ...
        for (f = resp->fields; f < resp->fields + resp->fields_count; f++)
        {
            ...
            nxt_debug(task, "header%s: %*s: %*s", (field->skip ? " skipped" : ""), (size_t)field->name_length,
                      field->name, (size_t)field->value_length, field->value);
        ...
        if (nxt_buf_mem_used_size(&b->mem) == 0)
        {
            ...
            nxt_work_queue_add(&task->thread->engine->fast_work_queue, b->completion_handler, task, b, b->parent);
                                               ...
                                               nxt_request_rpc_data_t *req_rpc_data)
{
    ...
    nxt_debug(task, "stream #%uD: got ack from %PI:%d", req_rpc_data->stream, msg->port_msg.pid,
              msg->port_msg.reply_port);

    nxt_port_rpc_ex_set_peer(task, msg->port, req_rpc_data, msg->port_msg.pid);
    ...
    app_port = nxt_port_hash_find(&app->port_hash, msg->port_msg.pid, msg->port_msg.reply_port);
    if (nxt_slow_path(app_port == NULL))
    {
        nxt_thread_mutex_unlock(&app->mutex);
    ...
    if (nxt_queue_chk_remove(&main_app_port->idle_link))
    {
        ...
        nxt_debug(task, "app '%V' move port %PI:%d out of %s (ack)", &app->name, main_app_port->pid, main_app_port->id,
                  (main_app_port->idle_start ? "idle_ports" : "spare_ports"));

        /* Check port was in 'spare_ports' using idle_start field. */
        if (main_app_port->idle_start == 0 && app->idle_processes >= app->spare_processes)
        {
            /*
            ...
            nxt_debug(task,
                      "app '%V' move port %PI:%d from idle_ports "
                      "to spare_ports",
    ...
    {
        nxt_debug(task, "stream #%uD: send body fd %d", req_rpc_data->stream, req_rpc_data->msg_info.body_fd);
        ...
        res = nxt_port_socket_write(task, app_port, NXT_PORT_MSG_REQ_BODY, req_rpc_data->msg_info.body_fd,
                                    req_rpc_data->stream, task->thread->engine->port->id, b);
...
static const nxt_http_request_state_t nxt_http_request_send_state nxt_aligned(64) = {
...
static void nxt_router_response_error_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg, void *data)
{
    nxt_request_rpc_data_t *req_rpc_data;
    ...
    {
        nxt_http_request_error(task, req_rpc_data->request, NXT_HTTP_SERVICE_UNAVAILABLE);
    }
...
static void nxt_router_app_port_ready(nxt_task_t *task, nxt_port_recv_msg_t *msg, void *data)
{
    ...
    if (nxt_slow_path(restarted))
    {
        ...
        start_process =
            !task->thread->engine->shutdown && nxt_router_app_can_start(app) && nxt_router_app_need_start(app);
    ...
    nxt_debug(task, "app '%V' new port ready, pid %PI, %d/%d", &app->name, port->pid, app->processes,
              app->pending_processes);
...
static void nxt_router_app_port_error(nxt_task_t *task, nxt_port_recv_msg_t *msg, void *data)
{
    ...
    while (link != NULL)
    {
        ...
        if (app->processes == 0 && app->pending_processes == 0 && !nxt_queue_is_empty(&app->ack_waiting_req))
        {
            link = nxt_queue_first(&app->ack_waiting_req);
...
nxt_inline nxt_port_t *nxt_router_app_get_port_for_quit(nxt_task_t *task, nxt_app_t *app)
{
    nxt_port_t *port;
    ...
    nxt_queue_each(port, &app->ports, nxt_port_t, app_link)
    {
        ...
        if (nxt_queue_chk_remove(&port->idle_link))
        {
            ...
            nxt_debug(task, "app '%V' move port %PI:%d out of %s for quit", &app->name, port->pid, port->id,
...
static void nxt_router_app_port_release(nxt_task_t *task, nxt_app_t *app, nxt_port_t *port, nxt_apr_action_t action)
{
    ...
    nxt_debug(task, "app '%V' release port %PI:%d: %d %d", &app->name, port->pid, port->id, (int)inc_use,
              (int)got_response);
    ...
    if (main_app_port->pair[1] != -1 && main_app_port->active_requests == 0 && main_app_port->active_websockets == 0 &&
        main_app_port->idle_link.next == NULL)
    {
        if (app->idle_processes == app->spare_processes && app->adjust_idle_work.data == NULL)
        {
            adjust_idle_timer = 1;
        ...
        if (app->idle_processes < app->spare_processes)
        {
            ...
            nxt_debug(task, "app '%V' move port %PI:%d to spare_ports", &app->name, main_app_port->pid,
                      main_app_port->id);
        }
        else
        {
            ...
            nxt_debug(task, "app '%V' move port %PI:%d to idle_ports", &app->name, main_app_port->pid,
                      main_app_port->id);
        }
    ...
    {
        nxt_debug(task, "app '%V' %p port %p already closed (pid %PI dead?)", &app->name, app, main_app_port,
                  main_app_port->pid);

        goto adjust_use;
    }

    nxt_debug(task, "app '%V' %p requests queue is empty, keep the port", &app->name, app);
...
void nxt_router_app_port_close(nxt_task_t *task, nxt_port_t *port)
{
    ...
    if (port == app->proto_port)
    {
        ...
        nxt_debug(task, "app '%V' prototype pid %PI closed", &app->name, port->pid);
    ...
    if (port->id != 0)
    {
        ...
        nxt_debug(task, "app '%V' port (%PI, %d) closed", &app->name, port->pid, port->id);
    ...
    if (nxt_queue_chk_remove(&port->idle_link))
    {
        ...
        nxt_debug(task, "app '%V' move port %PI:%d out of %s before close", &app->name, port->pid, port->id,
                  (port->idle_start ? "idle_ports" : "spare_ports"));
    ...
    start_process = !task->thread->engine->shutdown && nxt_router_app_can_start(app) && nxt_router_app_need_start(app);
...
static void nxt_router_adjust_idle_timer(nxt_task_t *task, void *obj, void *data)
{
    ...
    nxt_debug(task, "nxt_router_adjust_idle_timer: app \"%V\", queued %b", &app->name, queued);
    ...
    nxt_debug(task, "app '%V' idle_processes %d, spare_processes %d", &app->name, (int)app->idle_processes,
              (int)app->spare_processes);
    ...
    while (app->idle_processes > app->spare_processes)
    {
        ...
        nxt_debug(task, "app '%V' pid %PI, start %M, timeout %M, threshold %M", &app->name, port->pid, port->idle_start,
                  timeout, threshold);
        ...
        nxt_debug(task, "app '%V' move port %PI:%d out of idle_ports (timeout)", &app->name, port->pid, port->id);
        ...
        nxt_debug(task, "app '%V' send QUIT to idle port %PI", &app->name, port->pid);
...
static void nxt_router_free_app(nxt_task_t *task, void *obj, void *data)
{
    ...
    if (proto_port != NULL)
    {
        nxt_port_socket_write(task, proto_port, NXT_PORT_MSG_QUIT, -1, 0, 0, NULL);
...
void nxt_router_process_http_request(nxt_task_t *task, nxt_http_request_t *r, nxt_http_action_t *action)
{
    nxt_event_engine_t *engine;
    nxt_http_app_conf_t *conf;
    nxt_request_rpc_data_t *req_rpc_data;
    ...
    req_rpc_data = nxt_port_rpc_register_handler_ex(task, engine->port, nxt_router_response_ready_handler,
                                                    nxt_router_response_error_handler, sizeof(nxt_request_rpc_data_t));
...
static void nxt_router_app_prepare_request(nxt_task_t *task, nxt_request_rpc_data_t *req_rpc_data)
{
    ...
    buf = nxt_router_prepare_msg(task, req_rpc_data->request, app, nxt_app_msg_prefix[app->type]);
    if (nxt_slow_path(buf == NULL))
    {
        nxt_alert(task, "stream #%uD, app '%V': failed to prepare app message", req_rpc_data->stream, &app->name);

        nxt_http_request_error(task, req_rpc_data->request, NXT_HTTP_INTERNAL_SERVER_ERROR);

        return;
    }

    nxt_debug(task, "about to send %O bytes buffer to app process port %d", nxt_buf_used_size(buf), port->socket.fd);
    ...
    res = nxt_app_queue_send(port->queue, &msg, sizeof(msg), req_rpc_data->stream, &notify,
                             &req_rpc_data->msg_info.tracking_cookie);
    if (nxt_fast_path(res == NXT_OK))
    {
        if (notify != 0)
        {
            (void)nxt_port_socket_write(task, port, NXT_PORT_MSG_READ_QUEUE, -1, req_rpc_data->stream, reply_port->id,
                                        NULL);
        }
        else
        {
    ...
    {
        nxt_alert(task, "stream #%uD, app '%V': failed to send app message", req_rpc_data->stream, &app->name);
...
static nxt_http_field_t *nxt_fields_part_first(nxt_list_part_t *part, nxt_fields_iter_t *i)
{
    if (part == NULL)
    {
        return NULL;
...
static nxt_http_field_t *nxt_fields_first(nxt_list_t *fields, nxt_fields_iter_t *i)
{
    return nxt_fields_part_first(nxt_list_part(fields), i);
}

static nxt_http_field_t *nxt_fields_next(nxt_fields_iter_t *i)
{
                                         ...
                                         const nxt_str_t *prefix)
{
    ...
    req_size = sizeof(nxt_unit_request_t) + r->method->length + 1 + r->version.length + 1 + r->remote->address_length +
               1 + r->local->address_length + 1 + nxt_sockaddr_port_length(r->local) + 1 + r->server_name.length + 1 +
               r->target.length + 1 + (r->path->start != r->target.start ? r->path->length + 1 : 0);
    ...
    nxt_list_each(field, r->fields)
    {
        ...
        req_size += field->name_length + prefix->length + 1 + field->value_length + 1;
    }
    nxt_list_loop;
    ...
    {
        nxt_alert(task, "headers to big to fit in shared memory (%d)", (int)req_size);

        return NULL;
    }

    out = nxt_port_mmap_get_buf(task, &app->outgoing, nxt_min(req_size + content_length, PORT_MMAP_DATA_SIZE));
    if (nxt_slow_path(out == NULL))
    {
        return NULL;
    }
    ...
    *p++ = '\0';
    ...
    p = nxt_cpymem(p, nxt_sockaddr_address(r->remote), r->remote->address_length);
    *p++ = '\0';
    ...
    *p++ = '\0';
    ...
    p = nxt_cpymem(p, nxt_sockaddr_port(r->local), nxt_sockaddr_port_length(r->local));
    *p++ = '\0';
    ...
    *p++ = '\0';
    ...
    {
        query_pos = nxt_pointer_to(target_pos, r->args->start - r->target.start);
    ...
    for (field = nxt_fields_first(r->fields, &iter); field != NULL; field = nxt_fields_next(&iter))
    {
        if (field->skip)
        {
            continue;
        ...
        nxt_debug(task, "add field 0x%04Xd, %d, %d, %p : %d %p", (int)field->hash, (int)field->skip,
                  (int)field->name_length, field->name, (int)field->value_length, field->value);
        ...
        *p++ = '\0';
        ...
        if (prefix->length != 0)
        {
            ...
            for (dup = nxt_fields_next(&dup_iter); dup != NULL; dup = nxt_fields_next(&dup_iter))
            {
                if (dup->name_length != field->name_length || dup->skip || dup->hash != field->hash ||
    ...
    for (b = r->body; b != NULL; b = b->next)
    {
        ...
        while (size > 0)
        {
            ...
            else
            {
                ...
                if (free_size < size && nxt_port_mmap_increase_buf(task, buf, size, 1) == NXT_OK)
                {
                    free_size = nxt_buf_mem_free_size(&buf->mem);
                }
            }
...
static void nxt_router_oosm_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
    ...
    process = nxt_runtime_process_find(task->thread->runtime, msg->port_msg.pid);
    ...
    for (i = 0; i < process->incoming.size; i++)
    {
        ...
        for (mi = 0; mi < MAX_FREE_IDX; mi++)
        {
            if (m[mi] != 0)
            {
                ...
                nxt_debug(task, "oosm: already free #%uD %uz = 0x%08xA", i, mi, m[mi]);
...
static void nxt_router_get_mmap_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
    ...
    port = nxt_runtime_port_find(rt, msg->port_msg.pid, msg->port_msg.reply_port);
    if (nxt_slow_path(port == NULL))
    {
        nxt_alert(task, "get_mmap_handler: reply_port %PI:%d not found", msg->port_msg.pid, msg->port_msg.reply_port);

        return;
    }

    if (nxt_slow_path(nxt_buf_used_size(msg->buf) < (int)sizeof(nxt_port_msg_get_mmap_t)))
    {
        nxt_alert(task, "get_mmap_handler: message buffer too small (%d)", (int)nxt_buf_used_size(msg->buf));
    ...
    {
        nxt_alert(task, "get_mmap_handler: app == NULL for reply port %PI:%d", port->pid, port->id);
    ...
    if (nxt_slow_path(get_mmap_msg->id >= mmaps->size))
    {
        ...
        nxt_alert(task, "get_mmap_handler: mmap id is too big (%d)", (int)get_mmap_msg->id);
    ...
    nxt_debug(task, "get mmap %PI:%d found", msg->port_msg.pid, (int)get_mmap_msg->id);
...
static void nxt_router_get_port_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
    ...
    reply_port = nxt_runtime_port_find(rt, msg->port_msg.pid, msg->port_msg.reply_port);
    if (nxt_slow_path(reply_port == NULL))
    {
        nxt_alert(task, "get_port_handler: reply_port %PI:%d not found", msg->port_msg.pid, msg->port_msg.reply_port);

        return;
    }

    if (nxt_slow_path(nxt_buf_used_size(msg->buf) < (int)sizeof(nxt_port_msg_get_port_t)))
    {
        nxt_alert(task, "get_port_handler: message buffer too small (%d)", (int)nxt_buf_used_size(msg->buf));
    ...
    if (nxt_slow_path(port == NULL))
    {
        nxt_alert(task, "get_port_handler: port %PI:%d not found", get_port_msg->pid, get_port_msg->id);

        return;
    }

    nxt_debug(task, "get port %PI:%d found", get_port_msg->pid, get_port_msg->id);
