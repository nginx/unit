...
static void nxt_conn_proxy_client_buffer_alloc(nxt_task_t* task,
                                               void* obj,
                                               void* data);
static void nxt_conn_proxy_peer_connect(nxt_task_t* task,
                                        void* obj,
                                        void* data);
...
static void nxt_conn_proxy_client_read_ready(nxt_task_t* task,
                                             void* obj,
                                             void* data);
static void nxt_conn_proxy_peer_read_ready(nxt_task_t* task,
                                           void* obj,
                                           void* data);
static void nxt_conn_proxy_read_process(nxt_task_t* task,
                                        nxt_conn_proxy_t* p,
                                        nxt_conn_t* source,
                                        nxt_conn_t* sink);
...
static void nxt_conn_proxy_client_write_ready(nxt_task_t* task,
                                              void* obj,
                                              void* data);
static void nxt_conn_proxy_peer_write_ready(nxt_task_t* task,
                                            void* obj,
                                            void* data);
static void nxt_conn_proxy_write_process(nxt_task_t* task,
                                         nxt_conn_proxy_t* p,
                                         nxt_conn_t* sink,
                                         nxt_conn_t* source);
...
static void nxt_conn_proxy_read_timeout(nxt_task_t* task,
                                        void* obj,
                                        void* data);
static void nxt_conn_proxy_write_timeout(nxt_task_t* task,
                                         void* obj,
                                         void* data);
...
static void nxt_conn_proxy_reconnect_handler(nxt_task_t* task,
                                             void* obj,
                                             void* data);
static void nxt_conn_proxy_shutdown(nxt_task_t* task,
                                    nxt_conn_proxy_t* p,
                                    nxt_conn_t* source,
                                    nxt_conn_t* sink);
...
static void nxt_conn_proxy_read_process(nxt_task_t* task,
  ...
  if (sink->socket.error != 0) {
    nxt_debug(task, "conn proxy sink fd:%d error:%d", sink->socket.fd,
              sink->socket.error);
  ...
  while (source->read != NULL) {
    ...
    if (rb->mem.start != rb->mem.end) {
      nxt_work_queue_add(source->read_work_queue, nxt_conn_proxy_read, task,
  ...
  if (second == NULL) {
    ...
    if (second->mem.end != b->mem.start) {
      nxt_thread_log_alert(
          "event conn proxy write: second buffer end:%p "
...
static void nxt_conn_proxy_write_process(nxt_task_t* task,
  ...
  nxt_work_queue_add(source->read_work_queue, nxt_conn_proxy_read, task, source,
                     source->socket.data);
}
...
static void nxt_conn_proxy_read_add(nxt_conn_t* c, nxt_buf_t* b) {
  ...
  if (second == NULL) {
    ...
    if (second->mem.end != b->mem.start) {
      nxt_thread_log_alert(
          "event conn proxy read: second buffer end:%p "
          "is not equal to added buffer start:%p",
          second->mem.end, b->mem.start);
      return;
    }
...
static void nxt_conn_proxy_shutdown(nxt_task_t* task,
  ...
  nxt_debug(source->socket.task,
            "conn proxy shutdown source fd:%d cl:%d err:%d", source->socket.fd,
            source->socket.closed, source->socket.error);

  nxt_debug(sink->socket.task, "conn proxy shutdown sink fd:%d cl:%d err:%d",
...
static void nxt_conn_proxy_write_error(nxt_task_t* task,
                                       void* obj,
                                       void* data) {
  nxt_conn_t *source, *sink;
  nxt_conn_proxy_t* p;
...
static void nxt_conn_proxy_completion(nxt_task_t* task, void* obj, void* data) {
  ...
  nxt_debug(p->client->socket.task, "conn proxy completion %d:%d:%d", p->retain,
            p->client->socket.fd, p->peer->socket.fd);
