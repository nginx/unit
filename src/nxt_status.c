...
nxt_conf_value_t *nxt_status_get(nxt_status_report_t *report, nxt_mp_t *mp)
{
     ...
     * first entry.
     */
    for (i = 1, nr_langs = 0, prev_type = NXT_APP_UNKNOWN; i < langs->nelts; i++)
    {
        type = modules[i].type;
    ...
    nxt_conf_set_member_integer(obj, &active_str, report->accepted_conns - report->closed_conns - report->idle_conns,
                                1);
