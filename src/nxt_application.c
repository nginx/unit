...
static nxt_int_t nxt_discovery_start(nxt_task_t *task, nxt_process_data_t *data);
static nxt_buf_t *nxt_discovery_modules(nxt_task_t *task, const char *path);
static nxt_int_t nxt_discovery_module(nxt_task_t *task, nxt_mp_t *mp, nxt_array_t *modules, const char *name);
static void nxt_discovery_completion_handler(nxt_task_t *task, void *obj, void *data);
static void nxt_discovery_quit(nxt_task_t *task, nxt_port_recv_msg_t *msg, void *data);
static nxt_app_module_t *nxt_app_module_load(nxt_task_t *task, const char *name);
...
static void nxt_proto_start_process_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg);
static void nxt_proto_quit_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg);
static void nxt_proto_process_created_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg);
...
const nxt_sig_event_t nxt_prototype_signals[] = {
    nxt_event_signal(SIGHUP, nxt_proto_signal_handler),   nxt_event_signal(SIGINT, nxt_proto_sigterm_handler),
    nxt_event_signal(SIGQUIT, nxt_proto_sigterm_handler), nxt_event_signal(SIGTERM, nxt_proto_sigterm_handler),
    nxt_event_signal(SIGCHLD, nxt_proto_sigchld_handler), nxt_event_signal_end,
};
...
static nxt_int_t nxt_discovery_start(nxt_task_t *task, nxt_process_data_t *data)
{
    ...
    stream = nxt_port_rpc_register_handler(task, discovery_port, nxt_discovery_quit, nxt_discovery_quit, main_port->pid,
                                           NULL);
    ...
    ret = nxt_port_socket_write(task, main_port, NXT_PORT_MSG_MODULES, -1, stream, discovery_port->id, b);
...
static nxt_buf_t *nxt_discovery_modules(nxt_task_t *task, const char *path)
{
    ...
    if (ret != 0)
    {
        nxt_log(task, NXT_LOG_NOTICE, "no modules matching: \"%s\" found", path);
        n = 0;
    }
    ...
    for (i = 0; i < n; i++)
    {
        nxt_debug(task, "module: %d %V %V", module[i].type, &module[i].version, &module[i].file);
        ...
        size += NXT_INT_T_LEN + module[i].version.length + module[i].name.length + module[i].file.length;
        ...
        {
            size += nxt_strlen(mnt[j].src) + nxt_strlen(mnt[j].dst) + nxt_strlen(mnt[j].name) + (2 * NXT_INT_T_LEN) +
    ...
    *p++ = '[';
    ...
    for (i = 0; i < n; i++)
    {
        ...
        p = nxt_sprintf(p, end,
                        ...
                        module[i].type, &module[i].name, &module[i].version, &module[i].file);
        ...
        for (j = 0; j < mounts->nelts; j++)
        {
            p = nxt_sprintf(p, end,
                            ...
                            "\"data\": \"%s\"},",
                            mnt[j].src, mnt[j].dst, mnt[j].name, mnt[j].type, mnt[j].flags,
...
static nxt_int_t nxt_discovery_module(nxt_task_t *task, nxt_mp_t *mp, nxt_array_t *modules, const char *name)
{
    ...
    if (app != NULL)
    {
        nxt_log(task, NXT_LOG_NOTICE, "module: %V %s \"%s\"", &app->type, app->version, name);

        if (app->compat_length != sizeof(compat) || memcmp(app->compat, compat, sizeof(compat)) != 0)
        {
        ...
        for (i = 0; i < n; i++)
        {
            if (type == module[i].type && nxt_strstr_eq(&module[i].version, &version))
            {
        ...
        module->mounts = nxt_array_create(mp, app->nmounts, sizeof(nxt_fs_mount_t));
...
static nxt_int_t nxt_proto_setup(nxt_task_t *task, nxt_process_t *process)
{
    ...
    if (nxt_app == NULL)
    {
        nxt_debug(task, "application language module: %s \"%s\"", lang->version, lang->file);
    ...
    if (nxt_slow_path(nxt_app_set_environment(app_conf->environment) != NXT_OK))
    {
    ...
    if (app_conf->working_directory != NULL && app_conf->working_directory[0] != 0)
    {
        ret = chdir(app_conf->working_directory);

        if (nxt_slow_path(ret != 0))
        {
            nxt_log(task, NXT_LOG_WARN, "chdir(%s) failed %E", app_conf->working_directory, nxt_errno);
...
static void nxt_proto_start_process_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
    ...
    *init = nxt_app_process;
    ...
    process->name = nxt_mp_alloc(process->mem_pool, nxt_app_conf->name.length + sizeof("\"\" application") + 1);
...
static void nxt_proto_process_created_handler(nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
    ...
    if (process->pid != pid)
    {
        nxt_debug(task, "app process %PI (aka %PI) is created", isolated_pid, pid);
...
static void nxt_proto_sigterm_handler(nxt_task_t *task, void *obj, void *data)
{
    nxt_trace(task, "signal signo:%d (%s) received", (int)(uintptr_t)obj, data);
...
static void nxt_proto_sigchld_handler(nxt_task_t *task, void *obj, void *data)
{
    ...
    nxt_debug(task, "proto sigchld handler signo:%d (%s)", (int)(uintptr_t)obj, data);
    ...
    for (;;)
    {
        ...
        if (WTERMSIG(status))
        {
            ...
            {
                nxt_alert(task,
                          "app process %PI (isolated %PI) "
                          "exited on signal %d%s",
                          process != NULL ? process->pid : 0, pid, WTERMSIG(status),
                          NXT_WCOREDUMP(status) ? " (core dumped)" : "");
            }
            else
            {
                nxt_alert(task, "app process %PI exited on signal %d%s", pid, WTERMSIG(status),
                          NXT_WCOREDUMP(status) ? " (core dumped)" : "");
            }
        }
        else
        {
            if (rt->is_pid_isolated)
            {
                nxt_trace(task,
                          "app process %PI (isolated %PI) "
                          "exited with code %d",
                          process != NULL ? process->pid : 0, pid, WEXITSTATUS(status));
            }
            else
            {
                nxt_trace(task, "app process %PI exited with code %d", pid, WEXITSTATUS(status));
            }
        }
...
static nxt_app_module_t *nxt_app_module_load(nxt_task_t *task, const char *name)
{
    ...
    {
        err = dlerror();
        nxt_alert(task, "dlopen(\"%s\") failed: \"%s\"", name, err != NULL ? err : "(null)");
        return NULL;
    }
    ...
    {
        err = dlerror();
        nxt_alert(task, "dlsym(\"%s\", \"nxt_app_module\") failed: \"%s\"", name, err != NULL ? err : "(null)");

        if (dlclose(dl) != 0)
        {
            err = dlerror();
            nxt_alert(task, "dlclose(\"%s\") failed: \"%s\"", name, err != NULL ? err : "(null)");
        }
    }
...
nxt_int_t nxt_app_set_logs(void)
{
    nxt_int_t ret;
    nxt_file_t file;
    nxt_task_t *task;
...
nxt_app_lang_module_t *nxt_app_lang_module(nxt_runtime_t *rt, nxt_str_t *name)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (lang[i].type == type && nxt_strvers_match(lang[i].version, version, version_length))
        {
            return &lang[i];
        }
    }
...
nxt_int_t nxt_unit_default_init(nxt_task_t *task, nxt_unit_init_t *init, nxt_common_app_conf_t *conf)
{
...
static nxt_int_t nxt_proto_lvlhsh_isolated_pid_test(nxt_lvlhsh_query_t *lhq, void *data)
{
    nxt_pid_t *qpid;
    nxt_process_t *process;
...
nxt_inline void nxt_proto_process_lhq_pid(nxt_lvlhsh_query_t *lhq, nxt_pid_t *pid)
{
...
static void nxt_proto_process_add(nxt_task_t *task, nxt_process_t *process)
{
    ...
    default:
        nxt_alert(task, "process (isolated %PI) failed to add", process->isolated_pid);
        break;
    }
}
