...
static nxt_int_t nxt_discovery_module(nxt_task_t* task,
                                      nxt_mp_t* mp,
                                      nxt_array_t* modules,
                                      const char* name);
static void nxt_discovery_completion_handler(nxt_task_t* task,
                                             void* obj,
                                             void* data);
static void nxt_discovery_quit(nxt_task_t* task,
...
static nxt_int_t nxt_discovery_start(nxt_task_t* task,
  ...
  stream =
      nxt_port_rpc_register_handler(task, discovery_port, nxt_discovery_quit,
                                    nxt_discovery_quit, main_port->pid, NULL);

  if (nxt_slow_path(stream == 0)) {
    return NXT_ERROR;
  }

  ret = nxt_port_socket_write(task, main_port, NXT_PORT_MSG_MODULES, -1, stream,
                              discovery_port->id, b);
...
static nxt_buf_t* nxt_discovery_modules(nxt_task_t* task, const char* path) {
  ...
  for (i = 0; i < n; i++) {
    nxt_debug(task, "module: %d %V %V", module[i].type, &module[i].version,
              &module[i].file);
    ...
    size += NXT_INT_T_LEN + module[i].version.length + module[i].name.length +
            module[i].file.length;

    mounts = module[i].mounts;

    size += mounts->nelts * nxt_length(
                                "{\"src\": \"\", \"dst\": \"\", "
                                "\"type\": , \"name\": \"\", "
                                "\"flags\": , \"data\": \"\"},");
...
static nxt_int_t nxt_discovery_module(nxt_task_t* task,
  ...
  if (app != NULL) {
    nxt_log(task, NXT_LOG_NOTICE, "module: %V %s \"%s\"", &app->type,
            app->version, name);
...
static nxt_int_t nxt_proto_setup(nxt_task_t* task, nxt_process_t* process) {
  ...
  if (nxt_app == NULL) {
    nxt_debug(task, "application language module: %s \"%s\"", lang->version,
              lang->file);
  ...
  *init = nxt_app_process;
  ...
  process->name =
      nxt_mp_alloc(process->mem_pool,
                   nxt_app_conf->name.length + sizeof("\"\" application") + 1);
...
static void nxt_proto_sigchld_handler(nxt_task_t* task, void* obj, void* data) {
  ...
  nxt_debug(task, "proto sigchld handler signo:%d (%s)", (int)(uintptr_t)obj,
            data);
  ...
  for (;;) {
    ...
    if (WTERMSIG(status)) {
      if (rt->is_pid_isolated) {
        ...
        nxt_alert(task, "app process %PI exited on signal %d%s", pid,
                  WTERMSIG(status),
                  NXT_WCOREDUMP(status) ? " (core dumped)" : "");
      }
      ...
      if (rt->is_pid_isolated) {
        ...
        nxt_trace(task, "app process %PI exited with code %d", pid,
                  WEXITSTATUS(status));
      }
    }
...
static nxt_app_module_t* nxt_app_module_load(nxt_task_t* task,
  ...
  if (nxt_slow_path(dl == NULL)) {
    err = dlerror();
    nxt_alert(task, "dlopen(\"%s\") failed: \"%s\"", name,
              err != NULL ? err : "(null)");
    return NULL;
  }
  ...
  if (nxt_slow_path(app == NULL)) {
    err = dlerror();
    nxt_alert(task, "dlsym(\"%s\", \"nxt_app_module\") failed: \"%s\"", name,
              err != NULL ? err : "(null)");

    if (dlclose(dl) != 0) {
      err = dlerror();
      nxt_alert(task, "dlclose(\"%s\") failed: \"%s\"", name,
                err != NULL ? err : "(null)");
    }
  }
...
nxt_int_t nxt_unit_default_init(nxt_task_t* task,
...
static nxt_int_t nxt_proto_lvlhsh_isolated_pid_test(nxt_lvlhsh_query_t* lhq,
                                                    void* data) {
  nxt_pid_t* qpid;
  nxt_process_t* process;
...
nxt_inline void nxt_proto_process_lhq_pid(nxt_lvlhsh_query_t* lhq,
                                          nxt_pid_t* pid) {
...
static nxt_process_t* nxt_proto_process_remove(nxt_task_t* task,
                                               nxt_pid_t pid) {
  nxt_runtime_t* rt;
  nxt_process_t* process;
  nxt_lvlhsh_query_t lhq;
