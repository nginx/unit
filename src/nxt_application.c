...
static void nxt_proto_quit_handler (nxt_task_t *task,
                                    nxt_port_recv_msg_t *msg);
...
static void nxt_proto_sigterm_handler (nxt_task_t *task, void *obj,
                                       void *data);
static void nxt_proto_sigchld_handler (nxt_task_t *task, void *obj,
                                       void *data);
...
nxt_discovery_start (nxt_task_t *task, nxt_process_data_t *data)
{
  ...
  stream = nxt_port_rpc_register_handler (
      task, discovery_port, nxt_discovery_quit, nxt_discovery_quit,
      main_port->pid, NULL);
...
nxt_discovery_modules (nxt_task_t *task, const char *path)
{
  ...
  if (ret != 0)
    {
      nxt_log (task, NXT_LOG_NOTICE, "no modules matching: \"%s\" found",
               path);
      n = 0;
    }
  ...
  if (app != NULL)
    {
      ...
      module->mounts
          = nxt_array_create (mp, app->nmounts, sizeof (nxt_fs_mount_t));
...
nxt_proto_start_process_handler (nxt_task_t *task, nxt_port_recv_msg_t *msg)
{
  ...
  *init = nxt_app_process;
  ...
  process->name = nxt_mp_alloc (process->mem_pool,
                                nxt_app_conf->name.length
          ...
          if (rt->is_pid_isolated)
            {
              nxt_alert (task,
                         ...
                         process != NULL ? process->pid : 0, pid,
                         WTERMSIG (status),
