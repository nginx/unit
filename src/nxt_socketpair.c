...
nxt_int_t nxt_socketpair_create(nxt_task_t *task, nxt_socket_t *pair) {
  ...
  if (nxt_slow_path(setsockopt(pair[0], SOL_SOCKET, SO_PASSCRED, &enable_creds,
                               sizeof(enable_creds)) == -1)) {
    nxt_alert(task, "failed to set SO_PASSCRED %E", nxt_errno);
    goto fail;
  }

  if (nxt_slow_path(setsockopt(pair[1], SOL_SOCKET, SO_PASSCRED, &enable_creds,
                               sizeof(enable_creds)) == -1)) {
    nxt_alert(task, "failed to set SO_PASSCRED %E", nxt_errno);
    goto fail;
  }
...
ssize_t nxt_socketpair_send(nxt_fd_event_t *ev, nxt_fd_t *fd, nxt_iobuf_t *iob,
                            nxt_uint_t niob) {
  ssize_t n;
  nxt_err_t err;
  nxt_send_oob_t oob;
  ...
  for (;;) {
    ...
    nxt_debug(ev->task, "sendmsg(%d, %FD, %FD, %ui): %z", ev->fd, fd[0], fd[1],
              niob, n);
    ...
    default:
      nxt_alert(ev->task, "sendmsg(%d, %FD, %FD, %ui) failed %E", ev->fd, fd[0],
                fd[1], niob, err);
...
ssize_t nxt_socketpair_recv(nxt_fd_event_t *ev, nxt_iobuf_t *iob,
  ...
  for (;;) {
    ...
    nxt_debug(ev->task, "recvmsg(%d, %ui, %uz): %z", ev->fd, niob,
              ((nxt_recv_oob_t *)oob)->size, n);
