...
static nxt_http_action_t *nxt_upstream_handler(nxt_task_t *task, nxt_http_request_t *r, nxt_http_action_t *action);
...
nxt_int_t nxt_upstreams_create(nxt_task_t *task, nxt_router_temp_conf_t *tmcf, nxt_conf_value_t *conf)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        ret = nxt_upstream_round_robin_create(task, tmcf, upcf, &upstreams->upstream[i]);
