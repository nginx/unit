...
static void nxt_conn_write_timer_handler(nxt_task_t *task, void *obj, void *data);
...
void nxt_conn_io_write(nxt_task_t *task, void *obj, void *data)
{
    ...
    nxt_debug(task, "conn write fd:%d er:%d bl:%d", c->socket.fd, c->socket.error, c->block_write);
...
error:
    ...
    nxt_work_queue_add(c->write_work_queue, c->write_state->error_handler, task, c, data);
}
...
static ssize_t nxt_conn_io_sendfile(nxt_task_t *task, nxt_sendbuf_t *sb)
{
    ...
    for (;;)
    {
        ...
        nxt_debug(task, "sendfile(%FD, %d, @%O, %uz): %z", b->file->fd, sb->socket, b->file_pos, size, n);
        ...
        {
            nxt_alert(task, "sendfile() reported that file was truncated at %O", b->file_pos);
        ...
        default:
            sb->error = err;
            nxt_log(task, nxt_socket_error_level(err), "sendfile(%FD, %d, @%O, %uz) failed %E", b->file->fd, sb->socket,
                    b->file_pos, size, err);
...
static ssize_t nxt_sendfile(int fd, int s, off_t pos, size_t size)
{
    ...
    map = nxt_mem_mmap(NULL, size + page_off, PROT_READ, MAP_SHARED, fd, pos - page_off);
...
ssize_t nxt_conn_io_writev(nxt_task_t *task, nxt_sendbuf_t *sb, struct iovec *iov, nxt_uint_t niov)
{
    ssize_t n;
    nxt_err_t err;
    ...
    for (;;)
    {
        ...
        default:
            sb->error = err;
            nxt_log(task, nxt_socket_error_level(err), "writev(%d, %ui) failed %E", sb->socket, niov, err);
...
ssize_t nxt_conn_io_send(nxt_task_t *task, nxt_sendbuf_t *sb, void *buf, size_t size)
{
    ...
    for (;;)
    {
        ...
        default:
            sb->error = err;
            nxt_log(task, nxt_socket_error_level(err), "send(%d, %p, %uz) failed %E", sb->socket, buf, size, err);
...
size_t nxt_event_conn_write_limit(nxt_conn_t *c)
{
    ...
    nxt_debug(c->socket.task, "event conn correction:%z average:%0.3f", correction, rate->average);
...
ssize_t nxt_event_conn_io_sendbuf(nxt_conn_t *c, nxt_buf_t *b, size_t limit)
{
    nxt_uint_t niob;
...
ssize_t nxt_event_conn_io_writev(nxt_conn_t *c, nxt_iobuf_t *iob, nxt_uint_t niob)
{
    ...
    for (;;)
    {
        ...
        default:
            c->socket.error = err;
            nxt_log(c->socket.task, nxt_socket_error_level(err), "writev(%d, %ui) failed %E", c->socket.fd, niob, err);
            return NXT_ERROR;
        }
    }
}
...
ssize_t nxt_event_conn_io_send(nxt_conn_t *c, void *buf, size_t size)
{
    ...
    for (;;)
    {
        ...
        nxt_debug(c->socket.task, "send(%d, %p, %uz): %z", c->socket.fd, buf, size, n);
        ...
        default:
            c->socket.error = err;
            nxt_log(c->socket.task, nxt_socket_error_level(err), "send(%d, %p, %uz) failed %E", c->socket.fd, buf, size,
                    err);
            return NXT_ERROR;
        }
    }
