...
static void nxt_conn_write_timer_handler(nxt_task_t* task,
                                         void* obj,
                                         void* data);
...
void nxt_conn_io_write(nxt_task_t* task, void* obj, void* data) {
  ...
  nxt_debug(task, "conn write fd:%d er:%d bl:%d", c->socket.fd, c->socket.error,
            c->block_write);
...
static ssize_t nxt_conn_io_sendfile(nxt_task_t* task, nxt_sendbuf_t* sb) {
  ...
  for (;;) {
    ...
    nxt_debug(task, "sendfile(%FD, %d, @%O, %uz): %z", b->file->fd, sb->socket,
              b->file_pos, size, n);
...
ssize_t nxt_conn_io_writev(nxt_task_t* task,
  ...
  for (;;) {
    ...
    switch (err) {
      ...
      default:
        sb->error = err;
        nxt_log(task, nxt_socket_error_level(err), "writev(%d, %ui) failed %E",
                sb->socket, niov, err);
...
ssize_t nxt_event_conn_io_send(nxt_conn_t* c, void* buf, size_t size) {
  ...
  for (;;) {
    ...
    nxt_debug(c->socket.task, "send(%d, %p, %uz): %z", c->socket.fd, buf, size,
              n);
