...
static u_char *nxt_listen_socket_log_handler(void *ctx, u_char *pos, u_char *last);

nxt_int_t nxt_listen_socket(nxt_task_t *task, nxt_socket_t s, int backlog)
{
...
nxt_int_t nxt_listen_socket_create(nxt_task_t *task, nxt_mp_t *mp, nxt_listen_socket_t *ls)
{
    nxt_log_t log, *old;
    nxt_uint_t family;
    nxt_socket_t s;
    ...
    if (family == AF_INET6 && ls->ipv6only)
    {
        ...
        (void)nxt_socket_setsockopt(task, s, IPPROTO_IPV6, IPV6_V6ONLY, ipv6only);
    }
    ...
    if (family == AF_UNIX && sa->type == SOCK_STREAM && sa->u.sockaddr_un.sun_path[0] != '\0')
    {
        orig_sa = sa;
        ...
        p = nxt_cpystr((u_char *)sa->u.sockaddr_un.sun_path, (u_char *)orig_sa->u.sockaddr_un.sun_path);
        nxt_memcpy(p, ".tmp", 4);
    ...
    {
        nxt_alert(task, "listen(%d, %d) failed %E", s, ls->backlog, nxt_socket_errno);
        goto listen_fail;
    }
    ...
    if (orig_sa != NULL)
    {
        ...
        if (ret == 0)
        {
            nxt_alert(task, "connect(%d, %*s) socket already in use", ts, (size_t)orig_sa->length,
        ...
        {
            nxt_alert(task, "connect(%d, %*s) failed %E", ts, (size_t)orig_sa->length, nxt_sockaddr_start(orig_sa),
                      err);
...
nxt_int_t nxt_listen_socket_update(nxt_task_t *task, nxt_listen_socket_t *ls, nxt_listen_socket_t *prev)
{
    nxt_log_t log, *old;
    nxt_thread_t *thr;
    ...
    {
        nxt_alert(task, "listen(%d, %d) failed %E", ls->socket, ls->backlog, nxt_socket_errno);
        goto fail;
    }
...
void nxt_listen_socket_remote_size(nxt_listen_socket_t *ls)
{
...
size_t nxt_listen_socket_pool_min_size(nxt_listen_socket_t *ls)
{
    size_t size;
    ...
    case AF_INET6:
        ...
        size =
            offsetof(nxt_sockaddr_t, u) + sizeof(struct sockaddr_in6) + NXT_INET6_ADDR_STR_LEN + nxt_length(":65535");
    ...
    default:
        ...
        size = offsetof(nxt_sockaddr_t, u) + sizeof(struct sockaddr_in) + NXT_INET_ADDR_STR_LEN + nxt_length(":65535");
    ...
    if (ls->tls)
    {
        size += 4 * sizeof(void *)                            /* SSL/TLS connection */
                + sizeof(nxt_buf_mem_t) + sizeof(nxt_work_t); /* nxt_mp_cleanup */
    }
...
static u_char *nxt_listen_socket_log_handler(void *ctx, u_char *pos, u_char *end)
{
    nxt_sockaddr_t *sa;

    sa = ctx;

    return nxt_sprintf(pos, end, " while creating listening socket on %*s", (size_t)sa->length, nxt_sockaddr_start(sa));
}
