...
static u_char* nxt_listen_socket_log_handler(void* ctx,
                                             u_char* pos,
                                             u_char* last);
...
nxt_int_t nxt_listen_socket_create(nxt_task_t* task,
                                   nxt_mp_t* mp,
  ...
  if (orig_sa != NULL) {
    ...
    if (err != NXT_ENOENT && err != NXT_ECONNREFUSED) {
      nxt_alert(task, "connect(%d, %*s) failed %E", ts, (size_t)orig_sa->length,
...
nxt_int_t nxt_listen_socket_update(nxt_task_t* task,
                                   nxt_listen_socket_t* ls,
