...
#define NXT_CONF_VLDT_ANY_TYPE                                         \
  (NXT_CONF_VLDT_NULL | NXT_CONF_VLDT_BOOLEAN | NXT_CONF_VLDT_NUMBER | \
   NXT_CONF_VLDT_STRING | NXT_CONF_VLDT_ARRAY | NXT_CONF_VLDT_OBJECT)
...
static nxt_int_t nxt_conf_vldt_environment(nxt_conf_validation_t* vldt,
                                           nxt_str_t* name,
                                           nxt_conf_value_t* value);
static nxt_int_t nxt_conf_vldt_targets_exclusive(nxt_conf_validation_t* vldt,
                                                 nxt_conf_value_t* value,
                                                 void* data);
...
static nxt_conf_vldt_object_t nxt_conf_vldt_session_members[] = {
    ...
    },
    {
        ...
        .type =
            NXT_CONF_VLDT_STRING | NXT_CONF_VLDT_ARRAY | NXT_CONF_VLDT_BOOLEAN,
#if (NXT_HAVE_OPENSSL_TLSEXT)
...
static nxt_int_t nxt_conf_vldt_tls_cache_size(nxt_conf_validation_t* vldt,
  ...
  if (cache_size < 0) {
    return nxt_conf_vldt_error(vldt,
                               "The \"cache_size\" number must not "
                               "be negative.");
  }
...
static nxt_int_t nxt_conf_vldt_type(nxt_conf_validation_t* vldt,
  ...
  return nxt_conf_vldt_error(vldt, "The \"%V\" value must be %V, but not %V.",
...
static nxt_int_t nxt_conf_vldt_listener(nxt_conf_validation_t* vldt,
  ...
  if (nxt_slow_path(sa == NULL)) {
    return nxt_conf_vldt_error(vldt, "The listener address \"%V\" is invalid.",
                               name);
  }
...
static nxt_int_t nxt_conf_vldt_match_patterns(nxt_conf_validation_t* vldt,
  ...
  if (nxt_conf_type(value) == NXT_CONF_ARRAY) {
    ret =
        nxt_conf_vldt_array_iterator(vldt, value, &nxt_conf_vldt_match_pattern);
...
#else
    return nxt_conf_vldt_error(vldt,
                               "Unit is built without support of "
...
static nxt_int_t nxt_conf_vldt_match_encoded_patterns_sets(
    ...
    void* data) {
  if (nxt_conf_type(value) == NXT_CONF_ARRAY) {
    return nxt_conf_vldt_array_iterator(
        vldt, value, &nxt_conf_vldt_match_encoded_patterns_set);
  }
...
static nxt_int_t nxt_conf_vldt_match_encoded_patterns_set(
  ...
  if (nxt_conf_type(value) != NXT_CONF_OBJECT) {
    return nxt_conf_vldt_error(vldt,
                               "The \"match\" pattern for "
                               "\"arguments\" must be an object.");
  }

  return nxt_conf_vldt_object_iterator(
      vldt, value, &nxt_conf_vldt_match_encoded_patterns_set_member);
}
...
static nxt_int_t nxt_conf_vldt_match_encoded_patterns_set_member(
  ...
  if (nxt_slow_path(name->length == 0)) {
    return nxt_conf_vldt_error(vldt,
                               "The \"match\" pattern objects must "
...
static nxt_int_t nxt_conf_vldt_match_addr(nxt_conf_validation_t* vldt,
  ...
  switch (nxt_http_route_addr_pattern_parse(vldt->pool, &pattern, value)) {
    ...
    case NXT_ADDR_PATTERN_CIDR_ERROR:
      return nxt_conf_vldt_error(vldt,
                                 "The \"address\" has an invalid CIDR "
                                 "prefix.");

    case NXT_ADDR_PATTERN_NO_IPv6_ERROR:
      return nxt_conf_vldt_error(vldt,
                                 "The \"address\" does not support "
...
static nxt_int_t nxt_conf_vldt_match_patterns_set(nxt_conf_validation_t* vldt,
  ...
  return nxt_conf_vldt_object_iterator(
      vldt, value, &nxt_conf_vldt_match_patterns_set_member);
}
...
static nxt_int_t nxt_conf_vldt_forwarded(nxt_conf_validation_t* vldt,
  ...
  if (client_ip == NULL && protocol == NULL) {
    return nxt_conf_vldt_error(vldt,
                               "The \"forwarded\" object must have "
...
static nxt_int_t nxt_conf_vldt_app(nxt_conf_validation_t* vldt,
  ...
  if (type_value == NULL) {
    return nxt_conf_vldt_error(
        vldt, "Application must have the \"type\" property set.");
  }
...
static nxt_int_t nxt_conf_vldt_processes(nxt_conf_validation_t* vldt,
  ...
  ret =
      nxt_conf_map_object(vldt->pool, value, nxt_conf_vldt_processes_conf_map,
...
static nxt_int_t nxt_conf_vldt_cgroup_path(nxt_conf_validation_t* vldt,
  ...
  if (cgpath.length == 0 || strstr(path, "/../") != NULL) {
    return nxt_conf_vldt_error(vldt, "The cgroup path \"%V\" is invalid.",
                               &cgpath);
  }
...
static nxt_int_t nxt_conf_vldt_server_weight(nxt_conf_validation_t* vldt,
  ...
  if (num_value < 0) {
    return nxt_conf_vldt_error(vldt,
                               "The \"weight\" number must be "
                               "positive.");
  }

  if (num_value > 1000000) {
    return nxt_conf_vldt_error(vldt,
                               "The \"weight\" number must "
...
static nxt_int_t nxt_conf_vldt_access_log(nxt_conf_validation_t* vldt,
  ...
  ret = nxt_conf_map_object(vldt->pool, value, nxt_conf_vldt_access_log_map,
                            nxt_nitems(nxt_conf_vldt_access_log_map), &conf);
  if (ret != NXT_OK) {
    return ret;
  }
