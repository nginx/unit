...
static nxt_int_t
nxt_conf_vldt_tls_timeout (nxt_conf_validation_t *vldt,
                           nxt_conf_value_t *value, void *data)
{
  int64_t timeout;
                   ...
                   nxt_str_t *value)
{
  ...
  if (nxt_tstr_test (vldt->tstr_state, value, error) != NXT_OK)
    {
      return nxt_conf_vldt_error (vldt, "%s in the \"%V\" value.", error,
                                  name);
    }
  ...
  if (nxt_slow_path (sa == NULL))
    {
      return nxt_conf_vldt_error (
          vldt, "The listener address \"%V\" is invalid.", name);
    }
  ...
  if (nxt_conf_type (value) == NXT_CONF_ARRAY)
    {
      ret = nxt_conf_vldt_array_iterator (
          vldt, value, &nxt_conf_vldt_match_encoded_pattern);
    }
  else
    {
      /* NXT_CONF_STRING */
...
static nxt_int_t
nxt_conf_vldt_match_addr (nxt_conf_validation_t *vldt, nxt_conf_value_t *value)
{
...
static nxt_int_t
nxt_conf_vldt_certificate (nxt_conf_validation_t *vldt,
                           nxt_conf_value_t *value, void *data)
{
  if (nxt_conf_type (value) == NXT_CONF_ARRAY)
    {
                         ...
                         void *data)
{
  ...
  ret = nxt_conf_map_object (
      vldt->pool, value, nxt_conf_vldt_processes_conf_map,
...
static nxt_int_t
nxt_conf_vldt_cgroup_path (nxt_conf_validation_t *vldt,
                           nxt_conf_value_t *value, void *data)
{
...
static nxt_int_t
nxt_conf_vldt_clone_uidmap (nxt_conf_validation_t *vldt,
                            nxt_conf_value_t *value)
{
  nxt_int_t ret;
...
static nxt_int_t
nxt_conf_vldt_clone_gidmap (nxt_conf_validation_t *vldt,
                            nxt_conf_value_t *value)
{
  nxt_int_t ret;
...
static nxt_int_t
nxt_conf_vldt_java_option (nxt_conf_validation_t *vldt,
                           nxt_conf_value_t *value)
{
  nxt_str_t str;
