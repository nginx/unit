...
static void nxt_h1p_conn_ws_frame_header_read(nxt_task_t *task, void *obj, void *data);
static void nxt_h1p_conn_ws_keepalive_disable(nxt_task_t *task, nxt_h1proto_t *h1p);
static void nxt_h1p_conn_ws_keepalive_enable(nxt_task_t *task, nxt_h1proto_t *h1p);
static void nxt_h1p_conn_ws_frame_process(nxt_task_t *task, nxt_conn_t *c, nxt_h1proto_t *h1p,
...
static void nxt_h1p_conn_ws_frame_payload_read(nxt_task_t *task, void *obj, void *data);
static void hxt_h1p_send_ws_error(nxt_task_t *task, nxt_http_request_t *r, const nxt_ws_error_t *err, ...);
...
static const nxt_ws_error_t nxt_ws_err_out_of_memory = {NXT_WEBSOCKET_CR_INTERNAL_SERVER_ERROR, 0,
                                                        nxt_string("Out of memory")};
static const nxt_ws_error_t nxt_ws_err_too_big = {NXT_WEBSOCKET_CR_MESSAGE_TOO_BIG, 1,
                                                  nxt_string("Message too big: %uL bytes")};
static const nxt_ws_error_t nxt_ws_err_invalid_close_code = {NXT_WEBSOCKET_CR_PROTOCOL_ERROR, 1,
                                                             nxt_string("Close code %ud is not valid")};
static const nxt_ws_error_t nxt_ws_err_going_away = {NXT_WEBSOCKET_CR_GOING_AWAY, 0,
                                                     nxt_string("Remote peer is going away")};
static const nxt_ws_error_t nxt_ws_err_not_masked = {NXT_WEBSOCKET_CR_PROTOCOL_ERROR, 0,
                                                     nxt_string("Not masked client frame")};
static const nxt_ws_error_t nxt_ws_err_ctrl_fragmented = {NXT_WEBSOCKET_CR_PROTOCOL_ERROR, 0,
                                                          nxt_string("Fragmented control frame")};
static const nxt_ws_error_t nxt_ws_err_ctrl_too_big = {NXT_WEBSOCKET_CR_PROTOCOL_ERROR, 1,
                                                       nxt_string("Control frame too big: %uL bytes")};
static const nxt_ws_error_t nxt_ws_err_invalid_close_len = {NXT_WEBSOCKET_CR_PROTOCOL_ERROR, 0,
                                                            nxt_string("Close frame payload length cannot be 1")};
static const nxt_ws_error_t nxt_ws_err_invalid_opcode = {NXT_WEBSOCKET_CR_PROTOCOL_ERROR, 1,
                                                         nxt_string("Unrecognized opcode %ud")};
static const nxt_ws_error_t nxt_ws_err_cont_expected = {NXT_WEBSOCKET_CR_PROTOCOL_ERROR, 1,
                                                        nxt_string("Continuation expected, but %ud opcode received")};
...
void nxt_h1p_websocket_first_frame_start(nxt_task_t *task, nxt_http_request_t *r, nxt_buf_t *ws_frame)
{
    ...
    {
        h1p->websocket_timer = nxt_mp_zget(c->mem_pool, sizeof(nxt_h1p_websocket_timer_t));
...
static const nxt_conn_state_t nxt_h1p_read_ws_frame_header_state nxt_aligned(64) = {
...
static void nxt_h1p_conn_ws_frame_header_read(nxt_task_t *task, void *obj, void *data)
{
    ...
    if ((wsh->opcode & NXT_WEBSOCKET_OP_CTRL) != 0)
    {
        ...
        if (nxt_slow_path(wsh->opcode != NXT_WEBSOCKET_OP_PING && wsh->opcode != NXT_WEBSOCKET_OP_PONG &&
                          wsh->opcode != NXT_WEBSOCKET_OP_CLOSE))
        {
            hxt_h1p_send_ws_error(task, r, &nxt_ws_err_invalid_opcode, wsh->opcode);
            return;
        }

        if (nxt_slow_path(wsh->payload_len > 125))
        {
            hxt_h1p_send_ws_error(task, r, &nxt_ws_err_ctrl_too_big, nxt_websocket_frame_payload_len(wsh));
            return;
        }

        if (nxt_slow_path(wsh->opcode == NXT_WEBSOCKET_OP_CLOSE && wsh->payload_len == 1))
        {
    ...
    else
    {
        if (h1p->websocket_cont_expected)
        {
            ...
            {
                hxt_h1p_send_ws_error(task, r, &nxt_ws_err_cont_expected, wsh->opcode);
                return;
            }
        }
        else
        {
            if (nxt_slow_path(wsh->opcode != NXT_WEBSOCKET_OP_BINARY && wsh->opcode != NXT_WEBSOCKET_OP_TEXT))
            {
                hxt_h1p_send_ws_error(task, r, &nxt_ws_err_invalid_opcode, wsh->opcode);
                return;
            }
    ...
    if (nxt_slow_path(hsize > max_frame_size || payload_len > (max_frame_size - hsize)))
    {
...
static void nxt_h1p_conn_ws_keepalive_enable(nxt_task_t *task, nxt_h1proto_t *h1p)
{
    ...
    nxt_timer_add(task->thread->engine, timer, h1p->websocket_timer->keepalive_interval);
}
                                          ...
                                          nxt_websocket_header_t *wsh)
{
    ...
    if (nxt_slow_path(wsh->opcode == NXT_WEBSOCKET_OP_CLOSE))
    {
        if (wsh->payload_len >= 2)
        {
            ...
            if (nxt_slow_path(code < 1000 || code >= 5000 || (code > 1003 && code < 1007) ||
                              (code > 1014 && code < 3000)))
            {
                hxt_h1p_send_ws_error(task, r, &nxt_ws_err_invalid_close_code, code);
                return;
            }
        }
...
static const nxt_conn_state_t nxt_h1p_read_ws_frame_payload_state nxt_aligned(64) = {
...
static void hxt_h1p_send_ws_error(nxt_task_t *task, nxt_http_request_t *r, const nxt_ws_error_t *err, ...)
{
    ...
    {
        va_start(args, err);
        p = nxt_vsprintf(buf, buf + sizeof(buf), (char *)err->desc.start, args);
        va_end(args);
