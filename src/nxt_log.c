...
nxt_str_t nxt_log_levels[6] = {nxt_string("alert"), nxt_string("error"),
                               nxt_string("warn"),  nxt_string("notice"),
                               nxt_string("info"),  nxt_string("debug")};
...
/* STUB */
nxt_log_t *nxt_log_set_ctx(nxt_log_t *log, nxt_log_ctx_handler_t handler,
                           void *ctx) {
  nxt_log_t *old;
  nxt_thread_t *thr;
...
void nxt_cdecl nxt_log_handler(nxt_uint_t level, nxt_log_t *log,
                               const char *fmt, ...) {
  u_char *p, *end;
#if 0
    u_char   *syslogmsg;
