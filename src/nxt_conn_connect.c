...
void nxt_conn_sys_socket(nxt_task_t *task, void *obj, void *data)
{
    ...
    nxt_work_queue_add(&task->thread->engine->connect_work_queue, handler, task, c, data);
}
...
static nxt_err_t nxt_conn_connect_test_error(nxt_task_t *task, nxt_conn_t *c)
{
    ...
    if (err != 0)
    {
        ...
        nxt_log(task, nxt_socket_error_level(err), "connect(%d, %*s) failed %E", c->socket.fd,
                (size_t)c->remote->length, nxt_sockaddr_start(c->remote), err);
    }
