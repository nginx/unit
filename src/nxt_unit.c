...
nxt_unit_remove_port_unsafe (nxt_unit_impl_t *lib,
                             nxt_unit_port_id_t *port_id);
...
static int nxt_unit_get_port (nxt_unit_ctx_t *ctx,
                              nxt_unit_port_id_t *port_id);
...
static int
nxt_unit_process_req_headers (nxt_unit_ctx_t *ctx,
                              nxt_unit_recv_msg_t *recv_msg,
                              nxt_unit_request_info_t **preq)
{
  int res;
  ...
  if (nxt_queue_is_empty (&ctx_impl->free_ws))
    {
      ...
      ws_impl
          = nxt_unit_malloc (ctx, sizeof (nxt_unit_websocket_frame_impl_t));
      if (nxt_slow_path (ws_impl == NULL))
        {
          return NULL;
        }
      ...
      if (notify)
        {
          ...
          if (lib->callbacks.port_send == NULL)
            ...
            {
              ret = lib->callbacks.port_send (
                  ctx, port, &msg, sizeof (nxt_port_msg_t), NULL, 0);
...
static ssize_t
nxt_unit_sendmsg (nxt_unit_ctx_t *ctx, int fd, const void *buf,
                  size_t buf_size, const nxt_send_oob_t *oob)
{
  int err;
  ssize_t n;
...
nxt_unit_lvlhsh_alloc (void *data, size_t size)
{
  ...
  if (nxt_fast_path (err == 0))
    {
      nxt_unit_debug (NULL, "posix_memalign(%d, %d): %p", (int)size, (int)size,
                      p);
      return p;
    }
