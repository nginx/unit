...
static int nxt_unit_ready(nxt_unit_ctx_t* ctx,
                          int ready_fd,
                          uint32_t stream,
                          int queue_fd);
static int nxt_unit_process_msg(nxt_unit_ctx_t* ctx,
...
static nxt_unit_mmap_buf_t* nxt_unit_request_preread(
    ...
    size_t size);
static ssize_t nxt_unit_buf_read(nxt_unit_buf_t** b,
                                 uint64_t* len,
                                 void* dst,
                                 size_t size);
...
static int nxt_unit_send_mmap(nxt_unit_ctx_t* ctx,
                              nxt_unit_port_t* port,
                              int fd);
...
static int nxt_unit_send_port(nxt_unit_ctx_t* ctx,
                              nxt_unit_port_t* dst,
                              nxt_unit_port_t* port,
                              int queue_fd);
...
static ssize_t nxt_unit_sendmsg(nxt_unit_ctx_t* ctx,
                                int fd,
                                const void* buf,
                                size_t buf_size,
                                const nxt_send_oob_t* oob);
static int nxt_unit_ctx_port_recv(nxt_unit_ctx_t* ctx,
                                  nxt_unit_port_t* port,
                                  nxt_unit_read_buf_t* rbuf);
nxt_inline void nxt_unit_rbuf_cpy(nxt_unit_read_buf_t* dst,
                                  nxt_unit_read_buf_t* src);
static int nxt_unit_shared_port_recv(nxt_unit_ctx_t* ctx,
                                     nxt_unit_port_t* port,
                                     nxt_unit_read_buf_t* rbuf);
static int nxt_unit_port_recv(nxt_unit_ctx_t* ctx,
                              nxt_unit_port_t* port,
                              nxt_unit_read_buf_t* rbuf);
static int nxt_unit_port_queue_recv(nxt_unit_port_t* port,
                                    nxt_unit_read_buf_t* rbuf);
static int nxt_unit_app_queue_recv(nxt_unit_ctx_t* ctx,
                                   nxt_unit_port_t* port,
                                   nxt_unit_read_buf_t* rbuf);
...
static int nxt_unit_request_hash_add(nxt_unit_ctx_t* ctx,
                                     nxt_unit_request_info_t* req);
static nxt_unit_request_info_t* nxt_unit_request_hash_find(nxt_unit_ctx_t* ctx,
                                                           uint32_t stream,
                                                           int remove);
...
nxt_unit_ctx_t* nxt_unit_init(nxt_unit_init_t* init) {
  ...
  if (init->ready_port.id.pid != 0 && init->ready_stream != 0 &&
    ...
    rc = nxt_unit_read_env(&ready_port, &router_port, &read_port,
                           &shared_port.in_fd, &shared_queue_fd, &lib->log_fd,
                           &ready_stream, &shm_limit, &request_limit);
  ...
  mem = mmap(NULL, sizeof(nxt_port_queue_t), PROT_READ | PROT_WRITE, MAP_SHARED,
             queue_fd, 0);
  if (nxt_slow_path(mem == MAP_FAILED)) {
    nxt_unit_alert(ctx, "mmap(%d) failed: %s (%d)", queue_fd, strerror(errno),
                   errno);
  ...
  mem = mmap(NULL, sizeof(nxt_app_queue_t), PROT_READ | PROT_WRITE, MAP_SHARED,
             shared_queue_fd, 0);
  if (nxt_slow_path(mem == MAP_FAILED)) {
...
static nxt_unit_impl_t* nxt_unit_create(nxt_unit_init_t* init) {
  ...
  lib =
      nxt_unit_malloc(NULL, sizeof(nxt_unit_impl_t) + init->request_data_size);
  if (nxt_slow_path(lib == NULL)) {
...
static int nxt_unit_read_env(nxt_unit_port_t* ready_port,
  ...
  rc = sscanf(vars,
              ...
              &read_out_fd, shared_port_fd, shared_queue_fd, log_fd, shm_limit,
              request_limit);
  ...
  switch (port_msg->type) {
    ...
    case _NXT_PORT_MSG_REMOVE_PID:
      if (nxt_slow_path(recv_msg.size != sizeof(pid))) {
        nxt_unit_alert(ctx,
                       "#%" PRIu32
                       ": remove_pid: invalid message size "
                       "(%d != %d)",
...
static int nxt_unit_process_req_headers(nxt_unit_ctx_t* ctx,
  ...
  if (nxt_slow_path(recv_msg->size < sizeof(nxt_unit_request_t))) {
    nxt_unit_warn(ctx,
                  "#%" PRIu32
                  ": data too short: %d while at least "
                  "%d expected",
                  recv_msg->stream, (int)recv_msg->size,
...
static int nxt_unit_send_req_headers_ack(nxt_unit_request_info_t* req) {
  ...
  res =
      nxt_unit_port_send(req->ctx, req->response_port, &msg, sizeof(msg), NULL);
...
static nxt_unit_request_info_impl_t* nxt_unit_request_info_get(
  ...
  if (nxt_queue_is_empty(&ctx_impl->free_req)) {
    ...
    req_impl = nxt_unit_malloc(
        ctx, sizeof(nxt_unit_request_info_impl_t) + lib->request_data_size);
    if (nxt_slow_path(req_impl == NULL)) {
      return NULL;
    }
...
static void nxt_unit_websocket_frame_free(
    nxt_unit_ctx_t* ctx,
    nxt_unit_websocket_frame_impl_t* ws_impl) {
    ...
    switch (fields[i].hash) {
      ...
      case NXT_UNIT_HASH_COOKIE:
        if (fields[i].name_length == cookie.length &&
            nxt_unit_memcasecmp(name, cookie.start, cookie.length) == 0) {
          r->cookie_field = i;
        }
...
int nxt_unit_response_add_field(nxt_unit_request_info_t* req,
  ...
  if (nxt_slow_path(req_impl->state != NXT_UNIT_RS_RESPONSE_INIT)) {
    nxt_unit_req_warn(req,
                      "add_field: response not initialized or "
                      "already sent");
  ...
  if (m.msg.mmap) {
    ...
    if (nxt_slow_path(mmap_buf->plain_ptr == NULL ||
      ...
      nxt_unit_alert(req->ctx,
                     "#%" PRIu32
                     ": failed to send plain memory buffer"
...
int nxt_unit_response_write(nxt_unit_request_info_t* req,
                            const void* start,
                            size_t size) {
  ssize_t res;
...
static nxt_unit_mmap_buf_t* nxt_unit_request_preread(
  ...
  if (res < 0) {
    nxt_unit_req_alert(req, "failed to read content: %s (%d)", strerror(errno),
                       errno);
...
void nxt_unit_request_done(nxt_unit_request_info_t* req, int rc) {
  ...
  if (nxt_slow_path(req_impl->state < NXT_UNIT_RS_RESPONSE_INIT)) {
    ...
    rc = nxt_unit_response_add_field(req, "Content-Type",
                                     nxt_length("Content-Type"), "text/plain",
...
int nxt_unit_websocket_send(nxt_unit_request_info_t* req,
                            uint8_t opcode,
                            uint8_t last,
                            const void* start,
                            size_t size) {
...
int nxt_unit_websocket_sendv(nxt_unit_request_info_t* req,
                             uint8_t opcode,
                             uint8_t last,
...
ssize_t nxt_unit_websocket_read(nxt_unit_websocket_frame_t* ws,
                                void* dst,
                                size_t size) {
  ssize_t res;
  uint8_t* b;
...
retry:
  ...
  for (mm = lib->outgoing.elts; mm < mm_end; mm++) {
    ...
    if (hdr->sent_over != 0xFFFFu &&
        (hdr->sent_over != port->id.id || mm->src_thread != pthread_self())) {
      continue;
    }
...
static nxt_port_mmap_header_t* nxt_unit_new_mmap(nxt_unit_ctx_t* ctx,
  ...
  if (nxt_slow_path(rc != NXT_UNIT_OK)) {
    ...
    nxt_unit_debug(ctx, "new mmap #%" PRIu32 " created for %d -> %d", hdr->id,
                   (int)lib->pid, (int)port->id.pid);
  }
...
static int nxt_unit_shm_open(nxt_unit_ctx_t* ctx, size_t size) {
  ...
  if (nxt_slow_path(fd == -1)) {
    nxt_unit_alert(ctx, "shm_open(SHM_ANON) failed: %s (%d)", strerror(errno),
                   errno);
  ...
  if (nxt_slow_path(fd == -1)) {
    nxt_unit_alert(ctx, "shm_open(%s) failed: %s (%d)", name, strerror(errno),
                   errno);
  ...
  if (nxt_slow_path(shm_unlink(name) == -1)) {
    nxt_unit_alert(ctx, "shm_unlink(%s) failed: %s (%d)", name, strerror(errno),
                   errno);
  }
...
#error No working shared memory implementation.
  ...
  if (nxt_slow_path(ftruncate(fd, size) == -1)) {
    nxt_unit_alert(ctx, "ftruncate(%d) failed: %s (%d)", fd, strerror(errno),
                   errno);
...
static int nxt_unit_incoming_mmap(nxt_unit_ctx_t* ctx, pid_t pid, int fd) {
  ...
  mem =
      mmap(NULL, mmap_stat.st_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
  if (nxt_slow_path(mem == MAP_FAILED)) {
...
static int nxt_unit_mmap_read(nxt_unit_ctx_t* ctx,
  ...
  for (; mmap_msg < end; mmap_msg++) {
    res = nxt_unit_check_rbuf_mmap(ctx, mmaps, recv_msg->pid, mmap_msg->mmap_id,
                                   &hdr, rbuf);
    ...
    nxt_unit_debug(ctx, "#%" PRIu32 ": mmap_read: [%p,%d] %d->%d,(%d,%d,%d)",
                   ...
                   (int)hdr->dst_pid, (int)hdr->id, (int)mmap_msg->chunk_id,
                   (int)mmap_msg->size);
  }
...
static inline void nxt_unit_process_lhq_pid(nxt_lvlhsh_query_t* lhq,
                                            pid_t* pid) {
...
retry:
  ...
  nxt_unit_alert(ctx, "poll(%d,%d): %d unexpected revents [%04uXi, %04uXi]",
                 fds[0].fd, fds[1].fd, nevents, fds[0].revents, fds[1].revents);
...
static int nxt_unit_chk_ready(nxt_unit_ctx_t* ctx) {
  ...
  return (ctx_impl->ready &&
          (lib->request_limit == 0 || lib->request_count < lib->request_limit));
}
...
nxt_unit_ctx_t* nxt_unit_ctx_alloc(nxt_unit_ctx_t* ctx, void* data) {
  ...
  new_ctx = nxt_unit_malloc(
      ctx, sizeof(nxt_unit_ctx_impl_t) + lib->request_data_size);
...
static void nxt_unit_ctx_free(nxt_unit_ctx_impl_t* ctx_impl) {
  ...
  nxt_queue_each(req_impl, &ctx_impl->active_req, nxt_unit_request_info_impl_t,
                 link) {
...
void nxt_unit_port_id_init(nxt_unit_port_id_t* port_id,
                           pid_t pid,
                           uint16_t id) {
...
static nxt_unit_port_t* nxt_unit_remove_port_unsafe(
  ...
  nxt_unit_debug(NULL, "remove_port: port{%d,%d}, fds %d,%d, data %p",
                 (int)port_id->pid, (int)port_id->id, port->in_fd, port->out_fd,
                 port->data);
...
static ssize_t nxt_unit_port_send(nxt_unit_ctx_t* ctx,
  ...
  if (port_impl->queue != NULL && (oob == NULL || oob->size == 0) &&
    ...
    nxt_unit_debug(ctx, "port{%d,%d} enqueue %d notify %d", (int)port->id.pid,
                   (int)port->id.id, (int)buf_size, notify);
    ...
    if (notify) {
      ...
      if (lib->callbacks.port_send == NULL) {
        ret = nxt_unit_sendmsg(ctx, port->out_fd, &msg, sizeof(nxt_port_msg_t),
                               NULL);

        nxt_unit_debug(ctx, "port{%d,%d} send %d read_queue", (int)port->id.pid,
                       (int)port->id.id, (int)ret);

      } else {
        ret = lib->callbacks.port_send(ctx, port, &msg, sizeof(nxt_port_msg_t),
                                       NULL, 0);
...
retry:
  ...
  if (nxt_slow_path(n == -1)) {
     ...
     * implementation.
     */
    nxt_unit_warn(ctx, "sendmsg(%d, %d) failed: %s (%d)", fd, (int)buf_size,
                  strerror(err), err);
...
retry:
  ...
  if (nxt_unit_is_read_queue(rbuf)) {
    nxt_unit_debug(ctx, "port{%d,%d} recv %d read_queue", (int)port->id.pid,
                   (int)port->id.id, (int)rbuf->size);
...
retry:
  ...
  if (nxt_slow_path(rbuf->size == -1)) {
    ...
    if (err == EAGAIN) {
      nxt_unit_debug(ctx, "recvmsg(%d) failed: %s (%d)", fd, strerror(err),
                     err);
    ...
    if (nxt_app_queue_cancel(queue, cookie, port_msg->stream)) {
      ...
      if (lib->request_limit != 0) {
        ...
        if (nxt_slow_path(lib->request_count >= lib->request_limit)) {
          ...
          (void)nxt_unit_port_send(ctx, lib->main_ctx.read_port, &m, sizeof(m),
                                   NULL);
        }
      }
...
static inline void nxt_unit_port_hash_lhq(nxt_lvlhsh_query_t* lhq,
                                          nxt_unit_port_hash_id_t* port_hash_id,
                                          nxt_unit_port_id_t* port_id) {
  ...
  if (nxt_fast_path(port_id->hash != 0)) {
    ...
    nxt_unit_debug(NULL, "calculate hash for port_id (%d, %d): %04X",
                   (int)port_id->pid, (int)port_id->id, (int)port_id->hash);
  }
...
static nxt_int_t nxt_unit_request_hash_test(nxt_lvlhsh_query_t* lhq,
                                            void* data) {
  return NXT_OK;
}
...
void nxt_unit_req_log(nxt_unit_request_info_t* req,
                      int level,
                      const char* fmt,
                      ...) {
  int log_fd, n;
...
#else
  p += snprintf(p, end - p, "%4d/%02d/%02d %02d:%02d:%02d ", tm.tm_year + 1900,
