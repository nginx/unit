...
static int nxt_unit_read_env(nxt_unit_port_t *ready_port,
                             ...
                             int *shared_queue_fd, int *log_fd,
                             uint32_t *stream, uint32_t *shm_limit,
...
static int nxt_unit_send_mmap(nxt_unit_ctx_t *ctx, nxt_unit_port_t *port,
                              int fd);
static int nxt_unit_get_outgoing_buf(nxt_unit_ctx_t *ctx, nxt_unit_port_t *port,
...
static int nxt_unit_check_rbuf_mmap(nxt_unit_ctx_t *ctx,
                                    nxt_unit_mmaps_t *mmaps, pid_t pid,
                                    uint32_t id, nxt_port_mmap_header_t **hdr,
                                    nxt_unit_read_buf_t *rbuf);
...
static void nxt_unit_mmap_release(nxt_unit_ctx_t *ctx,
                                  nxt_port_mmap_header_t *hdr, void *start,
                                  uint32_t size);
...
static ssize_t nxt_unit_port_send(nxt_unit_ctx_t *ctx, nxt_unit_port_t *port,
                                  const void *buf, size_t buf_size,
                                  const nxt_send_oob_t *oob);
static ssize_t nxt_unit_sendmsg(nxt_unit_ctx_t *ctx, int fd, const void *buf,
  ...
  for (i = 0; i < r->fields_count; i++) {
    ...
    case NXT_UNIT_HASH_CONTENT_TYPE:
      if (fields[i].name_length == content_type.length &&
          nxt_unit_memcasecmp(name, content_type.start, content_type.length) ==
              0) {
...
int nxt_unit_response_add_field(nxt_unit_request_info_t *req, const char *name,
...
int nxt_unit_response_add_content(nxt_unit_request_info_t *req, const void *src,
                                  uint32_t size) {
  nxt_unit_buf_t *buf;
  nxt_unit_response_t *resp;
  nxt_unit_request_info_impl_t *req_impl;
...
uint32_t nxt_unit_buf_max(void) { return PORT_MMAP_DATA_SIZE; }

uint32_t nxt_unit_buf_min(void) { return PORT_MMAP_CHUNK_SIZE; }
...
int nxt_unit_websocket_sendv(nxt_unit_request_info_t *req, uint8_t opcode,
                             uint8_t last, const struct iovec *iov,
                             int iovcnt) {
  int i, rc;
  size_t l, copy;
...
void nxt_unit_done(nxt_unit_ctx_t *ctx) { nxt_unit_ctx_release(ctx); }
