...
nxt_list_t *nxt_list_create(nxt_mp_t *mp, nxt_uint_t n, size_t size)
{
    nxt_list_t *list;
...
void *nxt_list_add(nxt_list_t *list)
{
    void *elt;
    ...
    if (last->nelts == list->nalloc)
    {
        ...
        last = nxt_mp_get(list->mem_pool, sizeof(nxt_list_part_t) + list->nalloc * list->size);
...
void *nxt_list_zero_add(nxt_list_t *list)
{
    void *p;
...
void *nxt_list_next(nxt_list_t *list, nxt_list_next_t *next)
{
    if (next->part != NULL)
    {
        next->elt++;
