...
ssize_t nxt_solaris_event_conn_io_sendfilev(nxt_event_conn_t *c, nxt_buf_t *b, size_t limit);
static size_t nxt_solaris_buf_coalesce(nxt_buf_t *b, sendfilevec_t *sfv, int32_t *nsfv, nxt_bool_t *sync, size_t limit);
...
ssize_t nxt_solaris_event_conn_io_sendfilev(nxt_event_conn_t *c, nxt_buf_t *b, size_t limit)
{
    ...
    if (n == -1)
    {
        ...
        default:
            c->socket.error = err;
            nxt_log(c->socket.task, nxt_socket_error_level(err), "sendfilev(%d, %D) failed %E", c->socket.fd, nsfv,
                    err);
