...
nxt_int_t nxt_file_open(nxt_task_t *task, nxt_file_t *file, nxt_uint_t mode, nxt_uint_t create,
                        nxt_file_access_t access)
{
    ...
    nxt_debug(task, "open(\"%FN\", 0x%uXi, 0x%uXi): %FD err:%d", file->name, mode, access, file->fd, file->error);
    ...
    if (file->log_level != 0)
    {
        nxt_log(task, file->log_level, "open(\"%FN\") failed %E", file->name, file->error);
    }
...
nxt_int_t nxt_file_openat2(nxt_task_t *task, nxt_file_t *file, nxt_uint_t mode, nxt_uint_t create,
                           nxt_file_access_t access, nxt_fd_t dfd, nxt_uint_t resolve)
{
    struct open_how how;
    ...
    nxt_debug(task, "openat2(%FD, \"%FN\"): %FD err:%d", dfd, file->name, file->fd, file->error);
    ...
    if (file->log_level != 0)
    {
        nxt_log(task, file->log_level, "openat2(%FD, \"%FN\") failed %E", dfd, file->name, file->error);
    }
...
void nxt_file_close(nxt_task_t *task, nxt_file_t *file)
{
    nxt_debug(task, "close(%FD)", file->fd);

    if (close(file->fd) != 0)
    {
        nxt_alert(task, "close(%FD, \"%FN\") failed %E", file->fd, file->name, nxt_errno);
    }
}

ssize_t nxt_file_write(nxt_file_t *file, const u_char *buf, size_t size, nxt_off_t offset)
{
    ssize_t n;
    ...
    nxt_log_debug(thr->log, "pwrite(%FD, %p, %uz, %O): %z", file->fd, buf, size, offset, n);
    ...
    nxt_thread_log_alert("pwrite(%FD, \"%FN\", %p, %uz, %O) failed %E", file->fd, file->name, buf, size, offset,
                         file->error);

    return NXT_ERROR;
}

ssize_t nxt_file_read(nxt_file_t *file, u_char *buf, size_t size, nxt_off_t offset)
{
    ssize_t n;
    ...
    nxt_log_debug(thr->log, "pread(%FD, %p, %uz, %O): %z", file->fd, buf, size, offset, n);
    ...
    nxt_thread_log_alert("pread(%FD, \"%FN\", %p, %uz, %O) failed %E", file->fd, file->name, buf, size, offset,
                         file->error);
...
void nxt_file_read_ahead(nxt_file_t *file, nxt_off_t offset, size_t size)
{
    int ret;
    u_char buf;
    ...
    nxt_thread_log_alert("fcntl(%FD, \"%FN\", F_READAHEAD, %uz) failed %E", file->fd, file->name, size, nxt_errno);
}
...
void nxt_file_read_ahead(nxt_file_t *file, nxt_off_t offset, size_t size)
{
    ...
    nxt_thread_log_debug("posix_fadvise(%FD, \"%FN\", %O, %uz, %d): %d", file->fd, file->name, offset, size,
    ...
    nxt_thread_log_alert("posix_fadvise(%FD, \"%FN\", %O, %uz, %d) failed %E", file->fd, file->name, offset, size,
                         POSIX_FADV_WILLNEED, err);
}
...
void nxt_file_read_ahead(nxt_file_t *file, nxt_off_t offset, size_t size)
{
    ...
    nxt_thread_log_alert("fcntl(%FD, \"%FN\", F_RDAHEAD, 1) failed %E", file->fd, file->name, nxt_errno);
}
...
nxt_int_t nxt_file_info(nxt_file_t *file, nxt_file_info_t *fi)
{
    int n;
    ...
    if (file->fd == NXT_FILE_INVALID)
    {
        ...
        if (file->log_level != 0)
        {
            nxt_thread_log_error(file->log_level, "stat(\"%FN\") failed %E", file->name, file->error);
        }
    ...
    else
    {
        ...
        nxt_thread_log_alert("fstat(%FD, \"%FN\") failed %E", file->fd, file->name, file->error);

        return NXT_ERROR;
    }
}

nxt_int_t nxt_file_delete(nxt_file_name_t *name)
{
...
nxt_int_t nxt_file_set_access(nxt_file_name_t *name, nxt_file_access_t access)
{
...
nxt_int_t nxt_file_chown(nxt_file_name_t *name, const char *owner, const char *group)
{
    int err;
    char *buf;
    long bufsize;
    ...
    if (owner != NULL)
    {
        ...
        if (result == NULL)
        {
            nxt_thread_log_alert("getpwnam_r(\"%s\", ...) failed %E %s", owner, nxt_errno,
    ...
    if (group != NULL)
    {
        ...
        if (result == NULL)
        {
            nxt_thread_log_alert("getgrnam_r(\"%s\", ...) failed %E %s", group, nxt_errno,
    ...
    nxt_thread_log_alert("chown(\"%FN\", %l, %l) failed %E", name, owner != NULL ? (long)uid : -1,
...
nxt_int_t nxt_file_rename(nxt_file_name_t *old_name, nxt_file_name_t *new_name)
{
    int ret;
    ...
    nxt_thread_log_alert("rename(\"%FN\", \"%FN\") failed %E", old_name, new_name, nxt_errno);
...
nxt_int_t nxt_fd_nonblocking(nxt_task_t *task, nxt_fd_t fd)
{
    int nb;
...
nxt_int_t nxt_fd_blocking(nxt_task_t *task, nxt_fd_t fd)
{
    int nb;
...
nxt_int_t nxt_fd_nonblocking(nxt_task_t *task, nxt_fd_t fd)
{
    ...
    {
        nxt_alert(task, "fcntl(%d, F_SETFL, O_NONBLOCK) failed %E", fd, nxt_errno);
        return NXT_ERROR;
    }
...
ssize_t nxt_fd_write(nxt_fd_t fd, u_char *buf, size_t size)
{
    ssize_t n;
    nxt_err_t err;
...
ssize_t nxt_fd_read(nxt_fd_t fd, u_char *buf, size_t size)
{
    ssize_t n;
    nxt_err_t err;
...
FILE *nxt_file_fopen(nxt_task_t *task, const char *pathname, const char *mode)
{
    int err;
    FILE *fp;
    ...
    nxt_debug(task, "fopen(\"%s\", \"%s\"): fp:%p err:%d", pathname, mode, fp, err);
...
void nxt_file_fclose(nxt_task_t *task, FILE *fp)
{
...
nxt_int_t nxt_file_redirect(nxt_file_t *file, nxt_fd_t fd)
{
    nxt_thread_log_debug("dup2(%FD, %FD, \"%FN\")", fd, file->fd, file->name);

    if (dup2(fd, file->fd) == -1)
    {
        nxt_thread_log_alert("dup2(%FD, %FD, \"%FN\") failed %E", fd, file->fd, file->name, nxt_errno);
        return NXT_ERROR;
    }

    if (close(fd) != 0)
    {
        nxt_thread_log_alert("close(%FD, \"%FN\") failed %E", fd, file->name, nxt_errno);
        return NXT_ERROR;
    }
...
nxt_int_t nxt_file_stdout(nxt_file_t *file)
{
    nxt_thread_log_debug("dup2(%FD, %FD, \"%FN\")", file->fd, STDOUT_FILENO, file->name);
    ...
    nxt_thread_log_alert("dup2(%FD, %FD, \"%FN\") failed %E", file->fd, STDOUT_FILENO, file->name, nxt_errno);
...
nxt_int_t nxt_file_stderr(nxt_file_t *file)
{
    nxt_thread_log_debug("dup2(%FD, %FD, \"%FN\")", file->fd, STDERR_FILENO, file->name);
    ...
    nxt_thread_log_alert("dup2(%FD, %FD, \"%FN\") failed %E", file->fd, STDERR_FILENO, file->name, nxt_errno);

    return NXT_ERROR;
}

nxt_int_t nxt_stderr_start(void)
{
    int flags, fd;
...
nxt_int_t nxt_pipe_create(nxt_task_t *task, nxt_fd_t *pp, nxt_bool_t nbread, nxt_bool_t nbwrite)
{
    if (pipe(pp) != 0)
    {
...
void nxt_pipe_close(nxt_task_t *task, nxt_fd_t *pp)
{
...
size_t nxt_dir_current(char *buf, size_t len)
{
