...
nxt_int_t nxt_file_open(nxt_task_t *task, nxt_file_t *file, nxt_uint_t mode,
  ...
  nxt_debug(task, "open(\"%FN\", 0x%uXi, 0x%uXi): %FD err:%d", file->name, mode,
            access, file->fd, file->error);
...
nxt_int_t nxt_file_openat2(nxt_task_t *task, nxt_file_t *file, nxt_uint_t mode,
                           nxt_uint_t create, nxt_file_access_t access,
                           nxt_fd_t dfd, nxt_uint_t resolve) {
  struct open_how how;
...
ssize_t nxt_file_write(nxt_file_t *file, const u_char *buf, size_t size,
                       nxt_off_t offset) {
  ssize_t n;
  ...
  nxt_thread_log_alert("pwrite(%FD, \"%FN\", %p, %uz, %O) failed %E", file->fd,
                       file->name, buf, size, offset, file->error);

  return NXT_ERROR;
}

ssize_t nxt_file_read(nxt_file_t *file, u_char *buf, size_t size,
                      nxt_off_t offset) {
  ssize_t n;
...
void nxt_file_read_ahead(nxt_file_t *file, nxt_off_t offset, size_t size) {
  ...
  nxt_thread_log_debug("posix_fadvise(%FD, \"%FN\", %O, %uz, %d): %d", file->fd,
                       file->name, offset, size, POSIX_FADV_WILLNEED, err);
  ...
  nxt_thread_log_alert("posix_fadvise(%FD, \"%FN\", %O, %uz, %d) failed %E",
                       file->fd, file->name, offset, size, POSIX_FADV_WILLNEED,
                       err);
}
...
void nxt_file_read_ahead(nxt_file_t *file, nxt_off_t offset, size_t size) {
  ...
  nxt_thread_log_alert("fcntl(%FD, \"%FN\", F_RDAHEAD, 1) failed %E", file->fd,
                       file->name, nxt_errno);
}
...
nxt_int_t nxt_file_info(nxt_file_t *file, nxt_file_info_t *fi) {
  ...
  if (file->fd == NXT_FILE_INVALID) {
    ...
    nxt_thread_log_alert("fstat(%FD, \"%FN\") failed %E", file->fd, file->name,
                         file->error);
...
nxt_int_t nxt_file_chown(nxt_file_name_t *name, const char *owner,
                         const char *group) {
  int err;
  char *buf;
  long bufsize;
  ...
  if (group != NULL) {
    ...
    if (result == NULL) {
      nxt_thread_log_alert("getgrnam_r(\"%s\", ...) failed %E %s", group,
                           nxt_errno, err == 0 ? "(Group not found)" : "");
      goto out_err_free;
    }
...
nxt_int_t nxt_file_rename(nxt_file_name_t *old_name,
  ...
  nxt_thread_log_alert("rename(\"%FN\", \"%FN\") failed %E", old_name, new_name,
                       nxt_errno);
...
nxt_int_t nxt_pipe_create(nxt_task_t *task, nxt_fd_t *pp, nxt_bool_t nbread,
                          nxt_bool_t nbwrite) {
  if (pipe(pp) != 0) {
