...
nxt_int_t nxt_service_add(nxt_array_t *services, const nxt_service_t *service) {
  ...
  while (n != 0) {
    ...
    nxt_thread_log_alert("service \"%s:%s\" is duplicate", service->type,
                         service->name);
    return NXT_ERROR;
...
const void *nxt_service_get(nxt_array_t *services, const char *type,
                            const char *name) {
  nxt_uint_t n;
  ...
  nxt_thread_log_alert("service \"%s%s%s\" not found", type,
                       (name != NULL) ? ":" : "", name);
