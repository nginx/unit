...
static void nxt_http_request_send_error_body(nxt_task_t *task, void *r, void *data);
...
void nxt_http_request_error(nxt_task_t *task, nxt_http_request_t *r, nxt_http_status_t status)
{
    ...
    nxt_http_request_header_send(task, r, nxt_http_request_send_error_body, NULL);
    return;
...
static const nxt_http_request_state_t nxt_http_request_send_error_body_state nxt_aligned(64) = {
    .error_handler = nxt_http_request_error_handler,
};

static void nxt_http_request_send_error_body(nxt_task_t *task, void *obj, void *data)
{
    nxt_buf_t *out;
    ...
    out->mem.free = nxt_sprintf(out->mem.pos, out->mem.end, error, r->status, r->status);
