...
static void nxt_thread_realtime_update(nxt_thread_t *thr, nxt_monotonic_time_t *now);
static u_char *nxt_thread_time_string_no_cache(nxt_thread_t *thr, nxt_time_string_t *ts, u_char *buf);
static nxt_atomic_uint_t nxt_thread_time_string_slot(nxt_time_string_t *ts);
static nxt_time_string_cache_t *nxt_thread_time_string_cache(nxt_thread_t *thr, nxt_atomic_uint_t slot);
...
void nxt_thread_time_update(nxt_thread_t *thr)
{
...
static void nxt_thread_time_shared(nxt_monotonic_time_t *now)
{
    ...
    for (;;)
    {
        *now = nxt_shared_time;
        ...
        if (now->realtime.sec == t && now->realtime.nsec == n && now->monotonic == m && now->update == u)
        {
            return;
        }
    }
...
nxt_time_t nxt_thread_time(nxt_thread_t *thr)
{
...
nxt_realtime_t *nxt_thread_realtime(nxt_thread_t *thr)
{
    nxt_thread_realtime_update(thr, &thr->time.now);
...
u_char *nxt_thread_time_string(nxt_thread_t *thr, nxt_time_string_t *ts, u_char *buf)
{
    u_char *p;
    struct tm *tm;
    nxt_time_t s;
    ...
    if (ts->msec == NXT_THREAD_TIME_MSEC && (nxt_slow_path(thr->log->level == NXT_LOG_DEBUG || nxt_debug)))
    {
        nxt_msec_t ms;
