...
#define nxt_file_name_str_set(file_name, mem_pool, name) ((file_name) = (nxt_file_name_t *)(name), NXT_OK)

#define nxt_file_name_alloc(mem_pool, len) nxt_mp_nget(mem_pool, len)

#define nxt_file_name_copy(dst, src, len) nxt_cpymem(dst, src, len)

#define nxt_file_name_add(dst, src, len) nxt_cpymem(dst, src, len)

#if (NXT_HAVE_CASELESS_FILESYSTEM)

/* MacOSX, Cygwin. */

#define nxt_file_name_eq(fn1, fn2) (nxt_strcasecmp(fn1, fn2) == 0)

#else

#define nxt_file_name_eq(fn1, fn2) (nxt_strcmp(fn1, fn2) == 0)

#endif

#define nxt_file_name_is_absolute(name) (name[0] == '/')
...
NXT_EXPORT nxt_int_t nxt_file_open(nxt_task_t *task, nxt_file_t *file, nxt_uint_t mode, nxt_uint_t create,
                                   nxt_file_access_t access);

#if (NXT_HAVE_OPENAT2)
NXT_EXPORT nxt_int_t nxt_file_openat2(nxt_task_t *task, nxt_file_t *file, nxt_uint_t mode, nxt_uint_t create,
...
NXT_EXPORT ssize_t nxt_file_write(nxt_file_t *file, const u_char *buf, size_t size, nxt_off_t offset);
NXT_EXPORT ssize_t nxt_file_read(nxt_file_t *file, u_char *buf, size_t size, nxt_off_t offset);
NXT_EXPORT void nxt_file_read_ahead(nxt_file_t *file, nxt_off_t offset, size_t size);
NXT_EXPORT nxt_int_t nxt_file_info(nxt_file_t *file, nxt_file_info_t *fi);

#define nxt_is_dir(fi) (S_ISDIR((fi)->st_mode))

#define nxt_is_file(fi) (S_ISREG((fi)->st_mode))

#define nxt_file_size(fi) (fi)->st_size

#define nxt_file_mtime(fi) (fi)->st_mtime

NXT_EXPORT nxt_int_t nxt_file_delete(nxt_file_name_t *name);
NXT_EXPORT nxt_int_t nxt_file_set_access(nxt_file_name_t *name, nxt_file_access_t access);
NXT_EXPORT nxt_int_t nxt_file_chown(nxt_file_name_t *name, const char *owner, const char *group);
NXT_EXPORT nxt_int_t nxt_file_rename(nxt_file_name_t *old_name, nxt_file_name_t *new_name);
...
NXT_EXPORT FILE *nxt_file_fopen(nxt_task_t *task, const char *pathname, const char *mode);
...
#define nxt_write_console(fd, buf, size) write(fd, buf, size)

#define nxt_write_syslog(priority, message) syslog(priority, "%s", message)

NXT_EXPORT nxt_int_t nxt_pipe_create(nxt_task_t *task, nxt_fd_t *pp, nxt_bool_t nbread, nxt_bool_t nbwrite);
