...
nxt_int_t nxt_http_set_headers_init(nxt_router_conf_t *rtcf, nxt_http_action_t *action, nxt_http_action_conf_t *acf)
{
    ...
    headers = nxt_array_create(rtcf->mem_pool, 4, sizeof(nxt_http_header_val_t));
...
static nxt_http_field_t *nxt_http_resp_header_find(nxt_http_request_t *r, u_char *name, size_t length)
{
    nxt_http_field_t *f;
    ...
    nxt_list_each(f, r->resp.fields)
    {
        ...
        if (length == f->name_length && nxt_memcasecmp(name, f->name, f->name_length) == 0)
        {
            return f;
        }
    }
