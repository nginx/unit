...
#define nxt_var_raw_start(var) ((var)->data + (var)->vars * sizeof(nxt_var_sub_t))
...
static nxt_var_ref_t *nxt_var_ref_get(nxt_tstr_state_t *state, nxt_str_t *name, nxt_mp_t *mp);

static nxt_int_t nxt_var_cache_test(nxt_lvlhsh_query_t *lhq, void *data);
static nxt_str_t *nxt_var_cache_value(nxt_task_t *task, nxt_tstr_state_t *state, nxt_var_cache_t *cache,
                                      nxt_var_ref_t *ref, void *ctx);
...
nxt_var_field_t *nxt_var_field_new(nxt_mp_t *mp, nxt_str_t *name, uint32_t hash)
{
    nxt_str_t *str;
...
nxt_int_t nxt_var_test(nxt_tstr_state_t *state, nxt_str_t *str, u_char *error)
{
    ...
    while (p < end)
    {
        ...
        if (next == NULL)
        {
            nxt_sprintf(error, error + NXT_MAX_ERROR_STR, "Invalid variable at position %uz%Z", p - str->start);
        ...
        if (part.start != NULL)
        {
            ...
            if (ref == NULL)
            {
                nxt_sprintf(error, error + NXT_MAX_ERROR_STR, "Unknown variable \"%V\"%Z", &part);
...
nxt_int_t nxt_var_interpreter(nxt_task_t *task, nxt_tstr_state_t *state, nxt_var_cache_t *cache, nxt_var_t *var,
...
nxt_str_t *nxt_var_get(nxt_task_t *task, nxt_tstr_state_t *state, nxt_var_cache_t *cache, nxt_str_t *name, void *ctx)
{
    nxt_var_ref_t *ref;
