...
ssize_t nxt_macosx_event_conn_io_sendfile(nxt_event_conn_t* c,
                                          nxt_buf_t* b,
                                          size_t limit);

static int nxt_sys_sendfile(int fd,
                            int s,
                            off_t offset,
                            off_t* len,
                            struct sf_hdtr* hdtr,
...
ssize_t nxt_macosx_event_conn_io_sendfile(nxt_conn_t* c,
  ...
  nxt_log_debug(c->socket.log,
                "sendfile(%FD, %d, @%O, %O) hd:%ui tr:%ui hs:%uz", fb->file->fd,
                c->socket.fd, fb->file_pos, sent, nhd, ntr, hd_size);
