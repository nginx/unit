...
ssize_t nxt_macosx_event_conn_io_sendfile(nxt_event_conn_t *c, nxt_buf_t *b, size_t limit);

static int nxt_sys_sendfile(int fd, int s, off_t offset, off_t *len, struct sf_hdtr *hdtr, int flags)
{
    return -1;
}
...
ssize_t nxt_macosx_event_conn_io_sendfile(nxt_conn_t *c, nxt_buf_t *b, size_t limit)
{
    ...
    nxt_log_debug(c->socket.log, "sendfile(%FD, %d, @%O, %O) hd:%ui tr:%ui hs:%uz", fb->file->fd, c->socket.fd,
                  fb->file_pos, sent, nhd, ntr, hd_size);

    n = nxt_sys_sendfile(fb->file->fd, c->socket.fd, fb->file_pos, &sent, ht, 0);
    ...
    if (n == -1)
    {
        ...
        default:
            ...
            nxt_log(c->socket.task, nxt_socket_error_level(err),
                    "sendfile(%FD, %d, %O, %O) failed %E \"%FN\" hd:%ui tr:%ui", fb->file->fd, c->socket.fd,
                    fb->file_pos, sent, err, fb->file->name, nhd, ntr);
    ...
    if (sent == 0)
    {
        nxt_log(c->socket.task, NXT_LOG_ERR, "file \"%FN\" was truncated while sendfile()", fb->file->name);
