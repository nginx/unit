...
static nxt_int_t nxt_java_setup(nxt_task_t *task, nxt_process_t *process, nxt_common_app_conf_t *conf);
static nxt_int_t nxt_java_start(nxt_task_t *task, nxt_process_data_t *data);
...
static void nxt_java_join_threads(nxt_unit_ctx_t *ctx, nxt_java_app_conf_t *c);
...
static char **nxt_java_module_jars(const char *jars[], int jar_count)
{
    char **res, *jurl;
    uint8_t pathsep;
    ...
    {
        jlen = nxt_length("file:") + modules_len + (!pathsep ? nxt_length("/") : 0) + nxt_strlen(*jar) + 1;
...
static void nxt_java_websocket_handler(nxt_unit_websocket_frame_t *ws)
{
    ...
    if (b != NULL)
    {
        ...
        if (jbuf != NULL)
        {
            nxt_java_Request_websocket(env, data->jreq, jbuf, ws->header->opcode, ws->header->fin);
...
static int nxt_java_ready_handler(nxt_unit_ctx_t *ctx)
{
    ...
    {
        res = pthread_create(&nxt_java_threads[i], nxt_java_thread_attr, nxt_java_thread_func, ctx);
...
static void *nxt_java_thread_func(void *data)
{
    int rc;
    JavaVM *jvm;
    JNIEnv *env;
...
static int nxt_java_init_threads(nxt_java_app_conf_t *c)
{
    ...
    nxt_java_threads = nxt_unit_malloc(NULL, sizeof(pthread_t) * (c->threads - 1));
...
static void nxt_java_join_threads(nxt_unit_ctx_t *ctx, nxt_java_app_conf_t *c)
{
    ...
    for (i = 0; i < c->threads - 1; i++)
    {
        ...
        {
            nxt_unit_alert(ctx, "thread #%d join failed: %s (%d)", (int)i, strerror(res), res);
        }
    }
