...
static nxt_http_action_t *nxt_http_static(nxt_task_t *task, nxt_http_request_t *r, nxt_http_action_t *action);
static void nxt_http_static_iterate(nxt_task_t *task, nxt_http_request_t *r, nxt_http_static_ctx_t *ctx);
static void nxt_http_static_send(nxt_task_t *task, nxt_http_request_t *r, nxt_http_static_ctx_t *ctx);
static void nxt_http_static_next(nxt_task_t *task, nxt_http_request_t *r, nxt_http_static_ctx_t *ctx,
                                 nxt_http_status_t status);
#if (NXT_HAVE_OPENAT2)
static u_char *nxt_http_static_chroot_match(u_char *chr, u_char *shr);
#endif
static void nxt_http_static_extract_extension(nxt_str_t *path, nxt_str_t *exten);
static void nxt_http_static_body_handler(nxt_task_t *task, void *obj, void *data);
static void nxt_http_static_buf_completion(nxt_task_t *task, void *obj, void *data);

static nxt_int_t nxt_http_static_mtypes_hash_test(nxt_lvlhsh_query_t *lhq, void *data);
                               ...
                               nxt_http_action_conf_t *acf)
{
    ...
    conf->shares = nxt_mp_zget(mp, sizeof(nxt_http_static_share_t) * conf->nshares);
    ...
    if (acf->chroot.length > 0)
    {
        ...
        conf->chroot = nxt_tstr_compile(rtcf->tstr_state, &acf->chroot, NXT_TSTR_STRZ);
        ...
        for (i = 0; i < conf->nshares; i++)
        {
            ...
            if (conf->shares[i].is_const)
            {
                ...
                conf->shares[i].fname = nxt_http_static_chroot_match(chr.start, shr.start);
            }
        }
    }

    if (acf->follow_symlinks != NULL && !nxt_conf_get_boolean(acf->follow_symlinks))
    {
        conf->resolve |= RESOLVE_NO_SYMLINKS;
    }

    if (acf->traverse_mounts != NULL && !nxt_conf_get_boolean(acf->traverse_mounts))
    {
    ...
    if (acf->fallback != NULL)
    {
        ...
        return nxt_http_action_init(task, tmcf, acf->fallback, action->fallback);
    }
...
static void nxt_http_static_iterate(nxt_task_t *task, nxt_http_request_t *r, nxt_http_static_ctx_t *ctx)
{
    ...
    nxt_debug(task, "http static: \"%V\", index: \"%V\" (chroot: \"%V\")", &shr, &idx, &chr);
#else
    ...
    else
    {
        ...
        if (conf->chroot != NULL && ctx->share_idx == 0)
        {
            ret = nxt_tstr_query(task, r->tstr_query, conf->chroot, &ctx->chroot);
            if (nxt_slow_path(ret != NXT_OK))
            {
                goto fail;
            }
...
static void nxt_http_static_send(nxt_task_t *task, nxt_http_request_t *r, nxt_http_static_ctx_t *ctx)
{
    ...
    else
    {
        ...
        else
        {
            ...
            ret = nxt_http_route_test_rule(r, conf->types, mtype->start, mtype->length);
    ...
    if (conf->resolve != 0 || ctx->chroot.length > 0)
    {
        ...
        if (chr->length > 0)
        {
            ...
            fname = share->is_const ? share->fname : nxt_http_static_chroot_match(chr->start, file.name);
        ...
        if (nxt_fast_path(ret == NXT_OK))
        {
            ...
            ret = nxt_file_openat2(task, &file, NXT_FILE_RDONLY, NXT_FILE_OPEN, 0, af.fd, resolve);
    ...
    if (nxt_slow_path(ret != NXT_OK))
    {
        ...
        if (status != NXT_HTTP_NOT_FOUND)
        {
            ...
            if (chr->length > 0)
            {
                nxt_log(task, level, "opening \"%s\" at \"%V\" failed %E", fname, chr, file.error);
            }
            else
            {
    ...
    *f = file;
    ...
    if (nxt_fast_path(nxt_is_file(&fi)))
    {
        ...
        field->value = p;
        field->value_length = nxt_sprintf(p, p + length, "\"%xT-%xO\"", nxt_file_mtime(&fi), nxt_file_size(&fi)) - p;
    ...
    else
    {
        ...
        if (nxt_slow_path(!nxt_is_dir(&fi) || shr->start[shr->length - 1] == '/'))
        {
            nxt_log(task, NXT_LOG_ERR, "\"%FN\" is not a regular file", f->name);
...
static const nxt_http_request_state_t nxt_http_static_send_state nxt_aligned(64) = {
...
static const nxt_lvlhsh_proto_t nxt_http_static_mtypes_hash_proto nxt_aligned(64) = {
    NXT_LVLHSH_DEFAULT,
