...
static nxt_http_action_t* nxt_http_static(nxt_task_t* task,
                                          ...
                                          nxt_http_action_t* action);
static void nxt_http_static_iterate(nxt_task_t* task,
                                    nxt_http_request_t* r,
                                    nxt_http_static_ctx_t* ctx);
static void nxt_http_static_send(nxt_task_t* task,
                                 nxt_http_request_t* r,
                                 nxt_http_static_ctx_t* ctx);
static void nxt_http_static_next(nxt_task_t* task,
                                 nxt_http_request_t* r,
                                 nxt_http_static_ctx_t* ctx,
...
static void nxt_http_static_extract_extension(nxt_str_t* path,
                                              nxt_str_t* exten);
static void nxt_http_static_body_handler(nxt_task_t* task,
                                         void* obj,
                                         void* data);
static void nxt_http_static_buf_completion(nxt_task_t* task,
                                           void* obj,
                                           void* data);
...
nxt_int_t nxt_http_static_init(nxt_task_t* task,
  ...
  conf->shares =
      nxt_mp_zget(mp, sizeof(nxt_http_static_share_t) * conf->nshares);
  ...
  if (acf->chroot.length > 0) {
    ...
    conf->chroot =
        nxt_tstr_compile(rtcf->tstr_state, &acf->chroot, NXT_TSTR_STRZ);
    ...
    for (i = 0; i < conf->nshares; i++) {
      ...
      if (conf->shares[i].is_const) {
        ...
        conf->shares[i].fname =
            nxt_http_static_chroot_match(chr.start, shr.start);
      }
    }
  }
...
static void nxt_http_static_iterate(nxt_task_t* task,
  ...
  nxt_debug(task, "http static: \"%V\", index: \"%V\" (chroot: \"%V\")", &shr,
            &idx, &chr);
#else
...
static void nxt_http_static_send(nxt_task_t* task,
  ...
  if (shr->start[shr->length - 1] == '/') {
    ...
    *p = '\0';
    ...
    if (conf->types == NULL) {
      ...
      ret =
          nxt_http_route_test_rule(r, conf->types, mtype->start, mtype->length);
  ...
  if (conf->resolve != 0 || ctx->chroot.length > 0) {
    ...
    if (nxt_fast_path(ret == NXT_OK)) {
      ...
      ret = nxt_file_openat2(task, &file, NXT_FILE_RDONLY, NXT_FILE_OPEN, 0,
                             af.fd, resolve);
  ...
  if (nxt_slow_path(ret != NXT_OK)) {
    ...
    if (status != NXT_HTTP_NOT_FOUND) {
      ...
      if (chr->length > 0) {
        nxt_log(task, level, "opening \"%s\" at \"%V\" failed %E", fname, chr,
                file.error);
  ...
  *f = file;
  ...
  if (nxt_fast_path(nxt_is_file(&fi))) {
    ...
    field->value_length = nxt_sprintf(p, p + length, "\"%xT-%xO\"",
                                      nxt_file_mtime(&fi), nxt_file_size(&fi)) -
                          p;
