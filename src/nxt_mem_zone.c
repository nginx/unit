...
static uint32_t nxt_mem_zone_pages(u_char* start,
                                   size_t zone_size,
...
static void* nxt_mem_zone_alloc_small(nxt_mem_zone_t* zone,
                                      ...
                                      size_t size);
static nxt_uint_t nxt_mem_zone_alloc_chunk(uint8_t* map,
                                           nxt_uint_t offset,
                                           nxt_uint_t size);
static void* nxt_mem_zone_alloc_large(nxt_mem_zone_t* zone,
                                      size_t alignment,
                                      size_t size);
static nxt_mem_zone_page_t* nxt_mem_zone_alloc_pages(nxt_mem_zone_t* zone,
                                                     size_t alignment,
                                                     uint32_t pages);
static nxt_mem_zone_free_block_t* nxt_mem_zone_find_free_block(
    nxt_mem_zone_t* zone,
...
nxt_mem_zone_t* nxt_mem_zone_init(u_char* start,
                                  size_t zone_size,
                                  nxt_uint_t page_size) {
  uint32_t pages;
  nxt_uint_t n;
...
static void* nxt_mem_zone_slots_init(nxt_mem_zone_t* zone,
  ...
  for (;;) {
    ...
    if (slot[n].chunks != chunks) {
      ...
      nxt_thread_log_debug("mem zone size:%uD chunks:%uD start:%uD map:%uD",
                           slot[n].size, slot[n].chunks + 1, slot[n].start,
                           slot[n].map_size);
...
void* nxt_mem_zone_align(nxt_mem_zone_t* zone, size_t alignment, size_t size) {
  ...
  if (size <= zone->max_chunk_size && alignment <= zone->max_chunk_size) {
    ...
    nxt_thread_log_debug("mem zone alloc: @%uz:%uz chunk:%uD", alignment, size,
                         slot->size);
...
static nxt_mem_zone_page_t* nxt_mem_zone_alloc_pages(nxt_mem_zone_t* zone,
  ...
  block = nxt_mem_zone_find_free_block(zone, nxt_rbtree_root(&zone->free_pages),
                                       alignment, pages);
...
static nxt_mem_zone_free_block_t* nxt_mem_zone_find_free_block(
  ...
  if (pages <= block->size) {
    free_block =
        nxt_mem_zone_find_free_block(zone, block->node.left, alignment, pages);
    if (free_block != NULL) {
      return free_block;
    }
